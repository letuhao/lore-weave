...
import { FIELD_TYPES, type FieldType, type SystemAttribute } from './types';
...
export function AttributesAdminPanel() {
  ...
  return (
    <section className="space-y-4">
      ...
      {selected && attributes.isLoading && (
        <p className="text-sm text-muted-foreground">Loading…</p>
      )}
      {selected && attributes.isError && (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {attributes.data.map((a) => (
                <tr key={a.attr_id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2 text-muted-foreground">{a.field_type}</td>
      ...
      {(creating || editing) && (
        <Modal title={editing ? 'Edit attribute' : 'New attribute'} onClose={close}>
          <form onSubmit={onSubmit} className="space-y-3">
            ...
            <Field label="Options (one per line — for select)">
              <textarea
                value={draft.options}
