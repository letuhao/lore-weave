...
const sourceLabel = (src: string, t: TFunction): string => t(`audio.source.${src}`, { defaultValue: src });
    ...
    <div className="flex flex-col gap-3 p-3 text-xs">
        ...
        <div className="flex flex-wrap gap-2">
          {Object.entries(counts).map(([src, count]) => (
            ...
            >
              ...
              {sourceLabel(src, t)}: {count}
            </span>
          ))}
        </div>
        <div className="mt-2 text-[10px] text-muted-foreground">
          {t('audio.summary', { speakable: speakable.length, missing: missingCount })}
        </div>
      </div>
      ...
      {missingCount > 0 && (
        <div className="rounded-md border border-purple-500/20 bg-purple-500/5 p-3">
          <div className="mb-1 text-[11px] font-medium text-purple-400">
            {t('audio.generate_missing')}
          </div>
          <div className="text-[10px] text-muted-foreground">
            {t('audio.block_chars', { count: missingCount, chars: totalChars.toLocaleString() })}
            {estimatedCost > 0 && (
              <> · {t('audio.est_cost', { cost: estimatedCost.toFixed(4) })}</>
            )}
          </div>
          {generating ? (
            <div className="mt-2">
              <div className="mb-1 text-[10px] text-muted-foreground">{t('audio.generating', { progress: genProgress, total: missingCount })}</div>
            ...
            <button
              ...
              onClick={async () => {
                ...
                if (!prefs.ttsModelId || !token || !bookId || !chapterId) {
                  alert(t('audio.select_model_alert'));
                  return;
                }
            ...
            >
              {t('audio.generate_all', { count: missingCount })}
            </button>
          )}
        </div>
      )}
      ...
      <div className="space-y-1">
        ...
        {rows.map(({ block, source }, i) => (
          <BlockRow key={i} block={block} source={source} t={t} />
        ))}
      </div>
    </div>
  );
...
function BlockRow({ block, source, t }: { block: SpeakableBlock; source: AudioSource; t: TFunction }) {
  const color = SOURCE_COLORS[source];
  const label = sourceLabel(source, t);
  const preview = block.type === 'audio'
    ? (block.subtitle || t('audio.audio_block_preview'))
    : block.text;
