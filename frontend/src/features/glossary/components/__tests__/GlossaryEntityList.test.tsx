...
describe('GlossaryEntityList (13_glossary_panels.md A3)', () => {
  ...
  it('renders legacy entities whose tags field is absent', async () => {
    apiMocks.listEntities.mockResolvedValue({
      items: [{ ...entitySummary('e1', 'Legacy entity'), tags: undefined }],
      total: 1,
    });
    renderList();
    expect(await screen.findByText('Legacy entity')).toBeInTheDocument();
  });
