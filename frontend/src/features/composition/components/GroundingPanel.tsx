...
export function GroundingPanel({ projectId, sceneId, token }: { projectId: string; sceneId: string; token: string | null }) {
  ...
  return (
    <div className="flex flex-col gap-2 p-3 text-sm">
      ...
      {g.warnings.length > 0 && (
        <div className="rounded bg-amber-50 p-1.5 text-xs text-amber-800 dark:bg-amber-950 dark:text-amber-300">
          {g.warnings.join(' · ')}
        </div>
      )}
      {BLOCK_ORDER.filter((b) => g.blocks[b]).map((b) => (
        <details key={b} className="rounded border border-neutral-200 dark:border-neutral-700">
