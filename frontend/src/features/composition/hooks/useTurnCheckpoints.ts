...
export function useTurnCheckpoints(bookId: string) {
  ...
  //
  // `preRevisionId` (optional) lets the caller pin the pre-edit revision
  // SYNCHRONOUSLY (a ref it maintains at load/save), removing both the per-edit
  // API round-trip AND its TOCTOU race with a concurrent manual save (Ctrl+S)
  // that could commit a POST-edit revision inside the async read. When it is
  // `undefined` (not passed) we fall back to the async listRevisions read for
  // backward compatibility; passing `null` explicitly pins "no restore point".
  const capture = useCallback(
    async (
      ...
      let pre: string | null;
      if (preRevisionId !== undefined) {
        pre = preRevisionId; // synchronously provided (may be null) — no race
      } else {
        pre = null;
        try {
          const r = await booksApi.listRevisions(accessToken, bookId, chapterId, { limit: 1, offset: 0 });
          pre = r.items[0]?.revision_id ?? null;
        } catch {
      ...
      setCheckpoints((prev) => {
        const last = prev[0];
        if (last && last.chapterId === chapterId && last.preRevisionId === pre) {
          // Fold: keep the FIRST edit's snippet (restore rolls back to before it),
          // only bump the count + recency. Overwriting with the newest snippet
          // mislabelled the row against what Restore actually reverts.
          const folded: TurnCheckpoint = { ...last, count: last.count + 1, at: Date.now() };
          return [folded, ...prev.slice(1)];
        }
        const cp: TurnCheckpoint = {
          id: `${chapterId}:${Date.now()}:${Math.random().toString(36).slice(2, 6)}`,
          chapterId, preRevisionId: pre, at: Date.now(), snippet: snippet.slice(0, 80), count: 1, kind,
        };
