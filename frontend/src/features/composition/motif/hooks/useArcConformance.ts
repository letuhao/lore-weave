...
export function useArcConformance(
  ...
  token: string | null,
  // deep=true adds the realized-from-PROSE pacing overlay (a cross-service read) — off by
  // default so opening an arc does the cheap coarse diff; the user opts in (a button).
  deep = false,
) {
  return useQuery<ArcConformance>({
    queryKey: ['composition', 'arc-conformance', projectId, arcTemplateId, deep],
    queryFn: () => motifApi.arcConformance(projectId!, arcTemplateId!, token!, deep),
