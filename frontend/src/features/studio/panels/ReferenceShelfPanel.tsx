...
export function ReferenceShelfPanel(props: IDockviewPanelProps) {
  ...
  if (!work?.project_id) {
    return (
      <div data-testid="reference-shelf-nowork" className="flex h-full items-center justify-center p-6 text-center text-sm text-muted-foreground">
        {t('panels.reference-shelf.noWork', { defaultValue: 'No writing project yet — set up a Work first, then curate its reference shelf here.' })}
      </div>
    );
  }
  return (
