...
// Logic in useArcTemplates; this file renders. Every user-facing string goes through t('motif.arc
// .templates.*') with an English defaultValue (translation-ready; the sibling deconstruct section
// uses the same motif.arc.* namespace).
    ...
    <div data-testid="studio-arc-templates-panel" className="flex h-full min-h-0 flex-col text-sm">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        {view === 'deconstruct' ? (
          ...
          <ArcDetail state={state} arc={state.selected} t={t} />
        ) : (
          <ArcLibrary state={state} t={t} />
        )}
      </div>
    </div>
  );
...
function ArcLibrary({ state, t }: { state: ArcTemplatesState; t: TFunction }) {
    ...
    <>
      <div className="flex items-center gap-2 border-b p-2">
        <div className="flex gap-1" role="tablist">
          {TIERS.map((tr) => (
            <button key={tr.key} type="button" data-testid={`arc-tier-${tr.key}`}
              ...
              onClick={() => state.setTier(tr.key)}>{t(`motif.arc.templates.tier_${tr.key}`, { defaultValue: tr.dflt })}</button>
          ))}
        </div>
        <button type="button" data-testid="arc-new" disabled={state.busy}
          ...
          onClick={() => setCreating(true)}>{t('motif.arc.templates.new', { defaultValue: '+ New' })}</button>
      </div>

      {creating && <CreateForm state={state} t={t} onDone={() => setCreating(false)} />}
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        {state.loading ? (
          <p data-testid="arc-templates-loading" className="p-4 text-center text-xs text-muted-foreground">{t('motif.arc.templates.loading', { defaultValue: 'Loading…' })}</p>
        ) : state.isError ? (
          <div className="p-4 text-center text-xs text-destructive">{t('motif.arc.templates.loadError', { defaultValue: 'Could not load templates.' })} <button type="button" className="underline" onClick={state.refetch}>{t('motif.arc.templates.retry', { defaultValue: 'Retry' })}</button></div>
...
function Row({ state, arc, t }: { state: ArcTemplatesState; arc: ArcTemplate; t: TFunction }) {
    ...
    <li className="flex items-center gap-2 border-b px-2 py-1.5 hover:bg-muted/50">
      <button type="button" data-testid={`arc-row-${arc.id}`} className="min-w-0 flex-1 text-left" onClick={() => state.select(arc)}>
        ...
        <span className="block truncate text-[10px] text-muted-foreground">
          {t('motif.arc.templates.chapters', { count: arc.chapter_span ?? 0, defaultValue: '{{count}} chapters' })}{arc.genre_tags.length ? ` · ${arc.genre_tags.join(', ')}` : ''}
        </span>
      </button>
      <span data-testid={`arc-tier-chip-${arc.id}`} className="shrink-0 rounded bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground">
        {tier === 'system' ? t('motif.arc.templates.chipSystem', { defaultValue: 'System' })
      ...
      {state.tier === 'archived' ? (
        ...
        <button type="button" data-testid={`arc-adopt-${arc.id}`} disabled={state.busy}
          className="shrink-0 text-[10px] text-primary underline-offset-2 hover:underline disabled:opacity-50"
          onClick={() => void state.adopt(arc.id)}>{t('motif.arc.templates.adopt', { defaultValue: 'adopt' })}</button>
      )}
    </li>
  );
}

function CreateForm({ state, t, onDone }: { state: ArcTemplatesState; t: TFunction; onDone: () => void }) {
  ...
  return (
    <div data-testid="arc-create-form" className="flex flex-col gap-1.5 border-b bg-muted/30 p-2">
      <input data-testid="arc-create-code" className="rounded border bg-background px-2 py-1 text-xs" placeholder={t('motif.arc.templates.createCode', { defaultValue: 'code (unique)' })} value={code} onChange={(e) => setCode(e.target.value)} />
      <input data-testid="arc-create-name" className="rounded border bg-background px-2 py-1 text-xs" placeholder={t('motif.arc.templates.createName', { defaultValue: 'name' })} value={name} onChange={(e) => setName(e.target.value)} />
      <label className="flex items-center gap-2 text-[11px] text-muted-foreground">
        ...
        {t('motif.arc.templates.createShare', { defaultValue: "Share with this book's collaborators (book tier)" })}
      </label>
      <div className="flex gap-2">
        <button type="button" data-testid="arc-create-submit" disabled={state.busy || !code.trim() || !name.trim()}
          className="rounded bg-primary px-2 py-0.5 text-xs font-semibold text-primary-fg disabled:opacity-50" onClick={() => void submit()}>{t('motif.arc.templates.create', { defaultValue: 'Create' })}</button>
        <button type="button" className="rounded px-2 py-0.5 text-xs text-muted-foreground hover:bg-muted" onClick={onDone}>{t('motif.arc.templates.cancel', { defaultValue: 'Cancel' })}</button>
      </div>
    </div>
  );
}

function CatalogView({ state, t }: { state: ArcTemplatesState; t: TFunction }) {
  ...
  if (cat.isLoading) return <p data-testid="arc-catalog-loading" className="p-4 text-center text-xs text-muted-foreground">{t('motif.arc.templates.loading', { defaultValue: 'Loading…' })}</p>;
  if (cat.isError) return <div className="p-4 text-center text-xs text-destructive">{t('motif.arc.templates.catalogError', { defaultValue: 'Could not load the catalog.' })} <button type="button" className="underline" onClick={() => cat.refetch()}>{t('motif.arc.templates.retry', { defaultValue: 'Retry' })}</button></div>;
  const items = cat.data?.items ?? [];
  if (items.length === 0) return <p data-testid="arc-catalog-empty" className="p-4 text-center text-xs text-muted-foreground">{t('motif.arc.templates.catalogEmpty', { defaultValue: 'No public arc templates yet.' })}</p>;
  return (
      ...
      {items.map((it) => (
        <li key={it.id} className="flex items-center gap-2 border-b px-2 py-1.5 hover:bg-muted/50">
          <span className="min-w-0 flex-1">
            ...
            <span className="block truncate text-[10px] text-muted-foreground">{t('motif.arc.templates.chapters', { count: it.chapter_span ?? 0, defaultValue: '{{count}} chapters' })}{it.genre_tags.length ? ` · ${it.genre_tags.join(', ')}` : ''}</span>
          </span>
          <button type="button" data-testid={`catalog-adopt-${it.id}`} disabled={state.busy}
            className="shrink-0 text-[10px] text-primary underline-offset-2 hover:underline disabled:opacity-50"
            onClick={() => void state.adopt(it.id)}>{t('motif.arc.templates.adopt', { defaultValue: 'adopt' })}</button>
        </li>
      ))}
    </ul>
  );
...
function ArcDetail({ state, arc, t }: { state: ArcTemplatesState; arc: ArcTemplate; t: TFunction }) {
  return (
    <div data-testid="arc-template-detail" className="flex h-full min-h-0 flex-col">
      <div className="flex items-center gap-2 border-b p-2">
        <button type="button" data-testid="arc-back" className="text-[11px] text-primary hover:underline" onClick={() => state.select(null)}>{t('motif.arc.templates.back', { defaultValue: '← All templates' })}</button>
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        ...
        <DriftSection state={state} template={arc} t={t} />
      </div>
    </div>
  );
}
...
function DriftSection({ state, template, t }: { state: ArcTemplatesState; template: ArcTemplate; t: TFunction }) {
  ...
  return (
    <div data-testid="arc-drift-section" className="border-t p-2 text-[11px]">
      <h4 className="mb-1 font-medium text-foreground/80">{t('motif.arc.templates.driftTitle', { defaultValue: 'Drift — used by this book' })}</h4>
      {usedBy.length === 0 ? (
        <p data-testid="arc-drift-unapplied" className="italic text-muted-foreground">{t('motif.arc.templates.driftUnapplied', { defaultValue: 'Not applied to this book yet — apply it above to compare drift.' })}</p>
      ) : (
          ...
          {usedBy.map((a) => (
            <li key={a.id} className="flex items-center gap-2">
              <button type="button" data-testid={`drift-arc-${a.id}`} className={`min-w-0 flex-1 truncate text-left ${openArc === a.id ? 'font-medium text-primary' : ''}`}
                onClick={() => setOpenArc(a.id)}>{a.title || t('motif.arc.templates.untitledArc', { defaultValue: '(untitled arc)' })}</button>
            </li>
          ))}
        </ul>
      )}
      {openArc && (
        drift.isLoading ? <p className="text-muted-foreground">{t('motif.arc.templates.driftLoading', { defaultValue: 'Loading drift…' })}</p>
          : drift.data?.state === 'no_provenance'
            ? <p data-testid="arc-drift-no-provenance" className="italic text-muted-foreground">{t('motif.arc.templates.driftNoProvenance', { defaultValue: 'This arc was authored directly — there is no template to drift from.' })}</p>
