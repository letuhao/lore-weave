...
func closedSetSchemaFor[T any](enums map[string][]any) *jsonschema.Schema {
	// Delegates to the kit (lwmcp.ClosedSetSchema). This helper was glossary-only, and that
	// is precisely how book-service shipped four enum-less closed-set args — including the
	// two dispatch discriminators the catalog unification introduced. One implementation now,
	// so the next service inherits the rule instead of rediscovering it.
	return lwmcp.ClosedSetSchema[T](enums)
}
...
func schemaPropAt(s *jsonschema.Schema, dotted string) *jsonschema.Schema {
	return lwmcp.SchemaPropAt(s, dotted)
}
