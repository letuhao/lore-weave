...
// PgTaskStore is glossary's PERSISTENT, multi-replica implementation of lwmcp.TaskStore
// (D-MCPTASKS-GO-STORE) — a straight mirror of book-service's PgTaskStore. It backs the
// durable ext-tasks human gate with the `mcp_gate_tasks` table so a propose on one replica
// and its accept on another (or after a restart/deploy) resolve the SAME task exactly once.
// It persists only DATA ({descriptor, owner_user_id, payload}); the write to run on accept
// is the resolver registered for the descriptor (reconstructed on any replica — no closure).
// The accept is a single-winner atomic UPDATE (input_required→working), so two concurrent
// accepts can't both run the resolver — the single-use equivalent of consumeToken (the jti
// ledger the confirm_token path uses).
//
// Only ProvideInput runs the (possibly slow) resolver, and it uses the caller's ctx so a
// client disconnect / turn-abort propagates into the real write. Create/Get/Cancel are
// sub-millisecond single-row queries against the PK; they use a bounded background context
// so they never hang a startup or a poll.
