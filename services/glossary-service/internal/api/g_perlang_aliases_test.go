...
func TestPerLanguageAliases_ResolverExcludesDeleted(t *testing.T) {
	...
	if got, _ := f.srv.findEntityByNameOrAlias(ctx, f.bookID, charKind, "幽灵"); got != uuid.Nil {
		t.Errorf("Step 1: a deleted entity's name must not resolve (got %v)", got)
	}
	if got, _ := f.srv.findEntityByNameOrAlias(ctx, f.bookID, charKind, "Ghost"); got != uuid.Nil {
