...
func TestAttrLookupIndexIsRegisteredInTheChain(t *testing.T) {
	...
	var found *Step
	for i := range chain {
		if chain[i].Name == "0056_attr_lookup_index" {
			found = &chain[i]
			break
		}
	}
	if found == nil {
		t.Fatal("0056_attr_lookup_index is not in the migration chain — the index " +
			"would only ever be created on a FRESH database")
	}
	if found.Fn == nil {
		t.Fatal("0056_attr_lookup_index has a nil migration func")
	}
	if last := chain[len(chain)-1].Name; last != "0056_attr_lookup_index" {
		...
		t.Logf("note: chain now ends at %s, not 0056_attr_lookup_index", last)
	}
}
