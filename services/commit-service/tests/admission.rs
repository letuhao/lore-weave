...
fn valid_proposal_admits_with_notrun_stages_recorded() {
    ...
    let rec = admit_t6(&proposal_json("p-1", "strike", "hostile-3"), EntityId(1), &vocab(), &verbs(), &mut dedup);
...
fn duplicate_triple_rejects_at_idempotency_gate() {
    ...
    assert!(matches!(admit_t6(&json, EntityId(1), &v, &verbs(), &mut dedup).outcome, AdmissionOutcome::Admitted(_)));
    let rec = admit_t6(&json, EntityId(1), &v, &verbs(), &mut dedup);
    assert!(matches!(
        rec.outcome,
    ...
    assert!(matches!(admit_t6(&other, EntityId(1), &v, &verbs(), &mut dedup).outcome, AdmissionOutcome::Admitted(_)));
}
...
fn schema_and_vocabulary_rejects_are_recorded() {
    ...
    let rec = admit_t6("{not json", EntityId(1), &v, &verbs(), &mut dedup);
    assert!(matches!(rec.outcome, AdmissionOutcome::Rejected { stage: "schema", .. }));

    let rec = admit_t6(&proposal_json("p-2", "cast_meteor", ""), EntityId(1), &v, &verbs(), &mut dedup);
    ...
    let rec = admit_t6(&proposal_json("p-3", "strike", "the-king"), EntityId(1), &v, &verbs(), &mut dedup);
    assert!(matches!(
        rec.outcome,
