...
⚠️ **`app/db/graph_repos/` is also adapter territory, and T16's gate must say so.**
...
filter, which is the failure mode `graph_repos` exists to prevent.

So T16's `no-cypher-outside-adapters` check must allow BOTH `app/adapters/` and
`app/db/graph_repos/`, or T17 must move the repos under `app/adapters/neo4j/`. Written
