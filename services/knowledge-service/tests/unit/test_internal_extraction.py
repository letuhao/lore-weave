...
@patch("app.routers.internal_extraction.get_provider_client")
...
def test_chapter_extraction_success(
    mock_settings, mock_extract, mock_provider, mock_neo4j,
):
...
def test_chapter_without_text_returns_422():
    ...
    with patch("app.routers.internal_extraction.settings") as ms:
        ...
        with patch("app.routers.internal_extraction.neo4j_session") as mn:
            ...
            with patch("app.routers.internal_extraction.get_provider_client"):
...
@patch("app.routers.internal_extraction.extract_pass2_chapter")
@patch("app.routers.internal_extraction.settings")
def test_retryable_error_returns_502(
    mock_settings, mock_extract, mock_provider, mock_neo4j,
):
    from app.clients.provider_client import ProviderTimeout

    mock_settings.neo4j_uri = "bolt://localhost:7687"
    mock_settings.internal_service_token = _TEST_TOKEN
    mock_extract.side_effect = ProviderTimeout("LLM timed out")
    mock_provider.return_value = MagicMock()
    ...
    assert "timed out" in data["detail"]["error"]
...
@patch("app.routers.internal_extraction.extract_pass2_chapter")
@patch("app.routers.internal_extraction.settings")
def test_permanent_error_returns_422(
    mock_settings, mock_extract, mock_provider, mock_neo4j,
):
    from app.clients.provider_client import ProviderAuthError

    mock_settings.neo4j_uri = "bolt://localhost:7687"
    mock_settings.internal_service_token = _TEST_TOKEN
    mock_extract.side_effect = ProviderAuthError("invalid API key")
    mock_provider.return_value = MagicMock()
