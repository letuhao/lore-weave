...
def test_the_merge_really_bumps_version_unconditionally():
    ...
    # T17/T57 REWROTE THE FORM, NOT THE BEHAVIOUR. `ON MATCH SET` is gone because AGE has
    # neither it nor `ON CREATE SET`, so both arms are now one `CASE WHEN existed`. The old
    # anchors were the KEYWORD and the bare assignment, and both stopped matching — `.index()`
    # raised, which is the loud failure this file wants. Re-anchored on the CASE, so the guard
    # still goes red the moment the match arm stops bumping.
    body = _read("db/graph_repos/entities.py")
    start = body.index("_MERGE_ENTITY_CYPHER = ")
    cypher = body[start: body.index('"""', body.index('"""', start) + 3)]
    assert "e.version = CASE WHEN existed THEN coalesce(e.version, 1) + 1 ELSE 1 END" in cypher, (
        "merge_entity no longer bumps version on the match arm — re-check whether "
