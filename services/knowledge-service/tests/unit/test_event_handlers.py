...
async def test_chapter_published_ingests_passages_at_pinned_revision(monkeypatch):
    ...
    with patch("app.config.settings") as ms:
        ...
        monkeypatch.setattr("app.db.graph.graph_session", fake_session)
        monkeypatch.setattr(
...
async def test_glossary_updated_triggers_sync(monkeypatch):
    ...
    with patch("app.config.settings") as ms:
        ms.neo4j_uri = "bolt://fake"
        monkeypatch.setattr("app.db.graph.graph_session", _fake_neo4j_session)
        monkeypatch.setattr(
...
async def test_chapter_deleted_decrements_evidence_instead_of_bare_detach_delete(monkeypatch):
    ...
    monkeypatch.setattr("app.db.graph.graph_session", lambda: ctx)
