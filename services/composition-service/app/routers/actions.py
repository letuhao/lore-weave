...
# The user-paid motif translate (spec 2026-07-29-motif-i18n §5). USER-scoped like adopt
# — the payload names motif_ids, not a Work — with an optional book_id when the targets
# are that book's SHARED tier, which is EDIT-re-gated at confirm below.
...
_ALL_DESCRIPTORS = (
    ...
    _MOTIF_ADOPT_DESCRIPTOR, _MOTIF_MINE_DESCRIPTOR, _MOTIF_TRANSLATE_DESCRIPTOR,
...
async def confirm_action(
    ...
    # ── translate is USER-scoped (the payload names motif_ids). A book_id is present
    ...
    if claims.descriptor == _MOTIF_TRANSLATE_DESCRIPTOR:
        if payload.get("book_id"):
            ...
            try:
                ...
                raise HTTPException(status_code=403, detail={"code": "action_error"}) from exc
        return await _execute_motif_translate(payload, envelope_user, token=token, claims=claims)
...
async def _execute_motif_translate(
    payload: dict[str, Any], envelope_user: UUID, *, token: str, claims: Any,
) -> dict[str, Any]:
    """composition.motif_translate effect — ledger-claim → usage-billing precheck →
    enqueue a `translate_motif` worker job (202+poll).

    This is the ONLY way a motif translation gets paid for. The platform's own motifs
    ship translated for free; a user's are translated when — and only when — the user
    confirms this spend (spec 2026-07-29-motif-i18n §5)."""
    await _claim_or_replay(token, claims)
    estimate = float(payload.get("estimate_usd") or 0.0)
    await _precheck_or_402(
        owner_user_id=envelope_user, job_id=_billing_job_id(token), estimate_usd=estimate)
    job_id = await _enqueue_motif_job(
        envelope_user=envelope_user, project_id=None, operation="translate_motif",
        spec={
