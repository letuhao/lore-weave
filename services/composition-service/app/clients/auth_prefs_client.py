...
async def get_user_preference(user_id: UUID, key: str) -> Any:
    """Read ONE preference value (or None if unset) from auth-service — the SAME store
    `set_user_preference` writes + the FE reads. Used to capture a PRIOR value so a Tier-A
    write can emit a precise, verified Undo (e.g. switch_active_work restoring the prior active
    Work, not always the canonical). Raises AuthPrefsError on transport failure / non-2xx."""
    ...
    prefs = (resp.json() or {}).get("prefs") or {}
