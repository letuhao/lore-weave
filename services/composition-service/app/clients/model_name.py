...
provider-registry's internal model-info endpoint, so a generation job's lifecycle
...
(network I/O; H1). The projection's COALESCE merge tolerates a null model.
"""
...
async def resolve_model_name(model_source: str | None, model_ref: str | None) -> str | None:
    ...
    if not model_source or not model_ref:
        return None
    base = settings.llm_gateway_internal_url.rstrip("/")
