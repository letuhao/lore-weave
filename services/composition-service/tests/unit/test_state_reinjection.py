...
class _Jobs:
    ...
    async def prior_scene_drafts(self, user_id, project_id, chapter_id, before_story_order):
...
async def test_draft_present_uses_draft_no_fallback():
    ...
    out = await gather_recent(_Book("a\nb\nc\nd"), BOOK, CH, "jwt", k=2,
                              jobs_repo=jobs, user_id=USER, project_id=PROJECT, story_order=5)
...
async def test_book_outage_then_fallback():
    ...
    out = await gather_recent(_Book(None), BOOK, CH, "jwt",
                              jobs_repo=jobs, user_id=USER, project_id=PROJECT, story_order=3)
...
async def test_fallback_skipped_when_story_order_none():
    ...
    jobs = _Jobs(["prose"])
    out = await gather_recent(_Book(""), BOOK, CH, "jwt",
                              jobs_repo=jobs, user_id=USER, project_id=PROJECT, story_order=None)
