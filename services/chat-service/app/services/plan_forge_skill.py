...
`mode="rules"` for a fast deterministic first pass; `mode="llm"` (model_ref REQUIRED) for \
the richer LLM proposal — it returns an async job, so tell the author it started and read \
the run back; never claim it finished.
...
an arc (blocked if validation fails). `run_pipeline=true` (model_ref required) also starts \
the drafting pipeline — say it STARTED and offer the job to watch; never claim chapters \
are written.

## Rules
- **model_ref is REQUIRED** for every LLM step (`mode="llm"` propose, interpret, apply, \
autofix, compile+pipeline). There is no default planner model — ask the author which model \
