...
- Add a node with `composition_outline_node_edit(op="create")` (pass `kind`, an optional \
`parent_id` to nest it, `title`, `goal`, `synopsis`, `status`). Edit one with \
`composition_outline_node_edit(op="update")` — **`expected_version` is REQUIRED** (from a prior \
read); a stale version is rejected (`outcome: "applied_conflict"`), not silently \
overwritten. Read before you write.
- `composition_outline_node_edit(op="delete")` soft-archives a node (and its \
descendants) — reversible via `op="restore"`. `op="move"` reparents/reorders it.
- Link two scenes with a setup/payoff relationship: `composition_scene_link_edit(op="create")` \
(`from_node_id`, `to_node_id`, `label`). `op="delete"` (with `link_id`) is a hard \
...
one scene node via `composition_outline_node_edit(op="create")` — `composition_generate` needs \
...
- Create one in your own library with `composition_motif_edit(op="create", target="user")` or \
...
with no tool exposed here.** Edit with `composition_motif_edit(op="patch")` (needs \
`expected_version`); soft-archive with `composition_motif_edit(op="archive")` — unlike a canon \
rule, this IS reversible: there's no dedicated "restore" tool, but \
`composition_motif_edit(op="restore", motif_id)` un-archives it.
- **Bind** a motif into a chapter to instantiate its beats as scene nodes: \
`composition_motif_bind_edit(op="bind", project_id, node_id, motif_id, role_bindings)` — map each \
motif role to a glossary entity id. Re-binding archives (never deletes) the prior \
scenes. `op="unbind"` reverses it — pass the `undo_token` a bind returned \
