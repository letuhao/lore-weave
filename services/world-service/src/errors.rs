...
pub enum ProvisionerError {
    ...
    /// `E1` — the registry holds an `entity_id` that is not an island id.
    ///
    /// `actors.entity_id` is `BIGINT` and `sim_core::EntityId` is `u64`, and
    /// `0022` put no `CHECK` on the column: `GENERATED BY DEFAULT AS IDENTITY`
    /// allocates positives but accepts a supplied value, and `adopt_actor`
    /// passed an operator's `--entity-id` straight through. So `-1` was
    /// creatable, grantable, and then permanently unresolvable — `subject.rs`
    /// refuses it with `NotAnEntityId` at turn time, which is a character that
    /// exists and can never act.
    ///
    /// `adopt_actor` refuses it at the write edge now. This variant is the
    /// READ edge saying the same thing about a row that predates the guard,
    /// and it is OURS (a 500): the caller asked a fair question and our data is
    /// wrong. Casting instead would hand the transport `18446744073709551615`
    /// as a well-typed entity that names nobody — a wrong subject presented as
    /// a valid one, which is the class `SEALED-SUBJECT` exists to end.
    #[error("reality {0} registry holds entity_id {1}, which is not an island id")]
    CorruptEntityId(String, i64),
}
