...
INSERT INTO workflows (tier, slug, title, description, surfaces, inputs, steps, notes_md, status, source) VALUES
  ('system','entity-triage','Clean up a book''s suggested items',
   ...
   '{}'::text[], '{}'::jsonb,
   '[
     {"id":"see-pile","tool":"glossary_curation_list","gate":"none"},
     {"id":"keep-and-reject","tool":"glossary_propose_curation","gate":"confirm"},
     {"id":"merge-duplicates","tool":"glossary_propose_curation","gate":"confirm"},
     {"id":"recheck","tool":"glossary_curation_list","gate":"none","done_when":"suggestions < 1"}
   ]'::jsonb,
   E'Use this when the user wants to clean up / tidy / sort the suggested items in their book — "clean up the suggestions", "keep the good ones", "these are junk", "these two are the same person", "how many are left". The suggested items are the AI-proposed entries awaiting review (internally: draft entities tagged ai-suggested). The job: keep the real ones, throw out the junk, and combine duplicates, so the pile drains to a clean list the user trusts.\n\nEXACTLY WHICH TOOL DOES WHAT — use these, and ONLY these, for the cleanup:\n- glossary_curation_list (view="ai_suggestions") — show the pile. Each item comes back with its entity_id; you will pass those ids to the tools below. Never ask the user for an id.\n- glossary_propose_curation (op="status_change") — KEEP or THROW OUT items, in BATCHES. To keep: call it once with status="active" and entity_ids = all the real ones. To throw out: call it once with status="rejected" and entity_ids = all the junk ones. One confirmation covers the whole batch.\n- glossary_propose_curation (op="merge") — COMBINE duplicates. Call it with winner_id = the one entry to keep and loser_ids = the other entries that are the same thing (they must be the same category). The losers fold into the winner.\n- glossary_propose_curation (op="reassign_kind") — only if an item is filed under the wrong category (uncommon).\nDo NOT use glossary_propose_entity_edit or any rename/edit tool to keep, throw out, or combine — editing a name or a field is NOT triage and will not drain the pile. Renaming "Dracula" to "Count Dracula" does not merge them; use glossary_propose_curation (op="merge"). Do NOT create new items — this is a cleanup.\n\nRail: (1) list the pile and tell the user in plain terms what is there — how many, which look like real people/places, which look empty or junk, which look like the same thing twice; (2) keep the real ones (status_change → active) and throw out the junk (status_change → rejected); (3) combine duplicates (merge winner + losers); (4) list the pile again so the user SEES it shrank, and give an honest count.\n\nSPEAK PLAINLY to the user: say "suggestions" or "items", never "entities"/"drafts"; "keep"/"throw out"/"combine", never "status change"/"reject"/"merge candidate". Give honest counts (e.g. "kept 4, threw out 2, combined 2 into 1 — 5 left"), and never say the pile is clean while items still remain.',
   'published','system')
ON CONFLICT (slug) WHERE tier = 'system' DO UPDATE SET
  title = EXCLUDED.title, description = EXCLUDED.description, surfaces = EXCLUDED.surfaces,
  inputs = EXCLUDED.inputs, steps = EXCLUDED.steps, notes_md = EXCLUDED.notes_md,
...
INSERT INTO workflows (tier, slug, title, description, surfaces, inputs, steps, notes_md, status, source) VALUES
  ('system','populate-from-notes','Turn pasted notes into glossary entries',
   ...
   '[
     ...
     {"id":"save","tool":"glossary_propose_entities","gate":"none","done_when":"cast > 0","repeat":true}
   ]'::jsonb,
...
-- to_nodes (sync) + kg_build (target="graph") (ASYNC — starts an extraction job; the async_job flag
...
-- book_list (kind="chapters") (sync) + book_read (sync, reads the draft prose straight from
...
-- book_read (sync, reads the target chapter + its current draft_version) + book_chapter_
...
-- book_read is REQUIRED before the write: save_draft hard-needs the chapter's own
...
-- (sync) + composition_outline_node_edit (op="create") (sync, builds the chapter outline). done_when "plan > 0"
...
-- tools (verified 2026-07-12): composition_authoring_run_manage (op="create") (sync, sets up the run OVER AN
...
INSERT INTO workflows (tier, slug, title, description, surfaces, inputs, steps, notes_md, status, source) VALUES
  ('system','autonomous-drafting','Draft the next chapters for me',
   ...
   '{book,editor,studio}'::text[], '{}'::jsonb,
   '[
     {"id":"preview-chapters","tool":"plan_bootstrap_propose","gate":"none"},
     {"id":"make-chapters","tool":"plan_bootstrap_apply","gate":"confirm","inputs_map":{"proposal_id":"preview-chapters.proposal_id"}},
     {"id":"set-up","tool":"composition_authoring_run_manage","gate":"none"},
     {"id":"draft","tool":"composition_authoring_run_manage","gate":"none","async_job":true},
     {"id":"watch","tool":"composition_authoring_run_get","gate":"none"}
   ]'::jsonb,
   E'Use this when the user wants the book to keep drafting on its own — "just write the next few chapters", "draft ahead", "keep going without me", or when they said yes to your offer to start the writing after the plan was ready. The job: turn the plan into real chapters, then set up + start the focused drafting, and watch it go.\n\nPREREQUISITE: this drafts from a COMPILED story plan. If the book has no compiled plan yet, STOP and offer to build/compile the plan first — do not start a drafting run with nothing to draft from.\n\nEXACTLY WHICH TOOL DOES WHAT:\n- plan_bootstrap_propose — FIRST, turn the compiled plan into real, draftable chapters. This PREVIEWS which chapters the plan will create (it writes nothing yet) and returns a proposal id. Tell the user in plain words how many chapters it will make.\n- plan_bootstrap_apply — CREATE those chapters (pass the proposal id from the previous step). This asks the user to confirm before anything is created. Without this the plan has no chapters for the run to write into — this is the bridge from "a plan" to "chapters that can be drafted".\n- composition_authoring_run_manage (op="create") — set up the drafting run over those chapters: which ones, and whether to pause after each for review. Nothing drafts yet.\n- composition_authoring_run_manage (op="start") — start the drafting. This SPENDS money and asks the user to approve before anything runs — never claim it started before that approval lands. It is a BACKGROUND process, done by the focused drafting specialist, that drafts chapter after chapter; it is NOT done when the tool returns.\n- composition_authoring_run_get — watch the progress and tell the user what has been drafted so far. A drafted part becomes part of the book only once it is accepted.\n\nORDER MATTERS: make the chapters from the plan (preview → confirm-create), set up the run, get approval to start, then watch — never start spending before the user has said yes.\n\nSPEAK PLAINLY: say "chapters", "drafting", "pause to review", never "bootstrap"/"materialize"/"authoring run"/"units"/"tokens"/"job". Tell the user what is happening in their terms ("I''ll set up 10 chapters from your plan, then start drafting them, pausing after each so you can read them") and never claim chapters are written before their text is actually saved.',
   'published','system')
ON CONFLICT (slug) WHERE tier = 'system' DO UPDATE SET
  title = EXCLUDED.title, description = EXCLUDED.description, surfaces = EXCLUDED.surfaces,
  inputs = EXCLUDED.inputs, steps = EXCLUDED.steps, notes_md = EXCLUDED.notes_md,
