...
func TestInMemEmitter_RecordsAllMethods(t *testing.T) {
	e := NewInMemEmitter()
	e.SetProjectionLagSeconds("r-1", "region_projection", 12.5)
	e.SetProjectionDriftCount("r-1", "region_projection", 3)
	e.ObserveCheckDuration("daily", "region_projection", 1.2)
	e.IncCheckRun("daily", OutcomeOK)
	e.IncCheckRun("daily", OutcomeError)
	e.IncCheckRun("daily", OutcomeOK)

	if e.Lag["r-1|region_projection"] != 12.5 {
		t.Errorf("lag not recorded; got map=%v", e.Lag)
	}
	if e.Drift["r-1|region_projection"] != 3 {
...
func TestPromEmitter_DriftGaugeValueAndRunsCounter(t *testing.T) {
	e := NewPromEmitter()
	e.SetProjectionDriftCount("r-9", "world_kv_projection", 7)
	if v := testutil.ToFloat64(e.drift.WithLabelValues("r-9", "world_kv_projection")); v != 7 {
...
func TestPromEmitter_PushTransmitsFamilies(t *testing.T) {
	...
	e := NewPromEmitter()
	e.SetProjectionDriftCount("r-1", "region_projection", 5)
