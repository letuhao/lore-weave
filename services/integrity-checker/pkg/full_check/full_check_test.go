...
// addRow seeds one canon row whose live payload is `live`; the replay returns
...
func (r *rig) addRow(rid uuid.UUID, livePayload, replayPayload string) {
	canonID := uuid.New().String()
	ev := uuid.New()
	r.src.AddRow(rid, "canon_projection", live.SampledRow{
...
func TestRun_WalksAllRowsViaCursorBatching(t *testing.T) {
	...
	stats, err := r.loop.Run(context.Background(), rid, "postgres://shard", []types.TableConfig{
		{TableName: "canon_projection", FullScanBatchSize: 500},
	})
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRun_DetectsDriftAcrossFullScan(t *testing.T) {
	...
	stats, err := r.loop.Run(context.Background(), rid, "postgres://shard", []types.TableConfig{
		{TableName: "canon_projection", FullScanBatchSize: 25},
	})
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRun_MonthlyDelayWritten(t *testing.T) {
	...
	r.src.AddRow(rid, "canon_projection", live.SampledRow{
		PK: map[string]string{"canon_entry_id": canonID}, EventID: uuid.New(),
		Owning: []tablemap.OwningAggregate{{Type: "canon", ID: canonID}},
	})
...
func TestRun_AbortsOnStuckCursor(t *testing.T) {
	...
	_, err := loop.Run(context.Background(), uuid.New(), "postgres://shard", []types.TableConfig{
		{TableName: "canon_projection", FullScanBatchSize: 100},
	})
	if err == nil {
...
func (s *stuckCursorSource) NextBatch(_ context.Context, _ uuid.UUID, _, _ string, _ int) ([]live.SampledRow, string, error) {
	...
	rows := []live.SampledRow{{
		PK: map[string]string{"canon_entry_id": uuid.New().String()}, EventID: uuid.New(),
