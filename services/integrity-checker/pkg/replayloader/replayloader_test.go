...
func TestReplay_BuildsArgsAndPassesDSNAsEnv(t *testing.T) {
	...
	if argValue(fr.gotArgs, "--projection") != "canon_projection" {
	...
	if !strings.Contains(pk, `"canon_entry_id":"c-1"`) || !strings.Contains(pk, `"book_id":"b-1"`) {
...
func TestReplay_CrossAggregate(t *testing.T) {
	...
	l, _ := New(fr)
	req := baseReq()
	req.Projection = "canon_projection"
	req.Owning = []tablemap.OwningAggregate{{Type: "session", ID: "s-9"}, {Type: "npc", ID: "n-3"}}
	req.PK = map[string]string{"canon_entry_id": "c-3"}
