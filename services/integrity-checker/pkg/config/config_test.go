...
`
	...
	if cfg.Tables[0].TableName != "canon_projection" || cfg.Tables[0].SampleSize != 50 || cfg.Tables[0].FullScanBatchSize != 1000 {
		t.Errorf("table[0] mismatch: %+v", cfg.Tables[0])
	}
	if cfg.Tables[1].TableName != "canon_projection" || cfg.Tables[1].SampleSize != 25 {
...
tables:
  - name: canon_projection  # a surviving L3.A table
    sample_size: 20
    full_scan_batch_size: 500
`
	cfg, err := parse(yaml)
	...
	if !strings.HasPrefix(cfg.Tables[0].TableName, "canon_projection") {
