...
// ============================================================================
// Civ Ship 8 — political PNG + SVG export
// ============================================================================

/// **Civ Ship 8** — build a `HashMap` from `(plate_id, zone_id,
/// subzone_id)` to the civ-layer cell index that the political /
/// settlement / hydrology vectors are keyed by. Iteration order matches
/// [`build_civ_view`] — plates outer, zones middle, subzones inner —
/// so the mapping is a stable function of `world.plates`.
fn cell_index_map(world: &FlatWorld) -> std::collections::HashMap<(usize, usize, usize), u32> {
...
/// Deterministic per-state colour. Stays stable across renders so the
/// political map can be compared frame-to-frame.
...
/// Simple HSV → RGB helper (saturation+value in `[0, 1]`, hue in degrees).
...
///
/// For each pixel: find the covering plate → its sub-zone via
/// [`crate::flatworld::Plate::zone_at_polygon`] +
/// [`crate::flatworld::Zone::subzone_at_polygon`] → look up
/// `political.province_of[cell] → political.provinces[…].state` → colour
/// by `state_color`. Void pixels render as dark ocean blue; settlement
/// cells get a white dot overlay.
///
...
pub fn render_civ_political_png(
    ...
    const VOID: [u8; 3] = [12, 22, 48]; // deep-ocean blue
    let w = world.width as usize;
    let h = world.height as usize;
    ...
    // Settlement dots — radius 3 px filled white.
    for s in settlements {
        let cell = s.cell as usize;
        // Reverse-lookup centre from cell index by re-walking the
        // plate/zone/subzone tree. Cheap because settlements is small.
...
fn cell_index_to_center(world: &FlatWorld, target: usize) -> Option<(f32, f32)> {
    let mut idx: usize = 0;
    for plate in &world.plates {
...
/// 2. Plate polygons filled with their state hue (light alpha).
/// 3. Route polylines connecting cell centres along each route's path.
/// 4. Settlement circles + text labels with their synthetic name.
/// 5. Province name labels at the capital cell centre.
///
/// Designed for diff-friendly text output; not optimised for visual
/// fidelity. PNG remains the production render.
...
pub fn render_civ_svg(
    ...
    // Plates filled with their primary subzone's state colour.
    let idx_of = cell_index_map(world);
    for plate in &world.plates {
        // Use the first sub-zone's state as the plate "tint" — good
        // enough for an at-a-glance view.
        let tint = plate
            .zones
            .iter()
            .enumerate()
    ...
    // Routes — emit polylines along each route's `path` (cell index
    // list). Skip zero-hop or path-less routes.
    for r in routes_v {
        if r.path.len() < 2 {
            continue;
        }
...
mod tests {
    ...
    fn political_png_has_correct_byte_length() {
        // Ship 8: PNG buffer is `width * height * 3` bytes (RGB).
        let world = generate(&FlatParams::default());
    ...
    fn political_png_paints_void_pixels_with_ocean_blue() {
        // Pixels outside any plate must end up the dark-ocean colour.
        let world = generate(&FlatParams::default());
        let (_, _, political, _, settlements, _, _) = build_culture(
            &world,
            &WorldClimateParams::default(),
    ...
    fn civ_svg_export_well_formed_and_contains_features() {
        // Ship 8: SVG export must start with <?xml + <svg, contain
        // <polygon> for plates, <circle> for settlements, and <text>
        // for labels.
        let world = generate(&FlatParams::default());
