...
pub struct Zone {
    ...
    pub shape_seed: u32,
    pub shape_kind: ShapeKind,
    /// **v4.2a→d**: templated sub-zones (one templated polygon per
    /// sub-zone, generated by the dispatcher at `depth=2`). Post-v4.2d
    /// this is the single source of truth for L2 sub-zone layout —
    /// climate, render, and `Plate::subzone_at` all index
    /// `zones[zi].subzones[si].center` directly.
...
impl Zone {
    ...
    /// migration; v4.2a SCHEMA left zonegen.rs reading `subzone_sites`,
    /// v4.2c RENDER migrated SubAttr emission to `zones[zi].subzones`,
    /// v4.2d CLEANUP dropped `subzone_sites` entirely.
...
impl Plate {
    ...
    /// containing L1 zone, then the nearest sub-zone centre **within that
    /// zone**. `None` if the plate has no zones.
    ///
    /// **v4.2d**: now reads `self.zones[l1].subzones[].center` instead of
    /// the dropped `subzone_sites` Vec. Output unchanged because the
    /// v4.2a parallel-population invariant guaranteed the two were
    /// bit-for-bit identical (asserted by
    /// `v4_2a_subzones_parallel_to_subzone_sites` for all of v4.2a-c).
    pub fn subzone_at(&self, x: f32, y: f32) -> Option<(usize, usize)> {
        let l1 = self.zone_at(x, y)?;
        let l2 = self
            .zones
            .get(l1)
            .and_then(|zone| nearest_zone_center(&zone.subzones, x, y))
            .unwrap_or(0);
        Some((l1, l2))
    }
}
...
pub fn generate(params: &FlatParams) -> FlatWorld {
    ...
    let plates = centers
        ...
        .map(|(id, (cx, cy))| {
            ...
            // **v4.2d**: sub-zone centres are sampled into a local
            // `Vec<Vec<(f32,f32)>>` instead of being stored on
            // `Plate.subzone_sites`. Each `Plate.zones[zi]` then carries
            // its own `subzones: Vec<Zone>` as the post-v4.2 single source
            // of truth. The local Vec is consumed by the zone loop and
            // drops at scope end.
            let max_sub = params.max_subzones.max(params.min_subzones).max(1);
            let subzone_centers_per_zone: Vec<Vec<(f32, f32)>> = (0..zone_centers.len())
                .map(|l1| {
            ...
            for (zi, &zone_center) in zone_centers.iter().enumerate() {
                ...
                // **v4.2a→d**: depth=2 sub-zone templating. Each sampled
                // sub-zone centre becomes a `Zone` with its own
                // dispatcher-generated polygon. v4.2d drops the legacy
                // `plate.subzone_sites` Vec — sub-zone centres now live
                // only on `Zone.subzones[si].center`.
                ...
                let subzone_centers = &subzone_centers_per_zone[zi];
                ...
                .map(|(z, zone)| {
                    ...
                    ZoneData {
                        ...
                        subzones: zone
                            ...
                            .iter()
                            .enumerate()
                            .map(|(sz, sub)| SubZoneData {
                                path: vec![p.id as u32, z as u32, sz as u32],
                                site: [sub.center.0, sub.center.1],
                            })
                            .collect(),
                    }
                })
...
mod tests {
    ...
    #[test]
    fn v4_2a_subzones_size_within_configured_bounds() {
        // v4.2a originally asserted parity against the legacy
        // `Plate.subzone_sites` Vec; v4.2d dropped that field and made
        // `zones[zi].subzones` the single source of truth. The parity
        // invariant is no longer meaningful — instead assert each
        // zone's sub-zone count falls inside `[min_subzones, max_subzones]`
        // as configured. Centre-drift detection is now caught at
        // construction time (the dispatcher always uses the sampled
        // centres).
        let params = FlatParams::default();
        let world = generate(&params);
        let min = params.min_subzones;
        let max = params.max_subzones.max(min).max(1);
        for plate in &world.plates {
            for zone in &plate.zones {
                let n = zone.subzones.len();
                assert!(
                    n >= min && n <= max,
                    "plate {} zone {}: subzones.len() = {} outside [{min}, {max}]",
                    plate.id, zone.id, n,
                );
            }
        }
    ...
    fn square(id: usize, c: (f32, f32), r: f32, velocity: (f32, f32)) -> Plate {
        Plate {
            ...
            velocity,
            // v4.2d: test helpers don't exercise zones — empty Vec keeps
    ...
    fn each_zone_is_subdivided_into_subzones() {
        ...
        let world = generate(&p);
        for plate in &world.plates {
            for (l1, zone) in plate.zones.iter().enumerate() {
                assert_eq!(
                    zone.subzones.len(),
                    2,
                    "plate {} zone {l1} subzone count",
                    plate.id,
                );
                // Each sub-zone centre lies in its own L1 zone cell.
                for sub in &zone.subzones {
                    let (sx, sy) = sub.center;
