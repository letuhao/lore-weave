...
- **SCOPE-3 · SSOT vs derived is explicit and one-directional.** Every derived store (projection, cache, index, embedding, snapshot) declares its **single source of truth** and is **regenerable from it with no loss** — derived data is never authored directly. (INV-FACTS: `entity_facts` is truth; the EAV projection + prose snapshot are regenerable caches.)
  - ⚠️ **glossary↔knowledge is NOT an SSOT/derived pair, and calling it one licensed data loss.** This rule read *"glossary is authored SSOT, knowledge is the derived fuzzy/semantic layer"* until 2026-08-14. Measured on the live stack that day, knowledge holds truth glossary **cannot represent**: **1184 `:Event`** (glossary has no events table), **35 `:EntityStatus` of which 0 are glossary-anchored** against 3 status facts in `entity_facts`, and **567 `:Entity` with no anchor at all**. `entity_facts.entity_id` is an FK to `glossary_entities`, so an unanchored status is not merely absent — it is *unwritable*. Acting on the old wording — *"regenerable from it with no loss"*, *"derived data is never authored directly"* — would sanction rebuilding the KG from glossary and destroying every one of those rows.
  - ✅ **The honest characterisation: two stores with DISJOINT truth, joined by an anchor.** Glossary owns the authored, curated entity; knowledge owns the extracted story-positioned graph. `glossary_entity_id` is a *join key*, not a derivation. Either side can hold a row the other cannot express, which is precisely why T32's liveness producer had to **emit** from knowledge into glossary rather than glossary reading it back.
  - 📐 **The destination is ONE store** (O1, sealed 2026-08-09; `T46` merges them). Until that lands the two-store split is the design, not a defect to be tidied — and the rule above must not describe it as a derivation in the meantime.
  - 🔒 **The code already knew, which is how this was caught.** `knowledge-service/app/jobs/graph_rebuild.py` — the only rebuild-from-Postgres path — says in its own docstring that it restores **identity (nodes, anchors), not the full extracted edge set**, *"because those have no Postgres original to rebuild from"* (`D-T41-RELATIONS-NOT-REBUILDABLE`). **That module, not this sentence, is what protects the data**: a rebuild cannot destroy the edges because it never attempts them. What the wording above protects is the DECISION — someone reading SCOPE-3 and concluding the KG is disposable. Both are needed, and only one of them runs.
...
| SCOPE-3 SSOT/derived | **partly enforced** | INV-FACTS + two-layer FK anchoring (knowledge tests). ⚠️ The "generalize the *derived is regenerable* assertion" plan is **withdrawn for glossary↔knowledge** — measured 2026-08-14, the pair is not SSOT/derived and the assertion would be false by construction there. It still holds for the EAV projection and the prose snapshot, which ARE regenerable from `entity_facts`. |
