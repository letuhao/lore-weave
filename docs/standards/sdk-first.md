...
- **SDK-3 · Anti-orphan: if a shared module exists, ADOPT it.** Hand-rolling a capability that already has a shared module is a violation from the other side. Today `contracts/logging`, `contracts/errors`, and `contracts/dependencies/client_factory.go` exist with **zero importers** while services hand-roll the same thing — an orphan SDK is a defect.
...
- **`loreweave_logging`** (+ `TraceIdMiddleware`) — folds the 3× copy-pasted `logging_config.py` + 5× drifted `trace_id` middleware ([Logging LG-6](./logging.md)). *Or adopt the orphan `contracts/logging`.*
...
- **Adopt the orphans:** `contracts/logging`, `contracts/errors`, `contracts/dependencies/client_factory.go` (currently 0 importers).
...
| SDK-3 anti-orphan | **to build** | adoption check: assert each Python service `main.py` imports the shared logging/trace/authn SDK; assert `contracts/logging`/`contracts/errors` have >0 importers (orphan → red) |
