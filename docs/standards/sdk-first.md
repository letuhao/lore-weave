...
- ✅ **`loreweave_authn`** (Python) + **`contracts/platformjwt`** (Go) — one shared user-JWT verifier, DONE ([Security SEC-2](./security.md)): all 7 Go domain services import `platformjwt` (auth is the minter); all 8 Python user-facing services use `loreweave_authn` after **P3** migrated the last two hand-rollers (translation, video-gen). Only lore-enrichment's **intentional** `verify_signature=False` stub remains (deferred to its auth-wiring cycle). Regression-guarded by the `py-jwt-verifier` rule in `sdk-duplication-gate.py` (assignment-anchored HS256 `jwt.decode` → red). TS: the api-gateway-bff `jwt.verify` sites are now all HS256-pinned (alg-confusion defense).
...
- ✅ **`loreweave_internal_client`** (Python, ✅ CREATED + ADOPTED P3) — the shared async S2S HTTP mechanics the ~48 per-service `*_client.py` re-declared: `build_internal_client` (token+JSON headers + uniform per-request `X-Trace-Id` + `httpx.Timeout`), `InternalClientError`+`is_retryable_status`/`RETRYABLE_STATUSES`, `resolve_model_name`. **Composable mechanics, not imposed policy** (fail-posture is per-method — degrade/raise/swallow/verbatim all coexist). Adopted across all waves:
  - **W1** — the 6 `resolve_model_name` copies → shims; **W4** — worker-ai's last client-METHOD copy → SDK delegation, so **every** model_name copy is gone and the `py-model-name-copy` gate rule now has an **empty baseline**.
  - **W2 + W2-tail** — `InternalClientError`/`is_retryable_status` adopted by the 5 lore-enrichment error classes, knowledge + composition `EmbeddingError` (raise-posture), campaign `BookServiceError`/`DispatchError`, and worker-ai's degrade-posture result dataclasses. The inline `retryable = status in (502,503,429)` re-derivation is regression-guarded by the `py-inline-retryable` gate rule (matches exactly the 3-element set, so lore-enrichment's 504-INCLUSIVE list is correctly NOT flagged — see the `RETRYABLE_STATUSES` caveat).
  - **W3 + W4** — `build_internal_client` adopted by 13 single-auth clients (lore-enrichment book/kal; knowledge book/book_profile/embedding/reranker/translation/ontology + glossary [circuit breaker preserved, transport-only swap]; composition embedding/web_search/glossary/kal), removing the hand-rolled construction + per-request `X-Trace-Id` threading. The composition grant shim's forgotten `trace_id_provider` (a real live trace gap) was wired.
  - **Documented-bespoke (the SDK's acknowledged per-method exceptions — NOT drift):** clients that forward the caller's **JWT Bearer** rather than the internal token (comp `book_client`/`knowledge_client`, lore-enrichment `glossary`/`knowledge` dual-auth) cannot bake a single token; **multi-host** clients (LE `writeback`, 3 base URLs) don't fit a single-`base_url` factory; **verbatim status passthrough** (jobs `control`) and **2-host + MCP transport** (chat `knowledge_client`) keep their bespoke transport. Where their error semantics are load-bearing (e.g. comp `BookClientError` carries router-mapped status/code/detail), the error class also stays bespoke. This is the "composable mechanics" contract working as designed, not an un-migrated backlog.
