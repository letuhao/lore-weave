...
- 🔄 **`loreweave_internal_client`** (Python, ✅ CREATED P3 — adoption in waves) — the shared async S2S HTTP mechanics the ~48 per-service `*_client.py` re-declare: `build_internal_client` (token+JSON headers + uniform per-request `X-Trace-Id` + `httpx.Timeout`), `InternalClientError`+`is_retryable_status`/`RETRYABLE_STATUSES`, `resolve_model_name`. **Composable mechanics, not imposed policy** (fail-posture is per-method — degrade/raise/swallow/verbatim all coexist). Adopted so far: the 6 `resolve_model_name` copies → shims (W1), the 5 lore-enrichment error classes → `InternalClientError` (W2). Remaining: trace-uniformity + factory adoption for the simple clients (W3) + the 6 hard per-method cases (W4). Regression-guarded by the `py-model-name-copy` rule in `sdk-duplication-gate.py` (worker-ai's client-method copy baselined until its wave).
