...
|---|---|
| **A new MCP tool (input args, low error)** | [Frontend-Tool Contract](#frontend-tool-contract) (agent→GUI args) · knowledge **4-source discipline** ([§ Code invariants](#c-code-level-invariants)) · [MCP-first invariant](#b-cross-cutting-machine-contracts) · ⚠️ input-discipline is **fragmented** — see [Gaps](#known-gaps--caveats) |
| **An MCP tool's return shape (no context bloat)** | [Context Budget Law §6](../specs/2026-07-03-context-budget-law.md) — L1 reference-first / L2 detail·fields·limit / L3 concise-wire. Status **DRAFT** (T0–T2 shipped, L3 lint live) |
...
2. **MCP tool I/O discipline is fragmented across 4 places** — the exact pain this index was built to expose. *Input* rules live in the Frontend-Tool Contract (GUI tools only) + the knowledge 4-source discipline (code + drift tests) + the deferred `[[llm-client-first-tool-refactor]]` memory; *output* rules live in the DRAFT Context Budget Law §6. There is **no single "MCP Tool I/O Standard" doc yet** — a natural next promotion (`docs/standards/mcp-tool-io.md`).
