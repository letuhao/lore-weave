...
The rules you read **before building** on the main LoreWeave product. Many are stated in **[`AGENTS.md`](../../AGENTS.md)** (the development guide every agent reads at session start — the hub); this table points to the *authoritative detail* for each.

> ⚠ **Never cite `CLAUDE.md` as a source.** It is a 15-line pointer that says of itself *"deliberately kept empty of rules"*. Until 2026-09-03 this paragraph called it "the de-facto hub" and sixteen rows below pointed their **Authoritative source** at `CLAUDE.md §Section` — every one of which resolved to nothing. They are repointed at `AGENTS.md`, which is where those sections actually live.
...
| **Slash-command registry parity** | every runner in `.claude/commands/` is named in AGENTS.md's Slash commands section, and every command named there exists; a retirement that leaves the prose behind is the failure it catches | [scripts/slash-command-doc-gate.py](../../scripts/slash-command-doc-gate.py) | ACTIVE | `scripts/slash-command-doc-gate.py` (pre-commit; fails loudly if the registry heading itself goes missing, rather than scanning nothing) |
...
| **Frontend-Tool Contract** <a id="frontend-tool-contract"></a> | agent→GUI tools span 2 services/2 langs: closed-set arg⇒enum, resolver never silently no-ops, one-name-one-concept, machine-checked both sides, verify-by-effect | [`AGENTS.md`](../../AGENTS.md) §Frontend-Tool Contract · SoT [contracts/browser-tools.contract.json](../../contracts/browser-tools.contract.json) | **LOCKED** | 4 tests (see [§D](#d-enforcement-mechanisms)) |
| **MCP-first invariant** | any AI *agent* capability MUST be an MCP tool-call through `ai-gateway` (never bespoke HTTP/raw-prompt); domain owns its tools | [`AGENTS.md`](../../AGENTS.md) · [docs/specs/2026-06-10-glossary-assistant-architecture.md](../specs/2026-06-10-glossary-assistant-architecture.md) | ENFORCED (convention) | review; new agentic HTTP tracked in Deferred |
| **Provider-gateway invariant** | no service imports a provider SDK / calls a provider API directly; all LLM/embed/**rerank**/image/audio/STT via `provider-registry-service`; local backends only as BYOK creds | [`AGENTS.md`](../../AGENTS.md) | **ENFORCED** | `scripts/ai-provider-gate.py` (pre-commit + CI) |
| **No hardcoded model names** | model names + pricing resolve from provider-registry, never literal in runtime code | [`AGENTS.md`](../../AGENTS.md) | **ENFORCED** | `scripts/ai-provider-gate.py` |
...
| **User Boundaries & Tenancy** | 3 scope tiers (System/Per-user/Per-book); no regular-user write to shared rows; every table carries a scope key; grant-gated cross-tenant; tier-merged resolution | [`AGENTS.md`](../../AGENTS.md) §User Boundaries | **LOCKED** | review + partial-UNIQUE(scope,name) schema pattern |
| **Data Persistence Rules** | server-is-SSOT; no localStorage for user data; multi-device; persist UI state to DB; prefs write-through | [`AGENTS.md`](../../AGENTS.md) · [docs/DATA_ARCHITECTURE.md](../DATA_ARCHITECTURE.md) | ACTIVE ("enforced") | review |
| **Frontend Architecture (React MVC)** | hooks=controllers · context=services · components=views; no `useEffect` for events; split context by update freq; ~100/200-line limits | [`AGENTS.md`](../../AGENTS.md) §Frontend Architecture Rules | ACTIVE | review |
| **Gateway invariant (I1)** | all external traffic through `api-gateway-bff` (one sanctioned exception: PRR-20 game-server WS) | [`AGENTS.md`](../../AGENTS.md) · [docs/ARCHITECTURE.md](../ARCHITECTURE.md) | ACTIVE | review |
| **Per-service DB ownership / no cross-DB FK** | each microservice owns its Postgres DB; integrate via HTTP/outbox, never shared tables | [docs/DATA_ARCHITECTURE.md](../DATA_ARCHITECTURE.md) · [`AGENTS.md`](../../AGENTS.md) | ACTIVE ("enforced") | review |
...
| **No-Defer-Drift** | FIX-NOW default; a deferral must earn a row via the 5-point gate; "missing infra" ≠ "blocked" | [`AGENTS.md`](../../AGENTS.md) §No Deadline · [docs/deferred/DEFERRED.md](../deferred/DEFERRED.md) | ACTIVE | review at each PLAN |
...
| **Debugging Protocol** | no fixes without root cause; INVEST→PATTERN→HYPOTHE→FIX; 3-strike hard-stop | [`AGENTS.md`](../../AGENTS.md) §Debugging Protocol | ACTIVE | convention |
| **Test Parallelization** | pytest-xdist `-n auto --dist loadgroup`; DB-touching tests carry `xdist_group("pg")` | [`AGENTS.md`](../../AGENTS.md) §Test Parallelization | ACTIVE | convention (a missing mark → interleaved counts) |
...
| [contracts/browser-tools.contract.json](../../contracts/browser-tools.contract.json) | agent→GUI frontend-tool arg schemas + enum rule (cross-language SoT) | ai-gateway `src/mcp/{confirm,ui,propose-edit}-tools.ts` + FE resolvers; edit the JSON BY HAND (regen closed 2026-09-03) |
...
| Test | Guards |
|---|---|
| `services/chat-service/tests/test_frontend_tools_contract.py` (BE) | Frontend-Tool Contract — snapshots every schema into `browser-tools.contract.json`, enforces closed-set-arg⇒enum |
...
3. **INV-KAL is enforced but under-documented in the hub** — two pre-commit gates guard it, yet it is not in AGENTS.md's "Key Rules". Its design home is `docs/specs/2026-06-29-incremental-temporal-knowledge-architecture.md` §6D.
