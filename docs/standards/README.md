...
| [scripts/design-lint.py](../../scripts/design-lint.py) | Layer-1 design-corpus integrity (`docs/03_planning/LLM_MMO_RPG/`): broken relative links · phantom "(registered …)" claims vs the ownership matrix · stable-ID prefixes vs the id catalog. Root cause it guards: *"documents are locked individually; correctness is a property of the set"* (`19` §14) | pre-commit `--staged --warn-check symbol` (link + phantom = **hard fail**; unregistered-prefix = warn); [README](../../scripts/design-lint.README.md) |
