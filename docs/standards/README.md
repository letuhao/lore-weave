...
| **Multilingual / Anti-Language-Bias** | language a first-class param; Unicode-correct normalization (NFKC+CJK, no naive lower); script-agnostic segmentation (no `[A-Z][a-z]`/space-split); no English-only rule logic; `ensure_ascii=False`; per-locale i18n from the Registry SoT (`languages.ts`); **ML-7 one authored `en` locale — all others tool-scaffolded (`scripts/i18n_translate.py`, verify+self-heal+resumable, no silent blanks), never hand-edited** | [docs/standards/multilingual.md](./multilingual.md) | ACTIVE (rules; enforcement P1) | `language-bias-gate.py` + golden fixtures + ML-7 locale-completeness/registry-parity check + parity tests (to build) |
