...
| **Multilingual / Anti-Language-Bias** | language a first-class param; Unicode-correct normalization (NFKC+CJK, no naive lower); script-agnostic segmentation (no `[A-Z][a-z]`/space-split); no English-only rule logic; `ensure_ascii=False`; per-locale i18n | [docs/standards/multilingual.md](./multilingual.md) | ACTIVE (rules; enforcement P1) | `language-bias-gate.py` + multi-language golden fixtures + parity tests (to build) |
