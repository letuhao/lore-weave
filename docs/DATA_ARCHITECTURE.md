...
LoreWeave is a multi-agent platform for multilingual novel workflows. The data layer spans **16 Postgres databases**, **Neo4j** (derived knowledge graph), **Redis**, **RabbitMQ**, and **MinIO** — too large to infer from any single service.
...
flowchart TB
  ...
  subgraph stores [Data stores]
    PG[(Postgres 18 per-service DBs)]
    Neo4j[(Neo4j derived KG)]
...
| **Redis 7** | Cache, rate limits, sessions, **event stream fan-out** | Ephemeral / cache; streams are relay buffer | gateway, workers, AI services |
...
|--------|------------------------|-------------------------|
| Postgres | Single server, many `loreweave_*` database names | Per-service RDS instances or schemas per team policy |
| Neo4j | Single container (`NEO4J_URI` optional for Track-1-only) | Dedicated graph instance |
| Redis | Single container | ElastiCache |
| MinIO | Local container (`:9123`) | S3 |
...
| `loreweave_composition` | composition-service | composition_work, structure_template, outline_node, scene_link, canon_rule, generation_job, outbox_events | `services/composition-service/app/db/migrate.py` |
| `loreweave_lore_enrichment` | lore-enrichment-service | source_corpus, enrichment_job, enrichment_proposal, enrichment_template, enrichment_book_profile, enrichment_upload | `services/lore-enrichment-service/app/db/migrate.py` |
...
| video-gen-service | Stateless BFF | ComfyUI workloads in sibling **local-image-generator-service** |
| worker-ai | Stateless worker | Reads/writes `loreweave_knowledge` only |
| worker-infra | Worker | Owns `loreweave_events`; polls outbox tables in source DBs |
...
| `:Project`, `:Session` | Scoping for multi-tenant graph |
| `:ExtractionSource` | Provenance for evidence edges |
| `:RELATES_TO` | Directed entity relations |
| `:EVIDENCED_BY` | Source → target evidence links |

### MinIO buckets (by service)

| Service | Typical buckets / paths |
|---------|-------------------------|
| book-service | `loreweave-dev-books` — covers, imports, media |
| chat-service | `lw-chat` — attachments, audio |
| lore-enrichment-service | `lore-enrichment-uploads` |
| provider-registry-service | Provider-related assets (when configured) |
...
### Outbox sources (current)

Relayed by `worker-infra` from:

- `loreweave_book` → chapter/book lifecycle events
- `loreweave_translation` → translation job events
- `loreweave_chat` → chat events
- `loreweave_glossary` → entity/wiki changes (feeds knowledge glossary-sync + learning corrections)
- `loreweave_knowledge` → correction events for learning-service

Central event log: `loreweave_events.event_log`.

### Living Worlds (separate track)

The novel workflow above uses **one DB per microservice**. The **LLM MMO RPG / Living Worlds** track adds **DB-per-reality** (thousands of Postgres DBs) with strict rules:
...
Script `scripts/inventory-databases.py` could parse `DATABASE_URL` / `*_DB_URL` from compose and grep `CREATE TABLE` from migrate files to generate an appendix — reduces drift like the stale table formerly in `E2E_REVIEW_BRIEF.md` §3.
