...
`extraction_runs` is the core join target for config-vs-outcome analysis, so best-effort gaps would inject the exact **selection bias** the plan §2.4 guards against. worker-ai already writes the cursor-advance (`_advance_cursor`) to Postgres at chapter success — the outbox insert goes in the **same transaction**. Run-emit and cursor-advance commit or roll back together: under normal operation a chapter that advanced the cursor always has a run row (no systematic gaps). **Telemetry is never load-bearing** (/review-impl MED-1): on a transaction failure the worker falls back to a plain best-effort cursor-advance — the chapter's real work already persisted, so it must NOT re-extract nor fail the job; the rare lost run is random, not systematic (`_advance_cursor_and_emit_run`). Adjustments stay best-effort (analytics on user actions, not the join target).
