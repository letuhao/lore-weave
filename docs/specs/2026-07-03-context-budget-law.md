...
4. **Budget = pure fraction-of-window, no absolute cap.** `surface_max = 0.75×window`, `floor = 0.1×window`; **soft target triggers compaction, hard ceiling = model window (never exceeded).** *(Revised 2026-07-07 twice: v1 `min(32K, 0.35×window)` went flat above a ~91K window; v2 `min(200K, 0.75×window)` just moved the same flat-cap bug to ~267K — any model bigger than the cap got the identical target regardless of how much larger its real window was, which a user hit directly with a 1M-context model. The absolute cap is dropped entirely: both `floor` and `surface_max` are now pure fractions of whatever `context_length` compute_target is called with, so a 1M window correctly gets floor=100K/surface_max=750K instead of being clamped to a 200K-window's numbers. Industry comparables (Claude Code ~83.5%, general agent-harness guidance 50-80%) also scale purely off the actual window, not a separate flat ceiling. `0.75×window` matches this codebase's own pre-existing flat-mode fallback trigger (0.75×window, see stream_service.py).)*
