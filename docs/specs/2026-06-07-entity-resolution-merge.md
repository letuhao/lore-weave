...
6. ✅ **VERIFY — cross-service live smoke PASSED** *(2026-06-07, HEAD da82ab14; DEFERRED 062 CLEARED)*: rebuilt glossary+knowledge → migrations created merge tables live → R5 merge via gateway (owner JWT, 玉虛宮←蓬萊) 200 → glossary loser soft-deleted + journal + `glossary.entity_merged` outbox → relay → knowledge consumer → KG `merge_entities` (loser node gone, **0 orphans**, winner absorbed edges+aliases) + `entity_alias_map` project-scoped (MED-2 confirmed) → un-merge revert 200 (loser restored). KG `op=unmerged` is best-effort no-op by design (§3.4).

**mui #1c is COMPLETE** — G-merge · K-sync · G-cand · K-detect · FE built + reviewed, and the end-to-end cross-service merge loop is **live-smoke verified** on the real stack.
