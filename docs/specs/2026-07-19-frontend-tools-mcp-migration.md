...
- **D2 — Adoption → REVISED 2026-07-19 (SP-0 evidence overturns the original).** ~~Adopt beta v2 SDKs now~~ — **the betas are NOT needed.** SP-0 spiked each SDK in isolation and found the human-gate primitive (**elicitation**) already ships in ALL current **stable** SDKs (Python `mcp` **1.28.1** — already installed & running in chat-service; Go `go-sdk` **v1.6.1**; TS `@modelcontextprotocol/sdk` **1.29.0**), and the **Tasks** extension already ships in Python stable (experimental namespace); `chat_suspended_runs` covers durability. The v2 betas add only stateless-core/MRTR/auth-hardening — orthogonal to Phases 1-2. Also `mcp==2.0.0b1` never existed (PyPI latest is 1.28.1). **DECISION: build Phases 1-2 on current STABLE SDKs; do not adopt betas.** Optional hygiene: pin chat-service `mcp>=1.28,<2`. Full matrix: build-plan doc §"SP-0 CAPABILITY MATRIX".
