...
- **D2 — Adoption → DECIDED.** **Adopt beta v2 SDKs now** (dev phase): Python `mcp==2.0.0b1`, Go `go-sdk@v1.7.0-pre.1`, TS `@modelcontextprotocol/server`+`client@2.0.0-beta.1`; pin exact versions; move to stable at **2026-07-28**. Gate on **elicitation + MRTR**; `chat_suspended_runs` is the durable layer; adopt native **Tasks** once **SP-0** confirms it.
