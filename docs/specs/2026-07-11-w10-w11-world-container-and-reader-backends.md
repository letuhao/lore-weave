...
- **Per-read wiring (all run as `user_id=str(owner)`):**
  ...
  - **Cast list** — `list_entities_filtered` (`entities.py:1957`) has NO window (returns full cast by name). GOTCHA: an entity first introduced AFTER the cutoff still appears by name (mild spoiler). For `lore_browse_entities`, drop entities whose windowed status is unknown/`from_order > before_order`, OR document the mild-spoiler-by-name as accepted.
