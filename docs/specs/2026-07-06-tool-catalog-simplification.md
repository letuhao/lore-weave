...
1. **CAT-4 + Part A schema/filter layer — DONE 2026-07-06.** Shipped this pass:
   ...
   - **Remaining wiring (small, next slice):** inject `group_directory_text()` into the actual system-prompt assembly in `stream_service.py` (the schema/filter side is done; the plain-text directory itself isn't yet riding the prompt).
2. **Part C+D (glossary upsert/delete Go handlers + manual-injection settings/FE)** — NOT yet built. Scoped and de-risked (§6 has exact schemas, §8 has edge cases resolved, the eval proved the schema design works), but the Go implementation reuses substantial existing per-level core logic (`createBookGenreCore`/`createBookKindCore`/`createBookAttributeCore`, `resolveBookPatch`, the confirm-token mint/redeem spine) that deserves its own focused build+test pass rather than being rushed alongside the discovery-layer work above. Touch list when picked up: new Go tool handlers dispatching to existing cores by `scope`+level+`base_version`-presence; the `pinned_legacy_tools` per-session setting (routers + FE); any FE surface naming the old 6 tools directly.
3. **Verify (for step 2 when built):** unit tests per new tool covering every §8 resolution + a test proving manual-pin activation surfaces the tool's schema next turn (SET-5) + cross-service live-smoke + a before/after token measurement on the book-scoped scenario the investigation doc used.
4. Composition/knowledge/translation unification are separate follow-on specs once this pilot's pattern is validated in production use — not committed to in this spec.
