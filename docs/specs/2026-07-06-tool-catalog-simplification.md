...
5. **Verify — DONE 2026-07-06, live against the real stack (rebuilt + restarted chat-service, ai-gateway, glossary-service).**
   - **Metadata passthrough, real wire:** `tools/list` against the real glossary-service `/mcp` confirms all 7 legacy-tagged tools (6 original + `glossary_propose_new_entity`, see item below) carry `_meta.visibility:"legacy"`, and both consolidated tools (`glossary_ontology_upsert`, `glossary_propose_entities`) are `discoverable` — Go→ai-gateway→chat-service `_meta` passthrough proven live, not just unit-tested.
   - **CAT-4 discovery ranking, the eval's exact queries, real 190-tool federated catalog:** re-ran "add a new kind to the book" / "create a genre" / "make a new attribute type" (the exact wording `docs/eval/tool-catalog-comprehension-2026-07-06.md` used) against the LIVE catalog. **Found a real bug the small offline eval catalog couldn't surface**: `glossary_ontology_upsert` didn't even place in the top 5 — the fuzzy-rescue branch's own docstring says it "rescues a tool with NO token overlap," but the code never checked that precondition, so an EXACT single-token overlap (ratio=1.0 for identical strings, e.g. both intent and an unrelated tool's synonym containing the word "book") auto-qualified as a "strong fuzzy hit" and overrode the whole score to 1.0 — at real catalog scale, dozens of unrelated tools share SOME generic word with any given intent, drowning out genuine matches. Root-caused, fixed in both `tool_discovery.py` and `find-tools.ts` (gate the fuzzy branch on `overlap == 0`, matching the already-written comment), 4 new regression tests (2 Python + 2 TS) proving both the fix and that the genuine near-miss-spelling rescue it was designed for still works. Re-verified live post-fix: `glossary_ontology_upsert` now ranks **1st** for all three queries.
   - **Token measurement, live:** book-scoped hot-seed (glossary+story domains) measured against the REAL current 190-tool catalog: raw hot_tool_names (post-CAT-4, pre-budget) ≈ 10,317 tokens; after the token-budget mechanism (`dda88c0dd`, cap 4,000) ≈ 3,930; + `group_directory_text()` 188 tokens ≈ **~4,118 tokens total**, vs the investigation doc's original ~24,000-token baseline — an **83% reduction**, live-confirmed.
   - **`glossary_propose_entities` cross-service live-smoke:** real `tools/call` against the real running stack + the real POC book (`019f1783-ebb4-…`, an actual multi-chapter book with a built glossary, not a throwaway fixture) — created 2 draft entities in one batch call, verified by effect via direct DB read (`status='draft'`, `tags={ai-suggested,assistant}`), then cleaned up (test-only entities, not left as product data).
   - **Full regression suites re-run after the fuzzy-rescue fix:** ai-gateway 139/139, glossary-service all packages green. Chat-service: 1082/1086 — the same 4 failures reproduce byte-identical WITH and WITHOUT the fuzzy-rescue fix (verified via a direct A/B file swap), confirming they're pre-existing test-isolation flakiness in an unrelated resume/admin-surface test cluster, not a regression from this work.
