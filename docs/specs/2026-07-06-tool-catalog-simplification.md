...
3. **Remaining (next slice):** the `pinned_legacy_tools` per-session manual-injection setting (routers + FE), `group_directory_text()` injection into the live system prompt, and any FE surface naming the old 6 tools directly.
4. **Verify (remaining):** cross-service live-smoke of the new tools through the real chat agent + a before/after token measurement on the book-scoped scenario the investigation doc used.
5. Composition/knowledge/translation unification are separate follow-on specs once this pilot's pattern is validated in production use — not committed to in this spec.
