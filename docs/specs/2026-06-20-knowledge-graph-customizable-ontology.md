...
### 3.2b Expected node-kinds + adopt strength (M1, gap S0)

Bộ node-kind template **kỳ vọng** (anchor glossary, soft) nay là **bảng con có structure** (không còn chỉ là chú thích), để mang `strength` cho adopt-gate per-kind (M1 🔒 LOCKED S0):

```
kg_schema_node_kinds
  schema_node_kind_id  uuid PK
  schema_id            uuid FK kg_graph_schemas   -- kế thừa scope của cha
  kind_code            text          -- mã kind glossary (soft ref): 'character', 'concept', ...
  strength             text  CHECK (strength IN ('required','optional'))
                                     -- required => adopt BLOCK nếu glossary thiếu kind (deep-link fix);
                                     -- optional => adopt proceed; mismatch lúc extraction → unknown_node_kind triage (§11)
  deprecated_at        timestamptz NULL
  UNIQUE (schema_id, kind_code)
```

> Adopt-gate (LC, §11/M1): resolve nguồn kind (book ontology nếu có `book_id`, else user glossary standards — §3.5); với mỗi `required` kind vắng mặt → 422 `needs_glossary{kinds:[...]}` deep-link; `optional` kind vắng → cho adopt, warn. Bảng này tham gia adopt deep-copy + tree-sync như edge/fact/vocab.
...
- **Node kinds kỳ vọng** (anchor glossary, soft) + **adopt strength** (M1 🔒 LOCKED S0, §3.2b):
  - `required` (adopt BLOCK nếu glossary thiếu): **character, organization, location, concept, technique**.
  - `optional` (warn + triage): **item, event, relationship**.
...
- **Fact/state types** narrative (thay bộ chat-memory) — **🔒 LOCKED S0, 9 type:** `realm_change`, `allegiance_shift`, `motivation_shift`, `death`, `breakthrough`, `battle_outcome`, `betrayal`, `bloodline_awakening`, `oath_or_vow` (rút từ event_type §3 VCTĐ `battle/betrayal/breakthrough/death/political` + state-delta + drive arc).
...
- **M1 (≙ §10-G1, CRITICAL) — reconciliation node-kind giữa glossary↔KG. 🔒 LOCKED 2026-06-20 (S0, revised).** Adopt template KG khi book glossary CHƯA có kind tương ứng thì xử sao? **CHỐT: adopt-gated PER-KIND STRENGTH** — mỗi expected node-kind của template mang `strength ∈ {required, optional}` (bảng con `kg_schema_node_kinds`, §3.2b). Adopt **BLOCK** nếu thiếu một kind `required` ("hãy adopt các kind này trong glossary trước" + deep-link); thiếu một kind `optional` → **warn + park `unknown_node_kind` triage** lúc extraction (không block adopt). Không cross-service write ngầm. **Refine (gap draft-01):** project **no-book** → nguồn node-kind = glossary standards của user (§3.5); adopt-gate khi blocked **deep-link** sang glossary rồi re-check (idempotent). Strength split cho `xianxia-harem` (§4): required = `character, organization, location, concept, technique`; optional = `item, event, relationship`.
- **M2 (≙ §10-C4, HIGH) — seam partition. 🔒 LOCKED 2026-06-20 (S0).** **CHỐT:** `graph_id` đặt **trên EDGE** (node dùng chung); "view→partition" KHÔNG phải continuum mà là feature riêng.
- **M3 (≙ §10-B4, HIGH) — schema versioning + edit policy. 🔒 LOCKED 2026-06-20 (S0).** **CHỐT:** `schema_version` stamp lên edge/fact; schema edit **additive** (thêm type OK; rename/remove = deprecate-only). **Refine (gap draft-06):** **sync-apply đổi RULE going-forward, KHÔNG retro-recompute data** mặc định; recompute là hành động **opt-in riêng** (dry-run + count), không nằm trong sync-apply.

### Còn lại

1. **Đa-schema-active per project**: v1 chốt "một project-schema active sau merge". Có cần adopt nhiều template song song (vd xianxia-harem + một template mystery) ngay v1 không, hay để lớp 4? *(liên đới Q5 — vocab collision, §10-F2)* — **🔒 v1 = một active (LOCKED S0).**
2. **Free-edge policy**: **🔒 LOCKED 2026-06-20 (S0): `allow_free_edges=true` mặc định** (giữ free-string predicate hôm nay; siết theo `kg_edge_types` là opt-in per-schema). Tránh vỡ project cũ.
3. **Fact/state types narrative**: **🔒 LOCKED 2026-06-20 (S0).** `xianxia-harem` seed 9 fact-type: `realm_change, allegiance_shift, motivation_shift, death, breakthrough, battle_outcome, betrayal, bloodline_awakening, oath_or_vow`. `general` giữ bộ hiện tại.
4. **Grant-level cho schema write trong project shared**: **🔒 LOCKED S0:** mirror **Manage-gate** glossary; view **per-user** (`UNIQUE project+user+code`).
5. **Drive vocab dùng chung hay per-template**: **🔒 LOCKED S0: v1 per-template** (`drive` trong `xianxia-harem`); shared system-level drive để lớp sau (liên đới Q1).
...
**Quyết định (M1) — 🔒 LOCKED 2026-06-20 (S0, revised to per-kind strength):** **adopt-gated PER-KIND STRENGTH.** Mỗi expected node-kind của template mang `strength ∈ {required, optional}` (§3.2b `kg_schema_node_kinds`). Adopt **BLOCK** khi thiếu kind `required` ("adopt các kind X,Y trong glossary trước" + deep-link); thiếu kind `optional` → **warn + park `unknown_node_kind` triage** lúc extraction, KHÔNG block. Không cross-service write ngầm. (a=auto-adopt kind sang glossary — cần internal route write, coupling cao, loại; thuần-advisory toàn bộ — kind structural thiếu vẫn cho adopt → edge core không anchor, loại.) Per-kind strength = trung dung: gate kind structural, warn kind enrichment → reuse triage machinery (§11) thay vì block cứng. Cần glossary internal ontology read (D1) để KG kiểm tra lúc adopt.
