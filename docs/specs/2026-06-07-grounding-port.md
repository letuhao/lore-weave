...
3. **K-adopt** — knowledge uses the SDK verifier in its precision filter (persist structured `VerifyFlag`s instead of ephemeral keep/drop) and/or emits `GroundingCite`s from L3 passages. **← NEXT**
