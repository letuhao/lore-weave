...
2. ✅ **LE-migrate** *(DONE 2026-06-07)* — lore-enrichment's `canon_verify.py`/`sanitize.py`/`regurgitation.py` are now thin **re-export shims** over `loreweave_grounding` (the `CanonVerifier` shim keeps the legacy `read_port=` ctor arg, accepted+ignored). **Parity proven: lore-enrichment's OWN suite green against the shims — 844 unit tests pass** (incl. all verify-trio consumers: fabrication/recook/draft_expand/wiring/writeback). `/review-impl` MED-1 (only-3-files-run coverage gap) closed by the broader run; the 5 env-only failures don't import the shim. **DEFERRED: the grounding-compose migration** (`GroundingRef`→`GroundingCite`) — `GroundingRef` is persisted in proposals' `source_refs_json`, so it needs a data-shape migration, not a thin shim (tracked follow-up D-GROUNDING-COMPOSE-MIGRATE).
3. ✅ **K-adopt** *(DONE 2026-06-07)* — the verifier/precision-filter framing didn't fit (precision filter = extraction-faithfulness, not canon-consistency); the real consolidation was knowledge's **injection-defense**, a narrower ancestor of the SDK's. `app/extraction/injection_defense.py` is now a shim over `loreweave_grounding.sanitize` — knowledge GAINS base64/zero-width/full-width/classical-Chinese/Korean detection (security upgrade), keeping the `(str,int)` contract + per-pattern metric + clean-text-unchanged (no NFKC of stored text). **126 tests green** (38 injection + 45 pass2_writer + 43 regenerate_summaries). `/review-impl`: 2 LOWs accept+documented (regen-summaries now rejects obfuscated-injection summaries = security win w/ small FP on classical narrative; Prometheus `pattern` label set renamed/added → Grafana ops note).
4. ⏸️ **C-adopt — DEFERRED** *(PO 2026-06-07, DEFERRED 064)* — investigation found NO clean/valuable slice: composition's `app/packer/sanitize.py` `neutralize` is structurally divergent from the SDK (uses a `⟦…⟧` marker not `[FICTIONAL] `; **escapes angle brackets** `<`→`＜` to protect its `<canon>`/`<guide>` assembly delimiters — and the SDK's NFKC prenormalize would fold `＜` back, **undoing** that defense), so adopting it would be a behavior change with real risk for little gain. The `GroundingCite`-emission framing is net-new (composition packs a prompt, not per-item cites) with **no downstream consumer**. mui#3's value is banked by the shared SDK + the two services with mature/alignable logic (lore-enrichment verify trio, knowledge injection-defense) unified. Revisit if a generation-side `GroundingCite` consumer appears or composition's sanitize is rebuilt.

**mui#3 substantially COMPLETE: SDK (G3-SDK) + LE-migrate + K-adopt done; C-adopt + the compose-migration (063) deferred with reasons.**
