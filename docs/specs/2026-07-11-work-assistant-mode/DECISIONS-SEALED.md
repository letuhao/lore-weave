...
| T-4 | assistant-session discriminator | **An explicit `chat_sessions.session_kind` column.** Deriving from `book_id`=diary is fragile and three consumers need it (day-window read · voice gate · search scoping) · **⚠️ AMENDMENT (2026-07-12, D-R15 — awaiting human ratification):** WS-1.8 shipped the `book_id`=diary derivation instead (the day-window read filters `s.book_id=$diary`); NO `session_kind` column was added. Rationale: `book_id` already exists + is stamped by WS-1.4 provisioning, and all three named consumers resolve off it (an assistant session's `book_id` is ALWAYS the diary; a non-assistant session's is a novel or NULL). Spec 02 §Q1 itself offered `book_id` as an option ("decide in PLAN"). This is a defensible alternative, but it OVERRODE this seal without prior amendment — flagged for the human to RATIFY (keep `book_id`) or REVERT (add the `session_kind` column). Stale downstream specs still say `session_kind`: `06-journal-distiller.md` §Q10 + `01-data-architecture.md` §"assistant session". |
