...
| T-4 | assistant-session discriminator | **An explicit `chat_sessions.session_kind` column.** Deriving from `book_id`=diary is fragile and three consumers need it (day-window read · voice gate · search scoping) |
