...
| 2 | **T3 `Compiler` class** | Mechanism complete (`build_system_message` + `compact_messages` + `Planner` seam, 2 consumers); only a *named* `Compiler` wrapper is absent — cosmetic | **CONSCIOUS WON'T-FINISH** (record) |
| 3 | **D-T1-SMALLRETURN-ENFORCE** | `@small_return:` is a self-report comment on ~13 tools across 3 services; the snapshot harness asserts no per-tool size budget | **DEFER → A5** (real fix = runtime byte-histogram; proxies are theater) |
