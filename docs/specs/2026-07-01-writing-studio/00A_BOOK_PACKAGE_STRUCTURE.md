...
| **DA-12** ✱ | **Nothing may infer narrative structure from `parts`.** `chapters.part_id` (physical) and `outline_node.structure_node_id` (narrative) are independent axes. | `parts` is importer output (`path`, `parse_version`). A folder layout must never dictate a story's arcs. | **BPS-9** (guard-rail on the parked question) |
...
| # | Question | **Decision** | Rationale |
|---|---|---|---|
| **BPS-9** | [`23`](23_book_architecture.md) OQ-4 / BA14 — `parts` (physical) vs `saga` (narrative) | **PARKED by the PO (2026-07-10), now with a guard-rail: DA-12.** No relation is added; `chapters.part_id` and `outline_node.structure_node_id` remain independent axes, and **nothing may infer narrative structure from `parts`.** | Parked ≠ undefended. Without DA-12 the next agent "helpfully" makes `parts` the saga level and an importer's folder tree silently becomes the book's arc structure. |
