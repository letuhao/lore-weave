...
| 13 | **Glossary dockable migration** — cycle-2 of the queue in #12, re-scoped as shared-foundation-then-fanout (7 sub-features + 3 DOCK-standard violations found: DOCK-8 page-replacement, DOCK-9 6× hand-rolled modal, DOCK-10 no entity hoist). **Phase A ✅ built** (entity hook + `loreweave.glossary-entity.v1` JSON provider + `glossary` list panel, palette+agent-openable + 1 modal precedent (`ResolveKindModal`→`FormDialog`) + Lane-B wiring + 1 route-defect fix). Phase B (parallel fanout, not started): 4 remaining panels (ontology/unknown/suggestions/merge) + 4 remaining modal migrations | [`13_glossary_panels.md`](13_glossary_panels.md) | 🟡 Phase A built |
| 14 | **Knowledge/KG dockable migration** — larger than #13 (~4,300 lines, 2 route-driven tab hubs collapsing to ~13 unique panels; 3 genuine DOCK-9 hand-rolled overlays [1 grep hit reclassified as an accepted popover pattern, not a violation], 3 DOCK-7 route-couplings, no book→project hoist; global cross-book hub migrates too, as user-scoped panels per the human's K1 call). Phase A (serial): `useBookKnowledgeProject` hook + `knowledge` hub launcher panel + `VersionsPanel` DOCK-9 fix + Lane-B wiring. Phase B (parallel fanout): 12 remaining panels (overview/entities/timeline/evidence/gap/proposals/schema-bundle/graph/insights/jobs/bio/privacy) | [`14_kg_panels.md`](14_kg_panels.md) | 📐 specced |
| … | Wiki · Translation · Reader/Compare (see #12 cycle queue) · Search nav · Quality nav · Jobs/Generation/Issues bottom panels · Planner/Cast/Timeline/… | — | ⏳ |
