...
Every row is decided here or explicitly PO-DECIDE with a recommended default.

| # | Question | Disposition |
|---|---|---|
| OQ-1 | **Checkpoint classes** — blocking at 2 + 4, advisory elsewhere? | **PO-DECIDE.** Recommended default = BPS-19b as stated (blocking 2 + 4; `auto_approve` arg for autonomous runs). BPS-19 itself flags "(b) is decided on a stated principle; the PO may override." |
| OQ-2 | **`genre_tags` source** (BPS-20's open sub-question) | **PO-DECIDE.** Recommended = PF-15: explicit optional field on run create, persisted on `plan_run`, omitted ⇒ `[]`. Alternatives considered and disliked: `spec.meta` (frozen `additionalProperties:false` contract + unreliable to mine), a per-book setting (a hidden default the run never surfaces — the settings-boundary bug class; can layer on later as a fallback without breaking the explicit arg). |
