...
|---|---|---|
| OQ-1 | Should a parse failure block publish? | **PO-DECIDE — recommended default: NO** (publish proceeds; index marked stale; sweeper heals — IX-2). Rationale: prose availability beats index freshness for a derived artifact, and the failure is visible (marker + `index_stale` reason), not silent. The alternative (publish 500s on a parser bug) holds user prose hostage to infrastructure. |
| OQ-2 | Should the scene browser index **drafts** (debounced re-parse on save) or canon only? | **PO-DECIDE — recommended default: canon-only in v1** (IX-1). Draft divergence is already visible via ④. Draft-indexing re-opens F7's split, needs debounce machinery, and serves only browsing-before-publish — revisit on a real user request, not speculation. |
| OQ-3 | How loudly does the Hub surface dirtiness? | **PO-DECIDE — recommended default: per-arc badges + one `stale_chapter_count` rollup line; no book-level banner.** A global banner for a normal authoring loop (edit → publish → eventually re-run conformance) trains users to ignore it. |
