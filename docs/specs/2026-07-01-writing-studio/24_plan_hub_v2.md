...
> **Status:** 📐 DRAFT (multi-agent authored 2026-07-10, adversarially reviewed) — for PO review
...
2. The initial camera's arcs (PO-DECIDE-2) expand → chapter windows (call 2) page in keyset order; scenes page in only for expanded chapters.
...
## View modes — data contracts only (PH22; visuals PO-DECIDE)
...
### Phase H7 — view modes (PH22) — after PO-DECIDE-1; timeline then worldmap, each its own cycle
...
|---|---|---|
| OQ-1 | **Timeline/worldmap visual treatment** (mandated PO-DECIDE). | **PO-DECIDE.** Recommended default: v1 ships narrative mode only; the two mode buttons render disabled with a "coming" tooltip (visible, honest — PH7's spirit). When built: timeline = per-track swimlanes with `story_time` captions; worldmap = location clusters with transition edges. Data contracts are locked either way (PH22). |
...
| OQ-5 | Initial camera on open — whole-book fit vs the working scope? | **PO-DECIDE.** Recommended default: when opened from an editing context, focus + expand the arc containing the editor's active chapter (the `m1b-working-scope-boost` lesson applied to the canvas — the author's working set is where attention goes); otherwise fit-whole-book with all arcs collapsed. |
| OQ-6 | Does OutlineTree's indented-list view survive as a toggle inside the Hub? (`21` explicitly deferred this to ask the user.) | **PO-DECIDE.** Recommended default: **no toggle in v1** — the Plan navigator rail (PH25) *is* the list rendering of the same data; a third in-panel list duplicates it. `OutlineTree.tsx` stays where it is until its retirement cycle. |
| OQ-7 | What does the toolbar's primary **"Ask AI about this plan"** button do in v1? | **PO-DECIDE.** Recommended default: opens the Compose chat with the current selection's ref pre-filled as context (existing surface, zero new contract). The dedicated plan-agent affordance (highlight-my-pending-edit on the canvas) remains Phase 4 / PH8 — it needs the `resource_ref` convention, which [`28`](28_agent_native_studio.md) (authored 2026-07-10) does **not** yet spec — that convention remains open contract work gating Phase 4 — and after `27` links PlanForge output the button finally has a real spec object to talk about (BPS-17/18). |
