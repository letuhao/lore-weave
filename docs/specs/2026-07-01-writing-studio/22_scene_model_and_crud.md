...
| **SC10** | **Two panels, not one.** `scene-browser` (book-wide table: filter/sort/group, bulk-act) and `scene-inspector` (single-scene full-field editor, folding in `scene_link` + `scene_grounding_pins`). `SceneRail` **stays** as the chapter-local strip beside the editor. | Same DOCK-8 reasoning as [`15_chapter_browser.md`](15_chapter_browser.md): tree/strip-for-writing vs table-for-triage-at-scale are different jobs. The inspector is a *detail pane over a selection*, not a third capability. |
...
> `word_count` backfill in [`15_chapter_browser.md`](15_chapter_browser.md) A1.
