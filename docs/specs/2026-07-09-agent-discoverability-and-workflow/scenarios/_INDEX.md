...
| [S03](S03-entity-triage.md) | Triage the entity inbox (approve / reject / merge / edit) | P2 Linh | write | P0 | no | 🟡 W3 authored 2026-07-11 — inbox visible (extraction DOES tag ai-suggested; baseline was stale data) + agent lists pile ✅; drain ❌ blocked by auto-mode cross-turn activation gap (tool_surface.py:341) — Track A mechanism | this session |
...
| **[S06 ★ FLAGSHIP](S06-flagship-idea-to-arc.md)** | **"I have a story in my head — help me write it"** — one long session: vision → world structure + cast + connections + arc plan + drafted chapter, all beneath the surface. **The front door; orchestrates S01–S05.** | P1 Mai | write | **P0 (ship target)** | yes (seed-doc→entities; glossary→KG nodes; long-session continuity guards) | 🔬 ❌ **re-test 2026-07-11** (baseline ❌ 2026-07-09) — 16/17 turns 0 tools; only the arc-plan lands (5 artifacts). Never calls the workflow: the rail fires on a REQUEST, not on an ASSENT to the agent's own offer ("yeah do it"). NEW: severe jargon leak (PlanForge ×27) — no rail ⇒ no vocabulary owner. Next: WS-3 offer→workflow binding + W6 vision-to-book. | this session |
