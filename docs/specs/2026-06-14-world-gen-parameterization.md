...
| **P8b ✅** | `RenderTheme` — all `render.rs` colours: per-style hypsometric ramps (`land_realistic/atlas`, `water_realistic/atlas` as fixed `[(f32,[u8;3]);N]`) + biome(14)/culture(16)/state(12)/boundary(7)/route(5)/settlement(6) palettes + plate base tints + tier borders + background + coast ink + choropleth sat/val + `supersample`. Stored as `[u8;3]` (serde/Copy-friendly; `render.rs` wraps in `Rgb`). Threaded through all 7 `*_image` fns + `political_svg` + `export::glb_globe`. **Added 8 render-hash pins first** (blake3 of seed-7 80×40 renders, both styles) as the byte-identical safety net — they hold. Render *math* (`relief.rs` hillshade/warp/detail + per-style `StyleParams`) stays fixed. The `BiomeKind`/`RouteKind`/`BoundaryKind`/`SettlementRole` methods that aren't render (cost tables etc.) are untouched. `/review-impl` clean — fix folded in: `ramp()`'s clamp is now order-tolerant (a descending-stop config would have panicked via `f32::clamp(min>max)`; now GIGO-safe), with a no-panic test. | L |
