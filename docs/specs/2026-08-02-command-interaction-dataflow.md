...
| **O-CI-16** | **Six types are named and never defined** — `ChanceSpec`, `InputKind`, arity's home, pair `subject`, two-role `EffectRow`, `RefKindMask`. Every consideration and every `success` row in a real manifest rests on an invented shape |
