...
### 3.2 · Position write — per-node PATCH with server-side merge + OCC
`PATCH /books/{book_id}/motif-graph/layout` — body `{ motif_id, x, y, if_version }`:
- **Server-side merge** (not a whole-blob overwrite): `UPDATE … SET positions = positions || jsonb_build_object($motif_id, …), version = version+1 WHERE owner_user_id=$caller AND book_id=$book AND version=$if_version` (upsert the row on first write). Per-node merge avoids blob races between two nodes dragged in quick succession.
...
- **Persist on drag END** ([SceneGraphCanvas.tsx:379-380](../../frontend/src/features/composition/components/SceneGraphCanvas.tsx#L379) pattern) — `onNodeDragStop` reads the cursor drop point (`screenToFlowPosition`), then a **debounced** (≈400ms) mutation `patchLayout({motif_id, x, y, if_version})`. Debounce the write (the XL upgrade over SceneGraphCanvas's persist-every-dragEnd) so a flurry of small nudges coalesces; the debounced write MUST bind its target `motif_id` + the latest `if_version` via a ref ([[debounced-write-must-bind-its-target-entity]]).
