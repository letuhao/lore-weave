...
| **5.4** | ✅ **BUILT + LIVE 2026-08-10.** The refusal now reads the DECLARED supplier: 266 missing-argument failures / 87 sessions, largest single case `book_read.book_id` at **78 calls / 46 sessions** — a CONTEXT value the runtime owes and the model was told to invent. **argument supplier** (23.7%) — every input declares model \| context \| plan | a `plan`-supplied input the model sends is **discarded** — CP-3.10 already does this; the contract makes it *declarable* rather than plan-only |
| **5.5** | ✅ **BUILT + LIVE 2026-08-10.** `call_outcome` stamped at the persistence chokepoint (`done`/`failed`) and **`deferred` at the site that suspends**; a `failed` fails closed to C-7's `terminal_permanent`; a message-less failure is marked countable. Live: a tier-W turn recorded `deferred`, un-inferred, no `error_class`, while the turn said `awaiting_input`. **error contract** — every failure carries a C-7 class **and a message**. 🔴 **THE DENOMINATOR IS SMALLER THAN §1 SAYS, MEASURED 2026-08-10 BEFORE BUILDING: 26 calls / 17 sessions (4.7%), not 41 / 29 (8.1%)** — because **15 of the 41 are not failures at all.** They are calls SUSPENDED awaiting a human (`task: {status: "input_required"}`, `confirm_action`, `glossary_adopt_standards`), recorded `ok:false` with no message because they are *waiting*, not broken. 🔴 **That is a second defect and a bigger one: `ok:false` is carrying two vocabularies — a call that FAILED and a call AWAITING INPUT — and nothing downstream can tell them apart.** Fixing only the message would give a suspension a plausible error string and make the conflation permanent | a failure with no message cannot be produced — **and a suspension is not recorded as a failure**. ⭐ **THE VOCABULARY ALREADY EXISTS AND HAS NEVER BEEN WRITTEN ONCE.** `observation.py` defines C-14's typed CALL outcome — *"which replaces `ok: bool`"*, nine members including **`deferred`** — plus C-7's `ERROR_CLASSES`, and an `Observation` type that already refuses a `failed` with no class. Measured over **7,990 recorded tool calls: `outcome` appears 0 times, `error_class` 0 times.** So 5.5 is not *"add a message to 26 calls"*; it is **wire the enum CP-1 already built**, which is the C-3…C-17 shape sitting inside the observation module itself. (The second `OUTCOMES` in `instrument.py` is **not** a drifted copy — it is the TURN vocabulary, which already has `awaiting_input`. Turn-outcome and call-outcome are two vocabularies that overlap only at `failed`.) |
| **5.6** | 🟡 **HALF BUILT.** 🔴 §1 calls completeness *"0% by construction"*; **measured, it is 88 truncated results across 87 SESSIONS** — one of the largest classes by the honest denominator, never counted. ✔ But *"the model is never told"* is not true as stated: `book_list` and friends return an explicit `page {total, returned, is_complete}`. **The real subject: of 36 paged tools, FIVE report completeness NEVER** — and one is `glossary_search`, the tool 5.3's resolver runs on, making it a CORRECTNESS bug: *exactly one exact match* is only true if the list was not truncated. Now refused as a distinct `truncated` outcome; a tool that declares `is_complete` is believed. ✅ **The `emits` half landed 2026-08-10:** `check_emit_against_contract` validates a plan's emit path against the DECLARED `output_contract.emit_paths` at plan-build time — §2's inversion turned back. A tool declaring nothing is not blocked; declaring EARNS the earlier error. 🔴 **Four of the first five declared shapes were WRONG** (written from tool descriptions: `book_list` is `{books, total}`, not `{items, page}`) and would have made this check reject the one path CP-3 uses. All five now come from RECORDED results and carry `_verified_against` | **output contract + completeness** | 🔴 an `emits` path is checked at **plan-build** time; a truncated result is a **declared field the runtime must surface** |
| **5.7** | ✅ **CLOSED 2026-08-10 — CODE · LIVE · DATA.** LIVE from driven traffic: 25 sessions produced a real emergent loop (`glossary_search` 3x identical), recorded `refused` / `repeated_read` / `repeat_count: 3`. 🔴 **2,189 of 4,181 recorded failures (52.4%) are our own BREAKER prose, not a tool failing.** The breaker ALREADY removes the cost (it short-circuits before dispatch), so nothing is cached — deliberately, since `read_call_results` holds a fingerprint and a count, not a body. What was wrong was the SIGNAL: a refusal typed as a failure. Now `call_outcome: refused` + `refusal_kind` + `repeat_count`, count still rising, breaker still escalating. ✖ Live not driven: **a compliant model will not loop on instruction** — a 566-repeat session is emergent — so the row stays open until one is observed | a cached repeat is still counted and still escalates |
| **5.8** | ✅ **CLOSED 2026-08-10 — and the EXIT WAS REWORDED to get there (PO decision).** 🔴 **414 calls / 82 sessions — the largest remaining population — and every failing tool ALREADY DECLARES what it needs** (`_meta.scope`: 194 book · 65 user · 33 project · 23 none), consulted by nothing. A project-scoped tool with no project in scope is now refused BEFORE the wire, naming `kg_project_list`/`kg_project_create` as the way out. **Gated on `project` ONLY** — `scope: book` is a scope KEY, not a precondition (`book_list` is itself `scope: book`), and the project bootstrap tools are `scope: user`, both verified before building and both guarded. 🔴 **The §4.3 ADVERTISEMENT gate is REFUSED ON EVIDENCE.** `tool_defs` is computed ONCE PER TURN (outside the pass loop), and **143 of 154 turns that create a project use a project-scoped tool in the SAME turn (93%)** — so gating advertisement would leave the model unable to touch the project it just made, silently, for the rest of the turn. The pre-dispatch check has no such failure mode. ⭐ **PO 2026-08-10: §4.3's advertisement clause is WITHDRAWN as unsafe on this surface; the exit is pre-dispatch ONLY.** A clause that cannot be satisfied without stranding 93% of a journey is not a bar the work failed to clear — it is a bar in the wrong place, and rewording it on the measurement is the honest resolution rather than leaving the row open forever against it | **preconditions** (18.7%) — checked **pre-dispatch** | a tool whose precondition is unmet is **refused before dispatch**, naming the state it needs and how to obtain it. ~~and used to gate advertisement (§4.3) — WITHDRAWN 2026-08-10~~ |
...
| **5.11** | ✅ **AUDITED + CLOSED 2026-08-10 — and the catalogue answered it itself.** Both federated candidates carry **`visibility: legacy` AND `superseded_by` in their own `_meta`**: `composition_write_prose → book_chapter_save_draft` (tier A), `composition_get_prose → book_get_chapter` (tier R). agent-registry's own migration calls `composition_write_prose` *"a DEPRECATED, discovery-hidden thin proxy"* and names `book_chapter_save_draft` as the canonical chapter-write tool. **The real co-writer path is `compose_prose`** — chat-service-local, run inline by `stream_service` against the composer model (`is_composer_tool`) — **with the write landing through `book_chapter_save_draft`, which is already in the set.** So `compose_prose` is admissible and the two proxies are dead by their own declaration. ⭐ **The derivation now READS that declaration**: a tool carrying `visibility: legacy` or `superseded_by` can never enter the essential set, so this class of error cannot recur from a list kept by hand | ~~audit before admitting~~ ✅ |
| ~~5.11 (original)~~ | ⭐ **PO 2026-08-10: audit the compose role BEFORE admitting either implementation.** `compose_prose` is chat-service-LOCAL (2 sessions, 4 calls, 100% ok) while `composition_write_prose` / `composition_get_prose` sit FEDERATED and completely unused (0 sessions, 0 calls). Two implementations of the same journey step, one of which may be dead. **Admitting the wrong one would contract-govern a dead path** — the `glossary_propose_entity_edit` failure inverted: not a name the model invented, but a real registered tool nobody calls | the real co-writer path is identified **from evidence**, the other is recorded as dead-or-deprecated with a reason, and only then is one admitted. Blocks `compose_prose`'s admission, not the other ten |
| **5.10** | ✅ **CLOSED 2026-08-10 — CODE · LIVE · DATA, on a REWORDED exit.** 🔴 **MISREAD BY ME, CORRECTED:** `glossary_propose_entity_edit` (101 calls / 12 sessions / 0%) was a **REAL tool, since removed** — 96 of 101 returned its own validation prose and ZERO returned unknown-tool. A retired tool still being called is a SUPERSESSION defect, not an invented name. 45 driven sessions produced zero invented names. **LIVE is `CANNOT DETERMINE`, computed:** genuine inventions are 2 of 363 sessions (0.55%), so 45 sessions expect 0.25 and P(zero)=0.78; **543 sessions** are needed for a 95% chance of one. Rule of three bounds the rate at **≤6.7%**. Not drivable — the mechanism is deployed and awaits a real occurrence. An undispatchable name is now refused BEFORE the wire, typed `refused` not `failed`. 🔴 The first measurement reported **17 phantoms** — but `workflow_load` (100% ok), `chat_search_sessions`, `run_subagent` and others are REAL consumer-local tools: **a catalogue excluding what chat-service implements manufactures phantoms**, the `compose_prose` error again. The check uses the turn's own indexes, guarded against a snapshot | **registry is the only name source** (10.1%) | ⭐ **EXIT REWORDED BY PO DECISION 2026-08-10, on the same grounds as 5.8's** — the original bar (*"`glossary_propose_entity_edit` … cannot be dispatched"*) rests on a case that turned out to be a **RETIRED tool, not an invented name**, and the genuine population is **0.55% of sessions**, which no driven run can clear (~543 sessions). A bar a 0.55% event cannot reach is a bar in the wrong place. **Reworded to what is verifiable, and verified:** the check is **deployed**, it **refuses an undispatchable name before the wire** with a suggestion drawn from the turn's own indexes, it **fails OPEN when the catalogue is unknown** so an outage cannot make it refuse everything, and it **provably does not refuse real tools — 353 real calls across 41 sessions and 15 distinct tools, ZERO false refusals**. The positive branch's first real occurrence will be recorded when it happens |
