...
| `single_call` | ~−62% in | unmeasured at scale | **measure next** — it isolates the delta instruction |
| `single_call_delta` | **−65.5%** | **−38.4%** | not a default; a deliberate trade for a cheap first pass |
| `edc_cited` (A9) | −54.3% in | **+63% new/ch, no kind abandoned** at POC scale | **the most promising, and unmeasured at book scale** |
