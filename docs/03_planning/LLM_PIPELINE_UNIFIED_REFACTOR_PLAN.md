...
| **book-service** | Unified gateway only (image_gen + audio_gen) | `media.go` ✅ Phase 5e-β.1; `audio.go` ✅ Phase 5e-β.2 | ✅ migrated | OK |
| **glossary-service**, **auth-service**, **catalog-service**, **sharing-service**, **usage-billing-service**, **api-gateway-bff** | No LLM calls | — | — | — |
...
| **5b** | Migrate chat-service voice STT/TTS off `/internal/proxy/v1/audio/*` onto the unified gateway. **Final shape (not the M estimate):** scope expanded during CLARIFY to add a **bytes-mode multipart STT submit** on `/v1/llm/jobs` so chat-service skips MinIO+presigned URL roundtrip; SDK `transcribe(audio: str\|bytes\|bytearray\|memoryview)` polymorphic dispatch; 3 NEW SDK audio error classes (`LLMAudioTooLarge`/`LLMAudioFetchFailed`/`LLMAudioURLDisallowed`); audio paths now in `isDeprecatedProxyPath` deny-list (410-Gone); 9 proxy_integration_test.go placeholder-using tests rewritten to synthetic `v1/responses` path (preserves regression-locks per /review-impl Fix #7,#8). **✅ shipped — design doc [LLM_PIPELINE_PHASE5B_DESIGN.md](./LLM_PIPELINE_PHASE5B_DESIGN.md); BUILD: openapi +SubmitSttBytesRequest+multipart variant on both /v1/llm/jobs and /internal/llm/jobs; adapters.go +AudioBytes/+ContentType +ErrTranscribeInputInvalid; openai_audio.go exactly-one pre-check + bytes branch; worker.ProcessAudioInline goroutine-closure handoff (no DB persistence); jobs_handler mime.ParseMediaType dispatch + http.MaxBytesReader cap + multipart parse + audio field-name diagnostic; SDK polymorphic transcribe + _submit_stt_bytes + new audio error classes routed via from_code on HTTP 4xx; chat-service voice_stream_service.py uses SDK; dead provider.resolve() removed + 3 grep-lock tests. Verify: gateway go test ALL GREEN; SDK pytest 174 pass (was 167; +7); chat-service pytest 180 pass (was 177; +3 lock tests). /review-impl on DESIGN found 3 HIGH+7 MED+5 LOW+1 COSMETIC all fixed inline.** | XL (was M) |
| **5c-α** | `image_gen` adapter + Python SDK + openapi. Async submit→poll via `POST /v1/llm/jobs operation=image_gen` (matches Phase 5a STT). Caller-side download (matches chat-service voice; gateway returns URL or b64_json in result). Multi-image n=1..4. Both `response_format=url` AND `response_format=b64_json`. 2 NEW sentinels (ErrImageContentPolicy + ErrImageGenerationFailed + ErrImageInvalidParams; 3 total counting invalid-params), 2 NEW SDK exception classes (LLMImageContentPolicy + LLMImageGenerationFailed), 2 NEW consts (MaxImagesPerJob=4 + MaxImageResponseBytes=8MB). JSON-first content-policy detection (avoids prompt-echo false-positive per /review-impl(DESIGN) MED#3). Adapter-level invariant pre-checks (belt-and-suspenders matching Phase 5b's MaxAudioBytes). **✅ shipped — design doc [LLM_PIPELINE_PHASE5C_DESIGN.md](./LLM_PIPELINE_PHASE5C_DESIGN.md); BUILD: openapi +ImageGenInput+ImageGenResult+ImageGenDataItem; adapters.go +GenerateImage method + types + sentinels + consts; openai_image.go NEW with full implementation; adapters_image.go NEW with 3 stub adapters; worker_image.go NEW with processImageGenJob+runImageGenJob+classifyImageError+imageJobOperations+ImageGenJobTimeout=10min; worker.go dispatch hook; jobs_handler.go validateImageGenInput; SDK Client.generate_image + ImageGenResult/ImageGenDataItem pydantic models + 2 audio error classes routed via from_code. Verify: gateway go test ALL GREEN (+22 tests: 14 adapter + 14 worker incl. disjoint + 7 handler); SDK pytest **184 passed** (was 174; +10 new); /review-impl(DESIGN) caught 0 HIGH+5 MED+6 LOW+1 COSMETIC all fixed inline. **No DB migration needed** — image_gen was in CHECK constraint + JobOperation enum + SDK Literal + handler validJobOperations from Phase 2b reservation.** | XL |
| **5c-β** (deferred) | image_edit + image_variation endpoints if a caller surfaces a need (`/v1/images/edits`, `/v1/images/variations`). | TBD |
| **5d** | `video_gen` adapter + Python SDK + openapi + 5-slot registration. Async submit→poll via `POST /v1/llm/jobs operation=video_gen`. Caller-side download. n=1 locked. url-only response_format (b64 rejected per /review-impl MED#3). Image-to-video via `init_image` base64 field. Path dispatch `/v1/videos/generations/text-to-video` vs `/v1/videos/generations/image-to-video` matching local-image-generator-service's actual routes (NOT singular `/v1/video/generations` per stale integration guide — /review-impl(DESIGN) HIGH#1). Shared `isContentPolicyRejection` helper refactored to `openai_content_policy.go` (used by image + video). VideoGenJobTimeout=30min. **✅ shipped — design doc [LLM_PIPELINE_PHASE5D_DESIGN.md](./LLM_PIPELINE_PHASE5D_DESIGN.md); BUILD: openapi +VideoGenInput+VideoGenResult+VideoGenDataItem + JobOperation enum +video_gen; migrate.go CHECK ALTER for video_gen; adapters.go +GenerateVideo + types + 3 sentinels (ErrVideoGenerationFailed/ErrVideoContentPolicy/ErrVideoInvalidParams) + MaxImg2VidInputBytes=10MB; openai_video.go NEW; adapters_video.go NEW (3 stubs); openai_content_policy.go NEW (refactored shared helper); worker_video.go NEW with classifyVideoError + videoJobOperations + VideoGenJobTimeout=30min; jobs_handler.go validateVideoGenInput + validJobOperations entry; notification-service consumer test updated; SDK Client.generate_video + VideoGenResult/VideoGenDataItem + 2 video error classes. Verify: gateway go build/vet/test ALL GREEN (+25+ new tests); SDK pytest **195 passed** (was 185; +10 new); /review-impl(DESIGN) caught 1 HIGH + 2 MED + 4 LOW + 1 COSMETIC all fixed inline (HIGH#1 path drift fixed: actual backend routes; MED#2 init_image size cap; MED#3 b64_json rejected for video). DB migration: 1 new ALTER block following Phase 4a-β precedent. | XL |
| **5e-α** | video-gen-service (Python) caller migration: `/v1/video-gen/generate` route uses `Client.generate_video()` instead of direct httpx + manual credential resolve. Per-call SDK Client (matches chat-service voice precedent). Caller-side download + MinIO + billing PRESERVED. First test suite for video-gen-service (13 tests). **✅ shipped — design doc [LLM_PIPELINE_PHASE5E_ALPHA_DESIGN.md](./LLM_PIPELINE_PHASE5E_ALPHA_DESIGN.md); /review-impl rounds DESIGN 0H+2M+3L+1C all fixed; BUILD 0H+1M+5L+2C, MED+3LOWs fixed. Verify: video-gen-service pytest 13/13; SDK pytest 196/196; chat-service pytest 180/180.** | XL |
| **5e-β.1** | First Go SDK in monorepo (`sdks/go/llmgw/`, module `github.com/loreweave/llmgw`, package `llmgw`) + book-service `media.go::generateChapterMedia` migration. Greenfield Go SDK with submit_job → poll → terminal-result flow mirroring Python SDK's `generate_image()`. Sync API; context cancellation only (HIGH#5 — Transport injection not full Client); sentinel-based `errors.Is`/`errors.As` matching via unexported `inner` + central `newErrorFromCode` constructor helper (HIGH#1). audio.go INTENTIONALLY unchanged (reserved for 5e-β.2). Plan-line correction: book-service was previously listed as "No LLM calls" — stale before media.go added. **Plan correction:** book-service has media.go (image_gen — now migrated) AND audio.go (TTS — Phase 5e-β.2). **✅ shipped — design doc [LLM_PIPELINE_PHASE5E_BETA1_DESIGN.md](./LLM_PIPELINE_PHASE5E_BETA1_DESIGN.md); /review-impl rounds DESIGN 6H+5M+6L+3C all actionable fixed inline; BUILD 3H+6M+7L+3C, 3H+5M+1L fixed inline. Verify: SDK go test 44/44; book-service go build/vet/test ALL GREEN (19 api tests + 3 config tests; grep-locks green; audio.go anti-bait green).** | XL |
| **5e-β.2** | Gateway `audio_gen` operation (batch TTS, 1..10 inputs, order-preserving) via `POST /v1/llm/jobs` + NEW MinIO `loreweave-audio-cache` bucket for URL-mode staging (public-read static URLs mirroring book-service `loreweave-media` pattern — design HIGH#1 redesigned from broken presigned-host-rewrite). Both response modes: b64_json (default) AND url. Python SDK `generate_audio()` + Go SDK `GenerateAudio` + NEW `ErrGatewayStorage` (distinguishes upstream success/gateway-storage-failed for retry-safety). book-service `audio.go` migrated; `PROVIDER_REGISTRY_SERVICE_URL` env DROPPED (audio.go was last consumer). **✅ shipped — design doc [LLM_PIPELINE_PHASE5E_BETA2_DESIGN.md](./LLM_PIPELINE_PHASE5E_BETA2_DESIGN.md); /review-impl 3 rounds (DESIGN 4H+11M+13L+3C; BUILD C#1+5H+10M+7L; FIX DELTA 1CRIT+2H+3M+2L). +~52 new tests across 5 files. After this: book-service has ZERO direct provider httpx; unified-gateway covers all platform LLM/audio/image/video except video-gen-service BFF (retired in 5f).** | XL |
| **5f** | ~~video-gen-service deletion + FE migration~~ — **redefined to hardening** (see [LLM_PIPELINE_PHASE5F_DESIGN.md](./LLM_PIPELINE_PHASE5F_DESIGN.md) §1). CLARIFY found deletion is not viable: video-gen-service is not a pure proxy (it downloads the gateway result to MinIO + records billing; the gateway returns a raw upstream URL a browser can't fetch for local backends). **User decision: KEEP video-gen-service as a permanent thin domain BFF**; harden it instead. **✅ shipped** — G1 removed dead `/models` endpoint + FE `listModels`/`VideoModel`; G2+G4 MinIO bucket bootstrap → `lifespan` startup + public-read policy on `loreweave-media` (fixes a browser-403 race) + per-request self-heal; G3 incoming JWTs HS256 signature-verified (was unverified base64 decode). 2 /review-impl rounds (DESIGN 1H+3M+3L+1C; BUILD 0H+1M+4L+2C, all fixed inline). Verify: video-gen-service pytest 25 passed (was 13); frontend tsc clean. The unified-gateway invariant is realized — video-gen-service is a domain BFF, not a violation. | XL (was M) |
