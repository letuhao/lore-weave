...
| **book-service** | Direct provider httpx (image_gen + tts) | `book-service/internal/api/media.go::generateChapterMedia` AND `book-service/internal/api/audio.go::generateAudio` → `/internal/credentials/*` + `/v1/images/generations` / `/v1/audio/speech` | **(image_gen: ✅ migrated Phase 5e-β.1; audio: 5e-β.2 next)** | OK / WIP |
...
| **5e-β.2** (next) | Gateway audio_gen adapter + Python SDK `generate_audio()` op + Go SDK `GenerateAudio` extension + book-service `audio.go::generateAudio` migration. Mirrors 5c-α/5d adapter shape but for TTS (binary mp3 response, not URL — different decode path). Anthropic/Ollama/LM Studio stubs (most lack TTS). After this: book-service has no direct provider httpx calls; `PROVIDER_REGISTRY_SERVICE_URL` env can be removed. ~25 files. | XL |
| **5f** (after 5e-β.2) | video-gen-service deletion + api-gateway-bff `/v1/video-gen/*` route retirement + FE migration. After this: unified-gateway invariant fully realized for chat + extraction + translation + audio + image + video. | M |
