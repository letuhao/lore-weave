...
| **4b-β** | knowledge-service adds `POST /internal/extraction/persist-pass2` accepting `Pass2Candidates` payload (uses existing `pass2_writer.write_pass2_extraction`). extract-item kept for back-compat. | M |
| **4b-γ** | worker-ai migration: drops `KnowledgeClient.extract_item`; uses `loreweave_extraction.extract_pass2(llm_client, ...)` then POSTs to `/persist-pass2`. extract-item endpoint deleted. **The 120s HTTP timeout finally goes away.** | L |
