...
| **5e** (next) | Caller migration: book-service (Go; image_gen) + video-gen-service (Python; uses new SDK). Needs Go SDK decision (full SDK vs thin shim). | TBD |
| **5f** (after 5e) | video-gen-service deletion + api-gateway-bff `/v1/video-gen/*` route retirement. After this: unified-gateway invariant fully realized for chat + extraction + translation + audio + image + video. | TBD |
