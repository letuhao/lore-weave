...
|---|---|---|---|
| **XST-R1** ✅ **SHIPPED** | Restore `factor = max(0, 1000 + Σpct)` and add AC-DF7-17 as a test | XST-D1 | ~~one line~~ **DONE** — `crates/game-rules/src/stats/resolve.rs:110` `let factor = (1000 + pct).max(0);`, and the line above it names its own kill-mutation (*"drop the max(0, …) on factor → yields a negative stat"*), so it carries a bite test rather than a claim. **Closed 2026-07-30** by a sweep that found the row still reading as open |
| **XST-R2** ✅ **SHIPPED** | Widen damage intermediates to **i128**; replace `saturating_mul` with a **declared** `MAX_HIT` cap that is observable | XST-D2 | ~~small; prerequisite for R6/R7~~ **DONE** — `crates/ruleset-core/src/combat.rs:49` *"The chain runs in `i128` so nothing overflows"*; `MAX_HIT` is now a **declared ruleset constant** in the digest-hashed struct (its `TODO(IMP-D5)` is discharged, `combat.rs:11`). Delivered by the F1/F2 ruleset-core arc, not by this row being worked. **Closed 2026-07-30** |
| **XST-R3** ✅ **SHIPPED** | Fix the roll band to be inclusive of 1150 | XST-D3 | ~~one line~~ **DONE** — `crates/ruleset-core/src/combat.rs:32` *"Variance band, **inclusive**: `[lo, hi]` in per-mille (850..=1150)"*, `roll_band_hi_pm: 1150` at :90, and the width is computed in `i128` (:124) so the inclusive `hi - lo + 1` cannot overflow. **Closed 2026-07-30** |
...
| **XST-R9** ⚙️ **NARROWED** | ~~Closed `TriggerPoint` + `Reaction { when, guard, then }` with a depth budget~~ → **the residue after `WSA-A11`**, which is much smaller | convergence #5 | ~~medium-large~~ **small-medium — re-scoped 2026-07-30, [REC-94](19_reconciliation_register.md)** |
| ↳ *what actually remains* | [`WSA-A11`](32_locus_as_actor.md) (SEALED) collapses the trigger problem into the actor problem: *"every WHEN is **some actor took a turn**"* — *"when does X happen?"* **is** *"who decided X should happen?"*. So there is **no second dialect to design**; the existing turn seam generalises. And a reaction being a **turn** means reaction depth is bounded by the **turn budget that already exists** — no new depth-limiting machinery, no recursion guard, which is the *"depth budget"* half of this row discharged by unification rather than by construction. [`WSA-Q4`/`Q5`](32_locus_as_actor.md) — *when* a locus takes its turn — are **RESOLVED** in [`34`](34_when_the_world_runs.md): a locus publishes `next_wake` in closed form; no tick, no cadence, and a locus at equilibrium costs nothing. **Residue:** (a) [`XST-R11`](#) declared `replacement_priority` — still required, because [`WSA-F5(a)`](32_locus_as_actor.md) states plainly that interleaved turns give *an* order, not the *right* one (the Gisela case: two reactions, two orders, 7 vs 8); (b) naming the closed `TriggerPoint` set that the scheduler dispatches on. **Absorbed `WSA-R18`** (same work, second id — [REC-94](19_reconciliation_register.md)). | — | — |
