...
| `D-ACTOR-BINDING-NOT-READ-BY-TRANSPORT` | 3 | **The durable user→actor binding now EXISTS (`actor_control_binding`, migration 034) and the transport does not read it.** `ChannelRoom.actorForUser` resolves from `LW_CHANNEL_ACTOR_MAP`, an explicit per-user dev map — which is not the defect that was fixed (that was the `?? '1'` DEFAULT, now `undefined`). Reaching the table needs a control-plane lookup game-server does not have: it ships no Postgres client and must not grow one (`I3` — TypeScript is gateway/realtime, and a transport owning a meta-DB connection is the boundary this repo spent a session drawing).

🔴 **PO, 2026-08-06 — the direction is SEALED, and it is not the one this row first proposed.** I wrote *"a gateway/BFF route or an internal endpoint"*, i.e. give the transport a way to ASK. The PO's correction: **it goes through the kernel. That is the architecture.** The transport does not learn the subject at all.

**Measured, and the correction lands on a defect rather than a preference.** `ChannelRoom` puts `actor: Number(actor)` on the proposal and `admission.rs` takes it: the producer SIGNATURE is verified, the SUBJECT is not. So `actor` is a field the caller supplies that IS a verdict — the exact `CMD-10` V4 test the substrate already applies to authored verbs, and the exact reason `submitter_class` was struck from the sealed manifest contract under `SEAM-1`/`AUTHOR-1`. **The same defect, on the wire instead of in the file.** `PID-D5`'s note sits *directly under* `pub actor: u64` making this argument about `event_category` — *"it used to ride here and SELECT the validator subset, so a proposal could elect its own trust tier"* — one field over, unapplied.

⇒ The proposal should carry the **user**; the authoritative side resolves user→actor from `actor_control_binding`. Not blocked: `commit-service` already opens the meta DB (`ruleset_boot.rs`'s `meta_url`), so this is buildable there. **Fixing it in the transport would have fixed one instance; moving the resolution kills the class**, because a subject the caller cannot assert cannot be forged. | **ASSERTED TRIGGER** — `an unmapped authenticated user is bound to NOBODY, not to a default actor` + `onJoin binds no session entry when the user drives nobody` in `services/game-server/src/rooms/ChannelRoom.test.ts` hold the SAFE state (absence refuses) so the gap cannot silently reopen while it waits; both bitten RED. The row closes when `actorForUser` reads the binding instead of the env map, at which point those two tests must be re-pointed at the new source. |
