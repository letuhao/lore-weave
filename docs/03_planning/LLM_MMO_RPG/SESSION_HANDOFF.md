...
> **PENDING (lock-gated):** `SL-*` / `SC-*` prefixes + the `cell` island aggregate are **not yet registered** in `_boundaries/` — needs a `[boundaries-lock-claim+release]` cycle.
> **NEXT:** `AUD-F8` `commit-service` (top blocker) · `sim-core` **S1** (skeleton — unblocked today, islands in the type signatures from commit one) · `AUD-F9` spawning/threat/loot · consider putting `language-rule-lint.sh` in pre-commit.
