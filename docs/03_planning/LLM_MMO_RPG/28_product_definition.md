...
| **E2 · Quantities** (new nouns) | a new stat / resource / status / activity **kind**, declared per ruleset | [XST-R6](27_extensibility_stress_test.md) ruleset-owned slot set + [XST-R8](27_extensibility_stress_test.md) tag bitset; [XST-R12](27_extensibility_stress_test.md) status instances | *"add `Nội lực` as a real resource with its own regen and caps"*; *"add `Danh vọng` as a quantity NPCs read"*; *"add fire resistance"* | a quantity that changes **when** something happens |
