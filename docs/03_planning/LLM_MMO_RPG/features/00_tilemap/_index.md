...
| TMP_009 | **Isometric Asset Pipeline** (TMP-ASSET) | Two-layer anchor static-prop pipeline + iteration review workflow (§12), exercised n=9 (1 entry × 3 biomes × 3 seeds) 2026-05-23 — **NOT production-ready**: cold-start sub-agent review (§12 canonical) = 0 clean / 3 partial / 6 failed; main-thread self-review had been lenient at 4/2/3 (§3.1 explains why the strict reading is right). Gen-time = `NAGuidance` node enables hardened negative at CFG=1.0 (dampens but doesn't eliminate baked ground). Post-process = RMBG-1.4 ONNX strip + scale-normalise + canonical 2:1 dimetric composite. §12 = small fixed fixture + structured review (no LLM-judge code, Claude main / sub-agent / human PO). Additive `facing: Option<Facing>` (TMP-A8). UNRESOLVED: TMP-ASSET-Q10 actor multi-facing · Q11 SAM2 semantic prop-vs-platform separation. Prior art HoMM3/VCMI + NAG (Chen et al. 2025). | DRAFT 2026-05-23 | ~450 | [`TMP_009_isometric_asset_pipeline.md`](TMP_009_isometric_asset_pipeline.md) |
