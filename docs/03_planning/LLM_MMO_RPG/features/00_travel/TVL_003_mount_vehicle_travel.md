...
- [`AIT_001`](../16_ai_tier/AIT_001_ai_tier_foundation.md) — Tracked-tier discipline; mounts are not AIT entities
- [`TDIL_001`](../17_time_dilation/TDIL_001_time_dilation_foundation.md) — the speed modifier scales journey fiction-duration
...
<!-- design-lint: ok count — HISTORICAL claim, not a current one: this sentence describes what TVL_001 shipped (2 variants), while this file's own §3 declares TVL_003's expanded 5. Both are true. Flagged by the spec-only count source on its first run (2026-07-30) — the hatch is doing its job; the alternative was teaching the matcher past tense, which is a language-biased rule (ML-4) and would fail on the corpus's Vietnamese prose. -->
**Third TVL feature by number; the conveyance layer.** TVL_001 built `TravelMode` as a 2-variant enum with `ByBoat` schema-reserved precisely for this; TVL_003 activates it and adds the rest — the ROUTE_001 water routes finally have vessels, travel speed becomes a real choice, and TVL_002's multi-modal composite (CTV-D5) is unblocked.
