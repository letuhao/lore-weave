...
> **Builds on:** [`GEO_004 ROUTE_001`](../00_geography/GEO_004_route_network_generator.md) (primary substrate — Route graph + RouteKind 5-variant + route.distance_units + route.default_fiction_duration + route.is_bidirectional) · [`GEO_003 SET_001`](../00_geography/GEO_003_settlement_generator.md) (Settlement.role for hospitality availability — Hamlet/Village/Town/City/Capital/Fortress) · [`GEO_002 POL_001`](../00_geography/GEO_002_political_layer.md) (Province + State + culture_tag for journey-narrative context) · [`GEO_001`](../00_geography/GEO_001_world_geometry.md) (biome + climate for narrative grounding) · [`TDIL_001`](../17_time_dilation/TDIL_001_time_dilation_foundation.md) (4-clock model — actor_clock + body_clock + realm_clock advance; soul_clock preserved unless BodyOrSoul::Soul form per TDIL-A2) · [`RES_001`](../00_resource/RES_001_resource_foundation.md) (food + water consumable cost per league traveled; ~~vital_pool Hunger/Thirst advancement~~ *REC-34: no such vital_pool fields — see §3 note*) · [`PL_006`](../04_play_loop/PL_006_status_effects.md) (Exhausted status applied on overnight no-inn travel; Wounded status if forced-rest combat) · [`AIT_001`](../16_ai_tier/AIT_001_ai_tier_foundation.md) (Tracked tier discipline — PCs + Major NPCs participate; Untracked NPCs excluded — no travel state) · [`EF_001`](../00_entity/EF_001_entity_foundation.md) (actor.current_cell_id read at travel-initiate; updated at travel-arrive)
...
pub struct ProvisionsConsumed {
    pub food_units: f32,                                    // deducted at Travel:Initiate from actor.resource_inventory.food (Consumable) — REC-34; was "via vital_pool Hunger advancement" (field does not exist)
    pub water_units: f32,                                   // deducted at Travel:Initiate from actor.resource_inventory.water (Consumable) — REC-34; was "via vital_pool Thirst advancement" (field does not exist)
}
...
> **⚠ CORRECTED 2026-07-26 (REC-34 / AUD-F17 #9): every residual `vital_pool.hunger`/`.thirst`
> site in this doc is superseded — those fields DO NOT EXIST.** RES_001's `VitalKind` is
> engine-fixed `{Hp, Stamina, Mana}` (RES_001 §4.1). Hunger body-state is a **PL_006
> `StatusFlag`**, and **thirst has no V1 representation at all** (RES-D5). The 2026-05-15 HIGH-4
> fix landed this in prose (TVL-V8, §2 TravelCost) but left **8 residual sites** still
> reading/writing `vital_pool`: the header Builds-on line, the §2 TravelCost NOTE (which
> described `vital_pool.hunger/.thirst` "body-state counters" that RES_001 never declared), the
> `ProvisionsConsumed` struct comments above, the §5 initiate cascade, the RES_001 cross-feature
> row, the §12 reject walkthrough, TVL-Q3's refund target, and AC-TVL-1/8. All are corrected this
> pass: provisions are exclusively `actor.resource_inventory.food/.water` **Consumable**
> deductions/refunds; TVL never touches `vital_pool`, and no hunger/thirst body-state mechanic
> exists beyond PL_006's flag.
...
- Provisions (HIGH-4 fix /review-impl 2026-05-15): actor MUST have ≥ `food_per_league × route.distance_units` Food units in **`actor.resource_inventory.food`** (Consumable possession counter) AND ≥ `water_per_league × route.distance_units` Water units in **`actor.resource_inventory.water`** at Travel:Initiate; insufficient → reject `travel.insufficient_provisions`. NOT a vital_pool check — ~~vital_pool tracks body-state (Hunger/Thirst counters), not possessions~~ *(REC-34: vital_pool has no hunger/thirst counters at all — `VitalKind` is `{Hp, Stamina, Mana}`; hunger is a PL_006 StatusFlag, thirst doesn't exist V1 per RES-D5)*.
...
Actor (PC OR Tracked NPC) issues Travel:Initiate { route_id, mode=OnFoot, direction=Forward }:
  ...
  ↓ EVT-T3 Derived cascade: RES_001 resource_inventory deductions (food -= food_cost; water -= water_cost).
    (⚠ REC-34 2026-07-26: was "vital_pool deductions (hunger += food_cost; thirst += water_cost)" —
     vital_pool has no such fields; see the REC-34 note in §3.)
...
When status transitions Active → Arrived (progress_fraction reaches 1.0):
  ...
  ↓ Hospitality check at arrival cell (HIGH-1 + HIGH-2 fixes /review-impl 2026-05-15):
    ...
    if settlement == None OR settlement.role ∈ {Hamlet, Fortress}:
      ...
      cascade to PL_006 apply_set_status per PL_006 StackPolicy (~~V1+30d StackPolicy::Replace for Exhausted — newer magnitude overrides older~~ **`StackPolicy::ReplaceIfHigher`**).
      > **⚠ CORRECTED 2026-07-26 (REC-35 / AUD-F17 #43):** `StackPolicy::Replace` is not a PL_006
      > variant, and the struck behaviour ("newer overrides older") was the **negation** of the
      > actual variant's semantics. The real variant is **`ReplaceIfHigher`: the higher magnitude
      > wins** — a fresh magnitude-3 Exhausted does NOT downgrade an existing magnitude-5 one.
...
| **RES_001** | **Critical sibling coordination** — provisions cost deducted at Travel:Initiate (TVL-V8 sufficient-provisions check, `resource_inventory` Consumable); ~~vital_pool Hunger + Thirst advancement~~ *(REC-34: no such fields — see §3 note)*; food_per_league + water_per_league defaults V1+30d (tunable per creative_seed.travel_cost_per_league V1+30d+ TVL-D11). |
...
Actor tieu_long_nu at cell:cao_thien_dai (high in mountains); issues Travel:Initiate {
  ...
  ↓ TVL-V8 sufficient-provisions check:
    ...
    tieu_long_nu.resource_inventory.food = 5 (only 5 units of food) < 8 → REJECT travel.insufficient_provisions.
    (⚠ REC-34 2026-07-26: was "vital_pool.hunger = 5" — the check reads the Consumable possession
     counter, and vital_pool.hunger does not exist; see the REC-34 note in §3.)
...
Actor tieu_long_nu travels Trail from a City to a remote Hamlet over 18 hours.
  ↓ At arrival: settlement.role == Hamlet → no inn available;
    wakeful_duration_at_arrival = 18h > 16h threshold → apply PL_006 status StatusFlag::Exhausted magnitude=3 via OutputDecl.
    (⚠ REC-35 2026-07-26: was "Tier 0" — PL_006 has no tier concept; magnitude is validated 1..=10,
     and "Tier 0" would reject at Stage 0. magnitude=3 per the §7 HIGH-2 mapping.)
...
| **AC-TVL-8** | Actor with insufficient food/water attempts Travel:Initiate → reject; ~~vital_pool~~ **`resource_inventory` unchanged** *(REC-34 — the assertion guards the possession counter; vital_pool has no hunger/thirst to assert on)*. | `travel.insufficient_provisions` |
...
| **TVL-Q3** | Travel cancel — provisions refund policy (full / proportional-to-remaining / none)? | V1+30d: proportional refund (remaining_distance / total_distance × provisions_consumed) refunded to RES_001 ~~vital_pool~~ **`resource_inventory` (Consumable)** *(REC-34 — a refund restores possessions, and vital_pool has no hunger/thirst to refund into)* at Forge:CancelJourney apply. Defensive: V1+30d+ self-cancel will inherit this policy. |
...
- [`TDIL_001`](../17_time_dilation/TDIL_001_time_dilation_foundation.md) — 4-clock model + BodyOrSoul gating
...
- [`AIT_001`](../16_ai_tier/AIT_001_ai_tier_foundation.md) — Tracked tier discipline
- [`EF_001`](../00_entity/EF_001_entity_foundation.md) — entity.travel_journey_id additive field (cross-feature dependency)
