...
>
> **Status (folder):** **IDF folder closure COMPLETE 2026-04-26** — all 5 IDF features at CANDIDATE-LOCK 2026-04-26 after 15-commit cycle (Phase 0 concept-notes → POST-SURVEY adjustments → 5 × DRAFT/Phase3/closure). Tier 5 Actor Substrate Foundation ready. Next priority: V1+ FF_001 Family Foundation post-IDF closure (per POST-SURVEY-Q4 + ORG-D12).
...
|---|---|---|---|---|---|
| IDF_001 | **Race** (RAC) | Race Foundation — closed-set RaceId per-reality + race_assignment T2/Reality aggregate; 6-variant SizeCategory (Tiny/Small/Medium/Large/Huge/Gargantuan; Pathfinder 2e full); lifespan / mortality_kind override / V1+ Lex axiom hook. RealityManifest extension `races: Vec<RaceDecl>` REQUIRED V1. **Path C V1 light content (2026-04-26):** Wuxia V1 ships 2 races (Phàm nhân + Cultivator); 3 V1+ deferred per RAC-D12 (Demon/Ghost/Beast). Modern V1 = 1 race (Human). Sci-fi V1 = not shipped. Schema PRESERVED; foundation tier discipline maintained; V1+ activation cheap (additive). 10 V1-testable AC + 6 V1+ deferred (3 original + 3 RAC-V1+4..6 from Path C). 12 deferrals (RAC-D11 + RAC-D12 NEW). | **CANDIDATE-LOCK** 2026-04-26 (Phase 3 + closure pass + Path C V1 light content) | [`IDF_001_race.md`](IDF_001_race.md) | ★ V1 essential (light) |
...
| IDF_005 | **Ideology** (IDL) | Ideology Foundation — closed-set IdeologyId per-reality + actor_ideology_stance T2/Reality aggregate (Vec<IdeologyStanceEntry>). **ONLY mutable IDF aggregate V1** per IDL-Q11. Multi-stance V1 per IDL-Q2 (Wuxia syncretism: Lý Minh holds Đạo + Phật + Nho). FervorLevel 5-level (None/Light/Moderate/Devout/Zealous). Apply/Drop/AdjustFervor events with audit. **Free V1 conversion per IDL-Q13 LOCKED (POST-SURVEY-Q3)** — IDL-D11 cost mechanic V1+. V1+ AxiomDecl.requires_ideology hook for Lex gate. RealityManifest extension `ideologies: Vec<IdeologyDecl>` REQUIRED V1. 10 V1-testable AC + 2 V1+ deferred; 11 deferrals (IDL-D1..D11). | **CANDIDATE-LOCK** 2026-04-26 (Phase 3 + closure pass) | [`IDF_005_ideology.md`](IDF_005_ideology.md) | V1+ stub V1 |
