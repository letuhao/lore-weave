...
>
> **⚠ MEDIUM-RECONCILIATION 2026-06-20 — V1 combat targeting REVERSED to TACTICAL GRID ([10 AUD-F1](../../10_medium_blast_radius_audit.md); user-decided):** Under the graphical 2D/2.5D medium ([00_VISION §0](../../00_VISION.md)), V1 combat uses a **tactical grid** (combatant positions + range / line-of-sight; movement via pathfinding) — **reversing** §6 "No Move verb V1", §11.1 "abstract arena (no zone graph)", and the §11.3 V2-deferral. **Rationale:** the *only* reason grid was deferred (lesson §139 — grid tactics is "TOKEN-EXPENSIVE for LLM narration") is **dissolved by the medium correction**: combat math is already engine-deterministic (LLM-zero-math, locked), the grid is **rendered visually** (not narrated per-cell), and narration is a **batched, optional sub-layer** post-resolution. Reuses **TMP_001** pathfinding (A*/Dijkstra) + the **CSC_001** zone graph (§85's "V2+: promote zone graph to tactical grid" → now **V1**) + the instanced combat scene (RTM-Q4). **Supersedes** the §11.2 Front/Back 2-row metaphor → real grid positions. **Scope flag:** this is a V1 stance reversal + scope addition; the detailed tactical-grid combat design (movement rules, range/LoS, AP/initiative interaction) is a **follow-up to resolve before COMB_001 DRAFT promotion**. Unchanged: LLM-zero-math, 3-layer architecture, the V1 damage law-chain, Q1-Q9.
