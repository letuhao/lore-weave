...
| **SpatialPreference** | Closed enum 16 variants (§4) — ⚠ **CORRECTED 2026-07-30**: read *14*, while §4 declares 16 (`ExplicitPosition` and `Any` were added without updating this cell). Found by `design-lint`'s new spec-only `count` source on its first corpus run replacing `(f32, f32)` for LLM-friendly spatial intent | V1+ additive per I14; CreativeSeed.schema_version 1→2 when GEO_001b LOCKs. V1=1 keeps `position_normalized` required; V1+=2 adds optional `spatial_preference` (at-least-one-Some validator). |
