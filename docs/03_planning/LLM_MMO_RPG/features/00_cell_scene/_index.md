...
**Active:** CSC_001 — **Cell Scene Composition** (DRAFT 2026-04-26 — Option C max scope)

**Folder closure status:** Open — DRAFT 2026-04-26. CANDIDATE-LOCK pending Phase 3 review cleanup + closure pass + downstream updates (PCS_001 brief reading list / PL_001b §16.3 lazy creation reopen).
...
|---|---|---|---|---|---|
| CSC_001 | **Cell Scene Composition** (CSC) | 4-layer composition pipeline: hand-authored skeleton templates (3 V1 Tavern + 1 default_generic_room fallback) + seeded procedural fixture placement (Counter/Table/Chair/Fireplace/Window; deterministic via blake3 seed) + LLM zone-assignment categorical decision (4 validators + 3-retry feedback loop + canonical fallback) + LLM Vietnamese xianxia narration (free-form; in-memory cache). Owns `cell_scene_layout` aggregate (T2/Channel-cell). Replaces architecturally-flawed LLM-as-grid-generator approach (v3 evidence: 30K reasoning tokens / 0 successful outputs) with bounded layered fallback chains. Owns `csc.*` RejectReason namespace (8 V1 rule_ids + 3 V1+ reservations). 10 V1-testable acceptance scenarios (AC-CSC-1..10) + 13 deferrals (CSC-D1..D13) + 5 open questions (CSC-Q1..Q5). | DRAFT 2026-04-26 | [`CSC_001_cell_scene_composition.md`](CSC_001_cell_scene_composition.md) | (this commit) |
