...
> **Status:** **CANDIDATE-LOCK 2026-04-26** (DRAFT 2026-04-26 → Phase 3 review cleanup 2026-04-26 → CANDIDATE-LOCK 2026-04-26 closure pass: §16 acceptance criteria walked AC-CSC-1..11; ACs already aligned with Phase 3 added rule_ids — `csc.zone_empty_fallback_used` covered in AC-CSC-3 fallback chain expansion; `csc.layer3_occupant_set_changed` V1+ reservation covered in AC-CSC-11. **0 rule_id mismatches** at closure (Phase 3 cleanup proactively aligned). Option C max scope per user direction "design now"; v3→v4 demo evidence in `_ui_drafts/CELL_SCENE_v1..v4` validates 4-layer architecture)
...
- **[TMP_001 Tilemap Foundation](../00_tilemap/TMP_001_tilemap_foundation.md)** — V1+30d non-cell-tier sibling (annotation added 2026-05-14 closure-pass-extension). TMP owns non-cell `tilemap_view`; CSC owns cell `cell_scene_layout`. Tier delineation per TMP-A1: a cell never has a `tilemap_view`; a non-cell channel never has a `cell_scene_layout`. From parent-tilemap rendering: each cell appears as a `TilemapObjectKind::Town` (faction-owned cells) or `TilemapObjectKind::Landmark` (wilderness cells); click → drill into the cell's CSC_001 4-layer composition. Direction: TMP_001 → CSC_001 (drill-down link only; no contract dependency in reverse). CSC's 4-layer LLM architecture (CSC-A1 zone-classifier + canonical fallback) is the prior-art pattern that TMP_008/TMP_008b reused for non-cell L3+L4 (AIT-A4 hybrid 2-stage shared by both features).
...
- [x] Closure-pass walk-through 2026-04-26 — §16 acceptance criteria walked AC-CSC-1..11; **0 rule_id mismatches** (Phase 3 cleanup proactively aligned ACs to new rule_ids — zone_empty_fallback_used in AC-CSC-3, layer3_occupant_set_changed in AC-CSC-11). No new ACs needed at closure pass.
- [x] **CANDIDATE-LOCK 2026-04-26** — boundary matrix `cell_scene_layout` row updated · _index.md status promoted · changelog appended. Downstream updates (PCS_001 brief §4.4f mandatory CSC_001 reading) tracked at consumer-feature design time
