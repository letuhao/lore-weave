...
| `title.binding.faction_membership_required` | TitleDecl.binding = Faction(faction_id) but actor_id not in actor_faction_membership for that faction at canonical seed OR Forge:GrantTitle | "Phải là thành viên của tổ chức này mới được nhận tước hiệu" |
| `title.binding.dynasty_membership_required` | TitleDecl.binding = Dynasty(dynasty_id) but actor_id not in FF_001 family_node lineage for that dynasty at canonical seed OR Forge:GrantTitle | "Phải là thành viên của gia tộc này mới được nhận tước hiệu" |
...
| RejectReason rule_ids | 9 V1 + 5 V1+ reservations |
