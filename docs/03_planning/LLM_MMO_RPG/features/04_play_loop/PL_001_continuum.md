...
> RealityManifest extended (additively, per foundation I14) with two new optional fields owned by TMP_001: `tilemap_templates: Vec<TilemapTemplateDecl>` + `tilemap_defaults: TilemapDefaults`. Cell-tier behavior unchanged (cells have no tilemap_view per TMP-A1); non-cell tiers (continent/country/district/town) gain procedural tile rendering at reality bootstrap. PL_001 §16 RealityManifest snippet is illustrative — full shape per `_boundaries/02_extension_contracts.md` §2 (now also registers `tilemap_templates`); PL_001 stays the consumer-of-contract authority. §13 Travel sequence at V2+ may consume TMP terrain speed modifier (TMP-D5 deferred) for non-cell-tier travel cost; V1+30d Travel uses MAP_001 `default_fiction_duration` unchanged. MEDIUM magnitude — additive RealityManifest fields only; CANDIDATE-LOCK status PRESERVED. Companion file `PL_001b` §16 + §19 carry the bootstrap and cross-reference detail.
