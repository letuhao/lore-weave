...
| ~~**ITM-V10**~~ | ~~canonical seed: `def_id` collides with no `resource_kinds.kind_id`~~ **WITHDRAWN 2026-08-02 (`IR-18`)** — it policed the `item_defs` ∩ `resource_kinds` boundary that `ITM-A2` invented, and `ITD-2` removes the boundary. **A validator whose entire job is to police a boundary you created is evidence the boundary should not exist.** | — | — |
...
| ~~**ITM-C2**~~ | ~~`item_defs.def_id` ∩ `resource_kinds.kind_id` = ∅~~ **WITHDRAWN 2026-08-02 (`IR-18`)** — same reason as `ITM-V10`. | — | — |
...
| **ITM-C4** | ⚠ **HALVED 2026-08-02 (`IR-6`).** The `HeldBy(actor_ref)` half survives as equip precondition `V2-9`. The lifecycle-lockstep half is **deleted**: `Suspended` is **residency** (`D-12`, axis 3), engine-owned and fiction-invisible, so there was never a contradiction to reconcile. Cascade auto-clear is now a property of `commit_with_modifiers` (`D-50`), not a rule. | PL_007 | `item.not_held` (write-time) |
...
| **12.5** | **DF07_001 Actor Stat Block** (landed 2026-07-26, parallel) | ⚠ **REVERSED 2026-08-02 (`IR-9`).** Was: *"PL_007 ships the `EquipmentStats` impl DF7-D1 deferred to it (§6.3) — DF07 needs **no change** to accept it."* **Both halves are now wrong.** The impl is deleted — a contribution is **DATA, never CODE** (`D-27`) — and DF07 needs a great deal of change: its closed `StatSlot` set is **combat's vocabulary in the engine core** (`D-100`), is **two concepts sharing one array** (`D-105`), and is being dismantled by `C-2a`/`b`/`c`. What survives is the *shape* of the prediction ("contract now, body later"); what it predicted has moved. Two additive items remain valid and are re-homed below. Two additive items DF07 should absorb at its next opening: (a) `StatTerm.instrument_match` gains the `ItemTag`/`ItemDef` variants so its own §6.1 `Some(Blade)` example is expressible (§6.4 / ITM-C7); (b) DF7-D3's V1+ `CarryCapacity` slot is the stat-side of PL_007b §4's slot accounting — when it activates, `inventory_cap.max_slots` becomes a resolved stat rather than a manifest constant. | **yes** |
...
PC submits EVT-T1 Item:Equip { instance: itm_greatsword }
  ...
  post-commit: ⚠ REWRITTEN 2026-08-02 (`IR-19`) — no version, no cache, no invalidation.
               The SAME commit wrote the greatsword's ModifierRows via
               commit_with_modifiers (D-50); phase 0 next tick folds them.
...
| **ITM-D7** | ✅ **PROMOTED FROM DEFERRAL TO DECISION 2026-08-02 (`IR-21`).** Its own wording was already right — *"a transfer ledger is an event-log query, not a field"* — and `D-23` is now the reason: **canon is what is written to the ledger.** The 3 fields in §4.3 are a **derived copy** and must carry `(reality_id, seq)` per `D-53`. Residue: a `Signature` may be **deliberately lossy** and that lossiness is content (Wurm). | decided, not deferred | — |
...
|---|---|---|
| **ITM-Q1** | ✅ **CLOSED 2026-08-02 — DISSOLVED, not answered (`IR-9`/`IR-10`).** There is no `equipment_version` and no `StatEpoch` equipment input: equipping writes `ModifierRow`s in the **same commit** (`D-50`), so nothing is derived at read time and nothing needs invalidating. **The question stopped existing.** Recorded in full below because it is a clean specimen of the class — *a correctness argument resting on a turn-economy accident* — and because the deferral was disciplined: it named its trigger. Original: **DEFERRED with a trigger, and the trigger is now watched.** Safe in V1 because one action per turn is structural (COMB_001 §3 / TG-A3), and out of combat a turn carries one submitted event. **Trigger:** the first design that admits two equipment writes in one turn — most likely a COMB action-economy change (ITM-D22 mid-combat swap) or a batched client action. Fix is a `version: u64` counter on `actor_equipment`, one field, no migration concern (a fresh field defaults 0). DF07 tracks the same residual as **DF7-D13**, so both sides see it. |
...
- [~] **Representation rule (ITM-A2) enforced, not asserted** — ITM-V10 / ITM-C2 / AC-ITM-2. ⚠ **2026-08-02 (`IR-20`): a ✅ on a design now replaced.** The *enforcement* was real; the *rule* was the wrong one (`ITD-2`). Kept, struck rather than deleted, because **the enforcement is what made the boundary's cost visible**
- [~] **DF07 seam implemented, not merely described** — ⚠ **2026-08-02 (`IR-20`): the seam is DELETED (`IR-9`) and both named defects DISSOLVE.** *Keep this line's history.* It called out the `blocked_by_primary` double-count and the `equipment_version` monotonicity limit **rather than leaving them latent**, and that is exactly why removing them cost a paragraph instead of a debugging session. **A review that names its own latent defects is what makes a later reversal cheap** — the strongest single argument in this feature's favour, on the day its central mechanism was removed
