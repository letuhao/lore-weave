...
| WA_006 | **Mortality** (MOR) | PC death model. Per-reality `MortalityConfig` with closed-set DeathMode (`Permadeath` default, `RespawnAtLocation`, `Ghost`). `pc_mortality_state` state machine. Death-trigger detection layer in A6 output filter (V1 keyword-match) + admin force-kill. Hot-path mortality check on every turn submission. False-positive dispute flow. Resolves PC-B1 + partial PC-E3. | DRAFT 2026-04-25 | [`WA_006_mortality.md`](WA_006_mortality.md) | uncommitted |
