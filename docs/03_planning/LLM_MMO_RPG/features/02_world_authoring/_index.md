...
| WA_003 | **Forge** (FRG) | Author Console — UX flow + API contract for editing Lex axioms, declaring Heresy ContaminationDecls, and (with admin escalation) advancing WorldStability stages. RBAC matrix (4 roles × ImpactClass), 12 V1 EditActions + 5 read views, dual-actor approval flow for Tier1 edits, audit log. ⚠ **OVER-EXTENDED** (post-WA-boundary-review 2026-04-25) — generic patterns (RBAC matrix, dual-actor flow, audit infrastructure) should extract to a future `CC_NNN_*` cross-cutting feature. Lex/Heresy-specific portions stay here. See file's over-extension notice. Resolves LX-D4 + HER-D10. | **PROVISIONAL** 2026-04-25 | [`WA_003_forge.md`](WA_003_forge.md) | 5903ccd |
...
| WA_006 | **Mortality** (MOR) | PC death model. Per-reality `MortalityConfig` with closed-set DeathMode (`Permadeath` default, `RespawnAtLocation`, `Ghost`). ⚠ **OVER-EXTENDED** — current draft includes territory belonging to PCS_001 (per-PC state), 05_llm_safety (A6 detection), PL_001/PL_002 (hot-path check + respawn flow). Pending rewrite to thin config-only (~180 lines). See file's over-extension notice. Resolves PC-B1 + partial PC-E3 (config layer only). | **PROVISIONAL** 2026-04-25 | [`WA_006_mortality.md`](WA_006_mortality.md) | 8aed4fa |
