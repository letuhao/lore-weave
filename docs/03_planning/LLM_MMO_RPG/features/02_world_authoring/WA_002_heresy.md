...
| HER-D8 | EVT-T11 WorldTick V1+30d activation; for V1 stage transitions, may emit as EVT-T8 AdminAction only | event-model agent Phase 4 (`08_scheduled_events.md`) |
| HER-D9 | LexSchema v1 → v2 migration sequencing (deploy v2 readers before any v2 writer) | implementation phase ops |
| HER-D10 | Author UI for editing ContaminationDecl + WorldStability monitoring | WA_003 future Author Console |
...
- [07_event_model/05_validator_pipeline.md] (Phase 3, not yet locked) — slot ordering for Heresy proposed §6.1
