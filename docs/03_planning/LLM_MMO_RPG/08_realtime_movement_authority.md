...
| **RTM-Q4** | Realtime↔turn handoff | ⚠ **REVERSED 2026-08-22 by [`SPG-D1`](36_map_architecture.md) / `SPG-A16` — recorded here per the `AUD-F1` precedent (`SPG-R8`).** This row read *"✅ Instanced — dedicated encounter scene — combat spins up a dedicated instance"*, and doc 36 decided the opposite **on 2026-07-29**: combat is sited **IN PLACE where the space allows**, and an `Arena` is carved only when it does not. `SPG-A16` states the general form — **combat always resolves on a tactical grid; only the GRID'S SOURCE varies** — sampled from the `Domain`'s floor plan when one exists, derived from the world field when it does not. **One mechanism, two sources, rather than two code paths.** Position checkpoint/restore (§5.4) is UNAFFECTED and still applies to the carved case. **The rot is the interesting part:** for three weeks this table told every reader combat is always instanced while doc 36 said it is not, and nothing connected the two — the reversal was recorded in the doc that made it and never at the doc it overruled. |
