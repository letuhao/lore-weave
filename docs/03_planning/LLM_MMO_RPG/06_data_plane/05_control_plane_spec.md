...
| **Lifecycle scheduler + pause auto-expiry (Phase 4)** | Periodic scan over each active reality (5-min cadence) for cell channels eligible for auto-dormant transition; 60-s cadence for pause auto-resume on `paused_until` expiry; **30-s cadence for turn-slot auto-timeout on `turn_expected_until` expiry** (DP-Ch52). Coordinates transitions through writer nodes; emits canonical `ChannelStateTransition` / `ChannelResumed` / `TurnSlotTimedOut` events. See [DP-Ch32](17_channel_lifecycle.md#dp-ch32--auto-dormant-trigger-cell-only) + [DP-Ch35](17_channel_lifecycle.md#dp-ch35--channel_pause--channel_resume-primitives) + [DP-Ch52](21_llm_turn_slot.md#dp-ch52--auto-timeout-scheduler--turnslottimedout-event). |
| **Session→node binding lookup (Phase 4)** | Existing `GetSessionNode` RPC (DP-C3) is now consumed by the session-writer transparent-routing path on stale-gateway routing failures. CP serves the binding from its session registry; SDKs cache for 300 s + invalidate on routing failure. See [DP-Ch41](18_causality_and_routing.md#dp-ch41--session-writer-transparent-routing). |
