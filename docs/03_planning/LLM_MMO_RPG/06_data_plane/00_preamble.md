...
| [_superseded/04_player_character/](../_superseded/04_player_character/) | PC aggregates pick a tier per field (e.g. currency = T3, position = T1). DF items coming from PC designs plug into DP through the tier they pick. | PC designs must declare tier choice per field in their own doc; DP does not pre-classify PC fields. |
