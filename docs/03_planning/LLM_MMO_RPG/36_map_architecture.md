...
| **SPG-R6** | `ACT_001` L3 | `control_source` enum → a first-class `control_binding (controller_id, actor_id, since, authority)` aggregate (SPG-A10) | **verified** |
| **SPG-R7** | `PCS-A4` / PC concept notes | relax the `cap=1` controller↔actor validator; possession is a core mechanic, not a V1+ deferral | **verified** |
