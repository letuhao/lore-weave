...
| `PGN-*` | **Progression generation pipeline** — the first element module under `CPL-A3`'s contract, and the pipeline POC: a 武俠 book's cultivation ladder becomes `ProgressionKindDecl`s a reality loads, with every value accountable | [`39_progression_generation_pipeline.md`](../39_progression_generation_pipeline.md) | next free per family | PGN-A0 = the FIVE decisions between a sentence of prose and a tier row have five different OWNERS (book · book · book-as-a-naming-pattern · model · policy), and the pipeline's shape is whatever keeps them separate · PGN-A2 = the brief is derived from the RECURSIVE schema closure and coverage is ASSERTED, not fingerprinted · PGN-A5 = a model may emit CARDINALITY, ORDER and NAMES THE BOOK CONTAINS, never MAGNITUDE · PGN-A8 = a fixture that already contains the answers makes every stage vacuous · PGN-A10 = S3 is a deterministic FOLD; no model runs at consolidation |
