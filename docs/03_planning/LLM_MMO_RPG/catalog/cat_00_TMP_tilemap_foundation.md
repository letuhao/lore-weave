...
| TMP-18 | RealityManifest extension: `tilemap_templates: HashMap<MapKind, TilemapTemplateRef>` (`SPG-R1` 2026-07-30: was `ChannelTier`) ⚠ `SPG-R13` (OPTIONAL V1+30d; engine-defaulted) + `tilemap_defaults: TilemapDefaults` | ✅ | V1+30d | TMP-2 | [`_boundaries/02_extension_contracts.md` §2](../_boundaries/02_extension_contracts.md) |
| TMP-19 | `tilemap.*` RejectReason namespace (17 V1+30d rule_ids + 6 V1+ reservations — see TMP_001 §9) | ✅ | V1+30d | TMP-1 | [TMP_001 §9](../features/00_tilemap/TMP_001_tilemap_foundation.md) |
...
| TMP-45 | LLM prompt 3-segment structure: cacheable system + cacheable reality_context + variable tilemap_payload — aligned with Anthropic prompt caching for 30-45% ongoing-cost reduction | 📋 | V2 | TMP-23, TMP-31, TMP-32 | [TMP_008b §2](../features/00_tilemap/TMP_008b_llm_contract_spec.md) |
| TMP-46 | Anthropic tool-use structured-output enforcement (`submit_zone_classifications` + `submit_zone_narrations` tools with strict `input_schema`) — eliminates malformed-JSON / markdown-fence / preamble failure modes | 📋 | V2 | TMP-31, TMP-32 | [TMP_008b §3](../features/00_tilemap/TMP_008b_llm_contract_spec.md) |
| TMP-47 | Structured per-case validation feedback (per-object error messages with received-value + allowed-set rather than flat error list) — empirically ~70-90% retry success vs ~20-40% with flat strings | 📋 | V2 | TMP-31, TMP-46 | [TMP_008b §4](../features/00_tilemap/TMP_008b_llm_contract_spec.md) |
| TMP-48 | Per-object retry granularity: accept good classifications, retry only failing subset; preserve partial successes; per-object canonical-default fallback (not all-or-nothing) | 📋 | V2 | TMP-31, TMP-47 | [TMP_008b §5-§6](../features/00_tilemap/TMP_008b_llm_contract_spec.md) |
| TMP-49 | Prompt-injection defense: `<author_text>...</author_text>` delimiting + tag-close-token escape + 05_llm_safety 3-intent classifier on raw author input + World Oracle semantic check — multi-layer defense for author-controlled freeform text in prompts | 📋 | V2 | TMP-31, TMP-32, 05_llm_safety | [TMP_008b §7](../features/00_tilemap/TMP_008b_llm_contract_spec.md) |
| TMP-50 | L4 cache key includes L3-classifications digest for the zone — Forge:OverridePlacement on L3 correctly invalidates stale L4 narration (was missing pre-revision) | 📋 | V2 | TMP-32 | [TMP_008b §8.2](../features/00_tilemap/TMP_008b_llm_contract_spec.md) |
| TMP-51 | Deterministic key-phrase extraction post-L4 (TF-IDF V2 / KeyBERT V2+30d) — removed from LLM output; saves ~50 tokens × zone; consistent quality across replays | 📋 | V2 | TMP-32 | [TMP_008b §10](../features/00_tilemap/TMP_008b_llm_contract_spec.md) |
| TMP-52 | Closed-enum style hints: `NarrativeTone` (Xianxia/Wuxia/HistFiction/Scifi/Modern/Fantasy/UrbanFantasy/Horror) + `NarrationLanguage` (Vi/En/Zh/Ja/Ko) + `NarrationVoice` (SecondPerson/ThirdPerson/Omniscient) — schema-additive extensions; prevents LLM emitting random open-string tone | 📋 | V2 | TMP-32 | [TMP_008b §11](../features/00_tilemap/TMP_008b_llm_contract_spec.md) |
