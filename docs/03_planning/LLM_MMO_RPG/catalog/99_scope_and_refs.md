...
**All 21 SA+DE adversarial + 13 Security (S1-S13) resolved.** Storage + multiverse design fully locked pending external-dependent V1 prototype data. **SRE / Incident Response review in progress (11/12 done)**: SR1 SLOs + SR2 Incident Classification + SR3 Runbook Library + SR4 Postmortem Process + SR5 Deploy Safety + Rollback + SR6 Dependency Failure Handling + SR7 Chaos Drill Cadence + SR8 Capacity Planning + Auto-Scaling (invariant I17 approved) + SR9 Alert Tuning + Pager Discipline + SR10 Supply Chain Security (invariant I18 approved) + **SR11 Turn-Based Game Reliability UX (2026-04-24) — 10 decisions + schema + governance, 8-state turn machine at `contracts/turn/` with idempotency-keyed submission preventing S6 double-charge, per-user `turn.status.update` WS indicators with tier-matched latency-budget phrasing + cancel/abort verb discipline, 6-value `PresenceState` enum (distinct from GoneState) + `session_participants` schema extension + debounced WS propagation, 3-policy disconnect matrix (suspend-all / proceed-if-turn-complete default / replace-with-npc) with per-session override + DF4 extension point, optimistic UX rules with conservative rollback + non-modal toast + divergence audit, degraded-mode UX mapping per SR6-D5 (5 modes → 5 banner + tone + allowed-action rules), turn fairness via FIFO + tier-bump with S6 budget cost + 60s cooldown + 30% hard cap + Gini alert, 30-min absolute abandoned-turn timeout + `turn_outcomes` audit (1y retention) with 4 derived SLIs, registered error code library at `contracts/errors/user_errors.yaml` with per-locale templates + CI lint + 7 initial codes + technical-context-never-user-visible discipline, **12-scenario V1 launch gate** with E2E + SR7 chaos-drill coverage; 3 new admin commands (`admin/session-unfreeze` Tier 2 + `admin/turn-abandon` Tier 3 + `admin/presence-reset` Tier 3); no direct problem-status moves (SR11 introduces new UX territory not previously cataloged)** — SR12 pending.
