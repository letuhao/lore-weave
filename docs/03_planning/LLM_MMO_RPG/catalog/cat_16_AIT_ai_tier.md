...
| AIT-6 | Hybrid 2-stage Untracked generation (Stage 1 template+RNG / Stage 2 LLM-flavor lazy) | ✅ | V1 | LLM service / EVT-A9 <!-- corrected 2026-07-27: this cell previously named an `RNG`-prefixed axiom that has no declaring doc anywhere in the corpus (design-lint's last surviving finding). AIT-4 one row up cites EVT-A9 for exactly this property (replay determinism) and the number matched; AIT-6 Stage 1 is template+RNG, which is precisely what must replay deterministically. Inference, not a sourced fact — AIT owner: revert if the original meant something else. --> | [AIT_001 §5](../features/16_ai_tier/AIT_001_ai_tier_foundation.md#5-untracked-npc-generation-pipeline-q4q5q11-locked) |
