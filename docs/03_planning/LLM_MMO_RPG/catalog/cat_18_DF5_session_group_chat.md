...
<!-- design-lint: ok count — NAME COLLISION, not drift. DF5's `PresenceState` is session MEMBERSHIP (Connected/Disconnected/Left) and is spec-only; `dp_kernel::lifecycle::PresenceState` is rich per-user PRESENCE (Active/Idle/Typing/WaitingAi/DisconnectedBrief/...). Two concepts, one name — the collision is the real finding; tracked for a rename decision. -->
