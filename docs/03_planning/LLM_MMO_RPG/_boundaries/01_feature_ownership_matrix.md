...
| ~~`actor_binding`~~ → `entity_binding` | T2 / Reality | **EF_001 Entity Foundation** (DRAFT 2026-04-26; transferred from PL_001 2026-04-26) | TRANSFERRED 2026-04-26: was "Where is X covers PCs + NPCs uniformly" under PL_001 §3.6. EF_001 extends scope to all 4 EntityType variants (Pc/Npc/Item/EnvObject) via 4-state `LocationKind` discriminator (InCell / HeldBy / InContainer / Embedded) + 4-state `LifecycleState` (Existing / Suspended / Destroyed / Removed) + per-instance `affordance_overrides`. PL_001 reopen: §3.6 now references EF_001 as owner. See [EF_001 §3.1](../features/00_entity/EF_001_entity_foundation.md#31-entity_binding-t2--reality--primary). |
| `entity_lifecycle_log` | T2 / Reality (append-only) | **EF_001 Entity Foundation** (DRAFT 2026-04-26) | Per-entity append-only audit trail of LifecycleState transitions. Causal-ref to triggering EVT-T1 Submitted. Reason kinds: CanonicalSeed / RuntimeSpawn / PcMortalityKill / NpcCold / AdminDecanonize / AdminRestore / InteractionDestructive / Unknown. See [EF_001 §3.2](../features/00_entity/EF_001_entity_foundation.md#32-entity_lifecycle_log-t2--reality-append-only). |
...
| **EVT-T4 System sub-type `EntityBorn`** (EF_001 2026-04-26) | 07_event_model envelope; sub-type feature-owned | **EF_001 Entity Foundation** owns `EntityBorn { entity_id, entity_type, cell_id }` emitted at canonical seed (RealityBootstrapper) and runtime spawn (world-service). Cell membership emitted alongside via DP-A18 MemberJoined. | Per EVT-A11 sub-type ownership discipline. |
| **EntityKind trait** (EF_001 2026-04-26) | **EF_001 Entity Foundation** §4 | PCS_001 implements `EntityKind for Pc` · NPC_001 implements `EntityKind for Npc` · future Item feature implements `EntityKind for Item` · future EnvObject feature implements `EntityKind for EnvObject`. Each consumer MUST implement `type_default_affordances()` (no default impl) — forces explicit affordance declaration. | 5 methods: `entity_id()` · `entity_type()` · `lifecycle_state(binding)` · `affordances(binding)` · `display_name(locale)`. See [EF_001 §4](../features/00_entity/EF_001_entity_foundation.md#4-entitykind-trait-specification). |
