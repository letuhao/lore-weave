...
| `place` | T2 / Channel-cell | **PF_001 Place Foundation** (DRAFT 2026-04-26) | Semantic place identity per cell-tier channel; 1:1 strict V1 (cell-only). Owns 10-variant PlaceType closed enum + 5-variant ConnectionKind + 4-state StructuralState machine + 11-variant EnvObjectKind + fixture-seed deterministic instantiation. RealityManifest extension `places: Vec<PlaceDecl>` registered. Cascades into EF_001 §6.1 on `Pristine/Damaged/Restored → Destroyed` (HolderCascade for embedded EnvObjects + at-cell Items). §15 acceptance: 10 V1 scenarios AC-PF-1..10. See [PF_001 §3.1](../features/00_place/PF_001_place_foundation.md#31-place-t2--channel-cell-scope--primary). |
...
| **EVT-T4 System sub-type `PlaceBorn`** (PF_001 DRAFT 2026-04-26) | 07_event_model envelope; sub-type feature-owned | **PF_001 Place Foundation** owns `PlaceBorn { place_id, place_type, parent_channel }` emitted at RealityManifest bootstrap (one per cell-tier channel) and V1+ runtime spawn via Forge. Cascades cell-channel `MemberJoined` for canonical actors who start at this cell. | Per EVT-A11 sub-type ownership discipline. |
| **EVT-T8 Administrative sub-shape `Forge:EditPlace`** (PF_001 DRAFT 2026-04-26) | event-model agent (Phase 2) — feature-defined sub-shape | **PF_001 Place Foundation** owns `Forge:EditPlace { place_id, edit_kind, before, after }`. Uses `forge_audit_log` (WA_003-owned) for audit trail. Edit kinds V1: UpdateNarrativeDrift / UpdateStructuralState / UpdateConnections / UpdateFixtureSeed / Rename / ChangePlaceType. | Joins existing EVT-T8 sub-shape registry alongside Charter*/Succession*/MortalityAdminKill. |
