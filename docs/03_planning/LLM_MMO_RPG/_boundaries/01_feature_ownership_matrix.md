...
| **EVT-T1 Submitted sub-types** (Option C redesign 2026-04-25) | 07_event_model envelope; sub-types feature-owned | PL_001 + PL_002 own `PCTurn` (Speak/Action/MetaCommand/FastForward); NPC_001 + NPC_002 own `NPCTurn`; future quest-engine owns `QuestOutcome` | Per EVT-A11 sub-type ownership discipline. Replaces former T2 NPCTurn category. |
| **EVT-T3 Derived sub-types** (Option C redesign 2026-04-25) | 07_event_model envelope; sub-discriminator = `aggregate_type` | Each aggregate-owner feature owns its delta-kinds (PL_001 owns fiction_clock/scene_state/actor_binding/participant_presence; NPC_001 owns npc_pc_relationship_projection/npc_node_binding; etc.). Calibration sub-shapes (DayPasses/MonthPasses/YearPasses) owned by PL_001 (derived from FictionClock advance). | Replaces former T7 CalibrationEvent category — calibration is mechanically Derived. |
...
| `EVT-A1..A12` / `EVT-T1`/`T3`/`T4`/`T5`/`T6`/`T8` (active) + `EVT-T2`/`T7`/`T9`/`T10`/`T11` (`_withdrawn` 2026-04-25 Option C redesign per I15) / `EVT-Q*` (and EVT-P*/V*/L*/S* pending) | 07_event_model | Phase 1 LOCKED + Option C redesign 2026-04-25 |
