...
| `map_layout` | T2 / Channel (all tiers) | **MAP_001 Map Foundation** (DRAFT 2026-04-26) | Visual graph layer per channel; covers all tiers (continent through cell). Owns 5-variant ChannelTier closed enum + author-positioned absolute u32 (0..=1000) per-tier viewport + Option<TierMetadata> conditional (Some non-cell, None cell — PF_001 supplies display_name/canon_ref) + 5-variant MapConnectionKind matching PF_001 + distance_units (canonical abstract leagues; invariant) + default_fiction_duration (OnFoot baseline V1; V1+ TVL_001 method matrix) + 3 image asset slots (icon/background/inline_artwork; V1 schema-only with V1+ MAP_002 phased pipeline) + 4-variant AssetSource + 3-variant AssetReviewState. Composes with PF_001 at cell tier (PF_001 owns place semantic + cell ConnectionDecl; MAP_001 owns cell visual layer + non-cell connections). RealityManifest extension `map_layout: Vec<MapLayoutDecl>` + `travel_defaults: TravelDefaults` REQUIRED V1. §15 acceptance: 10 V1 scenarios AC-MAP-1..10. See [MAP_001 §3.1](../features/00_map/MAP_001_map_foundation.md#31-map_layout-t2--channel-scope--primary). |
| `cell_scene_layout` | T2 / Channel-cell | **CSC_001 Cell Scene Composition** (DRAFT 2026-04-26) | 4-layer composition pipeline turning cell-tier place into renderable in-scene UI. Owns: Layer 1 hand-authored skeleton template registry (3 Tavern + 1 default_generic_room V1) + Layer 2 seeded procedural fixture placer (5 FixtureKinds: Counter/Table/Chair/Fireplace/Window; deterministic per blake3 seed) + Layer 3 LLM categorical zone-assignment (JSON contract + 4 validators + 3-retry feedback loop) + Layer 3 canonical default fallback (engine algorithm; always succeeds; no LLM) + Layer 4 LLM creative narration (Vietnamese xianxia free-form; in-memory LRU cache). Each layer's failure mode bounded; cell scene always renders. Architectural axiom CSC-A1: LLM tasks confined to categorical (L3) + creative (L4); spatial coord manipulation handled by deterministic engine (L1+L2). Demo evidence at `_ui_drafts/CELL_SCENE_v1..v4`: v3 LLM-as-grid 31K tokens (failed); v4 LLM-as-zone 2.5K tokens (succeeded) — 12.7× cost reduction. RealityManifest extension `scene_skeleton_overrides: HashMap<ChannelId, SkeletonId>` (optional V1 per-cell override). §16 acceptance: 10 V1 scenarios AC-CSC-1..10. See [CSC_001 §3.1](../features/00_cell_scene/CSC_001_cell_scene_composition.md#31-cell_scene_layout-t2--channel-cell-scope--primary). |
