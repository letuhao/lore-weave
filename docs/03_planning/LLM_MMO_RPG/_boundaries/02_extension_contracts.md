...
| `authoring.*` | GEO_001b CreativeSeed Authoring Flow (added 2026-05-13 DRAFT; D-S04-1..5 approval batch 2026-05-14 added 2 V1 rule_ids); **10 V1 rule_ids** — invalid_json (LLM JSON parse fail; transient — retry with error context) / schema_violation (LLM output fails schemars-generated JSON Schema validation; transient — retry) / cap_violation (culture_hints.len ≤ 16 OR canonical_settlements.len ≤ 50 OR position ∈ [0, 1] OR other §3 rules) / **duplicate_canonical_name (D-S04-1 — canonical_settlements[].name OR culture_hints[].naming_style_ref duplicated across the array; LLM re-prompted to remove duplicate)** / **spatial_preference_cycle (D-S04-2 — SpatialPreference::NearSettlement cycle detected via topological sort DFS with white/gray/black coloring; LLM re-prompted to break cycle by anchoring one settlement to a non-NearSettlement preference)** / content_safety_violation (PII scrubber + §12Y.L5 injection scanner reject on `lore_hooks_per_region.content` or `canonical_settlements.name` or culture_hints.value_tags or naming_styles corpus refs) / iteration_cap_exceeded (V1 cap N=10 author-LLM turns) / retry_cap_exceeded (V1 cap N=3 retries per iteration; falls back to author EditManually) / cost_cap_exceeded (S6-D2 inherited per-session cap: $5 paid / $20 premium) / spatial_intent_required (V1+ schema_version=2 validator: each culture_hint + canonical_settlement MUST have at least one of `position_normalized` OR `spatial_preference` Some); +4 V1+ reservations: canon_ref_unresolved (V1+ when knowledge-service ships; KnowledgeServiceExtracted producer validator step 4 fails resolution) / template_version_deprecated (V1+ when world_authoring/v1.tmpl superseded by v2; bumps trigger CI fixture update per S9 governance §12Y.L2) / import_format_unsupported (V1+ when Imported producer activates AzgaarFmgJson V1+ first; later WonderdraftJson + LoreWeaveManifest + Custom) / collaboration_conflict (V2+ multi-author per PLT_001 Charter co-authors). V1 user-facing rejects: cap_violation + iteration_cap_exceeded + retry_cap_exceeded + cost_cap_exceeded + spatial_intent_required (others are transient LLM-retry-resolvable OR schema-level). **Multi-turn iteration loop V1 caps**: iteration_count_max=10 / retry_per_iteration_max=3 / S6 cost cap / 24h session TTL. **No new aggregate**: BFF-held UX state; per-iteration LLM cost in S6 user_cost_ledger; final accepted CreativeSeed durable record via GeographyBorn payload only. **5 producers** (LlmGenerated V1 / AuthorManual V1 / Imported V1+ / KnowledgeServiceExtracted V1+ / Hybrid V1). **SpatialPreference 14-variant enum** introduced V1+ schema_version 2 (additive per I14; LLM-friendlier alternative to raw `(f32, f32)`); CreativeSeed.schema_version 1 → 2 migration path documented. **S9-registered template** at `contracts/prompt/templates/world_authoring/v1.tmpl` with 8-section structure per §12Y.L3 + schema-constrained generation REQUIRED + token budgets per §12Y.L6. i18n: V1 ships I18nBundle from day 1 per RES_001 §2 cross-cutting contract. |
| `travel.*` | TVL_001 Travel Mechanics (added 2026-05-14 DRAFT — V1+ first feature consuming V1+30d activation triangle POL + SET + ROUTE; primary consumer of ROUTE Route graph); **15 V1+30d rule_ids** — route_unknown / actor_not_at_journey_start / backward_one_way_route / actor_untracked_excluded / mode_unavailable_v1plus30d / actor_already_traveling / soul_form_no_physical_travel / insufficient_provisions / mode_route_incompatible / route_layer_not_activated / journey_id_collision / progress_non_monotonic / arrival_cell_unknown / cross_channel_initiate_forbidden + `route.remove_blocked_by_active_journey` (cross-feature gate added to ROUTE_001 RemoveRoute pipeline as TVL-V14; ROUTE-V12 sealane_range slot owner remains ROUTE_001 but TVL-V14 is OWNED by TVL_001 per cross-aggregate validator C-rule pattern from TIT-C1 + PO-C1 precedent); +2 V2+ reservations: travel.soul_projection_pending (V2+ astral-travel activation TVL-D7) + travel.mount_unavailable (V1+30d+ when ByBoat / OnHorseback activates TVL-D8). V1+30d schema-level rejects (8): route_unknown / actor_untracked_excluded / route_layer_not_activated / journey_id_collision / progress_non_monotonic / arrival_cell_unknown / cross_channel_initiate_forbidden / + cross-feature gate. V1+30d user-facing rejects (7): the rest. **Phase 0 TVL-D1..D7 LOCKED**: D1 own `00_travel/` folder / D2 pure mechanics V1+30d (encounter generation deferred V1+30d+) / D3 on-demand Dijkstra / D4 selective TDIL clocks (actor + body + realm; soul preserved unless BodyOrSoul::Soul form rejected at Travel:Initiate per TVL-V7) / D5 narrative-only via S9 [TRAVEL_CONTEXT] sub-section / D6 atomic single-segment / D7 PC + Tracked NPC parity (Untracked excluded per AIT-A8). **NEW aggregate** `actor_travel_state` (T2/Reality, sparse per-(actor, journey)); **NEW EVT-T sub-types** Travel:Initiate (EVT-T1) + Scheduled:TravelTick (EVT-T5 per EVT-G2 trigger source kind c FictionTimeMarker) + Travel:JourneyNarration (EVT-T6 LLM journey description V1+30d active); **NEW V1+30d service** travel-service. **No new capability claim** — travel is regular gameplay action via standard PC/NPC action authorization. **Cross-feature schema dependency**: EF_001 entity_binding gains `travel_journey_id: Option<JourneyId>` additive field per I14 (EF_001 schema_version 1 → 2 bump at TVL ship); coordinated via EF_001 closure pass. i18n: V1+30d ships I18nBundle per RES_001 §2 cross-cutting contract from day 1. **TVL_002 Composite Multi-Segment Travel (added 2026-05-16 DRAFT — V1+30d+ convenience layer over TVL_001 atomic travel; extends this `travel.*` namespace, no new namespace since composite travel is within the travel domain)**: **12 V1+30d+ composite rule_ids** (10 player-meaningful + 2 defensive) — `composite_actor_not_at_origin` / `actor_already_in_composite_journey` / `composite_cell_unknown` / `no_route_to_destination` / `composite_destination_is_origin` / `composite_insufficient_provisions` / `composite_mode_unavailable_v1plus30d` / `composite_path_too_long` (>20 segments) / `composite_replan_cap_exceeded` (defensive — re-plan cap ≤3 → Stranded) / `actor_in_composite_journey` (cross-feature gate on TVL_001 `Travel:Initiate` — manual atomic travel blocked while a composite is Active) / `composite_not_cancelable` / `composite_plan_discontinuous` (defensive). Reuses 5 TVL_001 rule_ids unchanged (`actor_untracked_excluded` / `soul_form_no_physical_travel` / `cross_channel_initiate_forbidden` / `route_layer_not_activated` / `actor_already_traveling`). **NEW EVT-T sub-types**: EVT-T1 `CompositeTravel:Initiate` + `CompositeTravel:Cancel` (player intent) + EVT-T8 `Forge:CancelCompositeJourney` (admin immediate cancel — registered in §4). **NEW aggregate** `composite_journey` (T2/Reality, sparse per-(actor, composite_journey); owned by the existing `travel-service` — no new service). **No new capability claim** — `CompositeTravel:Initiate`/`Cancel` are regular gameplay actions; `Forge:CancelCompositeJourney` uses the existing Forge admin capability. **Cross-feature schema dependency**: TVL_001 `actor_travel_state` gains `composite_journey_id: Option<CompositeJourneyId>` additive field per I14 (`actor_travel_state` schema_version bump; default-tolerant readers; coordinated via a TVL_001 closure pass at TVL_002 ship — mirrors the EF_001 `travel_journey_id` precedent). **Phase 0 CTV-D1..D7 LOCKED via single deep-dive 2026-05-16** with user `approve all` directive: D1 new `composite_journey` aggregate / D2 freeze-at-initiate + re-plan fallback / D3 smart overnight stops / D4 whole-journey provisions pre-pay / D5 destination-cell + reachability check / D6 self-cancel at next segment boundary / D7 read-only preview before commit. i18n: V1+30d+ ships I18nBundle per RES_001 §2 cross-cutting contract from day 1. **TVL_004 Travel Encounters (added 2026-05-16 DRAFT — V1+30d+ feature generating encounters during a journey; extends this `travel.*` namespace, no new namespace since encounters are within the travel domain)**: **10 NEW V1+30d+ encounter rule_ids** (6 user/admin-facing + 4 defensive) + 1 reused (`actor_untracked_excluded`) — `encounter_not_found` / `encounter_not_owned` / `encounter_already_resolved` / `encounter_approach_invalid` / `encounter_reroute_target_unknown` / `encounter_outcome_malformed` / `encounter_journey_not_active` (defensive) / `encounter_already_pending` (defensive) / `encounter_schedule_overflow` (defensive) / `encounter_table_unresolved` (defensive). **NEW EVT-T sub-types**: EVT-T1 `Encounter:Resolve` (player intent) + EVT-T6 `Encounter:SceneNarration` (LLM scene + outcome proposal); EVT-T8 `Forge:ResolveEncounter` (admin force-resolve/skip — registered in §4). **NEW aggregate** `travel_encounter` (T2/Reality, sparse per-(journey, encounter); owned by the existing `travel-service` — no new service). **No new capability claim** — `Encounter:Resolve` is regular gameplay; `Forge:ResolveEncounter` uses the existing Forge admin capability. **Cross-feature schema dependency** — TVL_001 `actor_travel_state` gains a NEW additive field `encounter_schedule: Vec<EncounterPoint>` (the pinned pre-rolled encounter schedule; schema_version bump per I14; HIGH-2 /review-impl fix — the schedule depends on mutable `world_geometry` biome + `route.kind`, so it must be pinned at `Travel:Initiate`, not re-derived). The TVL_001 closure pass for TVL_004 is therefore **schema + behavioral**: schema = the additive field + version bump; behavioral = the `Scheduled:TravelTick` generator gains clamp-to-crossed-point encounter detection + a pause that skips both `progress_fraction` and clock advancement. No `TravelStatus` enum change (the pause is signalled by an unresolved `travel_encounter` row). **NEW RealityManifest field** `encounter_tables: Option<Vec<EncounterTableDecl>>` (author-declared per `BiomeKind × RouteKind`; OPTIONAL — engine default table; immutable post-bootstrap). **Phase 0 CTE-D1..D7 LOCKED via single deep-dive 2026-05-16** with user `approve all` directive: D1 new `travel_encounter` aggregate + pause-via-unresolved-row / D2 deterministic pre-rolled schedule at Travel:Initiate / D3 EncounterKind 4-variant / D4 author-declared encounter tables + engine default / D5 pause + choice-based LLM-narrated + engine-validated resolution (combat abstracted) / D6 abstract ephemeral participants / D7 PC + Tracked NPC parity. i18n: V1+30d+ ships I18nBundle per RES_001 §2 cross-cutting contract from day 1. **TVL_003 Mount/Vehicle Travel (added 2026-05-16 DRAFT — V1+30d+ conveyance layer; extends this `travel.*` namespace, no new namespace)**: **8 NEW V1+30d+ mount rule_ids** (5 user-facing + 3 schema-level) + 2 reused (`mode_route_incompatible` + `cross_channel_initiate_forbidden`) — `mount_unknown` / `mount_not_owned` / `mount_kind_mode_mismatch` / `mount_not_at_origin` / `mount_in_transit` / `mode_requires_mount` / `mount_grant_cell_unknown` / `mount_owner_untracked` (the last + `mount_grant_cell_unknown` surface only at Forge-grant / reality-bootstrap time — defensive). Supersedes TVL_001's `travel.mount_unavailable` V2+ reservation. **NEW EVT-T8 sub-type** `Forge:GrantMount` (admin grant — registered in §4); mount selection otherwise rides TVL_001's `Travel:Initiate` (no new EVT-T1 sub-type). **NEW aggregate** `mount` (T2/Reality, sparse per-mount; owned by the existing `travel-service` — no new service). **No new capability claim** — `Travel:Initiate` is regular gameplay; `Forge:GrantMount` uses the existing Forge admin capability. **Cross-feature schema dependency** — TVL_001 `actor_travel_state` gains `mount_id: Option<MountId>` additive field, the `Travel:Initiate` EVT-T1 payload gains `mount_id`, and the `TravelMode` closed enum bumps 2 → 5 (R3 additive — activates the schema-reserved `ByBoat` + adds `OnHorseback`/`ByShip`/`ByCarriage`; the `mode_unavailable_v1plus30d` reject lifted for the four). All via the TVL_001 closure pass — **schema + behavioral** for TVL_003 (schema: the field + enum-bump above; behavioral: TVL-V5 lifts the `mode_unavailable` reject for the activated modes, TVL-V9 becomes the expanded mode↔route matrix, and the `speed_modifier(mode)` factor enters the tick + arrival formulas). The TVL_001 closure pass now serves three consumer features: TVL_002 `composite_journey_id` + TVL_004 `encounter_schedule` + TVL_003 `mount_id`/`TravelMode`. **NEW RealityManifest field** `canonical_mounts: Option<Vec<MountDecl>>` (author-declared starting mounts; OPTIONAL; market purchase deferred TVM-D1). **Phase 0 TVM-D1..D7 LOCKED via single deep-dive 2026-05-16** with user `approve all` directive: D1 NEW `mount` aggregate / D2 TravelMode 2→5 additive bump / D3 canonical + Forge acquisition / D4 hardcoded mode↔route matrix / D5 per-mode speed modifier / D6 minimal mount state / D7 ≤1 mount per journey + PC/Tracked parity. i18n: V1+30d+ ships I18nBundle per RES_001 §2 cross-cutting contract from day 1. **TVL_005 Group/Party Travel (added 2026-05-16 DRAFT — V1+30d+ group layer; extends this `travel.*` namespace, no new namespace)**: **12 NEW V1+30d+ party rule_ids** (10 user-facing + 2 schema-level) + 1 reused (`cross_channel_initiate_forbidden`) — `party_unknown` / `party_not_leader` / `party_not_forming` / `party_member_not_co_located` / `party_size_cap_exceeded` / `actor_already_in_party` / `party_member_untracked` / `party_member_insufficient_provisions` / `party_leave_while_traveling` / `party_member_already_traveling` / `actor_in_party` (the cross-feature `Travel:Initiate` gate — fires for a member of ANY non-Disbanded party) / `party_mode_unavailable_v1plus30d`. **NEW EVT-T1 sub-types** `Party:Form` / `Party:Join` / `Party:Leave` / `Party:Travel` (player intent); **NEW EVT-T8 sub-type** `Forge:DisbandParty` (admin — registered in §4). **NEW aggregate** `travel_party` (T2/Reality, sparse per-party; owned by the existing `travel-service` — no new service). **No new capability claim** — `Party:*` are regular gameplay; `Forge:DisbandParty` uses the existing Forge admin capability. **No cross-feature schema field** — a member's "party-bound" state is derived by querying `travel_party` (the aggregate is itself the membership index); the TVL_001 closure pass for TVL_005 is BEHAVIORAL ONLY (a cross-feature validator on `Travel:Initiate` blocking a member's solo travel while the party is Traveling, + the `Travel:Arrive` cascade extended to every bound-party member; no `actor_travel_state` field, no `schema_version` bump — unlike TVL_002 `composite_journey_id` / TVL_003 `mount_id` / TVL_004 `encounter_schedule`). **NEW RealityManifest field** `canonical_parties: Option<Vec<PartyDecl>>` (author-declared starting parties; OPTIONAL). **Phase 0 TVP-D1..D7 LOCKED via single deep-dive 2026-05-16** with user `approve all` directive: D1 NEW `travel_party` aggregate / D2 party rides the leader's TVL_001 journey, members bound / D3 Party:Form/Join/Leave formation / D4 ≤6 size cap / D5 OnFoot-only V1+30d+ / D6 encounter pauses the party, leader resolves, outcome to leader only / D7 PC + Tracked NPC members, ≤1 party per actor, leader committed mid-journey. i18n: V1+30d+ ships I18nBundle per RES_001 §2 cross-cutting contract from day 1. |
| `geography.*` | GEO_001 World Geometry Foundation (added 2026-05-13 DRAFT — 7th foundation feature; procedural geographic substrate beneath MAP_001 visual layer; fix cycle 2026-05-13 added 3 V1 rule_ids via /review-impl); **GEO_002 POL_001 Political Layer Generator (added 2026-05-14 DRAFT — V1+30d activation feature; /review-impl 2nd-pass 2026-05-14 added 5 MED-driven rule_ids bringing POL_001 total to 20 V1+30d; total 33 V1+30d when POL ships). GEO_003 SET_001 Settlement Generator (added 2026-05-14 DRAFT + /review-impl fix cycle — V1+30d sibling of POL_001; 4 HIGH inline-fixed + 4 MED added new rule_id `canonical_capital_collision`; adds 15 V1+30d rule_ids under shared namespace per SET-D7; total 48 V1+30d when SET ships) · **GEO_004 ROUTE_001 Route Network Generator (added 2026-05-14 DRAFT — V1+30d final sibling completing activation triangle POL + SET + ROUTE; adds 10 V1+30d rule_ids + 2 V2+ reservations under shared namespace per ROUTE-D7; total 58 V1+30d when ROUTE ships; `geography.layer_activation_deferred_v1` lifted for route layer at ROUTE ship)****; **13 V1 GEO_001 rule_ids** — duplicate_world_geometry / **invalid_channel_kind** — renamed from `invalid_channel_tier` by `SPG-R3` 2026-07-30; it read *"per MAP-2 ChannelTier::Continent per HIGH-2 fix"*, naming an enum retired by `SPG-R1` AND an inverted scope (a World contains continents, not the reverse) / cell_count_out_of_bounds (cells.len ∈ [1024, 16384]) / invalid_neighbor_degree (Voronoi cells have 3-12 neighbors) / parallel_array_length_mismatch (biomes/climate_zones/river_flux/is_coast parallel to cells) / sea_level_out_of_bounds (threshold ∈ [8192, 57344]) / delta_order_violation (prev_delta_id stale; parallel to TDIL-A8 worldline-monotonicity discipline) / creative_seed_immutable_v1 (post-bootstrap mutation requires V1+ T6 LLM proposal + T8 Forge approval via GEO-D12) / layer_activation_deferred_v1 (writes to political/settlement/route/culture/resource layers reject V1; activate via V1+ POL_001/SET_001/ROUTE_001/V2+ resource generator) / creative_seed_required_when_seeded (RealityManifest geography_seed declared without creative_seed) / **biome_override_water_transition_v1 (HIGH-1 fix — V1 admits only land-↔-land SetBiomeOverride; water↔land V1+ when biome+water-network re-derivation lands per GEO-D13)** / **pipeline_version_mismatch (MED-4 fix — promoted from V1+ reservation to V1 active; world_geometry pinned to its generator_pipeline_version at GeographyBorn; mid-life upgrades FORBIDDEN; new realities adopt latest pipeline_version)** / **cell_id_index_violation (LOW-1 fix — `cells[i].id == GeoCellId(i)` invariant enforced at GeographyBorn + every delta apply touching cells)**; +3 V1+ reservations: cross_reality_reference (V2+ Heresy axis) / delta_kind_v1plus_inactive (V1+ DeltaKind extensions: SetResourceOverride V2+ per MED-6 fix + MergeProvinces/SplitProvince/TransferProvinceToState/SetCultureRegion reject at V1) / resource_layer_activation_pending (V2+ resource distribution generator placeholder per GEO-D10). **Fix cycle changes**: 7 architectural improvements applied via /review-impl 2026-05-13 (HIGH-1 SetBiomeOverride coherence + HIGH-2 `ChannelTier` ref — retired since by `SPG-R1` + HIGH-3 delta_id namespace + MED-1 Forge:EditGeographyDelta registered in §4 EVT-T8 sub-shapes + MED-2 GeographyForkInherited reclassified EVT-T8→EVT-T4 System + MED-3 stage 4 sub-stages 4a/4b/4c for Lake-vs-Ocean connected-components + MED-4 pipeline_version pinned + MED-5 schema_version added + MED-6 SetResourceOverride V1→V1+ + LOW-1 GeoCellId==index invariant + LOW-2 applied_at_fiction_time dropped + LOW-3 RegionalLoreHook/NamingStyleDecl/CanonicalSettlementDecl declared + Settlement.canon_ref added). V1 user-facing rejects: delta_order_violation + creative_seed_immutable_v1 + layer_activation_deferred_v1 + creative_seed_required_when_seeded (others schema-level canonical seed validation). All 7 sub-decisions D1-D7 LOCKED via single deep-dive 2026-05-13 (D1 ChannelScoped continent / D2 single aggregate layered / D3 deterministic-base+delta-overlay / D4 single Voronoi mesh water-tags / D5 cells AND provinces two-tier / D6 explicit FK channel↔map / D7 inherit by reference deltas don't cascade). **8-stage generation pipeline** (V1 implements 1-4 substantively: Voronoi + heightmap + climate + biome+river; V1+ schema-reserves 5-8: political POL_001 / settlement SET_001 / route ROUTE_001 / culture spread). **CreativeSeed LLM-supplied direction** (12 WorldArchetype + 5 WorldScale + hemisphere + coastline + culture_hints + canonical_settlements + naming_styles); feeds procgen as constraints NOT post-hoc decoration. **Deterministic-base + delta-overlay editability** (replay = base + deltas in order; genuinely novel work — no Azgaar-style tool does this V1). **Single Voronoi mesh sea zone tagging** (water cells via Biome::Ocean/Lake/River; V2+ Paradox-style separate sea_zones+straits per GEO-D6 if naval gameplay needs explicit straits). **Multiverse snapshot fork inheritance** (deltas-at-fork-point copied; new child deltas stay local per MV6 + 4-layer canon L3-scope discipline). **Composition with foundation siblings** (MAP_001 V1+ position auto-derivation GEO-D5; PF_001 V1+ procedural place generation PF-D7; CSC_001 V1+ biome→skeleton selection; RES_001 V2+ resource distribution generator GEO-D10; PROG_001 V2+ cultivation-realm biome-conditioned training modifiers via GEO-D7 MagicalAnomaly ClimateZone extension). Algorithmic baseline: Patel dual-mesh (Apache 2.0) + O'Leary erosion (MIT) + Azgaar pipeline (MIT) per 2026-05-13 world-map landscape survey. **Strategy substrate readiness**: V1 schema reserves all strategy layers (provinces/states/settlements/routes/resources); V1+ POL/SET/ROUTE features activate via additive schema discipline; V2+ STRAT_001 consumes locked layers as read-only inputs. RealityManifest extension `continent_geometries: Vec<ContinentGeometryDecl>` OPTIONAL V1 (per §2 contract — single-cell SPIKE_01 realities omit; multi-cell realities REQUIRE per continent channel). i18n: V1 ships I18nBundle from day 1 per RES_001 §2 cross-cutting contract. **POL_001 V1+30d additive 20 rule_ids** (GEO_002 DRAFT 2026-05-14; Phase 0 D1-D7 LOCKED with user approval; /review-impl 2nd-pass 2026-05-14 added 5 MED-driven rule_ids): schema-level (7) — state_capital_not_in_province / province_state_mismatch / state_capital_cycle (defensive) / partition_not_total / transfer_target_state_unknown / **canonical_mode_no_seeds (MED-8 — Canonical mode + zero canonical_provinces degenerate)** / **canonical_state_province_mismatch (MED-9 — canonical_state.capital_province ↔ canonical_province.canonical_state cross-reference asymmetry)**; user-facing (13) — culture_hearth_unreachable_v1 (stage 8 hearth snap fail within radius 0.1) / province_seed_unreachable_v1 (stage 5 canonical seed snap fail) / partition_cap_exceeded (SplitProvince >8 partitions) / merge_source_count_invalid (MergeProvinces <2 sources) / merge_cross_state_forbidden (sources span different states) / merge_capital_orphan (would orphan state capital) / split_capital_no_heir (heir partition not declared) / transfer_capital_forbidden (province is state's capital) / culture_tag_unregistered (CultureTag not in creative_seed.naming_styles or existing culture_regions) / political_edit_capability_required (capability `can_edit_political_geography` missing — added §3 capability JWT) / **set_culture_region_empty (MED-1 — empty cell_ids payload wasted admin action)** / **split_partition_min_2 (MED-3 — separated from partition_cap_exceeded for distinct user-facing errors)** / **transfer_self_target (MED-4 — province.state_id == new_state_id defensive reject)**. **POL ship action**: `geography.layer_activation_deferred_v1` reject LIFTED for political layer specifically (stage 5/8 + 4 V1+30d DeltaKind paths no longer reject); other layers (settlement V1+30d SET_001 / route V1+30d ROUTE_001 / resource V2+) keep reject. **GeographyDeltaKind closed-enum bump per R3 additive**: 5 V1 + 4 V1+30d active (MergeProvinces / SplitProvince / TransferProvinceToState / SetCultureRegion). All 4 Tier 1 ImpactClass=Destructive per S5. **Capability addition** (§3): `can_edit_political_geography` JWT claim — required by all 4 V1+30d DeltaKinds AuthorizationGate; additive to existing `can_edit_geography` (V1 settlement/biome/route edits keep `can_edit_geography`; V1+30d political edits ALSO need new claim). **CreativeSeed schema_version 2 → 3 bump (HIGH-1 fix /review-impl 2026-05-14)** mirroring GEO_001b 1→2 precedent for additive evolution. Three additive fields: political_seed_mode (PoliticalSeedMode 3-variant — Canonical/Procedural/Hybrid default) + canonical_states (Vec<CanonicalStateDecl>) + procedural_density (ProceduralDensityHint 3-variant — Coarse/Medium/Fine default). Readers default-tolerant per R3: absent fields default to Hybrid / [] / Medium. LLM authoring template bumps `world_authoring/v1.tmpl` → `world_authoring/v2.tmpl` (§12Y.L2 CI fixture update; `authoring.template_version_deprecated` reservation in `authoring.*` namespace activates at POL ship). **+1 V2+ reservation NEW**: `geography.political_narrative_proposal_pending` (V2+ T6 NarrativePoliticalEdit Generator review per POL-D2 deferral). **Phase 0 D1-D7 LOCKED**: D1 politics+culture in one POL_001 / D2 hybrid capital seeds / D3 flood-fill+procedural fallback / D4 stateless provinces (Option<StateId>) / D5 culture × political independent overlays / D6 all 4 DeltaKinds V1+ / D7 share `geography.*` namespace. **SET_001 V1+30d additive 15 rule_ids (DRAFT 14 + /review-impl MED-2 canonical_capital_collision)** (GEO_003 DRAFT 2026-05-14; Phase 0 SET-D1..D7 LOCKED with user approval): schema-level (3) — settlement_not_found / settlement_density_hint_invalid / (informational under-fill metric for procedural Poisson-disk); user-facing (11) — canonical_settlement_unreachable_v1 (radius 0.1 snap fail) / canonical_settlement_cell_collision (earlier canonical occupied target) / settlement_target_uninhabitable (Relocate target = Ocean/Lake/Glacier) / settlement_cell_collision (one-per-cell invariant) / capital_relocate_outside_capital_province (Capital cannot relocate outside its state's capital province) / promote_settlement_no_change (PromoteSettlement at-least-one-Some requirement) / capital_role_invalid_context (PromoteSettlement to Capital — cell not in capital_province OR province already has Capital) / population_tier_out_of_range (∉ [0, 6]) / capital_demote_without_successor (demoting sole Capital without designating successor) / cannot_remove_capital_settlement (RemoveSettlement on Capital) / cannot_remove_settlement_with_channel (defensive V1+30d for V1+30d+ Settlement.channel_id activation per SET-D8) / settlement_edit_capability_required (capability `can_edit_settlement_geography` missing). **GeographyDeltaKind closed-enum bump per R3 additive** (extends POL bump): 5 V1 + 4 V1+30d POL + 3 V1+30d SET = **12 V1+30d active** (RelocateSettlement / PromoteSettlement / RemoveSettlement). RemoveSettlement Tier 1 ImpactClass=Destructive per S5 matching RemoveRoute V1 + 4 POL V1+30d; Relocate/Promote Tier 2 ImpactClass=Griefing matching AddNamedSettlement V1. **Capability addition** (§3): `can_edit_settlement_geography` JWT claim — additive to existing `can_edit_geography` + `can_edit_political_geography`; auth-service one-shot migration job at SET ship mirrors POL_001 MED-6 precedent. **CreativeSeed v3 → v4 additive fields** (1 additive field-set per I14 — mirrors POL_001 2→3 + GEO_001b 1→2): settlement_seed_mode (SettlementSeedMode 3-variant — Canonical/Procedural/Hybrid default) + settlement_density_hint (SettlementDensityHint 3-variant — Sparse/Medium/Dense default). LLM authoring template bump v2.tmpl → v3.tmpl. **+2 V2+ reservations NEW**: `geography.settlement_narrative_proposal_pending` (V2+ T6 NarrativeSettlementEdit Generator per SET-D2) / `geography.multi_settlement_per_cell_pending` (V2+ urban-cluster modeling per SET-D9). **POL coordination**: stage 5 (POL provinces+states) → stage 6 (SET settlements + Province.capital_settlement_id) → stage 8 (POL culture spread) order locked. SET_001 stage 6 STEP D1 reads POL_001 state.capital_province output for Capital role assignment; closes POL_001's V1+30d-standalone None capital_settlement_id linkage. **Phase 0 SET-D1..D7 LOCKED**: D1 pure placement+role V1+30d / D2 hybrid canonical+procedural / D3 hybrid population_tier / D4 hybrid role-assignment political-first-then-terrain / D5 SettlementDensityHint default Medium / D6 3 new V1+30d DeltaKinds / D7 share `geography.*` namespace. |
...
| `item.*` | **PL_007 Item Foundation + PL_007b Inventory & Capacity** (added 2026-07-26 DRAFT — closes audit finding AUD-F5 "*Item / equipment module absent — STRUCTURAL, V1-blocking*" + the "*Inventory 🟡 partial*" row; the file EF_001 §Defers-to named and never had). **13 validator-table V1 rule_ids (PL_007 §9.1)** — `unknown_def` (instance's `def_id` ∉ `RealityManifest.item_defs`; ITM-V1) / `not_held` (Equip/Unequip/Use/Give-tool where `entity_binding.location ≠ HeldBy(agent)`; ITM-V2) / `slot_unknown` (`equip.slot ∪ also_blocks` ⊄ active slot profile; ITM-V3) / `slot_occupied` (occupied by an item that cannot be implicitly unequipped — V1+ locked/cursed gear; ITM-V4) / `equip_requirement_unmet` (PROG_001 `ProgressionLevel` gate; ITM-V5) / `not_equippable` (`def.equip` is None; ITM-V6) / `no_charges` (Use with `charges == Some(0)`; ITM-V7) / `no_use_effect` (`def.use_effect` is None; ITM-V8) / `not_in_cell` (`Item:PickUp` where item ∉ agent's cell — **cell membership, NOT fine position**, because fine position is RTM-ephemeral (RTM-A1 / ILR-A2 layer 2) and a durable validator must not depend on state absent from the log; ITM-V9) / `def_collides_with_resource_kind` (canonical seed: an `ItemDefId` equal to any `resource_kinds.kind_id` — **this is what makes ITM-A2's representation rule enforced rather than asserted**; ITM-V10 / ITM-C2) / `strike_kind_unsupported` (`Strike.strike_kind` ∉ `def.equip.combat.strike_kinds` — closes the "a club cannot Slash" hole PL_005b §3.1 left open; ITM-V11) / `equipped_transfer_forbidden` (Drop/Give of an item currently in an `actor_equipment` slot — keeps ITM-C4 true; ITM-V12) / `engine_owned_field_supplied` (payload carries `charges`/damage/price — the ITM-A4 LLM-zero-item-math assertion; ITM-V13). **+4 V1 `item.inventory.*` rule_ids (PL_007b)** — `cap_exceeded` (single atomic transfer at capacity — hard reject; ITM-V14, dormant-by-config in V1 since `inventory_cap` is None, **but test-covered via AC-INV-4 with a cap forced on**, so it is not untested code at activation) / `cap_partial` (**a warning on an ACCEPTED turn**, not a reject — bulk drains partial-fill with the remainder left at source, because destroying it is unrecoverable and failing the whole drain wastes the action; ITM-V15) / `cell_cap_exceeded` (`Item:Drop` into a cell at `cell_item_soft_cap` — **live V1**; the cap governs *proposals* only, holder-death cascades merely warn, since a cascade is a consequence and must never fail; ITM-V16) / `digest_bound_violated` (the ITM-A9 fixed-size-context assertion — **live V1**; it CAN fail on a mis-tuned `digest_top_n`, a long-rendering locale, or a future 9th `ItemClass`, and fails loudly at assembly rather than silently inflating every prompt; ITM-V17). **+4 V1+ reservations**: `durability_exhausted` (RES-D4 wear) / `bound_to_other_actor` (soulbound ITM-D5) / `container_depth_exceeded` (ITM-D3 / EF-D3) / `ammunition_missing` (ITM-D8). **V1 user-facing rejects**: not_held · slot_occupied · equip_requirement_unmet · not_equippable · no_charges · no_use_effect · not_in_cell · strike_kind_unsupported · equipped_transfer_forbidden · all 3 non-internal `item.inventory.*`. **Schema/assertion-level**: unknown_def · slot_unknown · def_collides_with_resource_kind · engine_owned_field_supplied · digest_bound_violated. **NEW validator sub-stage 3.5.e `item_structural`** (registered in `03_validator_pipeline_slots.md`) — and **5 checks are explicitly DELEGATED, not duplicated**: item-exists + destroyed/removed + affordance-missing → EF_001 3.5.a; place-destroyed → PF_001 3.5.b; drop-tile-not-placeable → CSC_001 `csc.item_on_non_placeable` 3.5.d (PL_007 §9.1 non-vacuity discipline — a check that cannot fail is noise). **Supplies the missing Lex operand**: `ItemDefDecl.lex_tags` is the Stage-4 input for PL_005b §8.2's "*Use → Lex severity: CRITICAL — item × reality compatibility matrix*", which was declared as the primary cross-reality reject path and had no item side to match against. **9 axioms** ITM-A1 (def/instance split on the System tenancy tier — only `Forge:EditItemDef` writes a def) / A2 (**the representation rule** — instanced ⇒ entity, fungible ⇒ `resource_inventory`, exactly one per thing) / A3 (items carry **modifiers, never stats** — against DF07_001's closed `StatSlot` enum; PL_007 ships the `EquipmentStats` impl DF7-D1 deferred to it, with **zero DF07 change**, and supplies `InstrumentTag` to make PROG_001/DF07 `instrument_match` evaluable at all — **resolving PROG-D15** and repairing a three-way latent break where `InstrumentMatch::Specific(ResourceKind)` could no longer name a wielded item) / A4 (LLM-zero-item-math) / A5 (equipment is a slot assignment, not a location — `LocationKind` stays closed, no 5th variant) / A6 (V1 flat inventory — **resolves EF-D3 by decision**; V1 holder depth ≤1 is *why* `entity.cyclic_holder_graph` is structurally unreachable rather than merely untested) / A7 (item-management as 4 EVT-T1 `Item:*` sub-types, **not** new InteractionKinds — PL_005's closed-set proof unchanged; TG-A3 precedent) / A8 (one inventory, two stores, no third) / A9 (**inventory context is fixed-size** ≤29 lines for any inventory of any size — the flat-cost family with TG-A1/COMB-A1; discharges EF_001 §3.1's one-sentence AssemblePrompt-summarization promise). **Resolves**: EF-D3 (by decision) · **RES-D1 by withdrawal** (`ResourceKind::Item` withdrawn/never-activated; `ResourceBalance.instance_id` permanently unused, retained-never-reused per I15) · RES-D2 + RES-D4 (schema homes) · the `ItemId`/`ItemInstanceId` spelling drift (ITM-C1 alias) · EF_001 Q6b `inventory_cap` accounting. **6 cross-aggregate consistency rules** ITM-C1..C7. i18n: ships `I18nBundle` from day 1 per RES_001 §2. 23 acceptance criteria (AC-ITM-1..13 + AC-INV-1..10); 19 deferrals (ITM-D1..D19); 8 open questions (ITM-Q1..Q8). **Namespace total: 24 V1 rule_ids = 21 rejects + 3 warnings** (13 from the ITM-V1..V13 validator table + 2 from consistency rules ITM-C5/C6 (`def_invalid` / `unknown_holder`) + 3 from PL_007b (`inventory.cap_exceeded` / `cell_cap_exceeded` / `digest_bound_violated`); the 2 warnings are `instrument_tag_unreferenced` (ITM-C7) and `inventory.cap_partial`), + 4 V1+ reservations. |
...
pub struct RealityManifest {
    ...
    //   world_channel_id: ChannelId,                       // MUST resolve to a MapKind::World node; reject geography.invalid_channel_kind (SPG-R3: was continent-tier + invalid_channel_tier)
    ...
    //   kind: GeographyDeltaKind (5 V1 GEO_001: AddNamedSettlement / RenameRegion / SetBiomeOverride /
    //         AddRoute / RemoveRoute; +4 V1+30d POL_001 active per GEO_002 DRAFT 2026-05-14:
    //         MergeProvinces / SplitProvince / TransferProvinceToState / SetCultureRegion;
    //         +3 V1+30d SET_001 active per GEO_003 DRAFT 2026-05-14:
    //         RelocateSettlement / PromoteSettlement / RemoveSettlement — total 12 V1+30d active —
    //         closed-enum bump per R3 additive; +1 V2+ reservation: SetResourceOverride V2+ when GEO-D10 ships;
    //         +2 V2+ reservations from POL-D14: CreateState / DestroyState V2+),
...
> **⚠ RETYPED 2026-07-30 — `ChannelTier` → `MapKind` (`SPG-R1`, mechanical half).** Four fields below
> were keyed by `ChannelTier`, an enum **retired** by [`36_map_architecture.md`](../36_map_architecture.md)
> `SPG-R1`. This is a **machine contract** — the shape that becomes real Rust — so a retired type here is
> not a documentation defect, it is a compile error waiting to be written. The rename is not optional and
> is applied now.
>
> **⚠ The SEMANTIC half is NOT applied, and is a real open question — see `SPG-R13`.** Keying by
> `MapKind` is *type-correct* but now nearly degenerate. This contract was written when `ChannelTier`
> had **four** non-cell tiers, each with its own tilemap at a decreasing zoom (hence the
> `256 / 192 / 128 / 64` default — four values for four tilemap-bearing tiers; **the list was never
> wrong**). Under `MapKind`, `SPG-R9` puts the tilemap on **`Locale`** alone: `World` and `Region` are
> served by `GEO_001`'s Voronoi mesh — a render target that did not exist when TMP_001 was drafted —
> and `Domain` carries `CSC_001`'s interior composition instead. So a four-tier zoom hierarchy of
> tilemaps collapses toward **one** kind, and these maps would carry a single meaningful key.
>
> **Not resolved here on purpose.** `TMP_001` owns this contract and it is still DRAFT; collapsing
> another feature's schema while holding a `_boundaries` lock for an unrelated rename would be exactly
> the scope creep this folder's mutex exists to prevent. Recorded as `SPG-R13`, routed to TMP_001.

```rust
pub struct RealityManifest {
    // ... existing fields ...

    /// V1+30d: None → engine uses `tilemap_defaults.default_template_per_kind` lookup.
    /// Some(map) → per-kind template selection; kinds not in map fall back to defaults.
    /// SPG-R1: was `HashMap<ChannelTier, _>`. ⚠ SPG-R13 — under SPG-R9 only `Locale`
    /// bears a tilemap, so this map has one meaningful key; TMP_001 to collapse or justify.
    pub tilemap_templates: Option<HashMap<MapKind, TilemapTemplateRef>>,

    /// V1+30d: None → engine uses hardcoded sensible defaults (parallel mode, LLM off).
    /// Some(defaults) → author overrides grid sizes, LLM enablement, single-thread mode, skip list, etc.
    pub tilemap_defaults: Option<TilemapDefaults>,
}

pub struct TilemapDefaults {
    // SPG-R1: the three maps below were keyed `ChannelTier`. The 256/192/128/64 default
    // was the four NON-CELL tiers at decreasing zoom — correct as written, and the reason
    // SPG-R13 is a collapse question rather than a defect.
    pub grid_size_per_kind: HashMap<MapKind, GridSize>,              // ⚠ SPG-R13 (was 256×256 / 192×192 / 128×128 / 64×64 across 4 tiers)
    ...
    pub skip_kind: BTreeSet<MapKind>,                                // kinds to NOT generate tilemaps for; UI falls back to MAP_001 graph view. ⚠ SPG-R13
...
**Engine defaults when both fields are None:** every non-cell channel gets a `tilemap_view` at RealityManifest bootstrap via canonical default template per tier. Cell channels skipped (CSC_001 authoritative). ⚠ **This sentence is the `SPG-R13` question in prose** — "every non-cell channel" was a 4-tier set under `ChannelTier`; under `MapKind` the corresponding set is `Locale` alone (`SPG-R9`), with `World`/`Region` served by the geo mesh and `Domain` by `CSC_001`. Left as written pending TMP_001's decision, and flagged rather than silently reinterpreted.
...
### §2.Z DF07_001 Actor Stat Block RealityManifest extensions (DRAFT 2026-07-26)

**DF07_001 Actor Stat Block** adds 3 OPTIONAL (engine-defaulted) fields. All three may be absent: a reality
that declares neither `progression_kinds` nor `stat_slots` still resolves a valid block from engine defaults
and is combat-playable (DF7-A6) — the same discipline as PROG_001 §9.4's default formula.

- `stat_slots: Vec<StatSlotDecl>` — the author's **projection** of their PROG_001 kinds into the closed
  engine slot set. `StatSlotDecl { slot: StatSlot, base: i32, terms: Vec<StatTerm>, clamp: StatClamp }`.
  `StatTerm` is the PROG_001 §9.2 shape (`{ kind_id, weight, instrument_match }`), now DF7-owned; it
  **supersedes** `StrikeFormulaDecl.offense_terms` / `.defense_terms`. Weights are authored as decimals and
  evaluated as milli-units (DF7-A4 — no float in the stat path).
- `stat_archetypes: HashMap<ActorClassRef, StatArchetypeDecl>` — flat blocks for actors with **no**
  `actor_progression` row (Untracked NPCs per AIT-A8 quantum-observation). Keyed by the existing ACT_001
  `ActorClassRef`; no ACT schema change.
- `stat_tuning: Option<StatTuningDecl>` — `{ base_move: u8 = 3, speed_per_tile: u16 = 50, max_move: u8 = 10 }`;
  the constants of COMB_002 TG-A3's `move_range = base_move + ⌊speed / speed_per_tile⌋` formula. Authors tune
  the constants, never replace the formula. Stage-0 validated (`speed_per_tile ≥ 1` — it is a divisor;
  `max_move ≥ 1`; `base_move ≤ max_move`) → `stat.tuning_invalid`.

**Interaction with existing extensions:** `vital_profiles` (RES_001) is **not** removed — `VitalProfile.max_value`
degrades to the `base` of the `MaxHp` / `MaxStamina` slot decls, so existing declarations keep working and
progression/equipment/status layer on top (resolves RES-Q1). `strike_formula` (PROG_001) keeps only
`damage_floor` + `post_damage_hooks`; its stat terms move here. All additive per I14. See
[DF07_001 §5](../features/DF/DF07_pc_stats/DF07_001_actor_stat_block.md).

### §2.AA PL_007 / PL_007b Item + Inventory RealityManifest extensions (DRAFT 2026-07-26)

**PL_007 Item Foundation** adds 4 OPTIONAL (engine-defaulted) fields. A reality declaring none of them
has no instanced items — a valid configuration (a pure fungible-economy or pure-conversation reality),
which is why all four are optional rather than required:

- `item_defs: Vec<ItemDefDecl>` — the author-declared item templates. **System-tier per ITM-A1:** only
  `Forge:EditItemDef` writes them at runtime; no regular actor may mutate a def (the platform tenancy
  rule — a System row is admin-write, user-clone-not-edit). `ItemDefDecl` = `def_id` + `display_name`
  + `description` (both `I18nBundle`) + `class` (`ItemClass`, 8 V1) + `affordance_overrides` +
  `equip: Option<EquipDecl>` + `use_effect: Option<UseEffectDecl>` (7 V1 variants, each dispatching to
  an aggregate **another feature already owns** — RES_001 vital/resource, PL_006 status, Oracle) +
  `max_charges` + `consume_on_exhaust` + `price` (RES_001 `PriceDecl`) + `weight` (V2) + `lex_tags`
  (**the Stage-4 operand PL_005b §8.2's "CRITICAL item × reality compatibility matrix" declared and
  never had**) + `instrument_tags` (PROG_001/DF07 `instrument_match` operand) + `destructible`.
  A `def_id` colliding with any `resource_kinds.kind_id` **rejects bootstrap** (ITM-V10/ITM-C2) — this
  is what makes the ITM-A2 representation rule enforced rather than asserted.
- `equip_slot_profile: Option<EquipSlotProfileDecl>` — None ⇒ engine default **6 slots**
  (`main_hand`/`off_hand`/`head`/`body`/`feet`/`accessory`). Per-race body plans are ITM-D2 with the
  IDF_001 `RaceDecl` seam named; `profile_id` exists now so activation adds a reference, not a schema.
- `initial_item_distribution: Vec<ItemDistributionDecl>` — seed items at bootstrap;
  `{ holder: EntityRef (Actor or Cell V1), def_id, count, equip_on_seed }`. `count: u32` mints **n
  distinct instances** (ITM-A2 — no stacking).
- `inventory_defaults: InventoryDefaults` (PL_007b) — `{ default_cap: Option<CapacityProfile>,
  digest_top_n = 8, digest_resource_top_m = 6, cell_item_soft_cap = 100 }`. **`default_cap` is None in
  V1**, preserving the EF_001-Q6 / RES-D2 locked uncapped posture; activating a cap is therefore a
  **config** change, not a schema change. The two guards that make uncapped tolerable are mandatory,
  not optional: the ≤29-line `InventoryDigest` (ITM-A9) and `cell_item_soft_cap`.

**Interaction with existing extensions:** `resource_kinds` (RES_001) is untouched and remains the
fungible store — the two are partitioned by ITM-A2, not overlapping. `RES_001.prices` gains item
entries via `ItemDefDecl.price`. `vital_profiles` / `stat_slots` are unaffected: items reach stats only
as `StatModifier`s through DF07's resolution law (ITM-A3), never by declaring a stat value. All
...
| `Forge:CancelCompositeJourney { composite_journey_id, reason }` (added 2026-05-16 DRAFT — immediate admin cancel of a composite journey; cancels the active atomic segment + closes the `composite_journey` with status=Canceled; proportional provisions refund) | TVL_002 Composite Multi-Segment Travel |
...
| `Forge:SpawnItem { def_id, holder: EntityRef, count, equip_on_spawn }` (added 2026-07-26 PL_007 DRAFT — mints `count` instances with `ItemOrigin::ForgeSpawn`; each creates an `item_instance` + an EF_001 `entity_binding` row in ONE transaction per ITM-C3) | PL_007 Item Foundation |
