...
- **Owner:** main session 2026-04-26 (FAC_001 Faction Foundation DRAFT cycle — 3 commits: DRAFT + Phase 3 + closure+release)
- **Claimed at:** 2026-04-26T<current>
- **Expected work:** Promote FAC_001 from CONCEPT (Q1-Q10 LOCKED via 49a17ed) → DRAFT. 3-commit cycle (DRAFT + Phase 3 + closure pass with lock release at final commit). Boundary: register `faction` (sparse) + `actor_faction_membership` aggregates + `faction.*` 8 V1 rule_ids + `canonical_factions` + `canonical_faction_memberships` REQUIRED V1 RealityManifest + `FAC-*` prefix + 3 EVT-T8 Forge sub-shapes + 2 EVT-T4 System sub-types + 99_changelog. Resolves FF-D7 master-disciple + IDL-D2 sect membership. 3 Q-REVISIONS (Q2 Vec+cap=1 / Q4 numeric-only / Q7 defer sworn V1+).
