...
| `onboarding.*` | PO_001 Player Onboarding | 7 V1 | 5 V1+ | Stage 0 canonical seed (onboarding_config OPTIONAL V1 + canonical_pcs subset validation) + Stage 7 Forge admin (Forge:CompleteOnboarding V1 active orchestrator + 2 schema-reserved Create/UpdateDraft per PO-D3) + cross-aggregate cascade C30 (PC creation 14-feature chain on Forge:CompleteOnboarding) + 5 schema validators C31-C35 |

**Total V1 rule_ids across all namespaces:** ~44+ (entity/place/map/csc) + ~117+ (Tier 5 substrate + first user-visible feature; ~96 prior + 14 DF5 + 7 PO) = ~161 V1 reject rules total across the engine. ~120+ are **Stage 0 schema validators** (canonical seed + Forge admin + cross-aggregate cascade); ~44+ are LLM-output pipeline validators (Stage 3.5+).
...
| **C30 (PO-C1): PC creation cascade orchestration** | PO_001 (orchestrator over 14 features) | RUNTIME cross-aggregate cascade — Forge:CompleteOnboarding EVT-T8 triggers synchronous 14-feature chain same turn (PCS_001 Forge:RegisterPc + Forge:BindPcUser → ACT_001 ActorBorn → EF_001 EntityBorn at spawn_cell → IDF_001..005 Race/Language/Personality/Origin/Ideology Born → FF_001 FamilyBorn (if family declared) → FAC_001 FactionMembershipBorn (if faction declared) → REP_001 ReputationBorn (per declared rep row) → TIT_001 TitleGranted (V1+ origin pack default_titles per PO-D-#) → PROG_001 ProgressionInit (per progression_kind ClassDefaultDecl) → RES_001 ResourceInit (vital_pool + currencies + items) → TDIL_001 actor_clocks-init → SR11 first turn state machine init → emit OnboardingCompleted EVT-T1 narrative milestone) | (no reject; cascade applies all events atomically; per-feature reject rules fire if individual stage validation fails) |
| **C31 (PO-C2): OnboardingConfigDecl.canonical_pcs subset of canonical_actors[kind=Pc]** | PO_001 + ACT_001 | At RealityManifest bootstrap, verify each ActorRef in onboarding_config.canonical_pcs resolves to canonical_actors with kind=Pc; reject if any ref invalid | `onboarding.canonical_pc_unauthorized` (V1+ if needed; V1 silently filters invalid refs) |
| **C32 (PO-C3): OnboardingConfigDecl.default_spawn_cell ∈ RealityManifest.places (cell-tier)** | PO_001 + PF_001 | At RealityManifest bootstrap, verify default_spawn_cell resolves to cell-tier ChannelId in places | `onboarding.spawn_cell_unauthorized` (PO_001 namespace) |
