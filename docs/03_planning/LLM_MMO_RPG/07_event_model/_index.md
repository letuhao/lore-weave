...
| [features/00_tilemap/](../features/00_tilemap/) | **V1+30d consumer (annotation added 2026-05-14 closure-pass-extension).** TMP_001..TMP_008b registers sub-types under EXISTING categories — **no new EVT-T\* category, no new EVT-A\* axiom, no EVT-P\* / EVT-V\* / EVT-S\* additions.** Sub-type registrations per TMP_001 §2.5: (a) EVT-T4 System gains `TilemapBorn` + `ZonesPlaced` sub-types (TMP-owned, producer = DP-Internal RealityBootstrapper for `TilemapBorn`; tilemap-service zone-placer for `ZonesPlaced`); (b) EVT-T3 Derived gains 2 new `aggregate_type` values — `tilemap_view` + `tilemap_template`; (c) EVT-T8 Administrative gains 3 new `Forge:*` sub-shapes — `Forge:RegenTilemap` + `Forge:EditTemplate` active V1+30d, `Forge:OverridePlacement` schema-reserved V1+30d / active V3; (d) EVT-T5 Generated + EVT-T6 Proposal reservation for L3+L4 LLM payloads (V2; V1+30d engine-only per AC-TMP-10). All sub-types subject to EVT-A11 sub-type ownership discipline + EVT-A12 extensibility framework. Boundary registry rows owned in `_boundaries/01_feature_ownership_matrix.md` (now lists TMP as new sub-type owner alongside EF_001/PF_001/MAP_001 for T4; alongside MAP_001/PF_001 for T3; alongside WA_003 + DF05_001 for T8). |
