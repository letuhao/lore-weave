...
| T1b | a citation supports its claim | live bytes from the **sealed** corpus; quote mismatch = refusal; span disjointness for lists | ✅ `GamegenCorpus.verify_citation` fetches the bytes at `[chunk_id, span)` from the corpus the seal covers and refuses three ways — chunk outside the seal, span outside the chunk, bytes differ — **returning what the corpus actually says**, because rendering the claimed quote would have a reviewer compare the model against itself |
...
| T3 | nothing reaches players unreviewed | `gamegen_decision` with `approved_by`; **`batch_id`/`batch_size` make bulk visible**; S3 refuses a batch above a declared size | ✅ …and now the **S5 gate too**: `gamegen_candidate.review_status`, with a `CHECK` making a **refused candidate unapprovable**. v1 named a gate at S5 and the schema had nowhere to record that anyone looked |
