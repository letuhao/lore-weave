...
Users pick which reality to play in. UI shows:
- Reality name, description
- Origin ("from book" vs "forked from R_α at event 12345")
- Canonicality hint (discovery-only: `canon_attempt`, `divergent`, `pure_what_if`)
- Population (current players, cap)
- Notable events (recent L3 headlines: "Alice dies in chapter 3")

Flat list. No "main server." No nested tree.
...
### M1. Reality discovery problem (C3 variant)

With N realities per book, which does a player join? If discovery is poor, each reality is lonely. Mitigations:
- **Default landing reality** per book (the most populated `canon_attempt` reality)
- **Interest-based matching** (language, canonicality, population)
- **Friend-follow** (join the reality your friends are in)
