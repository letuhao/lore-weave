...
| ~~**T4**~~ ✅ **BUILT 2026-08-22** | `layer_registry` — one row per `LayerDef` | **per-ruleset** (pinned per reality-epoch) | ~~the whole layer model is new~~ **`0029_layer_registry`** — and building it is what let `SDF-R5` land | every feature |
...
| ~~**T9**~~ ✅ **BUILT 2026-08-22** | `encounter` (`R-6`) | per-reality | ~~`Arena` and `Encounter` are different things; doc 36 has only the node~~ **`0030_encounter`** — the closure, not the fight | ~~`SPG-D1`~~ **unblocked** |

~~**T3 and T9 are the two the sealed design does not merely lack — it has the wrong shape for them.**~~ **Both now BUILT** — `T3` as `0028_portal` (`SDF-R3`), `T9` as `0030_encounter` (`SDF-R4`). They were the two hardest because a wrong shape is worse than an absence, and both needed their amendment applied before a table could be written.
...
**Status: ALL NINE are APPLIED (2026-08-22).** `SDF-R4` landed as its **structural** half only — the boundary, where it sits and what crosses it — with damage, initiative and turn order left to `COMB_*` by name. Doc 36's `SPG-A17` now carries the integer `Transform`
...
| `SDF-R4` ✅ **APPLIED 2026-08-22** | `SPG-D1` | in-place combat needs an **Encounter closure**; `Arena` and `Encounter` are different things | **APPLIED as `SPG-A19` + `0030_encounter` — the STRUCTURAL half only, and the line is drawn explicitly.** `carved_arena` NULL/not-null is the whole branch, so `SPG-D1`'s hybrid siting is one line rather than two code paths; `opening_frame` carries `R-7`'s closure, without which `SPG-D1` **was not satisfiable** — an in-place fight in a shared `Locale` is an OPEN world. **Damage, initiative, turn order and participants are deliberately absent: `COMB_*`'s, and a column here would decide them by accident** |
| `SDF-R5` ✅ **APPLIED 2026-08-22** | `SPG-A2` | layers bind to `MapKind`; `home_kinds` required, validated on write | **APPLIED as `0029_layer_registry` (`T4`).** It had been PROPOSED for one reason — there was no registry to require anything OF. `home_kinds` is `NOT NULL` with **`cardinality > 0`**, because an empty set means *every node*, which is `R-29`'s 300 000-row outcome arriving as an **omission rather than a decision**; and every member must be a real `MapKind`. **Every policy column is `NOT NULL` with NO `DEFAULT`** — `SDF-A6`'s "omitting a field is a compile error", expressed as the schema's version of one |
