...
> decisions `SDF-D1..D6`, findings `SDF-F1..F9`, amendments `SDF-R1..R9` — **eight applied, one remaining** —
...
**Status: EIGHT of nine are APPLIED (2026-08-22). One remains — `SDF-R4`, an Encounter closure, which is `COMB_*`'s to design.** Doc 36's `SPG-A17` now carries the integer `Transform`
