...
| TR-4 | M04 acceptance evidence pack | NOT-BUILT (the one real translator gap) | Matrix shows "⚠️ Smoke only" | **Wiring confirmed (2026-05-31 code audit):** FE→gateway(`/v1/translation`, `gateway-setup.ts:268`)→translation-service:8087→**dedicated `translation-worker` container** (`docker-compose.yml:361`, `Dockerfile.worker`)→provider-registry LLM SDK→`chapter_translations` version→notification. Real RabbitMQ consumer (`worker.py`) with retry/DLQ/stale-recovery. Pipeline genuinely wired, **not a stub**. Outstanding = a **lightweight live acceptance run** (writer→translate→read) per OPS-3; TR-5 already live-smoked the start path. |
