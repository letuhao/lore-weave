...
> **Pre-existing failures found (NOT this cluster, flagged for triage):** (a) `test_generation_repair.py::test_scalar_value_coerced_and_recorded` contradicts the 046 do-now CJK-reject (confirmed failing on clean HEAD — the 046 commit `7be1b18d` didn't update it); (b) `test_readiness/api_contract/health/metrics` need `loreweave_obs` (image-only; not pip-installed in a local venv — F-LIVE-3 class).
