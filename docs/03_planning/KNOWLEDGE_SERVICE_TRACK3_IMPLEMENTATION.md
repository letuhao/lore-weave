...
| K19a.7 i18n polish (remaining) | ✅ | session 49 | Converted every hardcoded toast/label/body string in the knowledge feature to i18n keys. `useProjectState.runAction(t, labelKey, ...)` + `ProjectRow.runDestructive` same; PrivacyTab fully converted (15 strings); new `projects.toast.*` block + `privacy.*` top-level namespace × 4 locales. `ACTION_KEYS` const makes callsite typos a compile error. 5 review-impl findings (1 MED + 3 LOW + 1 COSMETIC); 4 fixed, 1 accepted silently. Zero hardcoded toast strings remaining in the feature. |
| K19a.8 Storybook | ✅ | session 49 | Shipped: Storybook 10 + @storybook/react-vite + addon-a11y + addon-docs; `.storybook/{main.ts,preview.tsx,MockAuthProvider.tsx}` with Vite alias `@/auth` → mock; 14 stories covering all 13 ProjectMemoryState kinds (Failed has 2: retryable + no-retry). `npm run storybook` + `build-storybook` both work. 5 review-impl findings (2 MED + 2 LOW + 1 COSMETIC); 4 fixed, 1 documented (Playwright binaries). Dialog stories deferred as D-K19a.8-01 (needs MSW). **Track 3 K19a cluster fully complete.** |
