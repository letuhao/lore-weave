...
### [D] 15. Event timeline lacks metadata (chapter/scene/block); needs in-book time detection
...
> **SUBSTANTIALLY ADDRESSED — the implementable metadata + filters already shipped (C6/C10/D-K19e timeline track):**
> - **Chapter provenance is surfaced** — `TimelineEventRow` renders the BE-resolved `chapter_title` (UUID-suffix fallback when the chapter was trashed; C6/D-K19e-β-01) plus the localized `time_cue` and date, so the timeline reads as a chronology with per-event chapter anchoring.
> - **The BE date/chronological filters ARE exposed in the FE** (the RC's "same gap as #12" is closed) — `TimelineFilters` (C10/D-K19e-α) ships an entity picker, an inclusive **chronological range** (after/before on `chronological_order`), and **ISO date-range bounds** (`event_date_iso`, partial `YYYY`/`YYYY-MM`/`YYYY-MM-DD` tolerated for fictional calendars). Plus free-text search + page-size from the #12 track.
> **Deferred (the genuine remainder — both clear the gate, structural / naturally-next):**
> 1. **Block/scene-level provenance** — events are extracted per-chapter so intra-chapter position is lost; adding a `block_index`/scene anchor needs an Event-model + extraction-prompt + Neo4j-repo change (schema change, gate 2) → `D-KG-EVENT-SCENE-PROVENANCE`.
> 2. **Advanced in-book-time detection** — inferring real in-story time beyond the already-captured `event_date_iso`/`time_cue` is an LLM-inference feature; value/feasibility unproven until the scene-anchor lands (gate 3, naturally-next) → `D-KG-INBOOK-TIME-DETECTION`.
> Marked `[D]`: the UX bug as filed (no metadata to trace events) is resolved; the residual is a tracked structural feature, not a quick fix.
...
### [x] 18. Glossary assistant planner loops forever (self-recheck loop)
...
> **DONE (`ce96406b`):** the MECHANICAL hard-stop the soft skill rule couldn't guarantee — `_stream_with_tools` caps `PLANNER_TOOLS` to `PLANNER_CALLS_PER_TURN_CAP=1` per turn. The first `glossary_plan` runs; a 2nd+ call in the same tool loop is short-circuited BEFORE the MCP transport (no ~39s planner run) with a tool result steering the model to present/confirm the existing plan (Kiro-style logic control). Mirrors the H7 Tier-A volume cap; cross-turn recovery re-plans (fresh pass/counter) are never blocked. +2 TDD tests (`TestPlannerHardStop`); test_stream_tools 40/40. The skill's now-stale anti-loop rationale was also corrected (`f61a83de`).

### [x] 19. Hardcoded `google/gemma-4-26b-a4b-qat` planner called after stopping session
...
>
> **DONE (`D-PLANNER-INFLIGHT-ABORT` closed) — the in-flight-abort propagation gap fixed.** Traced the whole cancellation chain and found EVERY hop already cancels on connection-drop EXCEPT one: `ai-gateway` `federation.executeTool` did `await client.callTool(params)` with **no abort signal**, so a chat-turn stop closed the INBOUND MCP server (controller's `res.on('close')` → `server.close()` → the SDK aborts the request handler's `extra.signal`) but left the OUTBOUND call to glossary running to completion (the ~39s zombie planner). **Fix** — thread `extra.signal` through `proxy-server.factory.ts` → `handleCallTool` → `executeTool` → `client.callTool(params, undefined, {signal})`. An abort now drops the glossary fetch → glossary `r.Context()` cancels → `runPlanner` `cctx` cancels → `client.Complete` aborts → provider-registry `r.Context()` cancels the provider call (the last hops were ALREADY ctx-wired — glossary `runPlanner` uses `context.WithTimeout(ctx,…)`; provider-registry [stream_handler.go](../../services/provider-registry-service/internal/api/stream_handler.go) ~13-16 documents r.Context() disconnect-propagation into the provider call). +1 unit test (signal→executeTool) + the 4-arg `executeTool` assertion updated to 5; ai-gateway handlers+federation 37/37; nest build clean; rebuilt+deployed, happy-path live smoke OK (real plan via ai-gateway on the fixed image). The rigorous end-to-end ABORT A/B is environment-blocked (the sync planner path writes no `llm_jobs`/log row; the local gemma is fast ~3s + non-serializing so a queuing-timing A/B can't detect cancellation) → deferred `D-PLANNER-INFLIGHT-ABORT-CAPTURE` (gate 4). The code gap is closed regardless. [[feedback_mock_only_coverage_hides_crossservice_bugs]]
...
> **✅ LIVE-SMOKE DONE (gemma-4-26b-qat, real stack):** rebuilt + deployed glossary + translation-worker; ran the REAL `run_resummarize_pass` against the protagonist 張若塵 (book `019efae6`) whose append-accumulated `appearance`/`description`/`personality` held **22 / 30 / 29** near-duplicate raw items — gemma collapsed each into ONE clean canonical (dirty cleared, synced stamped). Proves the full seam: glossary `canonical-dirty` → gemma via provider-registry → `write-canonical` compare-and-clear. **The smoke CAUGHT A REAL BUG** the mocked units missed: a reasoning model (gemma) spent the whole output budget on `reasoning_content` → empty `content` → 0 syntheses; FIXED by disabling hidden thinking (`reasoning_effort="none"`, mirroring the extraction worker) + a regression test asserting it.
> **Deferred:** `D-SUMMARIZE-MANUAL-BUTTON` (on-demand single-entity re-summarize — needs a new single-entity trigger endpoint; the automatic end-of-job path already delivers the value).

### [x] 27. Multiple agent confirm cards — only the first works (later cards expire)
...
> **FIXED (P2 server/FE coalesce — `3cce85d9` BE endpoints, `a721c93c` FE+chat, `cabbe35d` live-smoke, `038bdbff` /review-impl, `f61a83de` L3):** when a turn yields MORE THAN ONE live `confirm_token` (explicit pending cards + the FE's auto-rendered ones), `AssistantMessage` now folds them ALL into ONE generic `BatchConfirmCard` (deduped by token) and suppresses the individual cards — one review, one "Confirm all", each row still single-use + server-re-validated, the suspended run resumed ONCE with the aggregate outcome. No orphans. The dominant path turned out to be the FE auto-confirm synthesis (a weak model mints N tokens via `glossary_propose_*` but never calls the FE confirm tool → those auto-cards have no `runId` and POST straight to `/actions/confirm`, so a server-side suspend-point coalesce would never fire for them — the coalesce therefore lives in the FE where the cards are born). Glossary commits atomically via `/actions/confirm-batch`; other domains loop the existing single `/actions/confirm`. /review-impl locked the honest-partial invariant (apply the rest, burn the failed). Closed.
...
### [x] 29. KG schema lacks batch operations (agent proposes only 1–2 edges, second expires)
...
> **FIXED-BY-INHERITANCE (the #27/#30 P2 coalesce was built all-domain, deliberately covering KG):** `kg_schema_edit`/`kg_adopt`/`kg_sync_apply`/`kg_triage_*` each return `{confirm_token, descriptor}` (`graph_schema_tools.py` ~1224-1245) — the exact propose-result-with-a-live-token shape `AssistantMessage` coalesces. `descriptorDomain` routes every `kg_`-prefixed descriptor to the **`kg`** domain (`ConfirmActionCard.tsx:66`, with a dedicated test `descriptorDomain('kg_schema_edit')==='kg'`), so N KG schema-edit cards in one turn fold into ONE "Confirm all" and each commits via the single-confirm loop → `/v1/kg/actions/confirm` (`kg_actions.py:369`, dispatches `DESC_SCHEMA_EDIT` at :417). No orphan, no expiry — the user's exact symptom ("the second always fails/expired") is gone. Test-covered: `ConfirmActionCard.test.tsx:302-318` (kg-domain mapping + preview/confirm against the kg endpoint). Closed.
> **Deferred (optional residual, NOT the bug):** (1) a deterministic `kg_schema_propose_batch` MCP tool (mirror `glossary_propose_batch`) so the agent emits ONE call instead of N → `D-KG-SCHEMA-PROPOSE-BATCH` (token-efficiency, gate 2); (2) an atomic `/v1/kg/actions/confirm-batch` endpoint so KG commits atomically instead of the honest-partial loop → `D-KG-CONFIRM-BATCH-ENDPOINT` (gate 2). Both are optimizations; the loop already commits every row correctly today.

### [x] 30. Batch logic is poor; agent loses track mid-list — planner/executor must be stricter
...
> **P1 SHIPPED:** new deterministic `glossary_propose_batch` MCP tool — the agent passes ALL ops explicitly; they go through `ValidatePlan` → ONE `execute_plan` card → the existing deterministic executor (no planner LLM). Skill hardened (one-card-per-turn HARD rule + no-loop prohibition). Round-trip DB test green (mint→confirm→N kinds). See [docs/plans/2026-06-28-confirm-card-server-coalesce.md](../plans/2026-06-28-confirm-card-server-coalesce.md).
> **P2 SHIPPED — CLOSED (`a721c93c`/`cabbe35d`):** the all-domain FE/run-loop coalesce (see #27) is the safety net — even if the agent still loops N propose calls instead of using `glossary_propose_batch`, the stray cards fold into ONE "Confirm all" and commit honest-partial. The two fixes together (P1 deterministic batch tool + P2 coalesce) close the "agent loses track mid-list / second always fails" symptom. The #18 mechanical planner hard-stop additionally stops a self-recheck re-plan loop. Closed.
...
### [x] 41. Some MCP tool calls don't refresh the GUI (need F5); add reload-after-commit
Some tool calls from the MCP (agent) don't trigger a GUI refresh — I have to press F5 to see the change. Happens when I use the Agent to edit the ontology and graph. Add a reload button, and/or trigger a reload after a successful MCP tool call on the GUI.

> RC (VERIFIED — UNSOLVED, small root-cause-clear fix): after an agent commits an ontology/glossary edit via a chat confirm card, the viewing page does NOT refetch — the change only shows after F5. The 3 confirm components POST the commit, set local `state='done'`, resume the chat run, and **never invalidate any react-query cache** (repo-wide grep for `invalidateQueries`/`useQueryClient` in `frontend/src/features/chat` = ZERO hits): [ConfirmActionCard.tsx](../../frontend/src/features/chat/components/ConfirmActionCard.tsx) (the class-C card — handles `kg_schema_edit`/`kg_adopt`/`kg_sync_apply`, `descriptorDomain`→`kg`), [ConfirmCard.tsx](../../frontend/src/features/chat/components/ConfirmCard.tsx) (legacy glossary), [BatchConfirmCard.tsx](../../frontend/src/features/chat/components/BatchConfirmCard.tsx) (coalesced). The viewing tabs are react-query consumers keyed on `['kg-graph-schemas']`/`['kg-resolved-schema']`/`['glossary-entities']`/etc. that only refetch on key invalidation. The GUI's OWN mutation hooks (`useGraphSchema`, `useOntologyAdopt`, `useEntityMutations`) invalidate correctly — the gap is ONLY the chat/MCP confirm path, which bypasses them. The cards already know `domain`+`descriptor`, so a targeted invalidation is feasible.
> Fix (proposed, fix-now S/M, FE-only): in each confirm card's success branch add `queryClient.invalidateQueries` keyed on a `domain → key-prefixes[]` map (`kg` → kg-graph-schema(s)/resolved-schema/views/subgraph/entities/timeline; `glossary` → glossary-entities/ontology/kinds/suggestions/merge-candidates). `BatchConfirmCard` must invalidate the UNION of all committed domains (its `byDomain` loop). Optionally surface a manual "Refresh" button on the ontology/schema tabs (wire the already-exposed `refetch`).
> **DONE (FE-only):** new shared [invalidateAfterConfirm.ts](../../frontend/src/features/chat/utils/invalidateAfterConfirm.ts) maps the committed `domain` → query-key roots and invalidates via a `predicate` that string-prefixes the FIRST key element (so distinct roots like `knowledge-subgraph`/`knowledge-timeline` — which are NOT under `['knowledge']` for react-query's array-prefix match — are still caught). Wired into all 3 confirm cards' success branches: `ConfirmActionCard` (the committed `domain`), `ConfirmCard` (always `glossary`), `BatchConfirmCard` (the UNION of `byDomain` keys, only when `applied>0`). So an agent ontology/glossary edit now refreshes the open page with no F5. VERIFY: new `invalidateAfterConfirm` unit test (kg-vs-glossary routing, union, unknown-domain no-op) + the 4 confirm-card test files wrapped in a `QueryClientProvider` (new shared `test-utils/renderWithClient`) — 45/45 green; frontend tsc clean. The optional manual "Refresh" button is a follow-up nicety (the auto-invalidate fully removes the F5 need) → `D-KG-MANUAL-REFRESH-BUTTON`. Live browser smoke of the agent-edits-ontology→auto-refresh flow deferred (FE logic unit-proven; the viewing pages are confirmed react-query consumers of the invalidated keys) → `D-MCP-REFRESH-LIVE-SMOKE`.

### [x] 42. KG rebuild destroys all data; need incremental/accumulate (per-chapter, multilingual)
Does KG rebuild need to destroy all old data? I need ACCUMULATE / incremental KG generation. The novel isn't fixed — the writer keeps writing, so I need to re-generate a specific chapter after an edit, or add a new chapter, or translate to multiple languages. Many scenarios need a PARTIAL rebuild without destroying all data.

> RC (VERIFIED — PARTIAL: the capability mostly EXISTS, one ergonomic gap): the destructive DETACH DELETE (`_delete_project_graph`) is confined to **3 explicit endpoints** ([extraction.py](../../services/knowledge-service/app/routers/public/extraction.py) ~1159 delete-graph, ~1345 rebuild, ~1485 change-embedding-model) — the normal extraction path NEVER deletes. An incremental accumulate path **ships today**: `scope="chapters"` + `scope_range={chapter_range:[from,to]}` re-extracts a chapter on top of the existing graph (the "re-generate one chapter after an edit" case); `scope="chapters_pending"` auto-ingests newly-written chapters; ALL KG writes are **idempotent `MERGE` on a content-hash id** (`entities.py`/`facts.py`/`events.py`), so re-running accumulates + unions aliases/provenance + bumps version, no duplication. Multilingual is a derived translation layer (`event_text_translations`), no rebuild needed. **The gap:** the `/extraction/rebuild` endpoint (`RebuildRequest` ~1268) has NO `mode` param — it's hard-wired to `_delete_project_graph` + `scope="all"`. This is the still-open **`D-KG-UPDATE-MODE`** (deferred from #14; named in the bug doc + SESSION_HANDOFF but never rowed in `docs/deferred/DEFERRED.md`).
> Fix (proposed, fix-now S, BE): add `mode: Literal["replace","update"]="replace"` (+ optional `scope`/`scope_range`) to `RebuildRequest`; when `mode="update"` skip the `_delete_project_graph` call and route to the existing non-destructive `start_extraction_job` core with the caller's scope (the typed-confirm gate is only needed for `replace`). PLUS a **discoverability** fix — the user reached for "rebuild" (destructive) not knowing per-chapter extraction already accumulates; surface an "Update (add/refresh chapters)" affordance distinct from "Rebuild (wipe + redo)". Caveat: `mode=update` adds/updates but can't RETRACT entities removed from edited text (MERGE can't delete); a fuller retract path (`remove_evidence_for_natural_key`) exists but is out of scope for a first cut.
> **DONE (BE — `D-KG-UPDATE-MODE` closed):** [extraction.py](../../services/knowledge-service/app/routers/public/extraction.py) `RebuildRequest` gains `mode: Literal["replace","update"]="replace"` + `scope`/`scope_range`. `mode="replace"` is unchanged (the bug #14 destructive `?confirm` gate + delete). `mode="update"` is NON-destructive — it skips BOTH the confirm gate and `_delete_project_graph`, validates the `chapter_range` up front (mirrors StartJob), and routes to the same job-create with the caller's `scope`/`scope_range` (default `all`); the existing idempotent `MERGE` writes make it accumulate, so `scope="chapters"` + `{"chapter_range":[from,to]}` re-generates just the edited/new chapters. VERIFY: 4 new unit tests (update is non-destructive + needs no confirm → 0 Neo4j calls; scope/range forwarded to the count/job; reversed range → 422; unknown mode → 422) + the 6 pre-existing rebuild tests (which were RED from DI drift after #9 added book/glossary/pending deps — fixed the harness to override them) → **10/10 green** in a throwaway knowledge-image container. Caveat unchanged: update can't RETRACT entities deleted from edited text. **Follow-ups:** an FE "Update (add/refresh)" affordance distinct from "Rebuild (wipe)" → `D-KG-UPDATE-MODE-FE` (the BE capability is the core ask, shipped); a live smoke of a real update job → `D-KG-UPDATE-MODE-LIVE-SMOKE` (the routing is unit-proven + the incremental MERGE path was already verified to exist/accumulate).

### [x] 43. Glossary 3000+ same-name/different-kind duplicates (existing-dupe cleanup)
Some books have 3000+ glossary entries but almost all are duplicates — same name, DIFFERENT kind — from running multiple extractions (3000+ entries on 10 chapters).

> RC (VERIFIED — PARTIAL: prevention SOLVED, existing-row cleanup UNSOLVED): the going-forward fix shipped — `bulkExtractEntities` resolves a name **across all kinds** via `findEntityCrossKind` ([extraction_handler.go](../../services/glossary-service/internal/api/extraction_handler.go) ~706-717, ~1331-1348) on the app-maintained `normalized_name` fold, merging into the oldest entity under ITS kind (commit `c72225fe`, tested `extraction_crosskind_dedup_test.go`). But the user's 3000+ EXISTING rows predate that fix and **nothing auto-heals them**: the unique key is `(book_id, kind_id, normalized_name)` ([extraction_concurrency.go](../../services/glossary-service/internal/migrate/extraction_concurrency.go) ~54) so same-name/different-kind is a distinct row, AND both cleanup paths refuse cross-kind merges — `internalDedupNameVariants` groups by `{kind, normalize(name)}` ([dedup_name_variants_handler.go](../../services/glossary-service/internal/api/dedup_name_variants_handler.go) ~119, same-kind variants only, dry-run + on-demand), `proposeMergeCandidate` explicitly rejects cross-kind clusters (`merge_candidates_handler.go` ~108). Only manual `bulkDeleteEntities` (#40) removes them today. **This is exactly the already-tracked `D-CROSSKIND-DEDUP-REMEDIATION` residual of #38**, now with concrete user impact.
> Fix (proposed, fix-now S, BE): extend the remediation endpoint with a cross-kind mode — reuse `internalDedupNameVariants` but key groups on `normalize(name)` WITHOUT kind (behind `?cross_kind=true`), pick oldest/richest winner (`betterWinner`), merge losers via the journaled `mergeEntitiesCore` (winner keeps its kind, oldest-wins — matching the write-time resolver). Self-contained: one handler + a test; reuses existing merge machinery. User then runs `POST /internal/books/{book_id}/dedup-name-variants?apply=true&cross_kind=true` once per affected book. Optionally surface cross-kind merge candidates in the FE for interactive review.
> **RECLASSIFIED S→M during BUILD (the "reuse mergeEntitiesCore" assumption was WRONG):** [mergeOne](../../services/glossary-service/internal/api/merge_handler.go) **deliberately rejects cross-kind** (`loserKind != winnerKind` → skip `"different kind"`, ~L231-233), and its EAV repoint (~L305-310) is NOT kind-aware — so the dedup handler can't just group-without-kind and call `mergeEntitiesCore` (every loser would skip), and naively relaxing the gate would mis-attach a loser-kind's attribute values onto a different-kind winner. The same-kind rule is a **load-bearing invariant of the REVERSIBLE merge** (revert correctness depends on it). The correct fix threads a `crossKind` flag through `mergeEntitiesCore`→`mergeOne` that (a) skips the same-kind rejection AND (b) makes the EAV repoint kind-scoped to the winner's kind (compatible attrs land, incompatible skip — mirroring the write-time resolver), plus real-DB tests for cross-kind merge + revert + same-kind-unaffected. That is an **M** touching reversible-merge core, not the estimated S.
> **DONE (the careful M — user chose to build it now):** `crossKind` threaded through `mergeEntitiesCore` (required param; the 3 same-kind callers — user merge, pipeline confirm, plan-ops — pass `false`) → `mergeOne` (**variadic** so the ~14 same-kind test call sites stay unchanged). `mergeOne` skips the same-kind gate when `crossKind`, and the EAV repoint gains a SQL clause `($4=false OR attr_def_id IN (winner-kind attrs))` so cross-kind only repoints attrs valid for the WINNER's kind (a loser-kind-only attr is left on the soon-soft-deleted loser, never mis-attached) — **byte-identical for same-kind** (clause inert). The dedup handler adds `?cross_kind=true`: groups by normalized NAME only (kind zeroed), winner keeps ITS kind (`betterWinner` richest-then-oldest), merges losers via the journaled (reversible) `mergeEntitiesCore`. VERIFY: 2 new real-DB tests (cross-kind collapse → 1 live, winner keeps kind, loser SOFT-deleted/reversible; default mode keeps different kinds separate) + the existing `CrossKindDedup` write-path tests + ALL `Merge`/`Wiki` tests green (same-kind merge core unaffected); `go build`+`vet` clean. The user runs `POST /internal/books/{book_id}/dedup-name-variants?apply=true&cross_kind=true` once per affected book to collapse the 3000+ (DRY-RUN by default — omit `apply` to preview). **Follow-up:** a user-facing trigger (it's an internal/ops endpoint today, same as the same-kind `D-GLOSSARY-ST-DEDUP`) → `D-CROSSKIND-DEDUP-FE`.
