...
- **Cleanup:**
  - `D-S4C-ACCOUNTBALANCES-DROP` — ✅ **DONE 2026-06-17** (decision: REMOVE the endpoint, not re-map — no FE/gateway consumer found). `migrate.go` drops the `CREATE TABLE account_balances` + appends an idempotent `DROP TABLE IF EXISTS account_balances` (rollback = restore the CREATE, re-creates an EMPTY table — the wallet was never populated post-S4c-retirement). Removed the dead `GET /v1/model-billing/account-balance` handler + route. Rewrote `TestRecordInvocation_Idempotent_NoDoubleWrite` (was `…NoDoubleDeduct`) to assert idempotency on the `usage_logs` request_id UNIQUE (the real artifact; the token-quota check was vestigial). VERIFY: build+vet clean; **real-PG test PASS** (migrate.Up runs clean with the table removed, DROP removes an existing one, `/record` writes exactly 1 audit row on a duplicate request_id). Dev DB drops the table on next usage-billing restart.
