...
**Still genuinely open:** `D-RERANK-BYOK-LIVE-SMOKE` (rerank is a RETRIEVAL-path call — campaign extraction never invokes it; smoke via a read-only raw-search on a graph'd project with a rerank model) · `D-S2-IDEMPOTENCY-LIVE-SMOKE` + `D-S3A-GOVERNOR-LIVE-SMOKE` (need a real multi-chapter dispatch that runs, not cancel-early) · `D-S5A-ESTIMATE-LIVE-SMOKE` (`est_usd_low/high` NULL on completed runs — confirm not-recorded-for-local-$0 vs a gap) · `D-CAMPAIGN-BREAKER-PAUSE-LIVE-SMOKE` (must induce a provider circuit-open — non-deterministic, risks the real model's breaker state) · `D-S5C-LIVE-SMOKE` + `D-S6-LIVE-SMOKE` (**browser**) · relay/consumer (`D-S4B`/`D-S4C` — needs a PRICED non-local model to observe a non-zero spend land; B3-proven mechanism). *(Net: of the ~17, **7 evidenced** [dispatch, claim (transitive), S4a threading, eval, S3b backoff, **cancel + cancel-prop** (fixtured run)]; ~6 remain: rerank (read-only retrieval smoke), idempotency/governor (sustained run), estimate (confirm), breaker (non-deterministic), 2 browser, relay/consumer (priced run).)*
