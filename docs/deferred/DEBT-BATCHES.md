...
- `D-JOBS-P4-RETRY-COMPOSITION` — 🟡 **NOT a clean win (re-scoped 2026-06-17, deferred).** Unlike translation/extraction/video_gen (one create core, full params on the row), composition is **bifurcated**: the **auto/worker-op** path stores full params in `generation_job.input` (the worker `run_job` re-runs from it — reconstructible), but the **cowrite/streamed (SSE)** handlers in `engine.py` (`generate`/`selection_edit`/`generate_chapter`/`stitch_chapter`) PACK the prompt live from the current work/node/canon/grounding state and do the LLM call INLINE — the row keeps only metadata (`model_source`/`operation`/`prompt_estimate`/`worker_op`), NOT the packed prompt. So a failed streamed job can't be reconstructed from its row alone; retry semantics differ per operation/mode. Needs a design pass (likely: retry only the worker-op/auto jobs whose input is self-contained; the streamed cowrite path re-runs from the FE, not a job-control retry). `operation` is the kind, so the jobs-service gate would need each retryable operation enumerated. **Deferred — fresh-context design task, not autonomous-clean.**
