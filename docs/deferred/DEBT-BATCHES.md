...
> **🆕 PRODUCT BUG found during the B4a live run (2026-06-18) — `D-KNOWLEDGE-PROJECT-DELETE-NEO4J-ORPHAN` (MED).** `DELETE /v1/knowledge/projects/{id}` (`projects.py:690 delete_project`) calls only `ProjectsRepo.delete` (Postgres row) — **no Neo4j cleanup, no cypher session injected** → deleting a project ORPHANS its entire knowledge graph in Neo4j (Entity/Event/Fact/Passage/ExtractionSource nodes) forever. Observed ≥7 orphaned project graphs (`019ec73e`=279 nodes, `019eb683`=55, `019ec734`=30, …) for projects absent from Postgres. Impact: Neo4j storage leak + potential cross-project pollution if any retrieval query isn't strictly `project_id`-scoped. Fix: the delete handler must `MATCH (n {project_id}) DETACH DELETE n` (+ drop the per-(project,model) vector indexes) in the same op, OR emit a cleanup event. NOTE: this also means smoke cleanups that delete the Postgres row via SQL leak Neo4j — but the API path has the SAME gap, so it's a genuine product defect, not just smoke discipline.
