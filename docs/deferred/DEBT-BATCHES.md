...
**Still genuinely open:** `D-RERANK-BYOK-LIVE-SMOKE` (rerank is RETRIEVAL-path — campaign extraction never invokes it; needs a raw-search on a graph'd project WITH a rerank model. **Confirmed blocker 2026-06-18: NO test-account project has both a Postgres row AND a Neo4j graph** — after the orphan sweep below, Neo4j holds only the 2 live graphs and neither carries a rerank model → only a fresh full-extraction build unblocks it) · `D-S2-IDEMPOTENCY-LIVE-SMOKE` + `D-S3A-GOVERNOR-LIVE-SMOKE` (need a sustained multi-chapter run, not cancel-early) · `D-CAMPAIGN-BREAKER-PAUSE-LIVE-SMOKE` (must induce a provider circuit-open — non-deterministic) · `D-S5C-LIVE-SMOKE` + `D-S6-LIVE-SMOKE` (**browser**) · relay/consumer (`D-S4B`/`D-S4C` — needs a PRICED non-local model to observe a non-zero spend land; B3-proven mechanism). *(Net: of the ~17, **8 evidenced** [dispatch, claim (transitive), S4a threading, eval, S3b backoff, cancel + cancel-prop, **S5a estimate**]; ~5 remain: rerank (needs a graph), idempotency/governor (sustained run), breaker (non-deterministic), 2 browser, relay/consumer (priced run).)*
