...
| ID | Description | sev |
|---|---|---|
| `D-P5-M3-EXTRACTION-LIVE-SMOKE` | decoupled extraction with P5 on; assert `p5:knowledge:extraction:inflight:{user}` ZCARD ≤ cap. **Infra confirmed ready 2026-06-17** (LM Studio `/models` 200 from worker-ai; `P5_SCHED_ENABLED=true`; `p5:knowledge:extraction:inflight_total` key present). **Blocker: needs SEEDED extractable content** — the test account's books have 0 chapters; requires importing chapters + a bound knowledge project before a real extraction can run. | high |
| `D-P5-M2-MULTI-OWNER-LIVE-SMOKE` | 2-user interleave on the stack (single-owner cap-hold already proven). Same seeded-content blocker as P5-M3. | med |
...
| `D-JOBS-P3-KNOWLEDGE-CANCEL-SUCCESS-LIVE-SMOKE` | a successful cancel mutating a real running extraction row. Same seeded-content blocker (needs a real running extraction to cancel). | med |
