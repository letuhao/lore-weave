...
- `admin/alert-silence` (S5 Tier 2 Griefing — temporarily suppresses a matching alert pattern; mandatory reason ≥50 chars + max 30-day duration + SRE-Slack notification; incident-tagged silences auto-clear on resolve; `pattern='*'` requires dual-actor + 24h cooldown — see [02_storage/SR09_alert_tuning.md](../03_planning/LLM_MMO_RPG/02_storage/SR09_alert_tuning.md) §12AL.6)
- `admin/alert-threshold-update` (S5 Tier 2 Griefing — changes an alert rule's threshold at runtime; re-validates against SR1-D6 derivation-rule format + 7-day replay; reason required — see [02_storage/SR09_alert_tuning.md](../03_planning/LLM_MMO_RPG/02_storage/SR09_alert_tuning.md) §12AL.3)
- `admin/pager-rotation-swap` (S5 Tier 3 Informational — swaps primary/secondary on-call rotation coverage per SR2-D2; not platform-affecting but audited for rotation-load analytics per SR9-D7 — see [02_storage/SR09_alert_tuning.md](../03_planning/LLM_MMO_RPG/02_storage/SR09_alert_tuning.md) §12AL.8)
- `admin/cve-override` (S5 Tier 2 Griefing — allows deploy with known critical/high CVE; bounded 72h; requires incident ticket reference + remediation ETA + SRE approval; post-expire report + auto-escalation if unremediated at ETA — see [02_storage/SR10_supply_chain.md](../03_planning/LLM_MMO_RPG/02_storage/SR10_supply_chain.md) §12AM.5)
- `admin/dep-vet-approve` (S5 Tier 2 Griefing — adds a 3rd-party library to `contracts/supply_chain/dep_allowlist.yaml` after vetting; reviewer + architect co-sign required; annual review cadence — see [02_storage/SR10_supply_chain.md](../03_planning/LLM_MMO_RPG/02_storage/SR10_supply_chain.md) §12AM.6)
- `admin/supply-chain-freeze` (S5 Tier 1 Destructive — platform-wide deploy halt during active supply chain incident; dual-actor + 100+ char reason + incident ticket + 24h cooldown before re-enable; fifth SR5 freeze mechanism; emergency patches via `break-glass-deploy` label — see [02_storage/SR10_supply_chain.md](../03_planning/LLM_MMO_RPG/02_storage/SR10_supply_chain.md) §12AM.9)
