...
| 20 | **THE root cause** (PO-driven deep dive) | **HIGH→FIXED** | **Wire-request proof**: the injected glossary-skill instructions say *"Add new entities: `glossary_propose_entities`"* while the request's 21 advertised tools contain only `glossary_propose_entity_edit` — the budgeted hot seed dropped the named tool under sticky-domain pressure (kg/plan/book crowding one ceiling). The model was TOLD to call a tool it could not see, so the create intent landed on the similarly-named visible edit tool, every turn. Instructions were correct; visibility was broken. The authoring-time lint (named ⊆ hot_domains) can't protect against a runtime budget drop. | LM Studio request dump: tools[21] w/o propose_entities + instructions naming it |

- **#20 → FIXED** (D-SKILL-NAMED-TOOLS-RIDE) — every injected skill's prompt-NAMED tools ride the surface budget-exempt (`skill_named_tools`, cached prompt scan ∩ catalog), fresh + resume paths. The invariant: an injected instruction must never name a tool that is not on the wire.

| 21 | rail steps | **HIGH→FIXED** | Layer under #20: the rail text names `save-cast → glossary_propose_entities`, but save-cast has `done_when: cast > 0` and no `repeat` flag — once ONE character existed, done-suppression + budget done-exclusion took the create tool off the wire FOREVER while the recipe kept teaching it. Adding characters is inherently repeatable. | workflows row: no repeat flag; drop warning named exactly this tool all day |
| 22 | rail budget | **HIGH→FIXED** | The rail shared the 2000-tok discovery hot-seed budget (`scale_by_window` only grows near 1M ctx) — a 9-step rail with two large glossary schemas can never fully fit; something always dropped. A pinned rail is a curated recipe, not discovery sprawl. | `scale_by_window(2000,195000)=2000`; perpetual drop warnings |
| 23 | book_id UX | **HIGH→FIXED** | Model stalls hunting `book_id` ("mọi nỗ lực tự tìm book_id đều thất bại") though the backend auto-resolves it (ambient X-Book-Id). The advertised schemas still SHOWED book_id, so the weak model treated it as a demand; its history of "book_id must be a real UUID" errors (from the FE edit tool, pre-fix) cemented the belief. | agent's own words + schemas |

**Fixes:** #21 (`repeat:true` on see/adopt/apply/read-back/capture/save/connect-people — migrate.go seed + live row), #22 (`RAIL_STEP_TOKEN_BUDGET=6000`, own ceiling; done steps reordered to the budget tail, repeat-done outranks one-shot-done), #23 (D-AMBIENT-BOOK-SCHEMA-PROJECTION — on a book-bound session, `book_id` is projected OUT of ambient_book tools' advertised schemas).

---

## PIVOT DECISION (PO, 2026-07-27)

After 7 platform fixes the weak model STILL mis-picks tools in a FRESH session (3× entity_edit✗ before tool_list). **The conversational agent's tool-choice is the unfixable link for weak local models.** PO direction: stop patching the chat path — **build deterministic server-side workflows for glossary-build and KG-build** (the PlanForge pattern: state machine outside the agent, LLM only fills content inside steps, human checkpoints between phases). The 12 findings + 10 fixes above remain valid platform hardening regardless.
