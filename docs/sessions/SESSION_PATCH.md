...
- Last Updated: 2026-04-21 **(session 50 continued — K19b.2 + K19b.7-partial FS batched [XL]; Jobs tab live)** — Second K19b cycle. CLARIFY-Q2 user-override ("do c") reclassified the `project_name` column from FE-fanout to BE response-shape change (option c narrow). BE `ExtractionJob` gains `project_name: str | None = None`; `list_all_for_user` rewritten with aliased LEFT JOIN on `knowledge_projects`. FE ships `ExtractionJobsTab.tsx` (4 sections via native `<details>` — Running+pending / Paused / Complete capped 10 / Failed+cancelled highlighted when non-empty), per-group `ErrorBanner`s driven by hook's `activeError`/`historyError`, `JobRow` with `project_name ?? unknownProject` fallback, `JobProgressBar` wired per row. KnowledgePage replaces the `jobs` placeholder. `jobs.*` i18n block added to all 4 locales. Review-code caught 1 LOW (dead `countOverride` prop). /review-impl caught 4 LOW, all fixed in-cycle (L1 historyError banner placement above Complete, L2 broadened error text "Couldn't load job history." × 4 locales, L3 hoisted SHORT_DATE_FORMATTER to module scope, L4 dropped redundant `ExtractionJobStatus` cast). Next cycle: K19b.3 (detail panel) + K19b.5 (retry).
- Updated By: Assistant (session 50 — K19b.2 FS commit: 11 files. BE 2 mod (extraction_jobs.py repo +`project_name` field + rewritten list_all_for_user SQL with aliased LEFT JOIN; test_extraction_jobs_repo.py +2 K19b.2 integration tests — populated + null-via-session_replication_role). FE 9 changed (api.ts +`project_name`; NEW ExtractionJobsTab.tsx; NEW ExtractionJobsTab.test.tsx with 9 tests; KnowledgePage.tsx wires tab; 4 locale knowledge.json add `jobs.*` block; projectState.test.ts adds JOBS_KEYS iterator 14 paths × 4 locales; useProjectState/BuildGraphDialog/useExtractionJobs test fixtures add `project_name: null`). `pytest` → BE unit 1171 pass, integration repo 30 pass (+2 K19b.2). `vitest` → FE knowledge 138 pass (was 125 at K19b.1 end; +13 = 9 ExtractionJobsTab + 4 locale-iterator JOBS_KEYS). `tsc --noEmit` clean. DB schema recovery: first integration-test run used fragile ALTER TABLE DROP CONSTRAINT; left schema broken; manually TRUNCATE extraction_jobs CASCADE + ADD CONSTRAINT to restore; test rewritten to use SET LOCAL session_replication_role='replica' which is transaction-scoped and can't leak.)
- Active Branch: `main` (ahead of origin by sessions 38–50 commits — user pushes manually)
- HEAD: `4fb8b62` (K19b.2 + K19b.7-partial) at session 50 end (was `1c208ce` K19b.1+K19b.4, `2061b2d` K19a.8, `c6ee80a` K19a.7 HEAD backfill, `2cbcc7c` K19a.7, `7cf394f` K19a.6 HEAD backfill, `2226283` K19a.6, `1156193` K19a.5 HEAD backfill, `3148751` K19a.5)
...
| D-K19b.2-02 | K19b.2 plan (session 50) | **Row-level click → JobDetailPanel.** K19b.3 explicitly adds the slide-over; K19b.2 rows are non-interactive by design. When K19b.3 starts, `JobRow` needs an `onClick` prop + `role="button"` / `tabIndex={0}` accessibility, and `ExtractionJobsTab` needs slide-over state. | K19b.3 |
