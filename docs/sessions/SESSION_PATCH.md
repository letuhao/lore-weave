...
- Last Updated: 2026-04-19 (session 47 — T2-close-7 glossary trigger perf pass shipped. Pin toggle no longer bumps updated_at, self-trigger watch list rewritten from "updated_at catch-all" to specific fields + soft-delete defence. Remaining: T2-close-3 + polish-1/2a/2b/3 + 2 + 4 before Track 3.)
- Updated By: Assistant (session 47 — 14+ commits: 7a/7b/8a/8b/8c/9 + test-hygiene + Gate-13-report + 1a + 1b-BE + 1b-FE + 5 + 6 + 7. glossary-service 10 new targeted tests pass (1 updated_at-only skip + 7 watched-field subtests + 1 pin no-recalc + 1 regen no-op + regen snapshot_at advancement), go build clean, knowledge-service 1154 unit still green.)
- Active Branch: `main` (ahead of origin by session 38–47 commits — user pushes manually)
- HEAD: 01b8eda (T2-close-6 scope_range filter); T2-close-7 trigger perf about to land
...
| D-K18.9-01 | K18.9 scope (session 47) | **System-prompt cache_control on anthropic path.** Cycle 7b marks only the stable memory prefix (1 of Anthropic's 4 cache breakpoints). Session-level `system_prompt` is stable per-session too — adding a second `cache_control: ephemeral` marker on it would cache the persona prompt across turns as well. ~3 lines in stream_service.py; defer until we have a real user with a long persona prompt to measure against. | K18.9-sysprompt (post-Gate-13) |
