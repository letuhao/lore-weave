...
- Last Updated: 2026-04-21 **(session 50 continued — K19b.3 + K19b.5 + ETA FE batched [XL]; JobDetailPanel + retry live)** — Third K19b cycle. Ships JobDetailPanel (Radix slide-over from right) consuming the Cycle 2 ExtractionJobsTab row click. Panel surfaces full metadata + pause/resume/cancel buttons + conditional error block. K19b.5 retry: "Retry with different settings" button on failed rows (NOT cancelled per review-design R3) opens BuildGraphDialog with the prior job's settings pre-filled via new optional `initialValues` prop. ETA cleared D-K19b.4-01 via new `useJobProgressRate` EMA hook (α=0.3, 60s stale-reset). CLARIFY discovery: plan's "Log viewer" needs a real BE schema + extraction-worker instrumentation + endpoint — too big for this cycle, deferred as new K19b.8. "Current item being processed" similarly unusable without BE cursor enrichment or FE chapter-title lookup, deferred as D-K19b.3-01. Review-design added 3 refinements (R1 single retryIntent state, R2 close panel on retry, R3 retry only for failed); review-code caught 2 LOW fixed in-cycle (dead useRef, getProject error toast). Next cycle: K19b.6 CostSummary (blocked on Track 2 K16.12) OR K19b.8 log viewer.
- Updated By: Assistant (session 50 — K19b.3 + K19b.5 + ETA FE commit: 10 files. NEW 4 (useJobProgressRate.ts, useJobProgressRate.test.tsx, JobDetailPanel.tsx, JobDetailPanel.test.tsx); MOD 6 (BuildGraphDialog.tsx +initialValues prop, BuildGraphDialog.test.tsx +1 test, ExtractionJobsTab.tsx owns selectedJobId+retryIntent state + JobRow click/keyboard + retry getProject useQuery with error toast, ExtractionJobsTab.test.tsx +3 tests, 4 locale knowledge.json +17 keys each under jobs.detail.* + jobs.retry.*, projectState.test.ts JOBS_KEYS +17 paths). `vitest` → FE knowledge 155 pass (was 138 at K19b.2 end; +17 = 6 hook + 6 panel + 1 dialog initialValues + 3 tab + 1 JOBS_KEYS cross-locale). `tsc --noEmit` clean. No BE changes this cycle.)
- Active Branch: `main` (ahead of origin by sessions 38–50 commits — user pushes manually)
- HEAD: `5e00f7b` (K19b.3 + K19b.5 + ETA) at session 50 end (was `4fb8b62` K19b.2+K19b.7-partial, `1c208ce` K19b.1+K19b.4, `2061b2d` K19a.8, `c6ee80a` K19a.7 HEAD backfill, `2cbcc7c` K19a.7, `7cf394f` K19a.6 HEAD backfill, `2226283` K19a.6, `1156193` K19a.5 HEAD backfill, `3148751` K19a.5)
