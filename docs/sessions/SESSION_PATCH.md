...
- Last Updated: 2026-04-19 (session 47 — T2-close-6 scope_range filter shipped. CI sub-cycle + T2-polish-4 scoped out (no CI/CD). Remaining: T2-close-7 + 3 + polish-1/2a/2b/3 + 2 + 4 before Track 3.)
- Updated By: Assistant (session 47 — 13+ commits: 7a/7b/8a/8b/8c/9 + test-hygiene + Gate-13-report + 1a + 1b-BE + 1b-FE + 5 + 6. knowledge-service 1154 unit pass (scope_range path added 4 respx + 1 start-bad + 1 helper-DRY tests), book-service api tests pass incl. TestBuildSortRangeFilter.)
- Active Branch: `main` (ahead of origin by session 38–47 commits — user pushes manually)
- HEAD: ed9f13d (T2-close-5 pricing); T2-close-6 scope_range filter about to land
