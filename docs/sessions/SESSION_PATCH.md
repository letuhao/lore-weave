...
| D-K20.3-α-01 | K20.3 Cycle α CLARIFY (session 50) | **Scheduled global L0 summary regen loop.** K20.3 α ships the project-scope sweep only. Global L0 regen is harder because there's no `project_id` to look up a last-used `llm_model` against — we'd need a different model-resolution story (e.g. query the most-recent extraction_job across ALL the user's projects, or add a user-level preferred-regen-model column). Low urgency — global regen is infrequent (weekly cadence per plan) and users can manually trigger via the FE Regenerate button. | Track 3 polish (after global-scope usage shows scheduled regen is actually wanted) |
...
| D-K19d-γb-01 | K19d Cycle γ-b review-impl M1 (session 50) | **RELATES_TO merge rewire ON MATCH doesn't union `pending_validation` / `valid_from` / `valid_until` / `source_chapter`.** Target's values win on those fields; only `source_event_ids` is unioned and `confidence` is maxed. Concrete symptom: merging a Pass-2-validated edge (`pending_validation=false`) into a target's quarantined duplicate (`pending_validation=true`) keeps the merged edge quarantined — the validation signal should promote. Fix: add the 4 CASE branches to the ON MATCH clause (min valid_from, latest non-null valid_until, AND-combine pending_validation, concat source_chapter). Low impact at hobby scale. | Track 3 polish |
| D-K19d-γb-02 | K19d Cycle γ-b review-impl M2 (session 50) | **Merge is not atomic.** `merge_entities` runs 4–7 separate auto-commit writes (rewire RELATES_TO, rewire EVIDENCED_BY, update target with glossary pre-clear, DETACH DELETE source, optional dedupe). A Neo4j crash or connection drop between the glossary pre-clear and the DETACH DELETE would leave source orphaned with `glossary_entity_id = NULL` but otherwise intact. Fix requires the `session.begin_transaction()` pattern which isn't used elsewhere in knowledge-service; adopting it would need a pass across all multi-step writes. | Track 3 polish (if the partial-state risk ever surfaces in prod) |
