...
| D-K20α-01 | K20 Cycle α plan (session 50) | **Cost tracking on regen flow.** `_record_spending` exists on worker-ai runner.py for extraction jobs (cleared in D-K16.11-01), but the regen helpers don't hit it. KSA §7.6 calls out `summary_regen_cost_usd` as a metric; ops can't see regen LLM spend today. Pair with K20.7 metrics when that cycle lands. Low urgency because (a) regen costs are bounded by `max_tokens=500` + the 10/sec provider-client bucket, and (b) a single regen is small (~$0.01–0.05) vs a typical extraction job. | K20.7 / Track 3 polish |
