...
| D-CHAT-01 | K9.1 review | **SessionSettingsPanel close-during-debounce drops PATCH.** [SessionSettingsPanel.tsx:88-101](frontend/src/features/chat/components/SessionSettingsPanel.tsx#L88-L101) debounces every save by 500ms via a single shared `saveTimerRef`, and the unmount cleanup at line 81 clears the pending timer. The dominant UX pattern *"change one thing, click outside to dismiss"* fires the panel unmount inside the 500ms window — for sliders/textareas this is masked by continuous interaction, but for the Model selector (and any other discrete `<select>`) the change is **silently dropped**. K9.1's project picker bypasses the debounce inline as a targeted workaround. **General fix needs**: either (a) flush pending PATCH on unmount instead of clearing the timer, or (b) per-field timers so unmount can flush each one, or (c) drop the debounce entirely for discrete fields (selects, toggles) and keep it only for typing buffers. Same change should also fix the shared-timer coalescing bug where two unrelated patches within 500ms collapse into one (only the latest field is sent). |
