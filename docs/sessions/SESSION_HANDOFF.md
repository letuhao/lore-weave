...
- **▶ NEXT:** (1) PO answers [`00B`](../specs/2026-07-01-writing-studio/00B_EXECUTION_ROADMAP.md) **§6's 15 PO-DECIDE rows** (each has a recommended default — the only human input the cluster still needs); (2) build starts at **00B Stage 1** = `25` Deploy 1 (M0 fail-loud pre-flights → expand → backfill → cutover), `22-A1` in the parallel lane, RB-1 checkpoint after T1–T6.
...
**New deferred item:**
- **`D-WS4C-FE-TOGGLE`** (LOW, gate #1 out-of-scope — frontend) — `canon_capture_enabled` is PATCHable on `/projects/{id}` but has no settings UI (same shape as `save_raw_extraction` / `D-P2-FE-SAVE-RAW`). **The UI must show the EFFECTIVE value, not the stored one**: today, if an operator sets `CHAT_CANON_CAPTURE_ENABLED=false`, a user's toggle reads `true` while nothing happens — the exact SET-4 "silently-off" bug class. The only current observable is the per-turn log line.
