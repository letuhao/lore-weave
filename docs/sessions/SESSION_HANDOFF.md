...
**Next: Commit 3** (`llm.jobs.{kind}` queue + bounded consumer pool sized `governor.maxFor` → replaces acquire-or-die; lets us REVERT the band-aid below; needs the queue live-smoke = the incident regression). Then Phase 2 (convert worker-ai/knowledge/translation), Phase 3 (long tail).
