...
1. **D-WIKI-P2-KG-SWEEP** — the pull sweep's KG-neighbourhood recompute (GAP A: pipeline KG changes emit no event). Cross-service: needs knowledge's Neo4j neighbourhood hash; the version-drift half is done glossary-local.
2. **D-WIKI-P2B-COST-ESTIMATE** (remaining /review-impl LOW — precise N×$ vs the current count+spend-cap).
3. **D-WIKI-M8-LEARNING-CONSUMER** (already partly on main via the composition correction-flywheel) + **D-WIKI-M8-EVAL-PLUS**.
The wiki feature (M0→M8 + M7a + Phase-2) is functionally complete + live-proven.
