# Session Handoff — Auto-Draft Factory (feat/advanced-translation-pipeline) · `main` merged 2026-06-10
...
> Preserved from main's handoff at merge time (HEAD `ba1edf93`). This track is COMPLETE on main; kept here for cross-track context (ai-gateway/chat MCP changes now present on this branch).
...
**▶ NEXT (wiki follow-ups, all optional):**
1. **D-WIKI-M8-LEARNING-CONSUMER** (likely partly on main via the composition correction-flywheel — check before building) + **D-WIKI-M8-EVAL-PLUS** (LLM-judge groundedness + golden corpus).
2. **Live-smokes (deferred, all low-risk/unit-pinned):** **D-WIKI-P2-KG-SWEEP-LIVE-SMOKE** (the parity-on-real-stack proof: generate → sweep with no KG change → **expect 0 kg_drift** → mutate a relation → flag that one); **D-WIKI-P2B-COST-ESTIMATE-LIVE-SMOKE** (gen-config GET through the live stack).
3. **D-WIKI-P2-SWEEP-DISMISS-RESWEEP** — a *dismissed* drift row (recipe + kg) is re-filed by the next sweep (the `ON CONFLICT … WHERE status='pending'` guard doesn't see dismissed rows; the stored hash/version only changes on regen). Pre-existing with recipe-drift; needs a dismiss-vs-resweep suppression design.
4. **D-WIKI-P2-KG-SWEEP-PROJECT-PARITY** — the sweep resolves `projects[0]` fresh while generation froze `job.project_id`; consistent for single-project books but a multi-project book could pick a different project → false drift. Inherited from the trigger's `projects[0]` assumption. Store the generating `project_id` in `build_inputs` (or pass it) to make the sweep project-exact.
The wiki feature (M0→M8 + M7a + Phase-2 change-control: push source-drift + pull recipe/KG-drift) is functionally complete + live-proven.
