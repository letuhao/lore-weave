# Session Handoff — `feat/auto-draft-factory-gaps` · B0+B1+B2 + producer-emit Slice A · 2026-06-17
...
>   - **☐ Slice C — wiki-gen** (`D-JOBS-WIKI-GEN-UNWIRED`): knowledge `wiki_gen_jobs` — emit at repo mutations (`repositories/wiki_gen_jobs.py` create/mark_running/complete/fail/pause/resume/cancel) + reconcile UNION into `/internal/knowledge/jobs` (kind `wiki_gen`, `complete`→completed) + FE label. **NEXT.**
>   - **☐ Slice B — glossary-translate** (`D-JOBS-GLOSSARY-TRANSLATE-UNWIRED`): translation `glossary_translation_jobs` — emit in `glossary_translate_worker` + reconcile UNION (kind `glossary_translation`) + FE label.
>   - **☐ Slice D — book-import (Go)** (`D-JOBS-BOOK-IMPORT-UNWIRED`): book-service `import_jobs` — Go emit helper (parameterize `outbox.go insertOutboxEvent` aggregate_type; relay already routes `jobs`), emit at create/`updateImportJobStatus`, new `_RECONCILE["book"]` + `GET /internal/book/jobs` Go endpoint.
