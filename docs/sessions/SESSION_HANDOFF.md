...
**State: M0 + M1(a–d) + M2 + M3 + M4(a–d) + config-plumbing + M5(a–d) + M6a + M6b-1 DONE** (M6b-2 FE remainder open; **M1d + M4d-1 + M4d-2(a/b/c) + glossary-versioning(VG-1/2/3) + M6b-1 targeted-propagate DONE 2026-06-07/08**). **🏁 Translation V3 plan-of-record COMPLETE — all milestones M0–M6 shipped; M4d-2 full 2-pass cold-start + the glossary entity-versioning epic landed.**
...
**Pushed:** branch `feat/translation-pipeline-v3` on origin (budget fix `009b8d18` pushed). **`origin/main` merged in 2026-06-07** (`d0fbd404`; PR #20 glossary/ai-pipeline-v2 + #21 lore-enrichment/foundation — 29 commits, 0 conflicts; 434 py + FE translation green post-merge). **D-TRANSL-EXTRASYSTEM-BUDGET CLEARED**. **M1d (`bab368b7`) + M4d-1 (`3e6f7554`) + M4d-2a/2b (through `3a37e6e0`) pushed.** **Local-only (awaiting push):** M4d-2c (`f6229a64`) + M6b-1 (this commit).

**NEXT options:** (1) **push** the local M4d-2c (`f6229a64`) + M6b-1 commits · (2) **M6b-2 FE** — surface "N affected chapters" + a user-triggered "re-translate affected" batch action (the user-consent half of full-propagate) · (3) clear live-smokes (M4d1/M5c/M6a/**M6b**) on a rebuilt stack · (4) open the PR. V3 feature-complete; M6b-2 + live-smokes are the open refinements.
...
- **D-TRANSL-M6A-LIVE-SMOKE** (M6a) → **largely RESOLVED by M6b-1 (2026-06-08):** the open question ("does `glossary.entity_updated` fire on a translation sub-resource PATCH?") was answered — it did NOT, and M6b-1 **adds the emit** to `createTranslation`/`updateTranslation`/`deleteTranslation`. Remaining = the live-smoke proof of the full chain, folded into **D-TRANSL-M6B-LIVE-SMOKE**.
- **D-TRANSL-M6A-NOTES** (M6a /review-impl, LOW) — (a) `targetLang` ("vi") vs glossary `language_code` subtag drift could create a duplicate translation instead of patching — **note:** M6b-1's consumer now tolerates this drift (primary-subtag match), but the FE write-side dup risk remains; (b) dialog inputs not cleared on cancel/reopen (minor UX); (c) ~~full propagate~~ **DONE in M6b-1** (term→chapter index + per-language flag); the **auto "affected chapters" re-translate** half is the M6b-2 FE slice (user-triggered, PO 2026-06-08 — no auto-spend).
- **D-TRANSL-M6B-LIVE-SMOKE** (M6b-1) — the full chain (translation edit in glossary → outbox → relay → `loreweave:events:glossary` → translation consumer → **per-language** precise `is_glossary_stale` flip; + worker records `chapter_translation_glossary_usage` rows) is NOT live-verified (needs glossary+relay+redis+translation co-running + a rebuilt translation image). Consumer mirrors the prod-proven M5c loop; emit/endpoint/usage/consumer-dispatch unit-covered (the row-selection SQL is mock-only). Smoke with the M4d1/M5c/M6a smokes on a rebuilt stack.
