...
## ▶ KNOWLEDGE-ARCHITECTURE REFACTOR IS CLOSED — merged from `refactor/entity-lifecycle` (2026-08-31)

Two plans finished and were merged into this branch. Neither is in flight; both are countable
rather than remembered:

* **`2026-08-09-knowledge-architecture-refactor.md`** — 69 of 69, archived to
  `.ai-factory/archive/plans/`. `plan-final-verification` still reads it there.
* **`2026-08-30-runstate-leftovers.md`** — 7 of 7. `python scripts/goal-prompt.py --plan
  docs/plans/2026-08-30-runstate-leftovers.md --check` prints `0 open of 7`.

**STILL OWED, and none of it blocks this branch:**

* **The 32 causal labels** —
  [`2026-08-24-t33-causal-labelling-sheet.md`](../measurements/2026-08-24-t33-causal-labelling-sheet.md).
  Drafted and awaiting a signature; `labelled_by:` is blank and `--score` refuses an
  assistant. A `labels_proposed_by:` field records that an assistant drafted them, and
  `--score` prints it beside every number. Standing result once signed: precision 0.30,
  recall 0.207 over candidates.
* **§19's five merge-to-main steps** — the PO deferred these on 2026-08-30; they are listed
  in the archived section below, not started.
* **`event_order` under CONCURRENT extraction of one chapter** — §24. The deterministic half
  is fixed (the within-chapter index continues from the band's maximum); two jobs running at
  once still both read that maximum. `scripts/event-order-collision-gate.py` freezes the 51
  pre-existing collisions as a shrink-only ceiling.
* **Four gates with no red-ability proof** — `alive-column-deprecation`, `derived-entity-id`,
  `entity-lifecycle-outbox`, `gateway-domain-logic`. `NO_PROOF_BASELINE` moved 0 → 4 at this
  merge and each is named in `gate-teeth-gate.py`; the direction is down.

**DISCHARGED here, so it is not re-litigated:** `D-T33-CAUSAL-COVERAGE-UNMEASURED` (§26) —
REACH 3/82 projects, YIELD 131/1277 candidate pairs (10.26 %), CONSISTENCY 0 of 131 edges
outside the window algorithm. `python scripts/causal-coverage-gate.py` re-runs it.

> The 427-line `refactor/entity-lifecycle` handoff this replaces was moved to
> [`SESSION_ARCHIVE.md`](SESSION_ARCHIVE.md) **verbatim**, not deleted. That file's own header
> records what happens otherwise: PR #184 *"truncated the handoff without archiving what it
> cut."*
...
PRODUCTION RULE, run it rather than trust this sentence) · the **99**
