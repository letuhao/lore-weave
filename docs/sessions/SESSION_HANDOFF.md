...
- **WS-D4 · capability sweep 62 → 26 null, 0 broken.** Reached **$0** via authored args + creator chains + $0 DB seeds: arc family (8), authoring-run (6), plan_* (4), glossary book-standard (4), book steering, jobs (3), the **kg build chain** (set-model→benchmark→build_graph on **local bge-m3**), book_scene_get + the 5 translation-version tools (seeded world/scene/completed-translation). Manifest regenerated (`193 executes:true · 0 false · 26 null`, 2 service copies byte-identical). The 26 residue is WAIVED with per-tool gate reasons (13 buildable-next short chains: kg node-chain/motif/scene; the rest bespoke multi-FK fixtures / browser-JWT ontology / paid / cross-service). CD4 stays reject-on-`executes:false`.
- **Bookkeeping:** WS-D5 number-collision resolved (WS-D5a=description follow-up; WS-D5=frontend tools) in TRACK-D.md; BOARD ND3 → ✅(redefined per WS-D5c), N3 → 🔄(D-side ✅, blocked on C). **Deferred (gate #1 — Track-A federation files, partial coverage exists):** a *gateway-wide* `tools/list` prefix-drop test — the concurrent `695aa5f64` added only a book-service-scoped one; the general "every federated tool matches its provider prefix" check is unbuilt.
...
- **✅ Stage 4 BACKEND SHIPPED + D3 live-proven on dev, 2026-07-11 (RB-4-BE).** FE half (22-C + 24-H2.2) is the follow-on. 3 build agents → parent SC4 read-path fix → `/review-impl` (4 dims, 3-lens refute-verify).
  ...
  - **▶ The unblocked Stage-4 FE wave is COMPLETE.** `22`-D2 satisfied (frontend-tools contract already lists both scene panels; no new enum added). `22`-D1 (OpenAPI for the book-service `/v1` scene routes) is a **book-service Go task**, owned by the scene-route wave, and needs a drift-test to be worth writing — out of the FE wave's scope. **Everything else is Deploy-2-gated (needs `structure_node` at runtime):** C2b group-by-Arc + bulk + virtualization, C4b SceneRail identity-union, the Hub canvas (24-H2 consuming `laneLayout`). Do NOT build these blind against non-existent routes. FE verify = `tsc --noEmit` + vitest + a **built-image** browser smoke (`frontend-5174-is-baked-prod-nginx-not-vite`).
