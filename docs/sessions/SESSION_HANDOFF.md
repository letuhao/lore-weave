...
>     - **▶▶ NEXT M2.b–f:** translation (model + params + tokens SUM + cost at finalize, reconcile), composition (model/params + tokens from result; cost_usd col exists but unwritten → None), video-gen (model/params; no cost/tokens), lore-enrichment (model/params + `actual_cost_usd`), campaign (per-stage model names + `spent_usd` + params; NOT a reconcile source). glossary-translate = OUT (not on the unified emit plane). Then M3 (FE rewrite) + M4 (Playwright).
