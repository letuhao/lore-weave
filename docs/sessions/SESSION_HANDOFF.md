...
> - **▶ NEXT (P1 cont.):** (1) **worker-ai serial pass (money-path service):** migrate `summary_consumer` (Family-3; nuance: its retryable→never-drop vs the base retry→poison — acceptable for re-derivable summaries, document) **and** the money-path **`llm_extract_consumer`** — flag-gated, `/review-impl`, full extraction E2E. (2) **emit-wiring** (per-service `outbox_events` table + `OUTBOX_SOURCES` + `emit_job_event` — shared compose/relay → SERIAL). (3) batched live-smoke `D-JOBS-MIGRATE-LIVE-SMOKE`. Checklist (strip internal ack/retry; verify UUID job-id) in the P1 plan.
