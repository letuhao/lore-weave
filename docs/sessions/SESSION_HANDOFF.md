...
**4 commits LOCAL on `wiki/llm-gen` (off main `2ace6272`), NOT pushed:** `e9313ef0` plan + BookProfile-A · `12956b1e` M0 · `fc969f03` M1 BookProfile read · (this) M2 retriever+context.

**▶ NEXT SESSION = `/loom` M3 — `prompt.py` + `generate.py` (LLM→IR) + `rulegate.py`** (the core single-pass generation proof). Consumes M1 `BookProfileClient.get_profile` (worldview/voice/era/language) + M2 `gather_entity_context` (the cite-labelled `Source` table). Plan §I-**C2** (Markdown contract + cite-label protocol) + §II-**M3** + risks #2 (cite round-trip fragility), #6 (truncation), #14 (infobox-vs-body dup). LLM seam = clone knowledge `llm_client.py` (provider-registry `submit_and_wait`). First shippable proof = a grounded article generates (even if not yet persisted via the full job). M7a (citation-mark FE) stays parallel-able.
