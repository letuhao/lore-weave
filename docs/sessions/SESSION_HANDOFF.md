...
**NEXT options:** (1) **live-smokes** on a rebuilt stack — the deferred translation→learning chains (M7a-quality, M7b-accept, M7c-corrected, M7c3-name-confirm, M7d-3-feed→judge) + M6b-worker-usage + M4d1, all share one stack-up · (2) **PR** `feat/translation-pipeline-v3` → `main`. **PO: do NOT merge to main until translation is production-ready** — the M7 feedback track is part of that gate. **🏁 M7 COMPLETE: all 3 channels collect** (M7a-log, M7b-accept, M7c-edit-gold + reachable FE, M7c-3-name-confirm) **+ the online fidelity judge is wired + fed** (M7d-1 SDK → d-2 learning → d-3 worker feed). (Before *enabling* the online judge in prod: clear D-TRANSL-M7D-INLINE-JUDGE — keep both judge flags OFF until out-of-band.)
