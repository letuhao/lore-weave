# Session Handoff — `feat/glossary-assistant-coverage` · glossary GENRE·KIND·ATTRIBUTE re-arch — design locked + G1→G3 built (G3b book-tier CRUD) · 2026-06-19
...
> - **▶ NEXT — `D-GKA-ADOPT-USER-TIER` (focused follow-up, non-destructive), then the GATED hard-stop G4.** G3a/G3b adopt + copy from the **SYSTEM tier only**; D7 wants a book owner's **User-tier** overrides to shadow system by code at adopt. **Open design Q before building it:** the grant `/access` projection exposes `grant_level` + `lifecycle` but **NOT the book owner's user_id**, so "the *owner's* user tier" isn't resolvable without either (a) scoping the union to the **adopting caller** (no new contract; caller≈owner under the Manage gate; arguably more correct — the person doing setup uses their own customizations) or (b) a **new book-service owner-resolution contract**. Decide (a) vs (b) at the start of that slice. Then the **GATED hard-stop = G4** (needs explicit user go-ahead — destructive): entity repoint (glossary_entities.kind_id→book_kinds, eav.attr_def_id→book_attributes) + entity_genres + snapshot-fn rewrite + extraction/translation retarget + DESTRUCTIVE drops (`genre_tags[]`/`genre_groups`/`system_kind_attributes`) + wiki-truncate/KG-clear (`D-GKA-G4-CONSUMER-RETARGET`). Then **G5** sync (content_hash diff/apply; D-GKA-HASH-REFRESH) → **G6** FE (~39-file rebuild).
> - **Deferred (G-epic):** **`D-GKA-ADOPT-USER-TIER`** (NEW, non-destructive, next slice — union the User tier into adopt so user overrides shadow system by code per D7; blocked on the owner-vs-caller resolution decision above). `D-GKA-VOCAB-DROP` (MED — seed represents only the canonical genre vocab; exotic runtime genre_tags dropped in the transient G1→G4 window; moot under R2 full-reset, documented in-code). `D-GKA-HASH-REFRESH` (MED — the admin/user-attr edit path must recompute `content_hash` or G5 Sync misses system-side edits). `D-GKA-G4-CONSUMER-RETARGET` (the drops: `genre_tags[]`/`genre_groups`/`system_kind_attributes` + entity-FK repoint + snapshot-fn rewrite + extraction/translation retarget + wiki-truncate/KG-clear — all G4). **Recently cleared:** `D-GKA-BOOK-CRUD` ✅ (G3b this session); `D-GKA-USERBOOK-COVERAGE` ✅ (user_*/book_* insert/constraint coverage shipped across G2+G3b CRUD tests). Bulk-merge `createKindAlias` retarget + per-book KG `relation` level still deferred from SS-4.
