...
**State: Phase-1 + 2 + 3(A,D) DONE + P3-EVAL retrieval harness DONE + LIVE-SMOKE PASSED (2026-06-08).** Hybrid (lexical+semantic, RRF) end-to-end. New workstream — the missing **raw chapter-text** layer beneath glossary/knowledge/wiki (all lossy derivatives); serves authoring + extraction/trích-lục. Spec [`2026-06-07-raw-search.md`](../specs/2026-06-07-raw-search.md) (PART I design + PART II ATAM-lite eval; 7/7 confirm-at-BUILD resolved). Plan [`2026-06-07-raw-search.md`](../plans/2026-06-07-raw-search.md) (3 phases; ADJ-1..4 PO-acked).
...
**NEXT:** **E5 recall+score tuning** (raise lexical `limit`/rethink per-chapter cap; calibrated score / score-floor) — the production-quality gap. Then re-run the harness to quantify the lift. Other Phase-3: P3-B canon-lexical · P3-C char-offset dedup · P3-D' semantic MMR/rerank · P3-E semantic-on-draft. OR **push** the branch (10 commits).
...
- **D-RAWSEARCH-E5-TUNING** (P3-EVAL finding, **priority** — production-quality gap) — lexical oracle-recall 0.63 + per-chapter-cap=3/`limit=10` under-cover wide terms; score is positional RRF (0.0164 floor), non-calibrated. Tune lexical `limit`/cap, add a calibrated relevance score / score-floor cutoff, optional RRF_K sweep; re-run `scripts/run_rawsearch_eval.py` to quantify lift.
- **D-RAWSEARCH-E6-FE-KSELECTOR** (P3-EVAL OUT) — expose K (`limit`) as a UI control in `RawSearchPanel`; meaningful once E5 score-floor lands.
- **D-RAWSEARCH-EVAL-CI** (P3-EVAL OUT) — wire the eval to a threshold gate + persist runs to `project_embedding_benchmark_runs`; expand the golden set (>15 q, more paraphrase/hard bands, multi-book) for a higher-confidence number.
