...
**NEXT options:** (1) **M7c-2** — FE: an **editable** translation view (Tiptap) + Save → `POST .../versions/edit` (makes the human-edit gold actually reachable by users) · (2) **M7c-3** — name-confirm capture (glossary emit user-actor on `confidence→verified` → learning gold) · (3) **M7d** — Channel-3 online LLM-judge · (4) live-smokes (M7a/b/c/M6b-worker-usage/M4d1) on a rebuilt stack · (5) PR `feat/translation-pipeline-v3` → `main`. **PO: do NOT merge to main until translation is production-ready** — the M7 feedback track is part of that gate. V3 core feature-complete; M7a✓ M7b✓ M7c-1✓; M7c-2/c-3/d + live-smokes remain.
