...
**State: P0+P1+P2+P3+P4+P5 BUILT + verified — the glossary assistant is FEATURE-COMPLETE for v1.** Read + edit-existing + new-entity draft + schema-create, all human-gated, skill-guided, and reachable on every book surface (editor + glossary page + reader). Only P6 (grounding consolidation, fenced below the line) remains. (full-stack/browser smokes deferred: 067/068 + D-GLOSSARY-EDIT/SCHEMA/SURFACES-SMOKE).
...
**NEXT:** **P6 — grounding consolidation (mui#3)** — move the glossary-grounding `build_context` path behind the gateway/MCP with a **mandatory `[]`-on-failure fallback** (H2) and **retained direct fallback**; billing metering unchanged (SO-6). Fenced "below the line" — the assistant already SHIPS at P5, so P6 is its own optional follow-up, not a v1 gate. When the docker stack is up, run the deferred browser/full-stack smokes (067 + 068 + D-GLOSSARY-EDIT/SCHEMA/SURFACES-SMOKE). Also outstanding: the glossary LLM-flow migration review (DEFERRED 066).
