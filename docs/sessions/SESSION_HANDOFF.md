...
**▶ NEXT — 2b-T2 INTEGRATION (the engine de-risks it):** (1) a `llm_job_terminal` consumer in translation-service (model on `app/events/glossary_consumer.py` XREADGROUP, group `translation-llm-resume`): on a terminal event → `get_job` for the full Job → look up the chapter by `chapter_translations.provider_job_id` → call `decoupled_translate.resume(...)` with a `finalize_cb`. (2) Flag `translation_decouple_enabled` (default off) + rewire `chapter_worker._process_chapter` so under the flag the TEXT path calls `decoupled_translate.start_chapter(...)` (submit chunk 0 + return) instead of the synchronous `translate_chapter`; `finalize_cb` runs the existing post-translate (write body + emit `chapter.translated` + `_check_job_completion`). (3) Live-smoke a real chapter through the event path. THEN reuse the engine for the BLOCK path (`translate_batch_with_retry`) + V3 verify/correct (2b-T3) — the campaign uses v3-block. Then **2b-worker-ai** (extraction, HARD — needs `loreweave_extraction` per-op resume points). Or Phase 3.
