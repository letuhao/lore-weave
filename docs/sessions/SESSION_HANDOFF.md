...
**CHAT & AI SETTINGS UNIFY — M1a–M4 SHIPPED 2026-07-05 (spec [`docs/specs/2026-07-05-chat-ai-settings.md`](../specs/2026-07-05-chat-ai-settings.md); user authorized a full end-to-end autonomous run, audit-at-end).** The fragmented chat/AI settings (7 surfaces, 3-way model split, silent fallbacks) are consolidated onto one resolution cascade. **6 commits:** `0172a4514` (M1a backend), `5944da242` (M1a FE), `a383c2a5f` (M2), `8843c7ba7` (M3), `fa935c26d` (M4), + session checkpoints.
...
**Tests all green:** full chat suite 1002, composition 791, chat-ai-settings 10, settings 45; tsc-clean (the 1 `i18n/index.ts` error is PRE-EXISTING/unrelated, confirmed via git).
**DEFERRED (tracked, defer gate #2 large/structural + cross-service):**
- `D-CHATAI-M1B-BOOK-TIER` — backend resolver Book tier is a present-but-stubbed slot; wiring it for chat sessions + the grant-gated SHARED-BOOK owner-read needs book-service `owner_user_id` on `/internal/books/{id}/access` (Go) + `loreweave_grants` SDK + a composition internal `model-settings` route + a chat→composition client. The STUDIO already has two-tier via M1a's FE; this is a backend refinement for chat-session + collaborator cases.
- `D-CHATAI-M5-VOICE-UNIFY` — the voice consolidation: 2-store migration (auth-service `voice_prefs`+`media_prefs` ↔ chat-service `user_chat_ai_prefs.voice`, per-surface `voice.chat`/`voice.reading` §4.4, lazy cross-service seed + dual-write-back for rollback safety), `voice_stream_service` consumption rewrite (persist the session TTS voice → fixes the `af_heart` reset), and the FE `VoiceSettingsPanel`/`ReadingTab` TTS consolidation into the new panel. Voice stays functional via its existing panel today.
- `D-CHATAI-M4-TIER-CONSUMPTION` — fine-grained per-tier (T5/T4/D13a/trigger/cap) + compaction-path consumption of ai-prefs.context; deferred as those tiers are eval-proven inert + default-off and the consumption is deep compact_service surgery (the M4 mode-level Off switch delivers the real control now).
