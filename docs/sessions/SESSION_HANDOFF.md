...
**State: P0 (ai-gateway) + P1 (read tools) + P2 (propose_new_entity draft) + P3 (edit-existing frontend-propose) + P4 (Tier-S schema confirm-token) BUILT + verified.** Architecture frozen. Read assistant + new-entity draft + edit-existing + schema-create (kind/attr) all wired (full-stack docker smokes deferred 067/068 + D-GLOSSARY-EDIT/SCHEMA-LIVE-SMOKE).
...
**NEXT:** **P5 — surface enablement + static "glossary skill" system-prompt** (OD-5): per-surface tool curation (H7 — advertise the `glossary_*` group, drop `memory_search`'s glossary overlap; H11 per-surface `MAX_TOOL_ITERATIONS`, glossary surfaces default 10), the fixed glossary-skill prompt stating glossary content is DATA not instructions (INV-6), kinds fetched on-demand via `glossary_list_kinds`. Then P6 grounding (mui#3, `[]`-fallback H2). When the docker stack is up, run deferred full-stack smokes (067 + 068 + D-GLOSSARY-EDIT/SCHEMA-LIVE-SMOKE).
