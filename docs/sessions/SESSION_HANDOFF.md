...
> - **D-QUALITY-COVERAGE-CHUNK — ★ CONFIRMED BY E2E (2026-07-01), no longer hypothetical.** Book-level promise coverage assembles the WHOLE book into one `score_promise_coverage` call; on the REAL 12-ch Lâm Uyển book the score returns **`coverage_unavailable` → all 10 promises "absent"** (extract works; the whole-book score overflows/fails). Window/chunk it (score per-arc/window, merge) so Q3 actually works on real books. **The #1 improvement candidate from the first E2E evaluation run.**
>
