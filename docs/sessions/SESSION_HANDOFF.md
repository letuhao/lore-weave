...
**State: P0 (ai-gateway) + P1 (read tools) + P2 (propose_new_entity draft) BUILT + verified.** Architecture frozen. Read-only assistant + first write tool wired at unit/spike level (full-stack docker smokes deferred 067/068).
...
**NEXT:** **P3 — edit-existing as a FRONTEND propose tool + FE diff-card renderer** (consumer-side, NOT gateway-routed): the assistant proposes an edit to an existing entity (name/attribute); the run SUSPENDS, streams the proposed change to the browser, the user reviews a diff card and Apply/Dismiss; Apply is a **version-checked** patch (H5 — `If-Match: base_revision_id`, 409 on drift) and the resume reports the **real** outcome (H6). Reuse the chat `propose_edit` suspend/resume machinery. Then P4 Tier-S schema (confirm-token), P5 surfaces+prompt, P6 grounding. **`/amaw` for P3+ (write path).** When the docker stack is up, run deferred full-stack smokes (067 + 068).
