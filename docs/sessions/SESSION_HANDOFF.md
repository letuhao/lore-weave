...
**State: M0 + M1(a–c) + M2 + M3 + M4a + M4b + M4c DONE** (M1d, M4d deferred). M0 scaffold; M1 rule-tier verify + re-translate + romanization; M2 LLM verifier + QA loop; M3 semantic batching; M4a knowledge read-port; M4b relations+bios brief → Translator+Verifier; **M4c cross-chapter memo (prev story+names) → Translator + cold-start name harvest (G4 complete)**. All PO-approved + `/review-impl`'d. Full suite **403 passed**; **parity** preserved (default `pipeline_version='v2'`).
...
**NEXT options:** (1) **config plumbing** (small) — wire `qa_depth`/`max_qa_rounds`/`verifier_model` settings→job→coordinator→worker (enable `thorough` + per-role verifier model); (2) **M5** (living-book: staleness/invalidation · provenance + publish quality-gate · "needs review" UI · sync-path convergence TD2); (3) **M6** (human-correction → glossary confirm → targeted re-translate + propagate); (4) **D-TRANSL-EXTRASYSTEM-BUDGET** (account injected context in the batcher token budget).
