...
> - **M2 NEXT (frontend, `D-GLOSSARY-LIST-P1-FE`):** server-paged hook + `<Pager>` + page-size + debounced search + sort UI + raw-search toggle/highlights + `<EntityListBrowser>` (GlossaryTab), built against this committed contract. Land before RAID C21 (world-container-fe, ~2 cycles out) touches GlossaryTab.
...
> - **`D-GLOSSARY-SORT-BE`** ✅ **SHIPPED (M1 above)** — `entityOrderBy` whitelist (name/kind/status/alive/recency + raw-relevance) over `e.cached_name`; counts-sort still deferred.
