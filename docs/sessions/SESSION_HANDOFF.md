...
- **D-TRANSL-M6B-LIVE-SMOKE** (M6b-1) → **mostly CLEARED 2026-06-08 (live-smoke, rebuilt stack).** A 4-chapter-translation fixture (vi/en × used-E1/used-E2/no-index) + a real glossary translation PATCH proved the consumer's row-selection SQL live: **precise** (used-E1 vi → flagged), **per-language** (used-E1 *en* → NOT flagged), **precision** (used-E2 vi → NOT flagged), **legacy fallback** (no-index vi → flagged). The M6b-2 **coverage** endpoint surfaced `is_glossary_stale` correctly incl. the `latest`-version fallback branch (`/review-impl`'s mock-only concern — now live). **Residual (narrowed):** the **worker writing `chapter_translation_glossary_usage` rows during a real translation** is the only unproven link — it's a best-effort additive INSERT, unit-tested ×3, and its *output* (usage rows) is now proven consumed correctly live. **2026-06-08 LLM-pass attempt:** ran a real **V3 translation end-to-end** (first ever on the stack — book 封神演義 ch.3, zh→vi, qa_depth=rule_only, LM Studio Qwen3-35B, job completed, score=100) → the glossary-fetch + `_record_glossary_usage` path **executed without error** but recorded **0 usage rows because the dev glossary corpus has NO translations at all** (`attribute_translations` is empty stack-wide → `translation-glossary` returns 0 entries for every book/lang → `used_entity_ids` empty → nothing to record, which is *correct* no-false-usage behaviour). **Blocked by corpus data, not code.** Fully closing it needs a seeded corpus: a book whose glossary entities have `verified` vi translations whose source names appear in a (MinIO-stored) chapter, then re-translate + assert the row. Deferred as **corpus-data-blocked** (the consumer + write are unit/live-covered; this is the last cosmetic link).
...
- **D-TRANSL-M4D1-LIVE-SMOKE** (M4d-1) — **corpus-data-blocked (2026-06-08 LLM-pass attempt).** The wiring is **confirmed live**: the real V3 translation showed the worker holds `KNOWLEDGE_SERVICE_INTERNAL_URL` (NOT Null-gated) and made real `POST /internal/knowledge/wiki-neighborhood` 200s (the M4b sibling path). But the **timeline** half (`POST /internal/knowledge/timeline` → `knowledge_projects` resolve → `list_events_filtered` → memo in the Translator `extra_system`) can't be exercised: **no `knowledge_project` exists for any translatable book** and the timeline event store (Neo4j-derived) is **unseeded** → the endpoint resolves an empty window → empty memo (nothing to assert beyond "no crash"). knowledge-service is FRESH on the stack; endpoint + window math + client + wiring are unit-covered and the query layer is verbatim-reused from prod-proven `handlers.py`. To close: seed a `knowledge_project` + timeline events for a translatable book, then translate + inspect the worker prompt log. Lowest-priority.
