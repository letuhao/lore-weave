...
>   - **▶ NEXT — reconcile Increment B then P4:** (B) add the reconcile SOURCE `GET /internal/{svc}/jobs?since=` to **knowledge / video_gen / lore_enrichment / translation** + their entries in `reconcile._RECONCILE` (each maps its native row → canonical `JobEvent` payload; mind the per-service timestamp column — `translation_jobs` has **no `updated_at`**, use `GREATEST(created_at,started_at,finished_at)`; owner column `user_id` vs `owner_user_id`; status canonical-mapping). Then flip `reconcile_enabled` default ON once all 5 sources exist (`D-JOBS-P2-RECONCILE-CROSS-SVC`). (P4) **the unified Jobs GUI** — dashboard listing the user's jobs (live SSE), children grouped under campaigns (H3), filter by kind/status, per-row state-aware cancel/pause/resume gated on `control_caps` (generalize `MonitorControls`/`CampaignMonitor`); `kind=campaign` deep-links to the existing monitor. **NB: P4 is frontend — a concurrent process has been editing FE; re-check `git status` + coordinate before P4.**
