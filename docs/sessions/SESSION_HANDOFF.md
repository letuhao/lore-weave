...
**M5 surfacing layer COMPLETE:** M5a needs-review badge · M5b publish quality-gate (+auto-active gate) · M5c glossary-staleness (consumer + badge). Remaining M5 piece: **M5d** sync-path convergence (TD2).

**NEXT options:** (1) **M5d** sync-path convergence (TD2; refactor sync `/translate-*` onto worker path — risky) · (2) **M6** human-fix flywheel (correction → glossary confirm → re-translate + propagate) · (3) **D-TRANSL-EXTRASYSTEM-BUDGET** · (4) **push** (20 commits, needs explicit approval). All 5 original goals delivered; remaining are flywheel/tech-debt enhancements.
