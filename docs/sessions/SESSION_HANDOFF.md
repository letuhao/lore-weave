...
- **Deliberately out of scope this pass (flagged, not silently dropped)**: the bulk extraction pipeline's own cross-kind dedup (`findEntityCrossKind`) does not yet consider `scope_label` — same reasoning as `findEntityByNameOrAlias`'s `""` pass-through, that pipeline has no scope concept yet; `glossary_search`/list-endpoints don't surface `scope_label` in their response yet (only `glossary_get_entity` does) — a nice-to-have visibility gap, not a correctness gap (an agent can always look up a specific hit). Neither blocks the feature working correctly today.
