...
>   - **Remaining (lower priority):** full create→projection e2e for A/C/B real jobs (`D-PRODUCER-EMIT-*-FULL-E2E`, med — Python emit lib + relay routing already proven, worker wiring unit-tested) + `D-JOBS-SECONDARY-KIND-CONTROL` (wire unified control for the 4 view-only secondary kinds — currently view-only, controllable via native panels) + the B0/B1/B2 deferred smokes.
