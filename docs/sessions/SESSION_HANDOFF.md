...
**3 commits LOCAL on `wiki/llm-gen` (off main `2ace6272`), NOT pushed:** `e9313ef0` plan + BookProfile-A decision · `12956b1e` M0 · (this) M1 BookProfile read.

**▶ NEXT SESSION = `/loom` M2 — retriever `run_hybrid_search` in-process refactor + `context.py`** (attrs+KG+passages gather + sanitize; passage→cite-label assignment). Critical path. Plan §I-**C4/C10** + §II-**M2** + §III risks (#9 not_indexed degradation). M7a (citation-mark FE, needs only M0+M5) stays parallel-able; critical path = M2→M3→M4→M5→M6→M7b. The wiki prompt (M3) will consume `BookProfileClient.get_profile(book_id)` to shape worldview/voice/era/language.
