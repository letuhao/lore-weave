...
**State: M0 + M1(a–d) + M2 + M3 + M4(a–d) + config-plumbing + M5(a–d) + M6(a + b-1 + b-2) + M7(a + b + c-1 + c-2 + c-3 + d-1 + d-2) DONE** (M7 feedback→learning: M7a–c✓ M7d-1✓ M7d-2✓ · only M7d-3 left — the worker feed) (**M1d + M4d-1 + M4d-2(a/b/c) + glossary-versioning(VG-1/2/3) + M6b-1/b-2 targeted-propagate DONE 2026-06-07/08**). **🏁 Translation V3 plan-of-record COMPLETE — all milestones M0–M6 shipped incl. the full human-fix flywheel (M6a confirm → M6b-1 targeted per-language stale → M6b-2 surface + user-triggered re-translate); M4d-2 full 2-pass cold-start + the glossary entity-versioning epic landed.**
...
**NEXT options:** (1) **M7d-3** (the LAST M7 slice) — translation worker `_emit_translation_quality` carries truncated `source_text`+`translated_text` in `translation.quality` when a feed flag is on (off default); this activates the M7d-2 judge hook end-to-end · (2) live-smokes (M7a/b/c/c-3/M6b-worker-usage/M4d1) on a rebuilt stack · (3) PR `feat/translation-pipeline-v3` → `main`. **PO: do NOT merge to main until translation is production-ready.** After M7d-3, the whole M7 feedback track is shipped (all 3 channels + the judge); remaining = live-smokes + PR. (Before *enabling* the online judge in prod: clear D-TRANSL-M7D-INLINE-JUDGE.) **PO: do NOT merge to main until translation is production-ready** — the M7 feedback track is part of that gate. **All 3 feedback channels' human + auto-log signals now collect** (M7a-log, M7b-accept, M7c-edit-gold + reachable FE, M7c-3-name-confirm); only M7d (online judge) + live-smokes + PR remain.
