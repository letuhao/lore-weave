...
> - **▶ IN PROGRESS (P1 tail M-T2/M-T3/M-T4 — emit-wiring, this run):** (M-T2) new `outbox_events` tables for campaign/lore-enrichment/video-gen + 3 OUTBOX_SOURCES entries; (M-T3) `emit_job_event` at each in-scope service's status chokepoint (knowledge/worker-ai, translation, composition, campaign, lore-enrichment, video-gen; learning judges + chat + spend excluded) + wiring tests; (M-T4) batched boot live-smoke `D-JOBS-MIGRATE-LIVE-SMOKE`. **Caveat:** producer-half only until P2's jobs-service projection consumes `loreweave:events:jobs`.
