...
**▶ NEXT — WX-T3b (the one remaining WX piece — TURNKEY in the design doc) · then 2b-T3b (V3) · or backlog:**
- **WX-T3b** = the consumer + the runner control-flow **inversion** (consumer becomes the chapter-sequencer: drives entity→trio→[recovery]→[filter]→persist→advance+spend+wake, then the next chapter) on the **E0-3 billing + `knowledge.chapter_extracted` campaign** path. ~600 lines (shell submit-assembly/fold-dispatch + candidate serialize/deserialize + Redis consumer + runner in-flight-guard branch + the 25-field resume_state context). **Flag-gated** ⇒ sync path safe until `D-WX-LIVE-SMOKE` gates default-on. **Deliberately NOT built in the 18-commit `/loom` turn** — it's only verifiable via a cross-service live-smoke (mocks hide cross-service bugs, the repo lesson), so it needs focused execution + a real smoke, NOT a marathon-tail cram on money-path code. The full resume_state schema + shell fn list + consumer outline + runner branch + the live-smoke acceptance are in the design doc's **"WX-T3b TURNKEY BUILD PLAN"** — drop-in. Resume via `/loom continue`.
- (Older, superseded) ~~WX-T3 (extraction orchestrator)~~ →
