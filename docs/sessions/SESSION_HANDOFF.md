# Session Handoff — `feat/auto-draft-factory-gaps` · HEAD `57206d70` · 2026-06-15

> **🔨 T2-M2 IN PROGRESS (2026-06-15) — per-segment translation status + dirty-only re-translate.** Plan [`docs/plans/2026-06-15-translation-panel-t2-m2-segment-translate.md`](../plans/2026-06-15-translation-panel-t2-m2-segment-translate.md). Split at a risk boundary into **M2.1 (status foundation)** ✅ + **M2.2 (dirty-only re-translate)** ⏳ NEXT.
...
> - **⏳ M2.2 NEXT:** dirty-only re-translate via the existing job machinery — `CreateJobPayload`+`translation_jobs`+message gain `block_index_filter`+`seed_version_id`; worker block path translates only dirty blocks + overlays onto the seed version + finalizes a normal LLM version (human-guard is free via `_PROMOTE_ACTIVE_SQL`); new `POST /v1/translation/chapters/{id}/retranslate-dirty` (force_retranslate=true). **⚠️ `segment_index` is NOT stable across re-segmentation** (DELETE+re-INSERT renumbers) — re-translate must map by block-range/hash, not assume index identity; orphan `segment_translations` rows (segment-count shrink) are invisible to `compute` (LEFT JOIN from current) but a cleanup is owed.
> - **Deferred:** `D-TRANSL-T2M2-LIVE-SMOKE` (real stack: edit block → rebuild segments → status dirty → retranslate-dirty → only dirty segment re-translated + promoted).
