...
**NEXT options:** (1) **M5** (living-book: staleness/invalidation · provenance + publish quality-gate · "needs review" UI · sync-path convergence TD2); (2) **M6** (human-correction → glossary confirm → targeted re-translate + propagate); (3) **D-TRANSL-EXTRASYSTEM-BUDGET** (account injected context — romanization + knowledge brief + prev-memo — in the batcher token budget); (4) **push** the branch (15+ commits, needs explicit approval). All 5 original goals are delivered + config knobs wired; M5/M6 are living-book/flywheel enhancements.
