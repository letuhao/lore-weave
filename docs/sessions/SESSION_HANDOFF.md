...
>   - `ContextPicker` — **multi-book** picker; single-book browser doesn't fit directly (a per-expanded-book browser is possible later). **Defer.**
>   - `ChapterRangeStep` (campaign) — range-by-`sort_order` predicate; needs a reusable **`range` mode**. **Defer.** (StepBatchConfig **all/range** were fixed in /review-impl — see below.)
>   - **Deferred (genuine, not over-fit):** `D-CHAPTERBROWSER-RANGE-MODE` (a reusable range mode → migrate campaign `ChapterRangeStep`, still `limit:5000`→100-capped), `D-CHAPTERBROWSER-CONTEXTPICKER` (per-book multi). The matrix/ChaptersTab/Reader/Editor sites are **won't-migrate** (the browser is the wrong shape for them).
