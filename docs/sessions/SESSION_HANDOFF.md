...
1. **D-WIKI-P2-KG-SWEEP** — the pull sweep's KG-neighbourhood recompute (GAP A: pipeline KG changes emit no event). Cross-service: needs knowledge's Neo4j neighbourhood hash; the version-drift half is done glossary-local. *(The largest remaining wiki follow-up.)*
2. **D-WIKI-M8-LEARNING-CONSUMER** (already partly on main via the composition correction-flywheel) + **D-WIKI-M8-EVAL-PLUS**.
3. **D-WIKI-P2B-COST-ESTIMATE-LIVE-SMOKE** (new) — exercise the gen-config GET through the live FE→glossary→knowledge stack (rebuild glossary+knowledge; JWT + a book). Low-risk config proxy, unit-pinned; deferred at VERIFY rather than do a full rebuild for a config read.
The wiki feature (M0→M8 + M7a + Phase-2 + change-control DECIDE loop) is functionally complete + live-proven.
