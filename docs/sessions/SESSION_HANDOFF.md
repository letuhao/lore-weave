...
**▶ NEXT — wiki FE gap-closure COMPLETE (W1–W5 all shipped).** The 5-screen mockup↔code gaps are closed. Remaining wiki items are **live-smokes** (`D-WIKI-W4A-LIVE-SMOKE`, `D-WIKI-W5-LIVE-SMOKE`, the earlier P2/M8 ones) + the **`D-WIKI-W4-RESULTS-64KB`** proxy-cap fix — all low-risk/unit-pinned. Natural next moves: a **PR** for `wiki/phase2-change-control` → main, or the **platform system-config UI** epic below once its auth-conflict clears.
