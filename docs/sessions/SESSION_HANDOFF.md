...
**Phase 1 (provider-registry + SDK) BUILD COMPLETE + queue live-smoked — all 3 commits + follow-ups.** Next: **Phase 2** (convert worker-ai → knowledge → translation to event-resume: pass `event_redis_url` to the SDK client + enable the queue + per-service consumer wiring per spec §6), **Phase 3** (long tail). Enable + live-smoke the queue as part of Phase 2's stack-up.
