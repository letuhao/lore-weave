...
**State: M0 + M1(a–c) + M2 + M3 + M4a DONE** (M1d, M4d deferred). M0 readiness gate + V3 scaffold; M1 rule-tier verify + targeted re-translate + romanization; M2 LLM verifier (Tier-2) + multi-round QA loop; M3 dialogue/scene-aware semantic batching; **M4a knowledge read-port (`wiki-neighborhood` client)**. All PO-approved + `/review-impl`'d. Full suite **377 passed**; **parity** preserved (default `pipeline_version='v2'`).
...
**M4a shipped:** `app/workers/knowledge_client.py` (mirrors `glossary_client` — plain async fn + dataclass + degrade): `Relation`/`WikiNeighborhood` + `fetch_wiki_neighborhood(user_id, glossary_entity_id, rel_cap=200)` → POST wiki-neighborhood (X-Internal-Token). **Two gates:** Null (no `knowledge_service_internal_url` set ⇒ empty, no HTTP) + degrade-to-empty (non-200/transport/parse error). Faithful parse keeps per-relation `confidence`/`pending_validation`/`source_type` **and** entity-level `source_types`/`entity_source_type` for M4b's §11.2 trust ladder. `config.py` `+knowledge_service_internal_url=""` (opt-in); compose `+KNOWLEDGE_SERVICE_INTERNAL_URL` (svc+worker, port 8092). **Client only — NOT wired into the pipeline** (no dead-code; zero runtime change until M4b). **+9 tests** (377 total). **Live smoke: POST wiki-neighborhood → HTTP 200, auth OK, live shape matches parser.** `/review-impl`: 1 MED fixed (entity trust fields were dropped) + malformed-body test; 3 LOW → M4b deferred rows below.

**NEXT options:** (1) **M4b** — relations+bios → pronoun/honorific brief into Translator+Verifier (consumes M4a; see the 3 D-TRANSL-M4B rows); (2) **config plumbing** (small) — wire `qa_depth`/`max_qa_rounds`/`verifier_model` settings→job→coordinator→worker; (3) **M4c** cold-start memo; (4) **M5/M6**.
...
- **D-TRANSL-M4B-ENTITY-ID-SOURCE** (M4a review-impl, LOW → M4b must-do) — `fetch_wiki_neighborhood` needs a `glossary_entity_id`, but translation's existing `fetch_translation_glossary` returns `[{zh, vi, kind}]` with **no id**. M4b must source ids from glossary `select-for-context` (returns `entity_id`) — do NOT assume `translation-glossary` carries them.
- **D-TRANSL-M4B-FANOUT-CACHE** (M4a review-impl, LOW → M4b) — M4b will call `fetch_wiki_neighborhood` per-entity; 5s timeout × N entities **serial** = a stall. Batch/parallelize + add the deferred **Cached** layer (TTL per book/project, fetch-once-per-chapter) before per-entity fan-out ships.
- **D-TRANSL-M4B-INJECTION-DEFENSE** (M4a review-impl, LOW → M4b) — relation `subject_name`/`object_name` originate in knowledge-service and will enter the Translator/Verifier prompt in M4b; sanitize at that boundary (§11.4 injection-defense). M4a stores them raw (correct — it doesn't inject).
