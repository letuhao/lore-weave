...
> **📐 TRANSLATION PANEL OVERHAUL — DESIGN PARKED (2026-06-15).** Spec [`docs/specs/2026-06-15-translation-panel-overhaul.md`](../specs/2026-06-15-translation-panel-overhaul.md). Two deferred big-features designed (build deferred by user). Key findings: most of the translation UI already exists (build ON `SplitCompareView`/`TranslationViewer` edit/`BlockAlignedReview`/`ConfirmNameDialog`); a `scenes` table already exists (P1, used by extraction) so T2 extends it — but `.txt` bulk-imported chapters have NO scenes → T2 needs heuristic scene-gen. Translation code is **RAID-free** (deferral was bandwidth, not file conflict). Deferred rows: **`D-TRANSL-PANEL-T1-CORRECTION`** (T1 split correction panel, B-first = reuse `saveEditedVersion`, FE-only, M), **`D-TRANSL-PANEL-T2-SCENE-SPLIT`** (T2 persisted scene-split: `scene_translations` + heuristic scene-gen + per-scene pipeline + matrix drill-down + `limit:200`→100 fix; XL/cross-service/+amaw), **`D-TRANSL-PANEL-T1A-PER-BLOCK`** (per-block patch, after T2). Recommended order T1-B → T2 → T1-A.
