...
>   - **▶ NEXT — P3-3+:** wire **translation** (note: translation-service files are under concurrent T2-segment edits — coordinate) + **lore-enrichment** (already HAS pause/resume/cancel via its C8 state machine + resume-stream re-drive — wrap a `job_id`-keyed internal endpoint reusing `_transition_job`, and **add `enrichment_job` to jobs-service `contract._MULTI_UNIT_KINDS`** so pause is offered — it is genuinely multi-unit, the conservative comment is stale for it) onto the same `/internal/{svc}/jobs/{job_id}/{action}` contract + `control._CONTROL`. Then the **reconcile sweep** + `GET /internal/jobs?since=` per service (`D-JOBS-P2-RECONCILE-CROSS-SVC`). Then **P4: the unified Jobs GUI**.
>   - **Deferred:** `D-JOBS-P3-VIDEOGEN-PROVIDER-ABORT` (video-gen cancel doesn't abort the in-flight provider job) · `D-JOBS-P3-KNOWLEDGE-CANCEL-SUCCESS-LIVE-SMOKE` (successful cancel mutating a real running extraction) · `D-JOBS-P2-RECONCILE-CROSS-SVC` (reconcile sweeper + per-service `/internal/jobs?since=` → P3) · `D-JOBS-P2-SSE-LIVE-SMOKE` (real consumer-upsert→pub/sub→connected-client push on the stack; notify+publish+stream-framing are unit-covered) · `D-JOBS-P2-SSE-EVENTSOURCE-AUTH` (browser EventSource can't send a bearer — P4 GUI uses a fetch-stream; token-in-URL intentionally unsupported).
