...
**W10-M2 SHIPPED — maps primitive, 2026-07-11** (`f1871835c`). **▶▶ THE W10/W11 BACKEND IS NOW COMPLETE** (all 6 milestones: W11-M1/M2/M3 reader journey + W10-M1/M2 world container, each with a mandatory adversarial review). book-service `world_maps`/`map_markers`/`map_regions` tables (world_id-scoped; `x/y DOUBLE PRECISION [0,1]`; polygon JSONB; CASCADE world→maps→markers/regions; the G1-lock test scoped to `books` so `world_maps.world_id NOT NULL` doesn't false-match) + 5 `world_map_*` MCP tools (create/add_marker/add_region/get/list; owner-scoped; `world_` prefix federated). **Adversarial 3-dim review → 1 MED + 2 COSMETIC:** MED (fixed) — `world_map_get` swallowed marker/region sub-query + row errors (silent-success: a transient DB error dropped a map's pins as an authoritative empty result); COSMETIC (fixed) — removed the "Reversible" over-promise (no delete tool yet); COSMETIC (accepted) — a degenerate polygon stores faithfully (spec-faithful GIGO). **Verify:** book-service api + migrate suites green; map DB round-trip live (create→marker→region→get + owner-scoping). **▶ FOLLOW-ONS (Track C surfaces + small backend tails, NOT blocking the backend):** (1) the MinIO image UPLOAD REST that populates `world_maps.image_object_key` (schema + `s.minio` ready; reuse the `book_set_cover` path) + `world_map_delete`/`remove_marker`/`remove_region` tools (would restore the "Reversible" promise + an undo_hint); (2) the enforcement-gap `tools/list` prefix test (from W10-M1); (3) `D-WS4C-EFFECTIVE-VALUE` remains cleared. **Track C** owns all W8/W10/W11 SURFACES (onboarding fork, world-container UI, reader UI, map canvas) + the W1–W12 workflow objects — the backends they consume are now shipped.
...
- **✅ Stage 5 CORE SHIPPED + RB-5 live-proven cross-service, 2026-07-11 (RB-5).** The `26` indexing/staleness spine. 3 build agents (one per service) → parent contract reconcile → `/review-impl` (3 dims, 3-lens refute-verify). 26-D provenance + 26-F FE chips are follow-ons.
  ...
  - **▶ NEXT FE (remaining C-wave + chips):** **C4b** — SceneRail renders the book-service IDENTITY union (imported chapter's rail not empty pre-decompile) — deferred: entangles `ManuscriptUnitProvider`, best paired with the browser smoke; the scene-browser already renders the full union · **C2b** (group-by arc/POV/beat · bulk spec writes · virtualization · windowed spec paging — the spec_only suppression from the review is the interim correctness fix) · **C3** `scene-inspector` detail pane (reuse GroundingPanel/SceneGraphCanvas, OCC 412) · **26-F1..F3** state chips (consume IX-14 `/conformance/status`) · **22-D1/D2** OpenAPI + contract regen. **24-H2.2 lane-layout is Stage-7 Hub** (`features/plan-hub/` doesn't exist; gated on Deploy 2). **The C-wave DoD is a real browser smoke** (vite :5199 + Playwright MCP) — deferred this run because :5199 is owned by a live concurrent session (shared-checkout hygiene); do it in a dedicated session.
