...
**State: Phase-1 + 2 + 3(A,D) + P3-EVAL harness + E5 recall/score tuning + E5B cross-encoder rerank DONE + LIVE-SMOKE PASSED (2026-06-08).** Hybrid (lexical+semantic, RRF) + cross-encoder rerank end-to-end. New workstream — the missing **raw chapter-text** layer beneath glossary/knowledge/wiki (all lossy derivatives); serves authoring + extraction/trích-lục. Spec [`2026-06-07-raw-search.md`](../specs/2026-06-07-raw-search.md) (PART I design + PART II ATAM-lite eval; 7/7 confirm-at-BUILD resolved). Plan [`2026-06-07-raw-search.md`](../plans/2026-06-07-raw-search.md) (3 phases; ADJ-1..4 PO-acked).
...
**NEXT:** **E6 FE** (granularity navigate/mine toggle + `relevance`/rerank-score display + K-selector) · `D-RAWSEARCH-EVAL-CI` (threshold gate + persist + 2nd-book golden + per-band metrics) · warm-rerank latency benchmark under load (vram_mb=8 ⇒ confirm GPU vs CPU). Other Phase-3: P3-B canon-lexical · P3-C char-offset dedup · P3-D' MMR. OR **push** the branch (12 commits).
...
- **D-RAWSEARCH-E6-FE** (E5 OUT) — FE: granularity navigate/mine toggle, `relevance` display (now in the API), K (`limit`) selector in `RawSearchPanel`. Also realign the FE lexical fallback to an explicit granularity (E5 flipped the book-service default to `chapter`).
