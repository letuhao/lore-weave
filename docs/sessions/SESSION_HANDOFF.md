...
- **🆕 `D-CAMPAIGN-BESTEFFORT-EMIT-REDIS` (reliability, MED):** under heavy local inference the dev **redis times out**, losing worker-ai's *best-effort* `knowledge.chapter_extracted` XADD → the campaign stalls at `knowledge=dispatched` and the **S3 stuck-timeout reconcile did not recover it within ~10 min**. Either make the per-chapter emit durable (outbox, like translation's) or tighten/verify the S3 reconcile so a lost best-effort emit self-heals quickly.
