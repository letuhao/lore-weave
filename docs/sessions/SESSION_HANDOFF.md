...
**7 commits LOCAL on `wiki/llm-gen` (off main `2ace6272`), NOT pushed:** …`f982f7c4` M3 · `a3505367` M4 · (this) M5 writeback+clobber-guard+schema.

**▶ NEXT SESSION = `/loom` M6 — fresh wiki orchestrator + cost-cap + `wiki_gen_jobs` + per-book lock + stream consumer + glossary delegate branch (C9)** — batch orchestration that ties M2→M3→M4→M5 per entity. **BUILD FRESH ~150-line orchestrator reusing GENERIC job infra** (risk #12 VERIFIED: `JobStateMachine`/`JobRecord`, `cost.py` `JobCostBudget`/`CostCapExceeded`, `JobEventEmitter`, `UsageMeter`, `make_complete_fn`, embed/retrieval-seam — NOT clone the gap/proposal-coupled LE `JobRunner`). knowledge `wiki_gen_jobs` table (mirror `extraction_jobs`; reuse `state_machine.py`, states incl `paused`). Per-book lock (risk #13: ONE active job/book; 2nd 409s/joins). Stream consumer (clone LE `resume_consumer.py`, flag-gated, per-entity skip-done) draining `loreweave:events:wiki-gen`. Glossary `generateWikiStubs` gains `model_ref`→delegate branch → knowledge `POST /internal/knowledge/books/{id}/wiki/generate` (202+job_id). **Cross-service live-smoke (D-WIKI-M5-LIVE-SMOKE + D-WIKI-M3/M4-LIVE-SMOKE all collapse here): a real entity generates end-to-end (context→generate→verify→cite→writeback) on the live stack.** Plan §I-**C9** + §II-**M6** + §III risks #12/#13. M7a (citation-mark FE) still parallel-able.
