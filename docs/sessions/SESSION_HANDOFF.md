...
**▶ NEXT (Track D):** WS-D3 is **DONE** (sweep + CD4 warn gate shipped; see the 2026-07-11 block above). Open items: (1) **WS-D4 decision** — the hard REJECT gate is `executes:false` today; tightening warn→reject needs a call on the target: full `proven` (needs ~199 NL probes; G1 selection is irrelevant to workflows), `executes ∧ effect` (grow the sweep with per-tool read-back oracles — medium lift), or `executes` alone. Recommend `executes ∧ effect` scoped to the *workflow-critical* set, not all 199. (2) **Step 5 — NL probes** for the `proven` column (the F5 description-quality signal, $0 on local gemma; not the workflow gate). (3) **Residue** (78 `executes:null`, none blocking): authoring-run family needs a paid confirm, `job_id` family needs real async jobs, kg graph tools need an adopted ontology, registry `slug` needs admin approval — each a scoped follow-up, characterized in the capability-sweep doc.
