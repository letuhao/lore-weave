...
- **D-RAWSEARCH-FE-JUMP-SEMANTIC** (→ folded into **P3-C**) — semantic hits only open the chapter (top) because `chunkIndex`≠block index. Precise semantic scroll + char-level highlight need the chunk→block / char-offset mapping (P3-C).
- **D-RAWSEARCH-EVAL-CI** — wire the eval to a threshold gate + persist runs to `project_embedding_benchmark_runs`; add a 2nd-book golden + per-band metric breakdown for higher-confidence numbers.
