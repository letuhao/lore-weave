...
**▶ NEXT — W6b-2b: the diff endpoint + FE red/green view (cross-service XL, IN PROGRESS).** **before** = the stored `wiki_article_source_usage.source_text` (W6b-2a); **after** = re-gather the entity's CURRENT context (knowledge, reusing `gather_entity_context`) → the current text for (source_type, source_id) — one knowledge hop handles all types. Likely: knowledge `POST …/wiki/source-text` (re-gather → current text per source) → glossary `GET …/wiki/staleness/{id}/diff` {before, after} (owner-gated; NULL source_text → 404/no-diff) → FE "View diff" affordance on snapshot-bearing rows, red/green via the W1 `wikiDiff` lib (`diffLines`). Scope CLARIFY at the start (on-demand re-gather cost; the after-fetch reuse). **W1–W5 + W6a + W6b-1 + W6b-2a shipped.** **W1–W5 + W6a + W6b-1 shipped.** Other moves: a **PR** for `wiki/phase2-change-control` → main, or the **platform system-config UI** epic once its auth-conflict clears.
