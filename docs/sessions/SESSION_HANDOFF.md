...
**▶ M4 BACKEND COMPLETE — 5/5 ops decoupled** (decompose · stitch · generate · chapter-gen · selection-edit) + accept/persist endpoint + composition-worker docker svc, all flag-gated `COMPOSITION_WORKER_ENABLED` (default off → inline verbatim). Commits (local, **NOT pushed**): `d99898dc` (accept+svc) · `6b015936` (generate) · `e4ebaa49` (chapter-gen) · selection-edit (pending commit). Earlier pushed: `62082c71` (foundation) · `785570d2` (stitch).
