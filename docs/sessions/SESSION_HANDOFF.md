...
> - **✅ P1 M2 — video-gen consumer MIGRATED (pattern-proof, this session).** `video-gen-service/app/worker/consumer.py` now subclasses `BaseTerminalConsumer` (~110 lines of hand-rolled transport deleted; `complete_job`/`sweep_once` kept verbatim as `handle`/`sweep_once` hooks; `operation='video_gen'` pre-filter preserved; `__main__` unchanged). **38/38 video-gen + 28 SDK green** (behavior-preserving). SDK editable-installed in the host env so `loreweave_jobs` resolves.
> - **▶ NEXT — P1 M2 cont.: `/warp` the remaining ~11 transport migrations** as disjoint per-service slices against the frozen SDK (PO pace decision 2026-06-15). **Transport-only in the slices** (each writes ONLY its own consumer file — disjoint); `OUTBOX_SOURCES` (docker-compose) + the relay + per-service `outbox_events` migrations are a **shared-write magnet** → the **emit-wiring is done SERIALLY after reconcile**, NOT in slices. **worker-ai money-path (`llm_extract_consumer`) is NOT a warp slice** — migrated serially, flag-gated, `/review-impl`, full extraction E2E. Then batched live-smoke `D-JOBS-MIGRATE-LIVE-SMOKE` (rebuild all migrated images, confirm every group boots). Migration checklist (strip internal ack/retry from each `handle`; verify UUID job-id) in the P1 plan.
