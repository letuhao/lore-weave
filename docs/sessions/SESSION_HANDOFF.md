...
> - **▶ NEXT (P1 cont.):** (1) **Family 3** functional job-workers (composition `worker/job_consumer`, lore-enrichment `worker/resume_consumer`, worker-ai `summary_consumer`) → `BaseTerminalConsumer` with `start_id="0"` + function→class. (2) **worker-ai `llm_extract_consumer` money-path** — serial, flag-gated, `/review-impl`, full extraction E2E. (3) **emit-wiring** (per-service `outbox_events` table + `OUTBOX_SOURCES` + `emit_job_event` — shared compose/relay → SERIAL). (4) batched live-smoke `D-JOBS-MIGRATE-LIVE-SMOKE`. Checklist (strip internal ack/retry; verify UUID job-id) in the P1 plan.
