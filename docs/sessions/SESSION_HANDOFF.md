...
**State: P0 (ai-gateway) + P1 (read tools) + P2 (propose_new_entity draft) + P3 (edit-existing frontend-propose) BUILT + verified.** Architecture frozen. Read assistant + new-entity draft + edit-existing (version-checked Apply) all wired (full-stack docker smokes deferred 067/068 + D-GLOSSARY-EDIT-LIVE-SMOKE).
...
**NEXT:** **P4 — Tier-S schema tools (kind/attr) with 2-step confirm** (`INV-9` server-minted `confirm_token` bound to user+book+payload+expiry, un-bypassable even by a direct gateway call — H8). Then P5 surfaces+prompt (system "glossary skill", per-surface tool curation H7/H11), P6 grounding (mui#3, `[]`-fallback H2). When the docker stack is up, run deferred full-stack smokes (067 + 068 + D-GLOSSARY-EDIT-LIVE-SMOKE).
