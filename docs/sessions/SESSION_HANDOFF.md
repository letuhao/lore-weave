...
>
> **▶ Deferred (this track):**
> - **D-QUALITY-DROPPED-FP — ★ HIGH (from eval).** Per-chapter Quality Report reuses v1 `audit_promises` ("dropped = not-paid-in-this-chapter") → 30 false "dropped" on serialized fiction vs 0 truly abandoned. Reframe as **"open threads raised here"** (informational) + let the book-level coverage own paid/abandoned. The #1 trust fix.
> - **D-QUALITY-HONORIFIC-PRECHECK — MED (from eval).** 15/49 self-heal props are objective xưng-hô RULE fixes but arrive un-pre-checked (`semantic` + re-ranker OFF). Quantify rerank-ON ($0 local), widen the deterministic prefilter, or default the type-routed re-ranker ON for local models.
> - **D-QUALITY-CRITIC-HEAL-LINK — MED (from eval).** The critic's 10 canon violations ≈ self-heal's 15 honorific edits (same issue, shown as diagnostic AND edit). Link them in the Polish gate.
> - **D-QUALITY-COVERAGE-VARIANCE — LOW (from eval).** Book-coverage paid↔progressing flips run-to-run (model non-determinism); stabilize later with a multi-sample vote if it matters.
