...
**▶ NEXT — debt-paydown roadmap (Wave 1+2+3-rem+4 DONE + BOTH live-smokes PASSED; WX decouple FULL-FEATURED, /review-impl-cleared, live-proven):**
- **Default-on decision (READY):** the WX decoupled path now covers entity→trio→recovery→filter→persist (full sync parity), is review-cleared AND live-proven end-to-end (recovery+filter fan-outs + the stuck-resume sweeper). Flipping the compose default `EXTRACTION_DECOUPLE_ENABLED` (+ `TRANSLATION_DECOUPLE_ENABLED`) to true is the remaining gate — the user's call. (Stack is currently ephemeral-on; a plain restart reverts to the OFF compose default.)
