...
**✅ 2b WX-T1 + WX-T2 DONE (2026-06-11, worker-ai extraction decouple foundation).** Per the design doc [`-workerai-extraction-decouple-design`](../plans/2026-06-11-llm-rearch-phase2b-workerai-extraction-decouple-design.md).
- **WX-T1 (`96bb6369`):** `extraction_jobs += provider_job_ids JSONB` (the in-flight set for the concurrent relation/event/fact fan-in) `+ resume_state JSONB + pipeline_stage TEXT` (idempotent, NULL=legacy) + worker-ai `extraction_decouple_enabled` flag (default off). Migrate-DDL 47 + worker-ai 179 green; columns applied on live `loreweave_knowledge`.
- **WX-T2 (the pure-seam SDK refactor):** split the 4 core `loreweave_extraction` extractors (entity/relation/event/fact) into pure `build_<op>_submit_kwargs(...)` + `parse_<op>_job(job, on_dropped)`, with `_extract_via_llm_client` calling them — **byte-identical** (the decoupled WX-T3 orchestrator will `submit_job` from the same `build_` + `parse_<op>_job` on the terminal event). **VERIFY: SDK extraction 267 pass** (the 3 `summarize_level` prompt-fixture failures are PRE-EXISTING, documented prior session — neither prompts nor summarize touched) **+ worker-ai 179 + knowledge extraction units 405 + provider-gate OK.**
  - **Deferred `D-WX-T2B-RECOVERY-FILTER-SEAMS`** — `entity_recovery.py` + `pass2_filter.py` (the optional recovery/filter stages, structurally different — recovery is a distinct flow, filter is per-item fan-out) still embed their `submit_and_wait`. Give them the same pure seams before WX-T3 decouples those stages.
...
**▶ NEXT — WX-T3 (extraction orchestrator), 2b-T3b (V3 verify/correct decouple), or other backlog:**
1. **2b-T3b — V3:** the `v3/orchestrator` QA loop (rule-tier verify + LLM verify → re-translate HIGH-severity blocks (keep-if-improved) → re-verify, ×max_qa_rounds, + the two-pass cold-start re-translate) layers MORE LLM ops on top of the T3a translate stage. To decouple it, each verify/correct/re-translate becomes a decouple step (extra `awaiting` kinds in a v3 resume_state that wraps T3a's block state). Currently v3 stays SYNCHRONOUS under the flag (the `pipeline_version=='v2'` gate). The consumer + `_finalize_chapter` are reused. **NOTE the relative value:** Phase 1's queue+semaphore already fixed the incident (jobs wait, don't die; cancel frees the slot) — 2b is an optimization, and the v3 QA loop is the most complex surface, so weigh T3b vs Phase 3 / other work.
2. **2b-worker-ai (HARD):** extraction decouple — needs `loreweave_extraction` per-op resume points (4 ops × N chunks); `loreweave_extraction` is shared with knowledge (the /warp TRIAGE-OUT reason). Separate-consumer rewrite in worker-ai.
Or Phase 3.
