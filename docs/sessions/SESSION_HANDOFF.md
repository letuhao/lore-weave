...
**Branch `wiki/llm-gen` (off main `2ace6272`):** M0–M6 + E2E-smoke + M7a PUSHED to origin (`e9313ef0` plan · `12956b1e` M0 · `fc969f03` M1 · `1cdf53ee` M2 · `f982f7c4` M3 · `a3505367` M4 · `6e420c30` M5 · M6 · `24a8d973` E2E · `409956bd` M7a). M7b-1 (`fbf230e7`) + M7b-2a (`4c853c43`) + M7b-2b (this) committed local — push pending (3 commits unpushed; ask before pushing).
...
1. **D-WIKI-M7B-LIVE-SMOKE** (the FE is now complete) — rebuild knowledge + glossary + frontend, then drive the whole FE→glossary→knowledge path from the UI: pick a model → generate → poll the banner → see `needs_review`/`blocked` badges + the verify-flags panel → resume/cancel → single-article **Regenerate**. First real exercise of the M7b-1 job endpoints + the 2b badges/regenerate over live provenance.
...
- **D-WIKI-M7B-LIVE-SMOKE** (M7b-1/2a/2b, cross-service) — the BE (108 pytest + go) + FE (44 vitest across 2a+2b) are unit-proven, but the full FE→glossary→knowledge round-trip over running services isn't exercised yet (M6 E2E proved the trigger path; this adds status/resume/cancel + the 2b badges/regenerate over live provenance). **Now the recommended next step** — the wiki LLM-gen FE is complete. Rebuild knowledge+glossary+frontend, then from the UI: model-pick → generate → poll → badges + verify-flags → resume/cancel → single-article Regenerate.
