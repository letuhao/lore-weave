...
>
> **▶ THIS BRANCH'S NEXT — glossary-assistant coverage campaign.** Make the assistant cover all scenarios (S1–S35). Architecture **LOCKED** (decisions **D1–D13**, 3 campaign specs: `2026-06-10-glossary-assistant-{scenario-coverage,extended-scenarios,architecture-review}.md`). **Start:** Phase -1 **E0 collaboration epic** CLARIFY, or the **H-C async-delivery spike** (independent, high-risk — do early). The full campaign block + decisions live in the **"▶ (merged from main) glossary-assistant"** section below (the deferred list there now carries `GLOSSARY-ASSISTANT COVERAGE CAMPAIGN`, `D-MCP-TASKS-MIGRATION`, `D-MCP-DIRECT-RETURN`).
>
