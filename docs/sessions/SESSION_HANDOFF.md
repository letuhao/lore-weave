...
**State: M0 + M1(a–c) + M2 + M3 + M4a + M4b DONE** (M1d, M4d deferred). M0 readiness gate + V3 scaffold; M1 rule-tier verify + targeted re-translate + romanization; M2 LLM verifier (Tier-2) + multi-round QA loop; M3 dialogue/scene-aware semantic batching; M4a knowledge read-port; **M4b relations+bios → pronoun/honorific brief into Translator+Verifier (G4)**. All PO-approved + `/review-impl`'d. Full suite **393 passed**; **parity** preserved (default `pipeline_version='v2'`).
...
**NEXT options:** (1) **M4c** — cold-start in-run proper-noun record + memo translated-only fix (clears D-TRANSL-MEMO-M4); (2) **config plumbing** (small) — wire `qa_depth`/`max_qa_rounds`/`verifier_model` settings→job→coordinator→worker; (3) **M5/M6** (living-book surfacing · human-fix flywheel).
