...
> - **E0-2 deferred rows:**
>   - **D-E0-2-LIVE-SMOKE** — real-stack collaborator flow on book-service's own endpoints (B with `edit` PATCHes a book + creates a chapter; `view`-B 403 on PATCH; non-grantee 404 on GET). book-service has no real-PG harness; deny-test proves the *denial* side, not the allow-path. Fold into the next E0 stack-up.
>   - **D-FAVORITES-METADATA-LEAK** (MED, pre-existing, /review-impl) — `addFavorite` has no access check + `listFavorites` returns title/description/genre_tags of any favorited active book → a user can read a **private** book's metadata by favoriting its id. Out of E0-2 scope (personal routes excluded; favoriting public catalog books is legit). Fix = gate `listFavorites` metadata to ≥view OR public; needs a catalog-interaction decision.
>   - **D-E0-2-IMPORT-ATTRIBUTION** (LOW) — import jobs are attributed to the book **owner** (`import_jobs.user_id`=owner) so the async worker bills the owner's quota (preserves pre-E0 billing); editor-as-author attribution for imports deferred.
> - **NEXT: E0-3 (knowledge adopt)** → E0-4 (translation/campaign/composition) · E0-5 (FE panel).
>
