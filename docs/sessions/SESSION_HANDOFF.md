...
- **▶ NEXT: H5 drag mechanics (FE)** — the last Hub milestone. Enable React-Flow drag on chapter/scene nodes; on drop, map the drop position → target lane/rank, dispatch to the right REST mirror (**chapter→lane = `/arcs/assign-chapters`** setting `structure_node_id`; **scene = `/outline/nodes/{id}/reorder`**); optimistic + refetch shell → laneLayout recomputes; **412 → reload+toast** (SceneRail recovery precedent); undo via `_meta.undo_hint`. Wire one PH20 row at a time, each with an OCC test + a live drag smoke (the H8.2 harness above is the recipe: `vite dev` on a free port + SQL-seed a book + Playwright; a real drag must show the DB `structure_node_id` change + the overlay update, spec §DoD line 264). The seed helper needs a real **Work** row (project_id→book_id) so the drawer's `getNode` resolves — SQL-seed one or drive `commit_decomposed_tree`. H7 view-modes deferred (P-10). Remaining i18n: only `en` for `plan-hub` + `planNav`; run `scripts/i18n_translate.py`.
