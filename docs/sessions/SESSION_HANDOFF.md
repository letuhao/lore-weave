...
> - **NEXT: D-E0-3 Phase 2b** (route reopen — the feature goes live): `require_project_principals` (return owner+caller); `start_extraction` OWNER→EDIT; collaborator dimension-guard 409; **+LOW-1: extend `_create_and_start_job` INSERT to persist billing** (HARD prereq — else billing dropped on the start path); set `billing_*` on the collaborator job; benchmark stays owner/project-scoped; **+≥3-service live-smoke** (collaborator extraction billed to their key, stored under project model, searchable by owner). → **E0-4c** (composition) → **E0-4b** (campaign) → E0-5 (FE panel). Residual caller-pays rows: `D-E0-3-SEARCH-QUERY-CALLER-PAYS`, `D-E0-PASSAGE-EVENT-CALLER-PAYS`, `D-E0-ENTITY-BACKFILL-CALLER-PAYS`.
>
