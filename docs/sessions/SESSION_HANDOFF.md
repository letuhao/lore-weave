...
> - **⏳ M3 NEXT:** coverage rollup (per-chapter dirty/translated counts for the matrix) + **TranslationTab matrix drill-down FE** (segment-level status + a "re-translate changed" action calling `retranslate-dirty`) + glossary-staleness per-segment + fix `TranslationTab` `limit:200`→100 clamp consumer.
