...
>   - **▶▶ NEXT — P4-REDESIGN M2: producers assemble + emit whitelisted params/model/cost/tokens** (knowledge extraction · translation · composition · video-gen · lore-enrichment · campaign · glossary-translate) at their status chokepoints + reconcile `/internal/*/jobs` sources emit them too. **≥2-service milestone → live-smoke required.** Then M3 (FE rewrite to the mockup) + M4 (Playwright smoke).
