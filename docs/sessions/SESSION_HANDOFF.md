# Session Handoff — Session 106 (S4 design + S4a threading + S4b usage outbox/relay)
...
> **Date:** 2026-06-09 (Auto-Draft Factory **S1 + S2 + S3a/b/c + S4 design + S4a threading + S4b outbox/relay** shipped; human-in-loop v2.2).
...
- **`D-S4B-S4C-DEPLOY-PAIR`** (⚠️ hard pre-prod) — since jobs-path `RecordUsage` was REMOVED, **S4c (usage-billing stream consumer) MUST ship before/with S4b in prod**, else batch-job quota/credit billing pauses (the `:usage` stream accumulates unconsumed). Dev is unaffected (no consumer either way).
...
**▶ NEXT: S4c — usage-billing stream consumer + real-cost deduction** (Go). Add a Redis-stream consumer on `loreweave:events:usage` (group `usage-biller`) → the existing `recordInvocation` deduction logic refactored into a shared core that takes the event's real `cost_usd` (REMOVE the flat $0.000002/token at `server.go:258`), idempotent on `request_id` (existing `usage_logs` UNIQUE + deferred-deduction). Keep the `/record` HTTP endpoint for the streaming path. **Resolve O-1 (token-vs-USD `account_balances` column semantics — how `cost_usd` maps to the credits leg without breaking the token-quota window) + O-2 (no Go Redis-consumer precedent in usage-billing yet — write a minimal `xreadgroup` loop) BEFORE building** (design doc §5.3 + Open items). Then **S4d** (campaign `spent_usd` accumulation + reactive cap-pause; dedup `campaign_usage_seen`) → **S5/S6** (wizard + monitor FE). **AMAW recommended for S4c–d** (money path, schema, multi-system contract). The relay's wire-field contract is frozen in the S4b design doc §7.
