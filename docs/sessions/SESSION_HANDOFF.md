...
- **`D-FACTORY-EST-PROVIDER-KIND`** (#5, LOW) — the cloud/local provider badge on the estimate screen needs provider-registry to echo `provider_kind` in the estimate response (cross-service; per-stage USD already signals $0-local).
- **`D-FACTORY-INFLIGHT-PANEL`** (#4, LOW) — a live "processing" panel + recent activity log; redundant with the now-paginated attention table + G3 in-progress stat, and a meaningful version needs per-chapter sub-step state (batch/verify/backoff) we don't project + a per-chapter activity event feed.
- **`D-FACTORY-SWITCH-MODEL-RESUME`** (#7, deferred) — re-pick translation/knowledge model on resume (model mutation on a live campaign; workaround = cancel + new campaign).
