...
**▶ NEXT — finish the M8 flywheel (user directive: build+test fully, default-OFF operationally):**
1. **D-WIKI-M8-EVAL-PLUS Phase 2 (auto-sampled judge)** — sample `wiki.generated` events (a rate config, like the existing `online_eval` sampler) → judge via the Phase-1 core. Needs the article content (fetch from glossary, or enrich the event). The on-demand half + the judge core are done; this adds the continuous-but-controlled mode.
2. **D-WIKI-M8-FEWSHOT** — inject gold AI→human pairs as few-shot exemplars into wiki generation (`prompt.py` `exemplars` param + a gold-pair fetcher from glossary `wiki_revisions` by `article_id` + thread through `generate.py`/orchestrator), gated `WIKI_FEWSHOT_ENABLED` (default OFF). Cross-service (knowledge ← glossary).
3. **D-PLATFORM-SYSTEM-CONFIG-UI** (new epic, NOT wiki-specific) — there is **no admin/system-config surface today** (no admin role/`is_admin`, no feature_flags table; all ops flags are env-only + a per-user Settings page only). A UI to toggle ops flags needs: admin auth (`is_admin` + `require_admin`) + a `feature_flags`/`system_config` store + PATCH endpoints + an `/admin` FE page. Reusable for ALL flags (wiki-scoring/few-shot/translation-judge/precision-filter/wiki-gen). Build after the code halves.
