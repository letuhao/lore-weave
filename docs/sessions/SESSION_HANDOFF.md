...
**▶ NEXT — wiki FE gap-closure essentially DONE (W1–W4 shipped; W5 PO-gated/deferred):** per [`2026-06-11-wiki-fe-gap-closure.md`](../plans/2026-06-11-wiki-fe-gap-closure.md): **W5** (per-step prose/verify model) is **DEFERRED 076 `D-WIKI-PER-STEP-MODEL`** — a never-built design idea, build only if the PO wants it. Remaining wiki follow-ups are the live-smokes (incl. **`D-WIKI-W4A-LIVE-SMOKE`** + the **`D-WIKI-W4-RESULTS-64KB`** proxy-cap fix). Otherwise the next epic is the platform system-config UI below, once its auth-conflict clears.
