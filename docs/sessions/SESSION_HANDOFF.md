...
> - **NEXT: D-E0-3 Phase 2a-2** (thread billing through the 3 embedder sites: `persist-pass2`→`passage_ingester`; billing fields on `summary.*` msg→`summary_processor`; `entity_embedder` — all storage tags stay the project's) → **2b** (route reopen: `require_project_principals`, OWNER→EDIT, dimension-guard 409, +LOW-1 INSERT fix, +≥3-svc live-smoke) → **E0-4c** (composition) → **E0-4b** (campaign) → E0-5 (FE panel). Residual: `D-E0-3-SEARCH-QUERY-CALLER-PAYS`.
>
