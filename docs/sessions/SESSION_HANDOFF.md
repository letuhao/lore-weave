...
>   - **▶ M3 NEXT** — projection summary parent/child undercount (`count_summary` filters `parent_job_id IS NULL`; `store.py:376`) + real-PG test; FE overlay terminal-eviction (`JobsStreamProvider` Map only `.set()`s). Then M4 (retry=re-submit end-to-end).
