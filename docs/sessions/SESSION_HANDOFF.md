...
**W11-M2 SHIPPED — reader ask-the-lore facade, 2026-07-11.** Four Tier-R MCP tools (`lore_ask`/`lore_browse_entities`/`lore_entity`/`lore_timeline`) in `app/tools/reader_tools.py`, wired across all three schema sources (shim + ARG_MODELS + TOOL_DEFINITIONS; drift test 32→36). The spoiler cutoff is SERVER-enforced from the reader's OWN furthest-read chapter (there is no `before_chapter` arg), all reads resolve-to-owner after a `book_id≥VIEW` grant, fail-closed everywhere. **Adversarial 5-dim facade review (refute-verified) → 4 CONFIRMED (2 HIGH, 1 MED, 1 LOW), all fixed:** (1) **HIGH** glossary-id vs KG-id mismatch made `lore_entity` return empty for EVERY entity (browse→inspect silently dead) → `resolve_kg_entity_id_by_glossary_id`, project-scoped; (2) **HIGH** `kind` filter read `kind`/`entity_kind` but the glossary row serializes `kind_code` → fixed; (3) **MED** glossary default `recency_window=100` dropped long-absent entities from the "whole cast" → `recency_window=0` (spoiler cutoff only); (4) **LOW** `list_facts_for_entity` wasn't project-scoped (cross-book fact exposure to a VIEW-grantee via an out-of-band id) → added `project_id`; plus a `book_client`-None guard mirroring `story_search`. **Verify:** knowledge **3772 passed** (9 reader tests: fail-closed null→empty · anti-oracle · resolve-to-owner · glossary `+1` inclusion · `kind_code` · `recency_window=0` · project-scoped facts · no-KG-anchor honest empty). **▶ NEXT: W11-M3** — public/anonymous canon-only windowed lore route (unlisted-token gated, self-declared cutoff). **Highest-risk line DE-RISKED 2026-07-11** (spec §4.5 "VERIFIED MECHANISM"): canon-only holds today only IMPLICITLY (WS-4C AI-suggested entities are `status='draft'` with NO `chapter_entity_links`, so `min_frequency≥1` drops them; `known-entities` applies no status filter by default). **M3 MANDATE:** pass an EXPLICIT `status='active'` filter to `known-entities` (never rely on the implicit link-absence on a public surface) + keep `min_frequency≥1`; red-first test: a `draft`+`ai-suggested` entity WITH a chapter link is EXCLUDED. Leanest M3 = glossary canon-cast only (no KG facts → sidesteps review-status). Build in sharing-service (owns the unlisted-token public edge, `server.go:108-110`) calling glossary's internal `known-entities`; bad/non-public token → 404 (anti-oracle). Mandatory `/review-impl`. Then W10-M1 → W10-M2. (M2 groundwork: `ee3548773`/`e72c828c6`.)
