...
**NEXT = Q4 (L) online-eval consumer:**
- New `eval-runner` Redis consumer group sampling `knowledge.extraction_run_completed` (5-10%); **structural-only deterministic re-score default** (no source text); LLM judge only for `save_raw_extraction` opt-in projects; **host-orchestrate the judge via provider-registry** (NOT JIT-load a 26B in-container); **sampled-out events XACK immediately** (no PEL churn); sorted-set paced queue (Redis Streams lack time-based delivery); throughput budget/backpressure; `online_eval_rule` table. Call `panel_safety` + (when verdicts exist) `calibrate_judge` to gate trust. Persist results via Q1 `DbSink` (+ add `panel_safe` to eval_runs).
- Deferred: **D-Q3-CHAT-FEEDBACK-E2E** (full POST→outbox→relay→learning live-smoke — needs `docker compose build chat-service learning-service`).
