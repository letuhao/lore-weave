...
> - **▶ NEXT = G2c** — **attributes** (system read `GET /system-attributes?kind_id=&genre_id=`, user CRUD `/user-attributes` attach-by-code: kind_id/genre_id must be the caller's user-tier rows) + **kind↔genre links** (`/user-kinds/{id}/genres` list/PUT-set/PUT-one/DELETE) + their tenancy tests → commit G2c. Then **G3** book adopt (pick-list copy-down Moment A + book-local read Moment B) → **G4** entity repoint + extraction/translation retarget + the DESTRUCTIVE drops/reset (gated) → **G5** sync (content_hash diff/apply) → **G6** FE (~39-file rebuild from the 5 drafts).
