# Session Handoff — `feat/auto-draft-factory-gaps` · HEAD `7815acc2` · 2026-06-15

> **🔨 CREATION-UNBLOCK RAID COMPLETION (design [`docs/specs/2026-06-15-creation-unblock-completion-design.md`](../specs/2026-06-15-creation-unblock-completion-design.md), gap analysis [`…-raid-gap-analysis.md`](../specs/2026-06-15-creation-unblock-raid-gap-analysis.md)). 6 slices W1–W6. BE W1–W3 ✅ + reviewed; FE W4 ✅; W5–W6 ⏳ NEXT.**
...
> - **✅ W4 — pickers (G2):** shared `ProjectPicker` + `WorldPicker` (`components/shared/`, mirror BookPicker — load-once + client filter, emit id, empty=valid, optional inline `onCreateNew`, archived/unlisted chip resolved by id). Chat `SessionSettingsPanel` raw project `<select>` → `ProjectPicker` (dropped its `useProjects`/archived-placeholder branch). VERIFY: vitest 13 picker + 152 chat/shared suite green; tsc 0; eslint 0.
> - **⏳ W5 NEXT (M) — world-workspace populate CTAs (G1: add/attach book + create what-if) + render the W2 rollup graph (G4 FE, replaces bible-only WorldGraphSection).** This slice also lands the **W2 non-empty rollup live proof** (it rebuilds a graph + renders the rollup) and G5. **W6** after: cross-linking (G3) + onboarding funnel close.
...
> - **▶ NEXT (P1 — the stack-dependent tail; 11/12 consumers done):** (1) **money-path `worker-ai/llm_extract_consumer.py`** — the LAST + riskiest consumer (~624-line multi-stage SM: entity→trio→recovery→filter, FOR-UPDATE races, double-spend guards). Same transport-swap recipe (extract `_handle`→`handle`, `_sweep_once`→`sweep_once`, drop the module transport) BUT do it **flag-gated** (keep `consume_llm_terminal_stream` as the fallback; a flag in main.py default→OLD until live-smoked) + **`/review-impl`** + **full extraction E2E on the live stack** (rebuild worker-ai, run a real decoupled extraction, confirm entity→trio→persist + no double-spend). (2) **emit-wiring** (per-service `outbox_events` table + `OUTBOX_SOURCES` + `emit_job_event` at status transitions — shared compose/relay → SERIAL; needs stack to live-smoke). (3) batched live-smoke `D-JOBS-MIGRATE-LIVE-SMOKE` (rebuild ~12 images, confirm every group boots). **All three need the Docker stack up.** ⛔ campaign `spend_consumer` stays excluded (`D-JOBS-SPEND-CONSUMER-MISFIT`).
> - **Commits (all pushed, origin `a9c0c211`):** SDK `c9dfb49e` · video-gen `2c457841` · Family-1 `754e9e10` + review-impl `1bec15cd` · BaseProjectionConsumer `148a18de` · knowledge `5a672324` · Family-2 `43be3c1e` · Family-3 (composition+lore-enrichment) `89eecfde` · summary `a9c0c211`. **~−1,800 lines of duplicated Redis transport eliminated; 11/12 consumers on the two shared bases.**
