...
> - **NEXT: E0-4c** (composition — L/XL, 9 routers, work-anchored authz) → **E0-4b** (campaign) → **E0-5** (FE collaborators panel). **Residual caller-pays deferred rows** (NOT extraction-triggered — separate surfaces): `D-E0-3-SEARCH-QUERY-CALLER-PAYS` (raw_search/drawers query-embeddings, view-tier), `D-E0-PASSAGE-EVENT-CALLER-PAYS` (passage_ingester via chapter.updated events), `D-E0-ENTITY-BACKFILL-CALLER-PAYS` (embed-entities-backfill, externally triggered).
>
