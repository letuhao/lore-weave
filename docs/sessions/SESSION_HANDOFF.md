...
- **Verify:** book-service API suite green + 4 reading-position DB tests PASS live (`BOOK_TEST_DATABASE_URL`→:5555); knowledge **3750 passed** (+5). **▶ NEXT: W11-M2** — reader ask-the-lore facade (cutoff SERVER-enforced from `reading-position`, never an LLM arg) + `lore_*` MCP tools + KG grant-access via the facade (resolve-to-owner, §4.4 refined into M2). The M1 null-position→fail-closed *interpretation* test lives in M2.
