...
> - **▶ NEXT (P1 cont., in order):** (1) **worker-ai `llm_extract_consumer` money-path** — serial, flag-gated, `/review-impl`, full extraction E2E. (2) **design+build `BaseProjectionConsumer`** + migrate Families 2 & 3. (3) **emit-wiring** (per-service `outbox_events` table + `OUTBOX_SOURCES` relay entry + `emit_job_event` at status transitions — shared compose/relay files → SERIAL). (4) batched live-smoke `D-JOBS-MIGRATE-LIVE-SMOKE` (rebuild migrated images, confirm every group boots). Checklist (strip internal ack/retry; verify UUID job-id) in the P1 plan.
