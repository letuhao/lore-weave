...
> - **`D-GLOSSARY-LIST-P1-FE`** — pagination UI + page-size + debounced search + sort-UI (existing updated_at) + extract `<EntityListBrowser>` (GlossaryTab). FE-only, no RAID risk, buildable now.
> - **`D-GLOSSARY-SORT-BE`** — extend `sort` whitelist; **sort name via denormalized `e.cached_name`** (NOT an eav aggregation); ship name/kind/status/recency/alive first, DEFER counts-sort (correlated subqueries → expensive at 20K). Edits glossary-service `entity_handler.go` (RAID-coordinated).
> - **`D-GLOSSARY-RAW-SEARCH-BE`** — `search_mode=raw` lexical leg (ILIKE-exact + trigram) over the **denormalized `cached_name` + `cached_aliases`** (entity row, no aggregation) + `match` snippet/highlights + pg_trgm GIN on `cached_name` (+ expr index on `array_to_string(cached_aliases,' ')`). NB: existing `search_vector` FTS is `'simple'` → can't substring CJK, so trigram is required. RAID-coordinated.
