...
**Skill-authoring contract + all 5 domain skills (Part A + Part B Phase 1+2) — SHIPPED, adversarially fact-checked, 2026-07-07** (branch `feat/context-budget-law`, HEAD `pending`, spec [`docs/specs/2026-07-07-skill-authoring-and-mcp-exposure-standard.md`](../specs/2026-07-07-skill-authoring-and-mcp-exposure-standard.md)). Direct follow-on to the MCP tool-calling fix below — user's framing: tool count will keep growing, and skills need to be genuine "workflow definitions + tool-use guides," not bare "here's MCP, go find_tools yourself." This entry supersedes the prior Part A+B(Phase1) entry — same effort, continued same session via fan-out.

- **Part A — the skill-authoring contract.** `SkillDef` gained `hot_domains: frozenset[str]`; a permanent lint (`test_skill_registry.py::TestSkillClaimsLint`) scans every skill's prose for real catalog tool names and fails if the tool's domain isn't declared hot (or the tool is CAT-4-legacy-tagged). Building it found 3 pre-existing real bugs (knowledge_skill's undeclared claim, 7 stale glossary tool refs, the `kg_*`/`memory_*`→`"knowledge"` `GROUP_DIRECTORY` alias gap, fixed via `_DOMAIN_ALIASES`/`_domain_of()`).
- **Part B Phase 1 — composition + translation.** `composition_skill.py` (~56 tools) + `translation_skill.py` (12 tools). Surfaced + fixed a pure-studio skill-visibility gap (`studio: bool` threaded through `_surface_key`/`resolve_skills_to_inject`) and generalized `tool_surface.py`'s curated hot-domain union from 2 hand-authored cases into one loop over pinned skills (with `covered_domains` tracking so no domain is ever double-budgeted).
- **Part B Phase 2 (this pass) — book + settings + jobs, all 3 built via 2 parallel fan-out agents + 1 direct.** `book_skill.py` (21 tools, book-service Go), `settings_skill.py` (12 tools, provider-registry-service Go — BYOK provider/model registry, not literal "settings"), `jobs_skill.py` (5 tools, jobs-service Python). All 3 are **curated-pin-only** (never auto-injected — zero default-turn token-cost change on any surface); `book` visible on book/editor/studio, `settings`/`jobs` also visible on plain chat (account-level, no book needed). The existing generic curated hot-domain union (built in Phase 1) covered all 3 with zero new per-skill wiring — only 2 lint-allowlist additions needed (legitimate cross-domain contrastive mentions: `book_skill`→`story_search`, `jobs_skill`→`translation_job_control`/`translation_job_status`) + one test bound bump (L1 metadata line-count 6→9, expected as more skills become visible).
- **`/review-impl`-equivalent fact-check, ALL PHASES:** Phase 1's 2 cold-start sub-agents found **8 findings** (5 content bugs — wrong motif-archive-no-restore claim, translation_coverage false glossary-staleness claim, translation_list_versions false full-body claim, translation_job_control cancel/pause reversibility conflation, extraction reasoning-effort clamp falsely promised reported-back; 1 architecture bug — curated union gave each pinned skill its own token budget instead of one shared ceiling; 1 cosmetic FE domain-badge; 1 doc-gap), all fixed and independently re-verified. Phase 2's 3 parallel fact-check agents (one per new skill) found: `jobs_skill.py` and `settings_skill.py` **clean** (14 load-bearing claims verified line-by-line across both, zero bugs); `book_skill.py` had **1 WRONG + 2 IMPRECISE**, all fixed — (WRONG) the skill claimed `book_get_chapter` returns a `draft_version` an agent could read before its first save; no `book_*` read tool actually exposes it (only a prior save/restore response does, and a stale-version 409 doesn't reveal the correct value either — a genuine chat-tool dead end for any chapter not freshly created this turn) — fixed with the one safe deterministic case (freshly-created chapter always starts at `base_version=1`) and an explicit "tell the user to open the editor" fallback; (IMPRECISE ×2) "trash is recoverable" didn't mention recovery itself has no MCP tool (Studio-UI-only, now a 3rd "what you cannot do" item), and bulk-create's filename-dedup only scans active (not trashed) chapters.
- **§3.3 Part C measurement (informational, not a build):** queried `loreweave_auth.mcp_api_keys` — 5 active keys, bimodal (3 tiny `{read, domain:book}`→5 tools; 2 full-scope `{all tiers, all domains}`→all 161 `TOOL_POLICY` tools, almost certainly the external-agent keys from the original bug report). Too sparse to derive a precise threshold (any value ~6–160 classifies all 5 keys identically today) — flagged for a PO call at Part C build time, not guessed.
- **VERIFY (final, this pass):** chat-service 1150/1150 (full suite, unchanged after the book_skill fact-check fixes — prose-only). ai-gateway untouched (skills are chat-service-only, spec §8b.4) — not rerun, no files changed there.
- **Deferred:**
  - `D-SKILL-HOTDOMAIN-RUNTIME-WIRING` (gate #2, large/structural) — `knowledge_skill.hot_domains={"knowledge"}` states real intent but the runtime doesn't fulfill it on any surface yet; needs Part D (fold the hand-authored hot-domain constants into a skill-registry-derived lookup) + its own budget-verified rollout (knowledge auto-injects on every surface incl. universal chat, no small permission_mode-style patch like plan's).
  - `D-SKILL-LINT-LIVE-CATALOG` (gate #4→buildable, spec §8b.2) — the lint can't catch a stale/typo'd tool-name-shaped token matching NO real catalog entry (only catches real-but-wrong-domain + known-legacy). Buildable via a snapshot-harness mirroring `contracts/mcp-response-shapes/*.json`'s pattern — not yet built.
  - `mcp-public-gateway` Part C (scope-size-adaptive exposure) — the §3.3 measurement above gives real (if sparse) data; still needs a PO threshold call + the `scopeToolCount()` implementation before it can build.
  - Part D (fold `_BOOK_SCOPED_HOT_DOMAINS`/`_STUDIO_HOT_DOMAINS`/`PLAN_HOT_DOMAINS` into one skill-registry-derived lookup) — spec §8b.9 flags this as behavior-sensitive, needs a full before/after regression pass across every existing hot-seed-asserting test before it's "done," not a quick refactor.
  - Part E (skill eval/quality-gate loop, `scripts/eval/run_quality_gate.py` reuse) — deferred per user's explicit Phase-1 choice ("viết skill trước và đánh giá" — write skills first, evaluate after); all 5 domain skills now exist, so this is the natural next slice if picked up.
  - **Recently cleared this pass:** composition/translation Phase 2 (book/settings/jobs) — was listed as future work, now shipped above.
