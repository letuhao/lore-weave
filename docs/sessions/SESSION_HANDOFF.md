...
**▶ NEXT — M8 flywheel COMPLETE; remaining user-directed item:**
1. **D-PLATFORM-SYSTEM-CONFIG-UI** (new epic, NOT wiki-specific) — there is **no admin/system-config surface today** (no admin role/`is_admin`, no feature_flags table; all ops flags are env-only + a per-user Settings page only). A UI to toggle ops flags needs: admin auth (`is_admin` + `require_admin`) + a `feature_flags`/`system_config` store + PATCH endpoints + an `/admin` FE page. Reusable for ALL flags (wiki-scoring/few-shot/translation-judge/precision-filter/wiki-gen). This is what surfaces the M8 flags (`wiki_fewshot_enabled`, `wiki_llm_judge_*`) to operators without an env redeploy.
