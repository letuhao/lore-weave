...
**Pushed:** branch `feat/translation-pipeline-v3` on origin (budget fix `009b8d18` pushed). **`origin/main` merged in 2026-06-07** (`d0fbd404`; PR #20 glossary/ai-pipeline-v2 + #21 lore-enrichment/foundation — 29 commits, 0 conflicts; 434 py + FE translation green post-merge). **D-TRANSL-EXTRASYSTEM-BUDGET CLEARED**. **M1d (`bab368b7`) + M4d-1 (`3e6f7554`) pushed.** M4d-2a bilingual-extractor commit is local (not yet pushed).

**NEXT options:** (1) **push** the M4d-2a commit · (2) **M4d-2b** target writeback (cross-service) → **M4d-2c** orchestrator 2-pass — finish the 2-pass loop · (3) **M6 full-propagate** · (4) clear live-smokes (M4d1/M5c/M6a) on a rebuilt stack · (5) open the PR. V3 feature-complete; M4d-2 + live-smokes + full-propagate are the open refinements.
...
- **D-TRANSL-M5C-LIVE-SMOKE** (M5c-1) — full chain glossary edit → outbox → relay → `loreweave:events:glossary` → translation consumer → `is_glossary_stale` flip is NOT live-verified (needs glossary+relay+redis+translation co-running + a rebuilt translation image). Consumer mirrors a proven loop; handler/parse/group-offset unit-covered. Smoke when the stack is up.
- **D-TRANSL-M6A-LIVE-SMOKE** (M6a) — verify the full flywheel live: confirm a name in the review UI → glossary translation set `confidence='verified'` → **check `glossary.entity_updated` actually fires on a translation-confidence PATCH** (M5c trigger; M5c probe only confirmed entity create/PATCH + extract-entities emit it — a translation sub-resource PATCH is unverified) → M5c flags stale → re-translate. If the event does NOT fire on translation PATCH, M6a must also touch the entity (or glossary-service must emit on translation change).
- **D-TRANSL-M6A-NOTES** (M6a /review-impl, LOW, accepted) — (a) `targetLang` ("vi") vs glossary `language_code` subtag drift could create a duplicate translation instead of patching; (b) dialog inputs not cleared on cancel/reopen (minor UX); (c) full propagate (term→chapter index + auto "affected chapters" re-translate) is the deferred M6 remainder — today the loop is coarse via M5c book-level staleness.
...
- **D-TRANSL-M5C-COARSE-LANG** (M5c-1 /review-impl, LOW) — staleness flags ALL of a book's translations across every target language on any glossary change (the event/entity is language-agnostic; no glossary-term→chunk map). Refine to per-language when the event carries `target_language` or a term→chapter index exists.
...
- **D-TRANSL-M4D-2B** (M4d-2) — **BLOCKED on D-GLOSSARY-VERSIONING (PO 2026-06-07).** Target-translation writeback: extend glossary `extract-entities` (`extractedEntity`) with an optional per-entity target `{language_code, value}` → write an `attribute_translations` row at `confidence='machine'`; translation sends `NamePair` rows (M4d-2a) with targets + `default_tags=['ai-suggested']`. **Held because** the writeback overwrites `attribute_translations`, which has **NO version history** (UNIQUE row per (attr_value, language); `trig_trans_snapshot` is a current-state cache, not history) — a machine overwrite is destructive with no recovery. PO: do not ship destructive glossary writes until versioning exists. Resume after D-GLOSSARY-VERSIONING. (No code written; CLARIFY/DESIGN done, gate abandoned at build.)
- **D-TRANSL-M4D-2C** (M4d-2) — **BLOCKED on 2B.** orchestrator 2-pass + `cold_start_mode` config; needs 2b's writeback. (M4d-2a bilingual extractor shipped 2026-06-07 — standalone, no consumer until 2b/2c.)
- **D-GLOSSARY-VERSIONING** (NEW, PO 2026-06-07 — **HIGH / recovery-critical**) — glossary entity data (names/translations/descriptions) is destructive on edit/delete with **no history/undo** (unlike chapter translations, which version). **Spec:** [`docs/specs/2026-06-07-glossary-entity-versioning.md`](../specs/2026-06-07-glossary-entity-versioning.md). **Design LOCKED (PO 2026-06-07):** whole-entity snapshot into append-only `entity_revisions`, captured **ASYNC as a projection off the `glossary.entity_updated` outbox stream** (zero hot-path cost — NOT a sync trigger) · **actor-granularity** (human edits always; machine/bulk skip/rolling-N — reproducible + high-volume) · monthly partition + cold-archival for scale · whole-entity **restore** (reconcile-apply, id-preserving). Scale analysis: history = a projection, not a write tax; delta-log rejected (trades affordable storage for unaffordable replay-compute). **Sliced VG-1 (table+projection consumer) / VG-2 (history+restore API) / VG-3 (FE history+restore).** Unblocks M4d-2b/2c (2b's pipeline overwrites are exactly the machine-volume case granularity throttles).
  ...
  - **VG-3 (next, FE):** history panel + restore button in the glossary entity editor (`useEntityRevisions`/`useRestoreEntity` + `EntityHistoryPanel` + i18n + vitest). After VG-3, **revisit D-TRANSL-M4D-2B** (machine writeback now safe — recoverable).
- **D-TRANSL-M4D1-LIVE-SMOKE** (M4d-1) — the full chain (translation worker → `POST /internal/knowledge/timeline` → `knowledge_projects` resolve → `list_events_filtered` → memo in the Translator prompt) is NOT live-verified (needs a rebuilt knowledge-service image co-running with postgres+neo4j). Endpoint + window math + client + wiring are unit-covered; the query layer is verbatim-reused from prod-proven `handlers.py` + the C10 router. Also pins the `chapter_worker` `sort_order`→msg one-liner. Smoke with the M5c/M6a smokes on a rebuilt stack.
