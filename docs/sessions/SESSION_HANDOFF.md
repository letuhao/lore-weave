# Session Handoff — `feat/glossary-assistant-coverage` · glossary GENRE·KIND·ATTRIBUTE re-arch — design locked + G1 built · 2026-06-19
...
> - **▶ NEXT (non-destructive, before the G4 block):** **G3b** book-tier CRUD (`D-GKA-BOOK-CRUD` — create/patch/delete book genres/kinds/attributes + set active-genres + the kind×genre matrix read) + **`D-GKA-ADOPT-USER-TIER`** (G3a adopts from SYSTEM only; add the User-tier union so a book owner's user overrides shadow system by code at adopt — D7). Then the **GATED hard-stop = G4**: entity repoint (glossary_entities.kind_id→book_kinds, eav.attr_def_id→book_attributes) + entity_genres + snapshot-fn rewrite + extraction/translation retarget + the DESTRUCTIVE drops (`genre_tags[]`/`genre_groups`/`system_kind_attributes`) + wiki-truncate/KG-clear (`D-GKA-G4-CONSUMER-RETARGET`). Then **G5** sync (content_hash diff/apply; D-GKA-HASH-REFRESH) → **G6** FE (~39-file rebuild).
> - **Deferred (G-epic, opened this session):** `D-GKA-VOCAB-DROP` (MED — seed represents only the canonical genre vocab; exotic runtime genre_tags dropped in the transient G1→G4 window; moot under R2 full-reset, documented in-code). `D-GKA-HASH-REFRESH` (MED — the G2 admin-edit path must recompute `content_hash` or G5 Sync misses system-side edits). `D-GKA-USERBOOK-COVERAGE` (LOW — user_*/book_* insert/constraint coverage lands with G2 CRUD). `D-GKA-G4-CONSUMER-RETARGET` (the drops: `genre_tags[]`/`genre_groups`/`system_kind_attributes` + entity-FK repoint + snapshot-fn rewrite + extraction/translation retarget + wiki-truncate/KG-clear — all G4). Bulk-merge `createKindAlias` retarget + per-book KG `relation` level still deferred from SS-4.
