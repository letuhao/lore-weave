...
- **▶ NEXT: H5 Row-3 + the gesture live-confirm + polish.** (1) **Human/manual confirm** the three drag GESTURES end-to-end (chapter→lane, scene→chapter, arc-band→band): open `plan-hub`, expand arcs, drag, watch the DB flip. Every LINK is proven (hit-tests + handlers unit-tested; all three mutations + the 412 + the cycle-4xx live-proven; the node accepts a real drag-start) — only the composed d3-drag gesture resists automation (trusted pointer events + `pointer-events-none` bands). (2) **Row-3** chapter reorder WITHIN a lane — ⚠ **needs its own semantics, NOT a straight `reorder` call**: that route groups siblings by `parent_id`, but post-lift chapters have `parent_id NULL` and bind by `structure_node_id`, and the Hub's x-axis is global `story_order` (not rank) — so "reorder a chapter in its lane" really means restacking `story_order`. **Scope before building.** (3) Optimistic re-place (avoid the brief refetch snap) + undo via `_meta.undo_hint`. **Live-smoke seed recipe (proven):** insert a `composition_work` (project_id→book_id, created_by=test user) FIRST, else every by-id route (`getNode`, `reorder`) 404s on the grant resolve. H7 view-modes deferred (P-10). Remaining i18n: only `en` for `plan-hub` + `planNav`; run `scripts/i18n_translate.py`.
