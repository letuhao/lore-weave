...
**PLAN HUB v2 (spec 24, Stage 7) — Phases H1 (read surfaces) + H2 (canvas core) SHIPPED, 2026-07-11** (branch `feat/context-budget-law`; each built via a parallel fan-out + serial integrate/verify). The Hub is a real, openable panel rendering the package structure on a React Flow canvas; H3/H4/H5/H6 are next.
...
- **H2 conscious carry-forwards (small, folded into the next increment):** (1) **node-content titles** — `NodePosition` (laneLayout output) carries no title/status, so expanded chapter/scene CARDS show a `Ch N`/`Sc N` placeholder + union dot (arc titles DO show, via the lane-band labels). Thread a `nodeContent: Record<id,{title,status,tension,kind,chapter_id}>` through PlanHubView→PlanCanvasProps (usePlanWindows keeps the raw `SummaryNode`; usePlanHub merges it with arc titles) — folds naturally into H4's node-card work. (2) **H2.6 bus** (selection publish + editor active-chapter subscribe) + **camera focus (OQ-5)** both need the node→`chapter_id` map, so they ride with (1). (3) **multi-locale i18n** — only `en` added; other locales fall back until `scripts/i18n_translate.py` runs.
- **▶ NEXT:** thread node-content (1)+(2) [small], then **fan out H3 (drawer) ∥ H4 (decorations) ∥ H5 (interactions) ∥ H6 (rail)** — 24's own declared W5 disjoint-file fanout; H7 deferred (P-10); then **H8** (10k-fixture perf/EXPLAIN + the H8.2 live browser smoke — the pillar DoD, NOT yet run: rebuild FE + composition images first, `live-smoke-rebuild-stale-images-first`). Reuse targets for H3/H4 (DOCK-2, avoid forks): `ArcTimelineEditor`, `MotifDetailDrawer`, `ChapterMotifBindings`, the 23-C3 arc-inspector, `resolveStudioLink`.
