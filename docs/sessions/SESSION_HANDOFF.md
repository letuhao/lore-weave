...
- **End-to-end live-smoke = `D-WX-RECOVERY-FILTER-LIVE-SMOKE`** (extend `D-WX-LIVE-SMOKE` to a project with `extraction_config` recovery/filter — needs the stack + a configured project; not yet run).

**▶ NEXT — debt-paydown roadmap (Wave 1+2+3-rem+4 DONE; WX decouple now FULL-FEATURED + /review-impl-cleared):**
- **Default-on decision:** the WX decoupled path now covers entity→trio→recovery→filter→persist (full parity with sync) + is review-cleared. Flip the compose default `EXTRACTION_DECOUPLE_ENABLED` to true when ready (still ephemeral-on in the running stack). Suggest a recovery/filter live-smoke (`D-WX-RECOVERY-FILTER-LIVE-SMOKE`) before the flip.
