...
- **D-TRANSL-M4D-2C** (M4d-2, **last piece of the 2-pass loop**) — orchestrator: cold-start (empty glossary) ∧ `cold_start_mode='two_pass'` ∧ recurring pairs → pass1 translate → `extract_name_pairs` (2a) → `writeback_name_pairs` (2b) → re-fetch glossary → **pass2 re-translate**; + `cold_start_mode` plumbed settings→job→coordinator→worker (mirror `qa_depth`). Guard the 2× cost (skip when no recurring names) + the M4d-2a input token budget (review-impl LOW). 2a + 2b both shipped 2026-06-07 — only the wiring remains.
