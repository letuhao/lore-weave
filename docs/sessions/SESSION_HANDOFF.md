...
**▶ NEXT — wiki FE gap-closure W4b (W5 PO-gated):** per [`2026-06-11-wiki-fe-gap-closure.md`](../plans/2026-06-11-wiki-fe-gap-closure.md): **W4b** (FE) the screen-③ per-entity results table fed by `results` — ✓/⚠/⊗/⏳/queued rows + citation/flag counts + the live 4-pass indicator off `current_pass`; files `WikiGenJobBanner.tsx`/new `WikiGenJobDetail.tsx` · `useWikiGenJob.ts` (thread `results`/`current_*`) · types · i18n ×4 · vitest. (W4a BE is done + flows through the verbatim proxy.) Then the platform epic below once its conflict clears.
