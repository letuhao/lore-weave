...
**6 commits LOCAL on `wiki/llm-gen` (off main `2ace6272`), NOT pushed:** `e9313ef0` plan+BookProfile-A · `12956b1e` M0 · `fc969f03` M1 · `1cdf53ee` M2 · `f982f7c4` M3 · (this) M4 verify+cite+revise.

**▶ NEXT SESSION = `/loom` M5 — glossary writeback endpoint (Go) + clobber-guard + schema (C6) + §5.1 capture (C7)** — the WRITE path (can't retrofit). `POST /internal/books/{book_id}/wiki/articles` (X-Internal-Token) body = `{entity_id, body_json (TipTap from M0 `ir_to_tiptap`), author_type:'ai', generation_provenance:{build_inputs(C7 fingerprint), citations(M4 `compose_provenance_cites`), verify_flags(M4), publish_blocked(M4), grounding, model_ref, step_models}, source_usage(C7 reverse index), spoiler_horizon}`. Glossary Go: upsert-by-entity + **clobber-guard** (§4.6: AI overwrites only untouched `author_type='ai'` draft; else → `wiki_suggestion`); writes `wiki_article_source_usage`; flips `generation_status`; migrate stub revisions `owner→'system'` (risk #4). Plan §I-**C5/C6/C7** + §II-**M5** + risks #4 (two gen paths). **Cross-service** (knowledge calls glossary; live-smoke needed). **D-WIKI-M4-NEUTRALIZED**: when M5 persists a publish-blocked article as a suggestion, persist the SDK neutralized-safe text (M4 currently drops `VerifyResult.neutralized`). M7a (citation-mark FE) still parallel-able.
