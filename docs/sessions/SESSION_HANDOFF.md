...
- **✅ Stage 4 BACKEND SHIPPED + D3 live-proven on dev, 2026-07-11 (RB-4-BE).** FE half (22-C + 24-H2.2) is the follow-on. 3 build agents → parent SC4 read-path fix → `/review-impl` (4 dims, 3-lens refute-verify).
  ...
  - **▶ NEXT within Stage 4 (FE wave):** `22`-D1 (OpenAPI for the book-service scene routes — pure doc, contract-first) · `22`-D2 (`frontend-tools.contract.json` regen — verify the scene panels' enum is current) · **C3b** (pov/present/location entity-picker over glossary + inspector Links section reusing `scene_link` — buildable now: glossary + `scene_link` are shipped services, no `structure_node` needed). **Gated on Deploy 2 (structure_node):** C2b group-by-Arc, C4b SceneRail identity-union, the Hub canvas (24-H2). Then RB-4-FE. FE verify = `tsc --noEmit` + vitest + a **built-image** browser smoke (`frontend-5174-is-baked-prod-nginx-not-vite`).
