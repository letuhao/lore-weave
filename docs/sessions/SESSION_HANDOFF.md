...
- **VERIFY: 208 worker-ai unit (5 new sweeper tests) + AST main/consumer OK + provider-gate OK.** Only runs when the decouple flag is on (inert otherwise). Single-service; live-smoke = `D-WX-LIVE-SMOKE` (Wave 3).
...
**▶ NEXT — debt-paydown roadmap (Wave 1a+1b done; push-approved-per-wave):**
1. **Wave 2 — translation parity + observability:** `D-2B-DECOUPLE-FLAG-COUPLING` (startup assert), `D-2B-SUBMIT-PERSIST-GAP` (translation resume sweeper, mirror Wave 1b), block chunk-rows + `record_stage` port (`D-2B-T3A-BLOCK-CHUNK-ROWS`/`-OBSERVABILITY`), `D-2B-SHELL-UNIT-TESTS`. (`D-2B-RESUME-STATE-SIZE` = perf, defer unless TOAST bites.)
