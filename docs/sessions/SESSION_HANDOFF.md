...
>
> **▶ THIS BRANCH'S NEXT — glossary-assistant coverage campaign.** Make the assistant cover all scenarios (S1–S35). Architecture **LOCKED** (decisions **D1–D13**, 3 campaign specs: `2026-06-10-glossary-assistant-{scenario-coverage,extended-scenarios,architecture-review}.md`). **Phase -1 E0 (collaboration epic) IN PROGRESS — E0-0 shipped (see ▼ E0 PROGRESS).** **Start next:** **E0-1 (glossary adoption)** — wires `grantclient` into glossary's 57 ownership sites + the per-site need-mapping table; this is the slice the campaign's F1 actually needs. (Or the independent **H-C async-delivery spike**, high-risk — do early.) The full campaign block + decisions live in the **"▶ (merged from main) glossary-assistant"** section below (the deferred list there now carries `GLOSSARY-ASSISTANT COVERAGE CAMPAIGN`, `D-MCP-TASKS-MIGRATION`, `D-MCP-DIRECT-RETURN`).
>
...
> - **NEXT: E0-1 (glossary adoption)** → then E0-2 (book self-adopt) · E0-3 (knowledge, IDOR-heavy) · E0-4 (translation/campaign/composition) · E0-5 (FE panel). After E0-0+E0-1 ship, campaign F1 is satisfied.
>
