# Session Handoff — `feat/auto-draft-factory-gaps` · B0+B1+B2 cleared · 2026-06-17
...
>
> ▶ **NEXT: (a) PRODUCER-EMIT AUDIT (new, high) then (b) B3.** While testing glossary-extract the user hit 3 bugs → root cause `D-JOBS-GLOSSARY-EXTRACT-UNWIRED` (DEBT-BATCHES): the glossary-extract pipeline (translation-service `extraction_jobs`, `routers/extraction.py` + `workers/extraction_worker.py`) emits **NO** `emit_job_event` at any transition, has **NO** reconcile source (the `translation` source queries `translation_jobs`, not `extraction_jobs`), and the projection's `extraction` kind federates to **knowledge** not translation → the job is structurally invisible to the unified Jobs screen (+ FE pick-mode inherits the 'all' default → extracts all; create endpoint does N per-chapter INSERTs + 2 HTTP sync before 202 → wizard freeze). **User suspects this is one of SEVERAL producers missed during P1 emit-wiring → audit EVERY job-producing pipeline for (emit at create/running/terminal/cancel + a reconcile source + correct kind/service federation) BEFORE fixing.**
>   - **Live-smoke:** `D-B0-LIVE-SMOKE` + `D-B1-LIVE-SMOKE` (model NAME lands in projection; live campaign cost) + `D-B2-LIVE-SMOKE` (pause/resume/abort/compose-cancel) → all fold into B3.
