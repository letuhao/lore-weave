...
- **Default-on decision (READY):** the WX extraction + translation (v2 text/block + V3) decoupled paths are review-cleared AND live-proven end-to-end. Flipping the compose defaults `EXTRACTION_DECOUPLE_ENABLED` + `TRANSLATION_DECOUPLE_ENABLED` to true is the remaining gate — the user's call. (Stack is currently ephemeral-on; a plain restart reverts to the OFF compose default.)
...
- **Wave 6b — `D-PHASE0-CANCEL-LIVE-SMOKE`** (cancel a real slow job → assert the GPU slot frees in one tick — the original incident regression; needs the worker stack).
- **`D-V3-DECOUPLE-COLDSTART-2PASS`** (wire the 2-pass cold-start into the decoupled v3 path — narrow).
