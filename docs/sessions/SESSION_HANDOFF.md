...
**▶ NEXT (Track C) — 2026-07-12:** **P-1 = build the server-side STEP-RUNNER** (the DoD blocker; the mechanism is 100% built + review-proven, gemma just does one step and stops — see RUN-STATE §7 P-1). Then **P-3** (author the 8 remaining WS-5 workflows, mirroring the just-shipped W2/W4 — note W5/W9's audit tool-names are WRONG: use `translation_retranslate_dirty`/`translation_coverage` and `composition_canon_rule_*`), **P-4** (scenario coverage — prioritise **S00e**, the Phase-1 consent journey end-to-end), **P-5** (FE surfaces — binding UI + workflow rack are cheapest; backends all live). Everything below is the SUPERSEDED pre-run diagnosis, kept for the record.
