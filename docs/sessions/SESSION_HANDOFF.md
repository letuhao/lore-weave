...
- **`D-LEARNING-JUDGE-EMPTY-CONTENT`** (MED, env/gateway) — LM Studio chat completions for instruct models (gemma-4-26b, qwen3.5-9b) return empty assistant `content` via the provider-registry gateway for the judge prompt (well-formed system+user, max_tokens=1792, temp=0, finish_reason=stop). Repro: a judge `chat` job completes with `result.messages[0].content=""`. Investigate the gateway's LM Studio chat-content extraction / `chat_template_kwargs` (`enable_thinking`?) / response mapping. Until resolved, the online judges complete-but-score-nothing on this local stack; try a cloud judge model or a known-JSON-clean local model to observe a real `eval_fidelity_score`.
