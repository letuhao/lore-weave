...
**NEXT = Q3b (FE) then Q3.5:**
- **Q3b (FE) chat feedback UI** — thumbs up/down + regenerate-as-negative on `AssistantMessage.tsx` (server source of truth, no localStorage); `frontend/src/features/chat/api.ts` → `POST /v1/chat/messages/{id}/feedback`; i18n across 4 locales; vitest. Backend contract is LIVE + stable.
- **Q3.5 judge calibration** (gates Q4) — consumes Q2 gold_labels: judge-vs-human agreement (balanced acc + Cohen κ ≥ 75-90%); enforce disjoint-median exclude-generator in code (loreweave_eval).

(Critical path: Q0✓→Q1✓→Q2✓→Q3(a✓/b)→Q3.5→Q4→Q6a→Q6b→Q8; Q9 privacy gate before any cross-tenant Q7 surface. See track doc §5.)
