...
**Branch `wiki/llm-gen` (off main `2ace6272`):** M0–M6 + E2E-smoke + M7a PUSHED to origin (`e9313ef0` plan · `12956b1e` M0 · `fc969f03` M1 · `1cdf53ee` M2 · `f982f7c4` M3 · `a3505367` M4 · `6e420c30` M5 · M6 · `24a8d973` E2E · `409956bd` M7a). M7b-1 (this) committed local — push pending (last 1+ commit unpushed; ask before pushing).
...
**▶ NEXT SESSION — remaining wiki work:**
1. **M7b-2** (FE — model-picker dialog [reuse `aiModelsApi.listUserModels({capability:'chat'})`, mirror `BuildGraphDialog`] → `wikiApi.generateStubs` with `model_ref`/`model_source`/`max_spend_usd`, handle the delegate response shapes [202 `{job_id}` / 409 `{active_job}` / 404 not_indexed / 200 `{action:none}`]; job-progress polling via the new `GET /v1/glossary/books/{id}/wiki/job`; `needs_review`/`blocked` badges off `generation_status` + verify-flag display off `generation_provenance`; regenerate; resume/cancel buttons via the new endpoints). The BE contract is live + tested. **Rides D-WIKI-M7B-LIVE-SMOKE** (rebuild knowledge+glossary, drive the full FE→glossary→knowledge path end-to-end).
2. **M8** (feedback-emit `wiki.corrected`/`suggestion_reviewed` via glossary outbox + thin eval harness) — needs M5+M7.
...
- **D-WIKI-M7B-LIVE-SMOKE** (M7b-1, cross-service) — the job status/resume/cancel BE is unit-proven (108 pytest + 7 go) but the full FE→glossary→knowledge round-trip over running services isn't exercised yet (the M6 E2E proved the trigger path; this adds the status/resume/cancel hops). Collapses into the M7b-2 FE live-smoke (rebuild knowledge+glossary, drive a real generate→poll→resume/cancel from the UI).
- **D-WIKI-M7B-RUNNING-CANCEL** (M7b-1 DESIGN) — cancel is scoped to pending|paused (the stuck-lock case is budget-`paused`). A *running* job returns 409: the orchestrator processes entities sequentially without polling status mid-loop, so a cooperative running-cancel needs a per-entity status re-check (abort between entities). Add when an operator needs to stop a long in-flight run; today it self-terminates (complete/pause) and the F1 claim-guard prevents a cancelled job from being resurrected.
