...
**NEXT:** **the glossary-assistant arc (P0–P6) is DONE + the cross-service wiring is LIVE-SMOKED + the LOW-defer cleanup is COMPLETE.** Remaining follow-ups: (1) the LLM-driven chat-turn + browser smokes (`D-GLOSSARY-LIVE-SMOKE-BROWSER`) — one stack-up/Playwright pass; (2) the **glossary LLM-flow migration review** (DEFERRED 066): now that the glossary MCP tools exist + are live-federated, migrate the legacy prompt-driven glossary flows off raw HTTP; (3) **enhancement — `/loom` EDIT-ATOMIC**: multi-field-in-one-card glossary edit (a new transactional glossary endpoint taking `base_version` + several field changes in one tx + a multi-field FE diff card). Today edits are one-PATCH-target-per-proposal (atomic); EDIT-ATOMIC is a real feature, not debt — spec/plan when started.
...
- **D-GLOSSARY-LIVE-SMOKE-BROWSER** (P3/P4/P5) — the remaining smokes that need a browser/Playwright (lower marginal — handlers are real-PG-verified + the cross-service wiring is live-verified): the dock on glossary page + reader (P5), full chat→suspend→FE-Apply→BFF→glossary→resume (P3 edit), and chat→propose→confirm→`/v1/schema/confirm` (P4). One stack-up pass covers all three.
