...
- **D-TRANSL-M4D-2B** (M4d-2, PO full-spec 2026-06-07) — **target-translation writeback** (cross-service): extend glossary `extract-entities` (`extractedEntity`) with an optional per-entity target `{language_code, value}` → write an `attribute_translations` row at `confidence='machine'` (M1d trust ladder demotes → no false canon); translation sends `NamePair` rows (from M4d-2a) **with** targets + `default_tags=['ai-suggested']` (reuse mui#1 draft inbox + `ai-rejected` tombstone). Backward-compatible. **Must `_sanitize` names at the boundary.**
- **D-TRANSL-M4D-2C** (M4d-2) — **orchestrator 2-pass + config**: `cold_start_mode = single_pass(default)|two_pass` plumbed settings→job→coordinator→worker (mirror `qa_depth`); orchestrator: empty-glossary (cold start) ∧ `two_pass` ∧ recurring pairs found → pass1 → `extract_name_pairs` → writeback (2b) → re-fetch glossary → **pass2 re-translate**. Guard the 2× cost (skip when no recurring names) + input token budget. V2 untouched. (M4d-2a bilingual extractor shipped 2026-06-07.)
