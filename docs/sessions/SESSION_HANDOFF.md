...
**Branch `wiki/llm-gen` (off main `2ace6272`):** M0–M6 + E2E-smoke + M7a PUSHED to origin (`e9313ef0` plan · `12956b1e` M0 · `fc969f03` M1 · `1cdf53ee` M2 · `f982f7c4` M3 · `a3505367` M4 · `6e420c30` M5 · M6 · `24a8d973` E2E · `409956bd` M7a). M7b-1 (`fbf230e7`) + M7b-2a (this) committed local — push pending (2 commits unpushed; ask before pushing).
...
1. **M7b-2b** (FE + tiny glossary BE — `needs_review`/`blocked` **badges** off `generation_status` [now on list reads] + **verify-flag display** off `generation_provenance.verify_flags` `[{kind,dimension,evidence,severity}]` [on detail reads]; **regenerate** an article = add an optional `entity_ids` passthrough to glossary `generateWikiStubs` [~5 lines Go, PO decision A] so a single-article Regenerate re-runs just that entity). Then **D-WIKI-M7B-LIVE-SMOKE** (rebuild knowledge+glossary, drive a real generate→poll→resume/cancel + badges from the UI — the whole FE→glossary→knowledge path).
