...
**▶ NEXT — wiki FE gap-closure W2→W4 (W5 PO-gated):** per [`2026-06-11-wiki-fe-gap-closure.md`](../plans/2026-06-11-wiki-fe-gap-closure.md): **W2** change-feed richness (severity-bar + batch cost-estimate + dismiss-all + rescan proxy), **W3** generate-dialog/sidebar polish, **W4** job-progress detail (cross-service: BE per-entity outcomes → FE results table). Then the platform epic below once its conflict clears.
