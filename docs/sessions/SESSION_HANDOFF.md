...
1. **M8** (feedback-emit `wiki.corrected`/`suggestion_reviewed` via glossary outbox + thin eval harness) — needs M5+M7. **The whole M0→M7b LLM-gen feature is now built + live-proven; M8 is the feedback/eval flywheel on top.**
