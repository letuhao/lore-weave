...
>   - **`D-RAWSEARCH-CANON-WIRING-LIVE-SMOKE` ◑ WIRING PROVEN** (contract/deploy level, no embedding backend needed): through the gateway as the test account — `GET …/search?surface=all`→404 not_indexed (route+param live); `surface=bogus`→422 literal_error (validated through the proxy); `POST …/index-drafts` on a random book→404 not_indexed (new route prefix-proxied + owner-gate runs); on an OWNED-but-unindexed project→409 `project_not_indexed` (owner-gate PASSED → embedding gate fired); `surface=all` search on the owned project→200 `degraded:{semantic:not_indexed}` (owner-only `all` path executes + degrades cleanly). **Still deferred: the draft-vs-canon RETRIEVAL differentiation** (surface=all returns a draft that surface=canon excludes) — needs a real embedding backend + a built graph (same blocker as `D-RERANK-BYOK-LIVE-SMOKE`; the 10 test-account projects have no embedding model).
