...
1. **D-WIKI-M8-EVAL-PLUS (LLM-scoring)** — *(building next)* an online LLM-judge of wiki-article groundedness/quality, gated `WIKI_LLM_JUDGE_ENABLED` (default OFF), mirrors `_maybe_judge_translation`. Self-contained (learning + provider-registry).
2. **D-WIKI-M8-FEWSHOT** — inject gold AI→human pairs as few-shot exemplars into wiki generation (`prompt.py` `exemplars` param + a gold-pair fetcher from glossary `wiki_revisions` + thread through `generate.py`/orchestrator), gated `WIKI_FEWSHOT_ENABLED` (default OFF). Cross-service (knowledge ← glossary).
3. **D-PLATFORM-SYSTEM-CONFIG-UI** (new epic, NOT wiki-specific) — there is **no admin/system-config surface today** (no admin role/`is_admin`, no feature_flags table; all ops flags are env-only + a per-user Settings page only). A UI to toggle ops flags needs: admin auth (`is_admin` + `require_admin`) + a `feature_flags`/`system_config` store + PATCH endpoints + an `/admin` FE page. Reusable for ALL flags (wiki-scoring/few-shot/translation-judge/precision-filter/wiki-gen). Build after the two code halves.
