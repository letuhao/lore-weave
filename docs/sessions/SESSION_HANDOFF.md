...
> - **E0-2 deferred rows:**
>   - **D-E0-2-LIVE-SMOKE** — real-stack collaborator flow on book-service's own endpoints (B with `edit` PATCHes a book + creates a chapter; `view`-B 403 on PATCH; non-grantee 404 on GET; **+ favorites gate + async-import editor-attribution** from the cleanup below). book-service has no real-PG harness; deny-test proves the *denial* side, not the allow-path. **NEXT to run.**
