...
**Deferred:**
- `D-LMSTUDIO-TOOLCALL-FORMAT` (gate #4, needs investigation, HIGH user-flagged priority 2026-07-08) — gemma (and possibly other local models) sends blank args to nearly every tool call in this LM Studio setup. User's hypothesis: this is likely OUR request-format bug (LM Studio's console shows a warning), not a pure model defect, since the pattern is too uniform (100%) to be probabilistic model noise. Needs comparing this repo's OpenAI-compatible tool-calling request shape against LM Studio's actual expected function-calling format/spec.
