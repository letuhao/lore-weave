...
- **Part E — live judge-gate eval, first pass.** New harness `scripts/eval/run_skill_gate.py` (sibling of `run_quality_gate.py`, adds per-turn `book_context`/`editor_context`/`studio_context`/`enabled_skills` so a scenario can force the right surface+pin). 37 scenarios across 5 files (`scripts/eval/skill_scenarios/*.json`), run live against `gemma-4-26b-a4b-qat` on a freshly-rebuilt chat-service, judged by 5 independent Agents. **Headline: zero hallucinated tool names across all 37 scenarios** — the failure mode this whole effort exists to prevent never occurred. Most FAIL/WEAK scores trace to a cross-cutting `find_tools`-loop-then-give-up pattern, confirmed via a control run to be a PRE-EXISTING model/infra characteristic (reproduces identically on the untouched, already-shipped `glossary_skill`) — NOT a defect in this session's new skill prose, so no skill was edited based on this run. Full report: [`docs/eval/skill-authoring/2026-07-07-part-e-first-pass.md`](../eval/skill-authoring/2026-07-07-part-e-first-pass.md).
- **VERIFY (final, whole spec closure):** chat-service 1152/1152, mcp-public-gateway 230/230 (`tsc --noEmit` clean), live judge-gate run completed (37 scenarios, 5 skills). ai-gateway untouched throughout (skills/Part D/C are chat-service+mcp-public-gateway-only).
- **Deferred:**
  ...
  - `D-SKILL-EVAL-DISCOVERY-LOOP-FLAKE` (gate #2+#4, NEW) — the cross-cutting `find_tools`-loop-then-give-up/silence pattern seen across ~half of Part E's 37 scenarios (and reproduced on the untouched glossary skill in a control run). Needs root-cause investigation (search relevance? gemma's loop persistence? a `max_iterations`/termination heuristic in `stream_service.py`?) before any prose or agent-loop fix — do not guess-fix off this run's data alone.
  - `D-SKILL-EVAL-RERUN-AFTER-LOOP-FIX` (gate #3, naturally-next) — once the above is root-caused, re-run `scripts/eval/run_skill_gate.py` (harness + all 5 scenario files already built, reusable) to get a clean read on whether `cancel_job_irreversibility_warning` (translation) and `motif_not_connected_to_planforge` (composition) — the 2 FAILs that weren't loop-noise — are real prose gaps.
