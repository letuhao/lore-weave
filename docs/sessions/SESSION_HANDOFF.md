...
>   - **▶ NEXT — P3-reconcile then P4:** (1) the **reconcile sweep** — `jobs-service` periodically re-reads each service's job rows (`GET /internal/jobs?since=`, the endpoints land per-service alongside control) and upserts to heal outbox drift (`D-JOBS-P2-RECONCILE-CROSS-SVC`; the sweep scaffold + `reconcile_enabled`/`reconcile_interval_s` knobs already ship in jobs-service config, disabled). (2) **P4: the unified Jobs GUI** — dashboard listing the user's jobs (live SSE), children grouped under campaigns (H3), filter by kind/status, per-row state-aware cancel/pause/resume gated on `control_caps` (generalize `MonitorControls`/`CampaignMonitor`); `kind=campaign` deep-links to the existing monitor.
