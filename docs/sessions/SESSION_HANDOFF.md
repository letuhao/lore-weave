...
**NEXT:** `D-RAWSEARCH-EVAL-CI` (threshold gate + persist + 2nd-book golden + per-band metrics) · warm-rerank latency benchmark (vram_mb=8 ⇒ confirm GPU vs CPU). Other Phase-3: P3-B canon-lexical · P3-C char-offset dedup + copy-exact (folds in semantic precise-scroll). (P3-D' MMR closed — superseded by rerank.)
