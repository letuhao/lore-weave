...
>   - **▶ NEXT — P3: per-service control endpoints + routing + reconcile.** `POST /v1/jobs/{service}/{job_id}/{cancel|pause|resume}` → owner re-check on the projection → forward to the owning service's control endpoint (internal auth, **owning service RE-VERIFIES ownership** on the row — M4). Add missing cancel/pause/resume to composition/translation/video-gen/lore-enrichment. Build the reconcile sweep + `GET /internal/jobs?since=` per service (D-JOBS-P2-RECONCILE-CROSS-SVC). Then **P4: the unified Jobs GUI**.
>   - **Deferred:** `D-JOBS-P2-RECONCILE-CROSS-SVC` (reconcile sweeper + per-service `/internal/jobs?since=` → P3) · `D-JOBS-P2-SSE-LIVE-SMOKE` (real consumer-upsert→pub/sub→connected-client push on the stack; notify+publish+stream-framing are unit-covered) · `D-JOBS-P2-SSE-EVENTSOURCE-AUTH` (browser EventSource can't send a bearer — P4 GUI uses a fetch-stream; token-in-URL intentionally unsupported).
