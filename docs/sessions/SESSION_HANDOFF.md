...
**NEXT options:** (1) **M7b** — Channel-1a, wire existing human signals (set-active / publish-accept / name-confirm) → learning `source=human` · (2) **M7c** — Channel-1b, build the human-edit-translation feature + before/after gold (the richest signal; biggest slice) · (3) **M7d** — Channel-3 online LLM-judge · (4) live-smokes (M7a/M6b-worker-usage/M4d1) on a rebuilt stack · (5) PR `feat/translation-pipeline-v3` → `main`. **PO: do NOT merge to main until translation is production-ready** — the M7 feedback track is part of that gate. V3 core feature-complete; M7b/c/d + live-smokes remain.
