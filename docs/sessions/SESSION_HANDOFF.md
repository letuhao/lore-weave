...
**▶ NEXT — wiki FE gap-closure W4 (W5 PO-gated):** per [`2026-06-11-wiki-fe-gap-closure.md`](../plans/2026-06-11-wiki-fe-gap-closure.md): **W4** job-progress detail (cross-service, BE-first: W4a persist per-entity outcomes on `wiki_gen_jobs` + expose on the poll → W4b FE per-entity results table). Then the platform epic below once its conflict clears.
