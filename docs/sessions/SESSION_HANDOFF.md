...
> - **✅ `D-JOBS-P4-RETRY-VIDEOGEN` SHIPPED (2026-06-17).** NO migration needed — the "params not persisted" triage was WRONG: `video_gen_jobs.request_json` JSONB already stores prompt/model/duration/aspect/style. `video_gen` ∈ jobs-service `_RETRYABLE_KINDS`; `control_video_gen_job` routes `retry` → `_retry_video_gen_job` (owner 404 / 409-unless-failed / 409-empty-request_json / 404-stateless) → reconstruct `GenerateRequest` → reuse `_submit_decoupled` (new pending row + emits `running`). VERIFY: jobs-service caps 16 + video-gen 14 (+5 retry); provider-gate clean. **Retry now done for translation + extraction + video_gen; composition is the last clean candidate; lore stays blocked (sync in-process → async refactor).**
