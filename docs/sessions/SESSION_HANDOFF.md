...
**▶ NEXT — 2b-T3 (block path + V3 verify/correct decouple — the campaign uses v3-block, so THIS is the real-data path) then 2b-worker-ai:**
1. **2b-T3 block path:** reuse the T2 engine for `session_translator.translate_chapter_blocks` (batched `translate_batch_with_retry`). The block path aggregates per-batch (translated_blocks + translated_texts + counts) — model the resume_state on T2's but carrying the block list + per-batch outputs; finalize_cb sets `translated_body_json`, format='json', memo from translated-only texts (the existing block finalize inputs). Then **V3** (`v3/orchestrator`): the Translator→Verifier→Corrector loop has multiple LLM ops per block — each becomes a decouple step (verify/correct are extra `awaiting` kinds). The consumer + `_finalize_chapter` are REUSED unchanged. **Live-smoke on a real 万古神帝 chapter (flag on) closes `D-2B-T2-RESUME-LIVE-SMOKE` too.**
2. **2b-worker-ai (HARD):** extraction decouple — needs `loreweave_extraction` per-op resume points (4 ops × N chunks), and `loreweave_extraction` is shared with knowledge (the /warp TRIAGE-OUT reason). Separate-consumer rewrite in worker-ai.
Or Phase 3.
