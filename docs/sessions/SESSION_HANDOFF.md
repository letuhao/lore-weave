# Session Handoff — Session 106 (S4 design pass + S4a cost-attribution threading)
...
> **Date:** 2026-06-09 (Auto-Draft Factory **S1 + S2 + S3a/b/c + S4 design + S4a threading** shipped; human-in-loop v2.2).
...
**▶ NEXT: S4b — usage outbox + relay + dual Redis streams** (provider-registry, Go). Per the design doc §5.2/§7a: new `usage_outbox` table written **in the same tx as `Repo.Finalize`** (replaces fire-and-forget `RecordUsage`), carrying `cost_usd` (from `actualUSD()`) + `campaign_id` (from `job_meta`); a relay goroutine XADDs to `loreweave:events:usage` (all) + `loreweave:events:campaign_usage` (campaign-tagged, MAXLEN/G8). Then **S4c** (usage-billing consumer + real-cost deduction, fixes the flat $0.000002/token) → **S4d** (campaign `spent_usd` accumulation + reactive cap-pause; dedup via `campaign_usage_seen`) → **S5/S6** (wizard + monitor FE). **AMAW recommended for S4b–d** (money path, multi-system contract, schema). Open items O-1..O-5 in the design doc (notably: usage-billing has no Go Redis-consumer precedent yet; confirm token-vs-USD quota column semantics before S4c).
