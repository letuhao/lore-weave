...
**Deferred:**
- ~~`D-LMSTUDIO-TOOLCALL-FORMAT`~~ — **ROOT-CAUSED 2026-07-08, not a LoreWeave bug.** User correctly suspected the 100%-blank-args pattern was too uniform to be pure model noise. Web-search + LM Studio's own bug tracker confirm: `gemma-4-26b-a4b-qat`'s `chat_template.jinja` is missing a `format_type_argument` macro needed to correctly render function-call arguments (LM Studio bug tracker [#2012](https://github.com/lmstudio-ai/lmstudio-bug-tracker/issues/2012), [#1927](https://github.com/lmstudio-ai/lmstudio-bug-tracker/issues/1927) — exact model match; [#1756](https://github.com/lmstudio-ai/lmstudio-bug-tracker/issues/1756) independently confirms the "duplicate identical tool calls" pattern this session's B2 fix targeted was ALSO a known upstream LM Studio/Gemma4 template bug, not LoreWeave-introduced; mlx-swift-lm [#259](https://github.com/ml-explore/mlx-swift-lm/issues/259) confirms `GemmaFunctionParser uses outdated tags`). Verified our own `lmStudioAdapter` (`provider-registry-service/internal/provider/adapters.go:1546-1596`) sends a standard OpenAI-compat `tools`/`tool_choice` payload — no request-format bug on our side. **Fix is local/infra, not code**: re-download this model from the `lmstudio-community` HF namespace (maintains LM-Studio-corrected chat templates) instead of the raw google/unsloth build, or manually patch the Jinja prompt template (LM Studio → My Models → model settings → Prompt Template) per the fix in [this HF discussion](https://huggingface.co/google/gemma-4-26B-A4B-it/discussions/20). No action needed in this repo.
