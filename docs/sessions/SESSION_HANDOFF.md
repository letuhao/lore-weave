...
**Skill-authoring contract + composition/translation skills (Part A+B) — SHIPPED + `/review-impl` clean, 2026-07-07** (branch `feat/context-budget-law`, HEAD `pending`, spec [`docs/specs/2026-07-07-skill-authoring-and-mcp-exposure-standard.md`](../specs/2026-07-07-skill-authoring-and-mcp-exposure-standard.md)). Direct follow-on to the MCP tool-calling fix below — user's framing: tool count will keep growing, and skills need to be genuine "workflow definitions + tool-use guides," not bare "here's MCP, go find_tools yourself."

- **Part A — the skill-authoring contract.** `SkillDef` gained `hot_domains: frozenset[str]`; a new permanent lint (`test_skill_registry.py::TestSkillClaimsLint`) scans every skill's prose for real catalog tool names and fails if the tool's domain isn't declared hot (or the tool is CAT-4-legacy-tagged) — closes the exact bug class `plan_forge_skill` shipped with, for every current AND future skill. Building it immediately found 3 more real, live instances: `knowledge_skill` had the SAME undeclared-claim bug as plan_forge (now honestly declared, runtime wiring deferred — see below); `glossary_skill.py` still referenced 7 tools the 2026-07-06 CRUD-consolidation spec had already legacy-tagged (swapped to their replacements); `GROUP_DIRECTORY["knowledge"]` matched NOTHING in `find_tools`/hot-seeding because `kg_*`/`memory_*` tools carry those literal prefixes, never `knowledge_` (fixed with a `_DOMAIN_ALIASES`/`_domain_of()` layer, `tool_discovery.py` + `find-tools.ts` lockstep).
- **Part B — composition + translation skills.** Two new skills (`composition_skill.py` — the ~56-tool outline/prose/canon/motif domain; `translation_skill.py` — all 12 `translation_*` tools), researched from the real Python MCP server code, not guessed. Building it surfaced a latent gap where a pure-studio turn (studio_context set, editor/book_context absent) fell through to the generic "chat" surface in the SKILL-visibility system even though the tool-hot-seeding system already treated studio as independent — fixed by threading `studio: bool` through `_surface_key`/`resolve_skills_to_inject` (additive, retroactively fixes glossary_skill's studio visibility too). Also generalized `tool_surface.py`'s curated-mode hot-domain union from 2 hand-authored special cases (glossary, plan) into one loop over pinned skills, so `translation`/`composition` (off-studio) can be curated-pinned without re-hitting the seeding gap — NEITHER is auto-injected by default, so no session's default token cost changed. `translation` skill is curated-pin-only; `composition` auto-injects only on studio (where its tools were already hot).
- **`/review-impl` (8 findings, all fixed):** 2 cold-start sub-agents fact-checked every claim in the new skills against the real tool code and found the lint passing ≠ factually accurate — 5 content bugs (a wrong "motif archive has no restore" claim; `translation_coverage` doesn't report glossary-staleness; `translation_list_versions` never returns body text at any detail level; `translation_job_control`'s `cancel` is terminal, only `pause` is undoable; the extraction reasoning-effort clamp is never reported back in the tool's response) + 1 architecture bug (the curated hot-domain union gave each pinned skill its own separate token budget instead of one shared ceiling — fixed, regression-tested) + 1 cosmetic (FE tool-picker showed raw `kg`/`memory` instead of aliased `knowledge`) + 1 doc-gap (the lint's known live-catalog limitation wasn't a tracked Deferred item — now is, see below).
- **VERIFY (final):** chat-service 1144/1144, ai-gateway 142/142, all green.
- **Deferred:**
  - `D-SKILL-HOTDOMAIN-RUNTIME-WIRING` (gate #2, large/structural) — `knowledge_skill.hot_domains={"knowledge"}` states the skill's real intent (it claims `kg_*`/`memory_*` tools directly) but the runtime doesn't fulfill it on ANY surface yet — needs Part D (fold the 3 hand-authored hot-domain constants into a skill-registry-derived lookup) + its own token-budget-verified rollout before turning it on, since `knowledge` auto-injects on every surface including the universal chat surface (no small permission_mode-style patch like plan's).
  - `D-SKILL-LINT-LIVE-CATALOG` (gate #4→buildable, per spec §8b.2) — the skill-claims lint has no live-catalog access, so it can't catch a genuinely stale/typo'd tool-name-shaped token that matches NO real catalog entry (only catches real-but-wrong-domain and known-legacy names via hand-maintained sets). Buildable via a snapshot-harness mirroring `contracts/mcp-response-shapes/*.json`'s `assert_or_write_shape_snapshot` pattern (a regenerable JSON list of known tool names) — not yet built.
  - Composition/translation Phase 2 (`book_skill`, `settings_skill`, `jobs_skill`) — per spec §5, once Phase 1's pattern is validated.
  - `mcp-public-gateway` Part C (scope-size-adaptive exposure) — needs real key-scope-size measurement from `auth-service`'s `mcp_api_keys` before picking a threshold; independent track, not started this session.
