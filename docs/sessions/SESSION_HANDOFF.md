...
>     - **▶▶ NEXT:** `D-JOBS-P4-M2-LIVE-SMOKE` (consolidated real extraction/translation → model/cost/tokens/params land in `job_projection`), then **M3 (FE rewrite to the mockup — makes M1+M2 visible)** + M4 (Playwright). Deferred model-NAME follow-ons: `D-JOBS-P4-LORE-MODEL`, `D-JOBS-P4-CAMPAIGN-MODEL-NAMES`, `D-JOBS-P4-TRANSLATION-COST`, `D-JOBS-P4-COMPOSITION-GUARDED-MODEL`.
