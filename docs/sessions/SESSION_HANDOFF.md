...
**▶ NEXT (Track D):** WS-D3 — wire CD4 into `validateWorkflow` as **warn** + "no RED-G3 tool in `tool_list`". That is **ND3**, the real *before-we-ship-workflow* gate. Then WS-D2+ (harness beyond P0 — the async poller is now unblocked and can finally be live-proven). F5 (mid-tier models under-select Tier-W/async tools) is precisely what CD4 should block on.
