...
>   - **⏳ M3.2 (D):** `segment_glossary_usage` table (per-segment entity refs, populated best-effort at translate finalize by scanning segment_text vs the book glossary) + `segment_translations.is_glossary_stale` + glossary_consumer per-segment flag + `stale`/`needs` in status/coverage + retranslate-dirty includes stale.
