...
**State: Phase-1 + 2 + 3(A,D) + P3-EVAL harness + E5 recall/score tuning DONE + LIVE-SMOKE PASSED (2026-06-08).** Hybrid (lexical+semantic, RRF) end-to-end. New workstream — the missing **raw chapter-text** layer beneath glossary/knowledge/wiki (all lossy derivatives); serves authoring + extraction/trích-lục. Spec [`2026-06-07-raw-search.md`](../specs/2026-06-07-raw-search.md) (PART I design + PART II ATAM-lite eval; 7/7 confirm-at-BUILD resolved). Plan [`2026-06-07-raw-search.md`](../plans/2026-06-07-raw-search.md) (3 phases; ADJ-1..4 PO-acked).
...
**NEXT:** **E6 FE** (granularity navigate/mine toggle + `relevance` display + K-selector) · `D-RAWSEARCH-E5B-RERANK` (semantic junk-rejection / per-query score normalization) · `D-RAWSEARCH-EVAL-CI` (threshold gate + persist + 2nd-book golden). Other Phase-3: P3-B canon-lexical · P3-C char-offset dedup · P3-D' MMR. OR **push** the branch (11 commits).
...
- **D-RAWSEARCH-E5B-RERANK** (E5 calibration finding, **priority** for junk-rejection) — bge-m3 cosine on this corpus is compressed [0.68–0.82] with poor neg/pos separation, so a global `min_relevance` floor can't filter junk without dropping true hits (it's OFF by default). Real fix: a cross-encoder reranker or per-query score normalization (gap-to-top / z-score) on the semantic leg. The `relevance` field + opt-in floor already ship.
