...
**W11-M2 GROUNDWORK shipped + full BUILD RECIPE ready, 2026-07-11.** The reader-facade build surface is mapped end-to-end (4-agent grounding workflow) and the exact recipe is in the spec §4.2 "▶ BUILD RECIPE" — every signature, the grant→owner→position→3-axis flow, the **glossary `before_chapter_index = before_sort_order + 1` off-by-one** (glossary is exclusive `<`, verified `canon_at_chapter_handler.go:81`), the fail-closed rules per read, the 3 lockstep schema sources to register (FastMCP strips missing args), the test plan, and the **mandatory `/review-impl`** (this is the security-sensitive milestone). **Shipped this checkpoint:** `BookClient.get_reading_position` (knowledge `book_client.py`, fail-closed None on 200-null / any failure) + 3 respx tests (36 passed). **▶ NEXT (resume M2 from the recipe):** create `app/tools/reader_tools.py` (`lore_ask`/`lore_browse_entities`/`lore_entity`/`lore_timeline`, Tier-R scope=project), wire the 3 registration points, red-first tests (fail-closed / anti-oracle / resolve-to-owner / the `+1` inclusion), then `/review-impl`. The M1 null-position→fail-closed *interpretation* is proven here (empty windowed reads when position is None). Then W11-M3 → W10-M1 → W10-M2.
...
**Track-B defers still open:** **none.** `D-WS4C-EFFECTIVE-VALUE` was CLEARED this session (see the CLEARED row below). The only remaining Track-B scope is the W10/W11 backend build, which is unstarted design work, not a defer.
...
**▶ NEXT (Track D):** WS-D3 **DONE**; **WS-D4 partial DONE** + registry-slug residue cleared (commits `f9f841797` · `301f27bc6`). WS-D4 shipped `executes ∧ effect` for the workflow-critical set (derived live from `agent_registry.workflows` = `glossary-bootstrap`'s 4 steps): independent read-back (CD3), a silent success folds to `executes:false`. 3/4 critical tools effect-verified; `glossary_extract_entities_from_doc` is **paid** → an honest $0 gap in the sole curated workflow. Manifest gains informational `effect_verified` (`proven ⊆ effect_verified`). Sweep now **130 execute · 0 broken · 84 unchecked** (catalog grew to 216 under concurrent work). Open items: (1) **WS-D4 hard-reject tightening** — reject gate is still `executes:false`; moving warn→reject to full `proven` needs the NL probes below. (2) **Step 5 — NL probes** for the `proven` column (F5 description-quality signal, $0 on local gemma; not the workflow gate). (3) **Remaining residue** (84 `null`, none blocking): authoring-run needs a paid confirm, `job_id` family needs real async jobs (spend), kg-graph tools need an adopted ontology (deeper fixture) — characterized in the capability-sweep doc; the cheaply-seedable clusters (credential, registry-slug) are now done.
...
- **✅ Stage 4 BACKEND SHIPPED + D3 live-proven on dev, 2026-07-11 (RB-4-BE).** FE half (22-C + 24-H2.2) is the follow-on. 3 build agents → parent SC4 read-path fix → `/review-impl` (4 dims, 3-lens refute-verify).
  ...
  - **▶ NEXT within Stage 4 (FE wave):** `22`-C1..C5 (`scene-browser` + `scene-inspector` panels — the inspector renders SC4 intent now that the read path carries it; `SceneRail` identity + inline title; i18n) · `24`-H2.2 (the deterministic lane-layout pure fn, headless-tested, zero backend dep) · `22`-D1/D2 (OpenAPI for the book-service scene routes + `frontend-tools.contract.json` regen). Then RB-4-FE. FE verify = `tsc --noEmit` + vitest + a **built-image** browser smoke (`frontend-5174-is-baked-prod-nginx-not-vite`).
  - **⚠ Stage 3 (`25` Deploy 2, M4/M5) stays P-4-gated** — build + rehearse on a snapshot only, never run on dev unattended (deletes arc rows, no rollback).
...
- **✅ Stage 5 CORE SHIPPED + RB-5 live-proven cross-service, 2026-07-11 (RB-5).** The `26` indexing/staleness spine. 3 build agents (one per service) → parent contract reconcile → `/review-impl` (3 dims, 3-lens refute-verify). 26-D provenance + 26-F FE chips are follow-ons.
  ...
  - **✅ 26-D1 SHIPPED (`141641a73`):** `outline_node.source`/`decompile_key` + `structure_node.source` (IX-11); the decompiler stamps `source='decompiled'` + the key on mint and **never overwrites an authored node** (reports `skipped_authored`); `source` is readable for the "mined" badge. 7 decompiler DB tests; migration applied to dev. **Remaining 26-D follow-ons:** D2 (import-tail `source_scene_id` write-back, IX-12 — touches book-service parse.go + worker-infra) · D3 (`arc_import_analyze` stamps `source='decompiled'` on minted `structure_node`s). Small, same toolchain.
  ...
  - **26-D1 review — 2 accepted observations (deferred, each earns its gate):**
    - **`D-DECOMP-REIMPORT-RESURRECTS-DELETED-SCENE`** (LOW, gate #5 — conscious product decision). Because the probe skips archived rows, a re-import re-mints a scene the user soft-DELETED (a fresh LIVE node beside the tombstone). This is pre-D1 behavior, not introduced by the fix; whether reparse should *respect* a user's scene delete is a product-semantics call for the reparse/scene-CRUD phase, not a correctness bug. Trigger: 22 scene-CRUD FE wave or the first real import-then-delete-then-reimport authoring session.
    - **`D-DECOMP-KEY-COLLIDES-ON-SPEC-BRANCH`** (LOW, gate #3 — naturally-next-phase). `decompile_key` is unique per `(book_id, …)` but scenes only ever mint into the book's **canonical** partition today, so it can't collide across Works. When 23 BA8 spec-**branching** (derivative Works sharing a `book_id`) is built, a branch that copies `outline_node` rows MUST null `decompile_key` on the copy (or scope the key by `project_id`), else the copy collides. Trigger: implementing derivative-Work branch/copy.
  - **▶ THEN: the consolidated FE wave** (needs a real browser smoke — a session with browser tooling does this best): `22`-C1..C5 (scene-browser + scene-inspector, which now renders SC4 intent) · `24`-H2.2 (lane-layout pure fn) · `26`-F1..F3 (state chips: rail/inspector/browser/Hub) · `22`-D1/D2 (OpenAPI + contract regen). Then Stage 7 (`24` the Hub — its H8.2 browser smoke is the pillar-24 DoD).
  ...
  - **Stage-1 deferred (2 rows, both earn their gate):**
    - **`D-ENGINE-ERRORED-JOB-MARKED-COMPLETED`** (MED, gate #1 — out of scope: engine error-path, not the re-key). Observed live in T6: a `draft_scene` whose model failed to resolve emitted `{"type":"error"}`, then `{"type":"done","status":"completed"}`, and the `generation_job` row landed `status='completed'` with `output_tokens=0`, `result->>'error'` NULL. A retry/idempotency layer would treat it as done. **Proven pre-existing, not a Wave-1 regression**: `git diff routers/engine.py` shows only the mechanical actor-arg drop on `jobs.update_status` — the `completed`/`failed` choice is untouched (`engine.py:605/668` mark completed after the stream loop regardless of an emitted error event). Fixing it needs the SSE error taxonomy decided (a mid-stream error after partial content may legitimately be `completed`+`truncated`; a zero-token resolve failure is not). Trigger: the next engine/streaming cycle. Repro: POST `/v1/composition/works/{p}/generate` with a `model_ref` the caller does not own.
    - **`D-PKGREKEY-DDL-SCHEMA-QUALIFIER`** (LOW, gate #5 — conscious won't-fix for now). `package_rekey.py`'s guards query `information_schema.columns WHERE table_name = '…'` with no `table_schema` filter, so a same-named table in another schema could satisfy a guard. This DB has only `public`; adding the qualifier is trivial but touches every guard, and the migration is now marker-stamped on dev. Trigger: any multi-schema deployment, or the next edit to those guards.
