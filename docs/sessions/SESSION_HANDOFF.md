...
**▶ NEXT — debt-paydown roadmap (Wave 1a+1b+2a done; push-approved-per-wave):**
1. **Wave 2 remainder:** `D-2B-DECOUPLE-FLAG-COUPLING` (startup assert), block chunk-rows + `record_stage` port (`D-2B-T3A-BLOCK-CHUNK-ROWS`/`-OBSERVABILITY`), `D-2B-SHELL-UNIT-TESTS`. (`D-2B-RESUME-STATE-SIZE` = perf, defer unless TOAST bites.)
