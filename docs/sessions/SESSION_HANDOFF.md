...
- **▶ NEXT: H5 Row-2/3 + the gesture live-confirm.** (1) **Human/manual confirm** the drag GESTURES end-to-end (Row-1 chapter→lane, Row-4 scene→chapter): open `plan-hub`, expand arcs, drag, watch the DB flip. Every LINK is proven (hit-tests + handlers unit-tested; both mutations + the 412 live-proven; the node accepts a real drag-start) — only the composed d3-drag gesture resists automation (trusted pointer events + `pointer-events-none` bands). (2) **Row-2** arc band reorder/nest (`/arcs/{id}/move`). (3) **Row-3** chapter reorder WITHIN a lane — note the `reorder` route groups siblings by `parent_id`, and post-lift chapters have `parent_id NULL` + bind by `structure_node_id`, so an arc-scoped chapter rank/story_order reorder needs its own semantics (**not** a straight `reorder` call — scope it before building). (4) Optimistic re-place (avoid the brief refetch snap) + undo via `_meta.undo_hint`. **Live-smoke seed recipe (proven):** insert a `composition_work` (project_id→book_id, created_by=test user) FIRST, else every by-id route (`getNode`, `reorder`) 404s on the grant resolve. H7 view-modes deferred (P-10). Remaining i18n: only `en` for `plan-hub` + `planNav`; run `scripts/i18n_translate.py`.
