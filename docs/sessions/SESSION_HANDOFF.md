...
- **Deferred:** `D-MCP-PAYLOAD-DUPLICATION` (#9, gate #2 large/structural) — root cause is the MCP SDKs' OWN default behavior (Go SDK's `AddTool` auto-fills `content` from `structuredContent` when a handler leaves `Content` nil; Python FastMCP does the analogous thing) — no single chokepoint, ~150-250 handlers across ~19 services. Needs a new shared response helper in both `sdks/go/loreweave_mcp` and `sdks/python/loreweave_mcp` + a mechanical sweep; scoped but not built this pass. `D-MCP-ERROR-SHAPE-UNIFICATION-FULL` (#10 remainder, gate #2) — shape #1 (transport-level, intentionally different) and shape #3 (Pydantic, already independently started in knowledge/jobs/translation-service, absent in composition-service + 6 more Python MCP services) need cross-service surgery, not built this pass.
