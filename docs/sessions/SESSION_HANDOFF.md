...
>   - **▶ M4 NEXT (biggest)** — retry = re-submit (new job) end-to-end: SDK `ControlCap.RETRY` + jobs-service `VALID_ACTIONS`/`derive_control_caps` (valid only from `failed`) + each owning service re-creates a fresh job from stored request params + FE button. Cross-service control seam + FE.
>   - **Live-smoke:** `D-B1-LIVE-SMOKE` (model NAME lands in projection; live campaign cost) → folds into B3.
