...
**▶ M5 — video-gen decouple — DESIGNED, ready to BUILD (next).** PO CLARIFY decision LOCKED (2026-06-13): **Full DB, mirror M1.** Plan: [`2026-06-13-…-m5-videogen-decouple.md`](../plans/2026-06-13-llm-rearch-phase3-m5-videogen-decouple.md). **Key finding:** `Client.generate_video()` is just `submit_job(operation="video_gen")` + `wait_terminal()` — the gateway ALREADY runs video as a job with a `provider_job_id` + emits `loreweave:events:llm_job_terminal` (Phase 1), so M5 = the **same terminal-event consumer pattern as M1**, NOT a gateway change. video-gen-service is **entirely stateless today** (no DB/Redis), so M5 adds: `loreweave_video_gen` DB + `video_gen_jobs` table + a terminal-event consumer (download→MinIO→mark done) + `GET /v1/video-gen/jobs/{id}` poll + a `video-gen-worker` compose svc, all flag-gated `VIDEO_GEN_DECOUPLE_ENABLED` (default off → inline 201 verbatim). **Contract change** (callers poll). FE buries submit+poll in the api method (M2/M4-FE pattern). BUILD order + schema + gotchas in the plan. **Started at ~80% context → /compact recommended before BUILD.**
