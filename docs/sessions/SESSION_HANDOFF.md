...
**State: P0–P6 ALL BUILT + verified — the glossary-assistant arc is COMPLETE.** Read + edit-existing + new-entity draft + schema-create, all human-gated, skill-guided, reachable on every book surface; grounding now consolidated behind the gateway. The ai-gateway is the single AI integration layer (tools + grounding). Remaining: deferred full-stack/browser smokes (067/068 + D-GLOSSARY-EDIT/SCHEMA/SURFACES/GROUNDING-SMOKE) + the glossary LLM-flow migration review (DEFERRED 066).
...
**NEXT:** **the glossary-assistant arc (P0–P6) is DONE.** Immediate follow-ups when the docker stack is up: run the deferred full-stack/browser smokes (067 + 068 + D-GLOSSARY-EDIT/SCHEMA/SURFACES/GROUNDING-SMOKE) — these rebuild the ai-gateway + glossary + chat images and exercise the real chains end to end. Then the **glossary LLM-flow migration review** (DEFERRED 066): now that the glossary MCP tools exist, migrate the legacy prompt-driven glossary flows off raw HTTP. (No further glossary-assistant build phases remain.)

**Deferred (accepted, from /review-impl):**
- **067 / 068** — full-stack docker smokes (chat→gateway→knowledge / →glossary) — prod-traffic gates.
- **D-GLOSSARY-GROUNDING-SMOKE** (P6) — real chat→gateway→knowledge grounding chain (gateway-first + fallback) on a stack-up; needs ai-gateway + chat image rebuild. The proxy + fallback matrix is unit-verified (MockTransport routes by host).
- **D-GLOSSARY-SURFACES-SMOKE** (P5) — browser smoke: the assistant reachable + behaving (skill-guided) via the dock on the glossary page + reader. The skill/cap are unit-verified.
- **D-GLOSSARY-SURFACES-LOW1** (P5) — the `BookAssistantDock` unmounts when navigating GlossaryTab sub-views (kind editor / genres / inbox) — the book-scoped chat session persists server-side so reopening resumes the conversation; only an in-flight stream is dropped. Lifting the dock above GlossaryTab's early-returns needs a tab refactor. LOW.
- **D-GLOSSARY-SURFACES-LOW2** (P5) — the skill is injected whenever book-scoped + tools-enabled even if glossary federation is down (gateway returned only memory tools) → the prompt references glossary tools that aren't advertised (degraded, not broken). LOW.
- **D-GLOSSARY-SCHEMA-LIVE-SMOKE** (P4) — full chat→propose→`glossary_confirm_schema`→FE-Confirm→`/v1/schema/confirm` chain (needs glossary image rebuild + Playwright). The token+confirm *server* path is real-PG-verified.
- **D-GLOSSARY-SCHEMA-SINGLEUSE** (P4) — confirm token is stateless (no DB store) → replay within the 10-min window re-runs create (bounded by code-uniqueness → 409). Add a consumed-token store if strict once-only is wanted. LOW.
- **D-GLOSSARY-SCHEMA-LOW1** (P4) — `createKindFromParams`/`createAttrDefFromParams` cores don't re-validate code/name non-empty; both current callers validate upstream (manual handler + propose tool). Add a guard if a future caller skips it. LOW.
- **D-GLOSSARY-EDIT-LIVE-SMOKE** (P3) — full chat→suspend→FE-Apply→BFF→glossary→resume chain (needs glossary image rebuild + Playwright Apply). The H5 *server* path is real-PG-verified; the BFF `If-Match` forward is config-verified (CORS allowlist + http-proxy-middleware default-forward).
- **D-GLOSSARY-EDIT-ATOMIC** (P3) — multi-field-in-one-card edit (would fork the write/trigger logic into a new transactional endpoint). Today: one PATCH target per proposal (atomic).
