...
- **▶ NEXT:** build starts at **00B Stage 1** = `25` Deploy 1 (M0 fail-loud pre-flights → M1 expand → M2 backfill → M3 cutover incl. P-3's `.runs/` VIEW widening), `22-A1` in the parallel lane, **RB-1 commit + PO checkpoint** after T1–T6 — the highest-risk boundary in the plan. Stage-1.1 carries the task-1b residue notes already stamped into 22.
