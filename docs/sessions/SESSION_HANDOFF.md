...
**CHAT & AI SETTINGS UNIFY — M1a SHIPPED 2026-07-05 (spec [`docs/specs/2026-07-05-chat-ai-settings.md`](../specs/2026-07-05-chat-ai-settings.md); user authorized a full end-to-end autonomous run, audit-at-end).** The backend resolver + storage spine and the FE shared-model source are built, tested, committed (`0172a4514` backend, `5944da242` FE). **What works now:** (1) `user_chat_ai_prefs` (Per-user Account-tier blob) + session override columns + the pure cascade `settings_resolution.py` (Session▸Book▸Account▸System, field-by-field deep-merge with null=clear, per-tier model liveness → skip-dead/name-skipped/all-dead⇒no_model_configured); (2) `GET/PATCH /v1/chat/ai-prefs` (412 on version conflict, deep field-merge) + `GET /v1/chat/effective-settings` (resolved cascade, de-silenced System defaults VISIBLE); (3) FE `features/chat-ai-settings` (ChatAiSettingsProvider hoisted OUTSIDE LiveStateProvider, memoized; `useEffectiveModel(role)` optional); (4) every studio tool inherits through the CompositionPanel hub — **the studio now has real two-tier: book override (FE work.settings) wins over account default (backend resolver)**. Tests: 17 resolver+DDL, 5 router mock, 4 real-PG (deep-merge/version), 3 FE context, full chat suite 998 + composition 791 green. My FE diff is tsc-clean (1 pre-existing `i18n/index.ts` error is unrelated). **NEXT (this run continues):** M2 Models+Behavior consolidated panel → M3 grounding toggle (short-circuit the gate-off force-on) → M4 voice unify → M5 context panel. **DEFERRED (tracked, gate #2 large/structural — cross-service contract):** `D-CHATAI-M1B-BOOK-TIER` — the backend resolver's Book tier is a present-but-stubbed slot; wiring it for chat sessions + the grant-gated SHARED-BOOK owner-read needs book-service `owner_user_id` on `/internal/books/{id}/access` (Go) + the `loreweave_grants` SDK + a composition internal `model-settings` route + a chat→composition client. The studio already has two-tier via M1a's FE, so this is a backend-only correctness refinement for chat-session + collaborator cases.
