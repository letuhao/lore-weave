...
>   - **▶ NEXT — P3-2+:** wire translation/composition/video-gen/lore-enrichment onto the same `/internal/{svc}/jobs/{job_id}/{action}` contract + add each to jobs-service `control._CONTROL`; then the **reconcile sweep** + `GET /internal/jobs?since=` per service (`D-JOBS-P2-RECONCILE-CROSS-SVC`). Then **P4: the unified Jobs GUI**.
