...
>
> **▶ THIS BRANCH'S NEXT — glossary-assistant coverage campaign.** Make the assistant cover all scenarios (S1–S35). Architecture **LOCKED** (decisions **D1–D13**, 3 campaign specs: `2026-06-10-glossary-assistant-{scenario-coverage,extended-scenarios,architecture-review}.md`). **Phase -1 E0 (collaboration epic) IN PROGRESS — E0-0 + E0-1 shipped (see ▼ E0 PROGRESS).** **Start next:** **E0-2 (book-service self-adopt)** — book honors grants on its own per-book endpoints (reads/PATCH grant-gated; "my books" = UNION owned+collaborated; delete/trash/purge/transfer/visibility stay owner-only). Then E0-3 (knowledge, IDOR-heavy) · E0-4 (translation/campaign/composition) · E0-5 (FE panel). **After E0-0+E0-1, campaign F1 is satisfied.** (Independent **H-C async-delivery spike** also available.) The full campaign block + decisions live in the **"▶ (merged from main) glossary-assistant"** section below (deferred list carries `GLOSSARY-ASSISTANT COVERAGE CAMPAIGN`, `D-MCP-TASKS-MIGRATION`, `D-MCP-DIRECT-RETURN`).
>
...
>   - **E0-1 deferred (carry forward):** **D-E0-1-NEEDMAP-REVIEW** — `reassignEntityKind` mapped to **manage** (destructive: orphans kind-specific attrs); reconsider `edit` if kind-review proves too gated. **D-E0-1-WIKI-COST-AT-EDIT** (review LOW) — `generateWikiStubs` at `edit` lets a collaborator spend the owner's BYOK on AI wiki-gen (consistent w/ translations=edit; accept). **404→403** for missing book on glossary routes (intentional anti-oracle; FE treats both as no-access).
> - **✅ D-E0-LIVE-SMOKE CLEARED** (this session, real docker stack). Rebuilt book-service + glossary-service images (stale — predated E0) — **glossary Dockerfile needed `COPY sdks/go/grantclient`** (the E0-1 `replace` dir; build failed without it → fixed). Ran the full cross-service flow (book :8205 + glossary :8211, dev-secret JWTs): **B 403 before grant → allowed after grant** (none never cached → instant); **A revokes → B converges to 403 in ~66s** (60s positive-cache TTL + poll/offset; AC4 "≤60s" is nominal-TTL — tune `CacheTTL`→~45s if strict ≤60s wall-clock matters, row `D-E0-CACHE-TTL-TUNE`); **lifecycle gate: trashed book → edit 409, read 200**. Validates the DB-backed grant resolution + book↔glossary `/access` HTTP contract + cache TTL + lifecycle gate that unit/mock tests can't.
 - **NEXT: E0-2 (book-service self-adopt)** → E0-3 (knowledge) · E0-4 (translation/campaign/composition) · E0-5 (FE panel).
>
