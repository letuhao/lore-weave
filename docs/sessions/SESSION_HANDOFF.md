...
- **▶ NEXT:** **H5 drag mechanics (FE)** — enable React-Flow drag on chapter/scene nodes; on drop, map the drop position → target lane/rank, dispatch to the right REST mirror (chapter→lane = assign-chapters; scene = reorder), optimistic + refetch shell → laneLayout recomputes; 412 → reload+toast (SceneRail recovery precedent); undo via `_meta.undo_hint`. Wire one PH20 row at a time, each with an OCC test. Real proof = **H8.2 live browser smoke** (the pillar DoD, NOT yet run) — needs a **seeded book with `structure_node` arcs** (the dev composition DB currently has NONE — decompose one or SQL-seed) + FE served with the plan-hub code (baked `:5174` image is stale/pre-plan-hub → `vite dev` on a free port, avoid the concurrent `:5199/:5200`) + browser drive: open `plan-hub` → canvas renders → drag chapter across lanes → DB `structure_node_id` changed → overlay badge updates (spec §DoD line 264). H7 view-modes deferred (P-10). Remaining i18n: only `en` for `plan-hub` + `planNav`; run `scripts/i18n_translate.py`.
