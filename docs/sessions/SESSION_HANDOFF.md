...
- **▶ NEXT:** **H5 (drag mutations)** — reorder/reparent a node on the canvas → OCC PATCH; **needs the outline REST write-mirror verified first** (`composition_outline_node_update` / a move route + `expected_version`; RB drag → drop-target lane/rank → PATCH → shell refetch → laneLayout recomputes). Then **H8** (10k-fixture perf/EXPLAIN — the structure-axis query must use `idx_outline_node_structure_keyset` — + the H8.2 live browser smoke, the pillar DoD NOT yet run: rebuild FE + composition images first, `live-smoke-rebuild-stale-images-first`; the camera-pan + you-are-here are the smoke's visible checks). H7 view-modes deferred (P-10). Remaining i18n: only `en` for `plan-hub` + `planNav`; run `scripts/i18n_translate.py`.
