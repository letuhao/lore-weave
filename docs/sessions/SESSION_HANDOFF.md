...
> - **▶ NEXT (P1 cont.):** (1) migrate remaining **Family 2** — learning `events/consumer.py` (retry→DLQ), campaign `events/consumer.py` + `spend_consumer.py` (`ack_on_error=True`), learning `eval_runner.py` (single-stream `start_id="$"`, `ack_on_error`). (2) **Family 3** functional job-workers (composition `job_consumer`, lore-enrichment `resume_consumer`, worker-ai `summary_consumer`) → **`BaseTerminalConsumer` with `start_id="0"`** + function→class. (3) **worker-ai `llm_extract_consumer` money-path** — serial, flag-gated, `/review-impl`, full extraction E2E. (4) **emit-wiring** (per-service `outbox_events` table + `OUTBOX_SOURCES` + `emit_job_event` — shared compose/relay → SERIAL). (5) batched live-smoke `D-JOBS-MIGRATE-LIVE-SMOKE`. Checklist (strip internal ack/retry; verify UUID job-id) in the P1 plan.
