...
**▶ NEXT — 2b-T2 INTEGRATION (turnkey; the engine + tests de-risk it; do with FRESH focus — it touches the hot-path finalization + needs a live-smoke, NOT a session-tail cram):**
1. **`finalize_cb`** — the post-translate block is `chapter_worker._process_chapter:232-331` (persist completed+body+tokens → increment job counter → INSERT active_chapter_translation_versions with the campaign-aware `_on_conflict` → outbox `chapter.translated` → `_emit_translation_quality` → `_save_chapter_memo` → `record_stage` → `_emit_chapter_done` → `_check_job_completion`). For the TEXT path: `translated_body_text=body`, json=None, format="text", `memo_text=body`. **Two options:** (a) EXTRACT `_finalize_text_chapter(...)` from that block, called by BOTH the legacy path and the consumer (cleaner, but edits the monolith) — RECOMMENDED with care; (b) a flag-gated DUPLICATE finalize in the decoupled module reusing the same helpers (`_insert_outbox_event`/`_emit_chapter_done`/`_check_job_completion`/`_save_chapter_memo`/`_emit_translation_quality`) — additive, but tracked-debt `D-DECOUPLE-FINALIZE-DUP`.
2. **The consumer** (`app/events/llm_terminal_consumer.py`, model on `glossary_consumer.GlossaryStaleConsumer`): stream `loreweave:events:llm_job_terminal`, group `translation-llm-resume`, `socket_timeout=None`, `id="$"`, BUSYGROUP-safe, ack-on-success/bounded-retry. `_handle`: read `job_id` from the wire fields → `SELECT id FROM chapter_translations WHERE provider_job_id=$1` (ignore+ack if not ours — other services share the stream) → `llm_client.get_job(job_id)` for the full Job → `decoupled_translate.resume(pool, llm_client, job, chapter_translation_id, finalize_cb=…)` → ack.
3. **Flag** `translation_decouple_enabled` (config, default False) + rewire `_process_chapter`: under the flag AND the TEXT path (line 215), call `decoupled_translate.start_chapter(...)` + RETURN (chapter now in-flight; the consumer drives it) instead of `await _translate_text(...)`. Store `source_lang` in resume_state (finalize needs it). Legacy path unchanged when the flag is off.
4. **Wire the consumer** into `main.py` lifespan (alongside `GlossaryStaleConsumer`), gated on the flag.
5. **Live-smoke** a real chapter through the event path (flag on) → assert it translates + finalizes + emits `chapter.translated` via the consumer (no held coroutine). `D-2B-T2-LIVE-SMOKE`.
THEN reuse the engine for the BLOCK path (`translate_batch_with_retry`) + V3 verify/correct (**2b-T3** — the campaign uses v3-block). Then **2b-worker-ai** (extraction, HARD — needs `loreweave_extraction` per-op resume points). Or Phase 3.
