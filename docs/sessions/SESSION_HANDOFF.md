...
- **`D-CAMPAIGN-RECONCILE-LIVE-SMOKE`** — full force-a-stuck-row → self-heal E2E on a real stack: dispatch, drop the completion event, confirm the reconcile marks it `done` from downstream truth within one timeout window (needs campaign+worker-ai rebuild + a lowered `stuck_dispatch_timeout_s` + the heavy-inference stack). Subsumes the live observation of `D-S5BEVAL-LIVE-SMOKE`'s `eval_fidelity_score`.
