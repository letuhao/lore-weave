...
**▶ NEXT — debt-paydown roadmap (Wave 1a done; push-approved-per-wave):**
1. **Wave 1b — stuck-resume sweeper** (closes `D-WX-SUBMIT-PERSIST-GAP` + `D-2B-SUBMIT-PERSIST-GAP`; periodic re-drive of stale resume_state rows). Code-only, no stack.
2. **Wave 2 — translation parity + observability:** `D-2B-DECOUPLE-FLAG-COUPLING` (startup assert), block chunk-rows + `record_stage` port (`D-2B-T3A-BLOCK-CHUNK-ROWS`/`-OBSERVABILITY`), `D-2B-SHELL-UNIT-TESTS`. (`D-2B-RESUME-STATE-SIZE` = perf, defer unless TOAST bites.)
