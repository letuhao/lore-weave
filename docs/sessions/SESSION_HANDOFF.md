...
**Residuals (LOW, tracked — none block):** (1) merge-on-create is a conscious won't-fix but its `skipped_exists` result carries no per-attribute "discarded" list, so a weak model could silently lose passed attributes (`glossary/internal/api/mcp_server.go:522`); (2) `glossary_confirm_action` doc-drift is genuinely unaddressed — reclassified to Track A (a FRONTEND tool, not a Track-B file), so it is NOT "fixed", just moved; action it under A or record an explicit won't-fix so it stops re-surfacing; (3) a map base-image re-uploaded with a DIFFERENT extension orphans the prior MinIO object (best-effort hygiene; `world_map_delete` cleans the current key); (4) `world_map_delete` is Tier-A (no confirm) — deliberate, consistent with the owner-only/no-sharing world+map surface (blast radius = one re-creatable map).
