...
**▶ NEXT — debt-paydown roadmap (Wave 1+2 DONE):**
- **Wave 3 — bounce stack + live-smokes** (user bounces `docker compose down && up` for `D-WX-DNS-INFRA`): `D-WX-LIVE-SMOKE` (extraction decouple end-to-end persist) + `D-2B-TRANSL-SWEEP-LIVE-SMOKE` (translation stranded→re-drive) + `D-WX-RUN-SAMPLE-DECOUPLE`, then `/review-impl` the money-paths → flag default-on.
