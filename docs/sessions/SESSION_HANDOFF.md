...
>   - `ChapterRangeStep` (campaign) + StepBatchConfig **all/range** — range-by-`sort_order` predicate selection; needs a **`range` mode** the browser doesn't have yet. **Defer.**
>   - **Deferred (genuine, not over-fit):** `D-CHAPTERBROWSER-RANGE-MODE` (range mode → migrate campaign + StepBatchConfig all/range), `D-CHAPTERBROWSER-CONTEXTPICKER` (per-book multi). The matrix/ChaptersTab/Reader/Editor sites are **won't-migrate** (the browser is the wrong shape for them).
