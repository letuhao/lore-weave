...
**NEXT options:** (1) **M7c** — Channel-1b, **build the human-edit-translation feature** + before/after gold + **name-confirm** capture (the richest signal; biggest slice — adds a BE save-edit endpoint + FE editor) · (2) **M7d** — Channel-3 online LLM-judge · (3) live-smokes (M7a/M7b/M6b-worker-usage/M4d1) on a rebuilt stack · (4) PR `feat/translation-pipeline-v3` → `main`. **PO: do NOT merge to main until translation is production-ready** — the M7 feedback track is part of that gate. V3 core feature-complete; M7a✓ M7b✓; M7c/d + live-smokes remain.
