...
- **NEXT = build M4** (composition worker + queue FROM SCRATCH — composition-service has NO worker, only a job reaper; build a Redis-group consumer + resume pattern mirroring lore-enrichment, move `decompose`/`stitch` + auto-mode chapter-gen off the request path; streaming cowrite stays inline. The big lift — real value).
