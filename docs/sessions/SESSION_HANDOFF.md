...
- **✅ Stage 5 CORE SHIPPED + RB-5 live-proven cross-service, 2026-07-11 (RB-5).** The `26` indexing/staleness spine. 3 build agents (one per service) → parent contract reconcile → `/review-impl` (3 dims, 3-lens refute-verify). 26-D provenance + 26-F FE chips are follow-ons.
  ...
  - **▶ NEXT (backend, tractable to this bar): 26-D provenance** = `source`/`decompile_key` columns + the IX-11 never-overwrite-authored predicate in the decompiler (so a re-decompile can't clobber an authored node) + `mappings[]` + the import-tail `source_scene_id` write-back (IX-12) + `arc_import_analyze` stamps `source='decompiled'`. Small, on the same toolchain.
