...
>   - **▶ M2 NEXT** — translation `cost_usd` column (migration) + capture/accumulate/emit + real-PG token-SUM test (`D-JOBS-P4-TRANSLATION-COST`, `D-JOBS-P4-TRANSL-TOKENS-PG`). Then M3 (summary parent/child + FE overlay-evict), M4 (retry=re-submit end-to-end).
