...
**▶ NEXT — W6b (the LAST gap-closure item): true old→new source diff snapshots (XL, own /loom).** The change-feed's "xem thay đổi" per-staleness-row diff (audit gap #3 sub-item). **Finding from W6a CLARIFY:** `wiki_staleness.source_ref` is only `{source_type, source_id, event_id}` (a reference), and `build_inputs` stores a fingerprint **hash, not old source text** → **no stored before-state**. **PO chose to build true snapshots (XL+).** Likely shape: the "before" = the **`wiki_article_source_usage` snippet** captured at generation time (verify this holds the text), the "after" = the current source content (cross-service fetch per `source_type`: entity→glossary, chapter→book-service); render via the W1 `wikiDiff` lib. Scope CLARIFY at the start of that `/loom` (which source_types to support; degrade for kg/recipe drift which have no text "before"). **W1–W5 + W6a shipped — only W6b (+ live-smokes + `D-WIKI-W4-RESULTS-64KB`) remain.** Other moves: a **PR** for `wiki/phase2-change-control` → main, or the **platform system-config UI** epic below once its auth-conflict clears.
