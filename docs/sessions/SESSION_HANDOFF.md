...
- **`D-FACTORY-INFLIGHT-LOG`** (#4 residual, LOW) — the timestamped recent-activity log half of the in-flight panel; needs a per-chapter activity event feed we don't emit (the stage-level panel shipped; the log did not).
- **`D-FACTORY-SWITCH-VERIFIER-EVAL-UI`** (new, LOW) — the switch-model UI exposes translation+knowledge only; a campaign with an *explicit* cloud verifier/eval-judge can't switch those from the panel (BE PATCH accepts them → API-reachable; verifier follows the translator when unset, so the common case is covered).
