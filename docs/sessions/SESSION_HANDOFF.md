...
**NEXT = Q3.5 judge calibration (gates Q4):**
- Consume Q2 gold_labels (`GET /v1/learning/gold-labels`): compute judge-vs-human agreement (balanced accuracy + Cohen κ) per judge; require ≥75-90% before a judge panel is trusted as metric-of-record. Enforce the disjoint-median exclude-generator rule in code (`loreweave_eval`, already the default via `panel.excluded`). Block Q4 online-judge trust until a panel passes. (S/M — likely a `loreweave_eval` calibration module + a learning-service endpoint/threshold.)
