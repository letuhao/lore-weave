...
**PLAN HUB v2 (spec 24, Stage 7) — Phase H1 backend read surfaces SHIPPED, 2026-07-11** (branch `feat/context-budget-law`; built via a 3-slice parallel fan-out + serial integrate/verify). The Hub canvas data-contract layer is now live; H2 (FE canvas core) is next.
...
- **▶ NEXT: Phase H2 (canvas core).** Add `reactflow` (renders nodes at the FIXED positions `laneLayout` already computes — RF is the viewport/pan/zoom/edge shell, NOT its auto-layout), `plan-hub` panel shell + `panel_id` enum add (chat-service `frontend_tools.py:400` + `WRITE_FRONTEND_CONTRACT=1` regen), `usePlanWindows` keyset loader + rollup cards, RF node/edge components over `laneLayout`'s `NodePosition[]`, `useActualState` two-truths join, bus wiring. Then fan out H3(drawer)∥H4(decorations)∥H5(interactions)∥H6(rail); H7 deferred; H8 perf+live-smoke. Prereqs confirmed present: arc-list `GET /arcs` (read surface #1) + conformance/status (#7).
