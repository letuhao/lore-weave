...
> - **▶ NEXT (P1 final — stack-dependent):** (1) **flip + live-smoke the money-path** — `extraction_consumer_use_sdk=true`, rebuild worker-ai, run a real decoupled extraction (entity→trio→persist, no double-spend), then make it default. (2) **emit-wiring** (per-service `outbox_events` table + `OUTBOX_SOURCES` + `emit_job_event` — shared compose/relay → SERIAL; needs stack). (3) batched live-smoke `D-JOBS-MIGRATE-LIVE-SMOKE` (rebuild ~12 images, confirm every group boots). ⛔ campaign `spend_consumer` stays excluded (`D-JOBS-SPEND-CONSUMER-MISFIT`).
