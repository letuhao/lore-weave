...
>
> **Deferred — shared chapter-list browser + `limit>100→20` bug:** `docs/specs/2026-06-14-shared-chapter-list-browser-epic.md`. `book-service parseLimitOffset` returns the DEFAULT 20 (not a clamp) for `limit>100`, so TranslateModal(200)/TranslationTab(200)/ExtractionWizard(500)/ChapterEditorPage(200)/campaign ChapterRangeStep(5000) all silently get **20 chapters** — the "translator shows only 20" report. Fix = a 1-line clamp (B0) + a paginated `<ChapterListBrowser>` (B1) reused across ~8 call sites. **B0 SHIPPED** (2026-06-14, with the translator-wizard work above — `parseLimitOffset` now clamps to 100). **B1 still deferred to avoid conflict with the concurrent RAID** (composition/* + ChapterEditorPage). The import flow above is separate (client-side file list, not the server browser).
