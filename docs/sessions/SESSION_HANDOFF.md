...
> - **▶ NEXT — P1 Milestone 2: migrate `video-gen` consumer** onto `BaseTerminalConsumer` (pattern-proof) + wire `emit_job_event` (video-gen needs a new `outbox_events` table + `OUTBOX_SOURCES` relay entry) + transport live-smoke; then translation/learning/knowledge/composition/campaign/lore-enrichment, **worker-ai money-path LAST** (full extraction E2E). Each: behavior-preserving, flag+fallback, live-smoked, checkpoint-committed. Migration checklist (strip internal ack/retry from each `handle`; verify UUID job-id) in the P1 plan.
