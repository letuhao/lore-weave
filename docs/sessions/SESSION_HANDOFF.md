...
- **▶ NEXT:** **H5 (drag mutations)** — needs the outline REST write-mirror verified (reorder/reparent → OCC PATCH); **H2.6 bus** (selection publish + editor active-chapter subscribe) + **camera focus (OQ-5)** — unblocked now that `chapterId` + `selectedId` are on the canvas; then **H8** (10k-fixture perf/EXPLAIN — the structure-axis query must use `idx_outline_node_structure_keyset` — + the H8.2 live browser smoke, the pillar DoD NOT yet run: rebuild FE + composition images first, `live-smoke-rebuild-stale-images-first`). H7 view-modes deferred (P-10). Remaining i18n: only `en` for `plan-hub` + `planNav`; run `scripts/i18n_translate.py`.
