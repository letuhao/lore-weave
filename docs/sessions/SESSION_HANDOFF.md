...
**▶ NEXT — debt-paydown roadmap (Wave 1+2 DONE; Wave 3 WX live-smoke PASSED):**
- **Wave 3 remainder:** `D-2B-TRANSL-SWEEP-LIVE-SMOKE` (translation stranded→re-drive — the stack is up + flags on; induce a stranded chapter or accept the sweeper-started + flag-guard live evidence) + `D-WX-RUN-SAMPLE-DECOUPLE` (port `persist_run_sample`) + `/review-impl` the WX money-path → then flag default-on.
