...
**▶ NEXT — debt-paydown roadmap (Wave 1a+1b+2a + 2-remainder-A done):**
1. **Wave 2 remainder B:** block chunk-rows + `record_stage` port into the decoupled block engine (`D-2B-T3A-BLOCK-CHUNK-ROWS` / `-OBSERVABILITY`) — observability parity with the sync path (per-batch `chapter_translation_chunks` rows: validation_errors/warnings, glossary_corrections, retry_count + `record_stage("translation.batch")`). (`D-2B-RESUME-STATE-SIZE` = perf, defer unless TOAST bites.)
