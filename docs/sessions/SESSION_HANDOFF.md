# Session Handoff — `feat/auto-draft-factory-gaps` · blocked-bucket unblock: M1 D-PHASE5E DONE, M2 D-S5BEVAL-LEARNING-OUTBOX in flight · 2026-06-18
...
>   - **▶ M2 `D-S5BEVAL-LEARNING-OUTBOX` (NEXT) — transactional outbox for the `translation.eval_judged` emit.** Currently best-effort XADD ([decoupled_judge.py:393](../../services/learning-service/app/judges/decoupled_judge.py#L393) + handlers.py mirror) → a lost emit drops a campaign `eval_fidelity_score`. Add `outbox_events` to learning-service's idempotent DDL ([migrate.py](../../services/learning-service/app/db/migrate.py)); swap the terminal XADD for an in-tx outbox INSERT in the judge-persist tx (ordering already crash-safe per /review-impl MED#1); register learning-service as a worker-infra relay Source → republish to `loreweave:events:translation_eval` (campaign projection upserts the score → redelivery-safe). Schema change → migration care + /review-impl; PO authorized /amaw for it.
