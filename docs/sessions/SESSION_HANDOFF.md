...
**State: Phase-1 + 2 + 3(A,D) + P3-EVAL harness + E5 tuning + E5B rerank + E6 FE DONE + LIVE-SMOKE PASSED (2026-06-08).** Hybrid (lexical+semantic, RRF) + cross-encoder rerank end-to-end, surfaced in the search UI. New workstream — the missing **raw chapter-text** layer beneath glossary/knowledge/wiki (all lossy derivatives); serves authoring + extraction/trích-lục. Spec [`2026-06-07-raw-search.md`](../specs/2026-06-07-raw-search.md) (PART I design + PART II ATAM-lite eval; 7/7 confirm-at-BUILD resolved). Plan [`2026-06-07-raw-search.md`](../plans/2026-06-07-raw-search.md) (3 phases; ADJ-1..4 PO-acked).
...
- **D-RAWSEARCH-FE-MINOR** (FE-1 LOW-4/COSMETIC) — book-page launch-button strings hardcoded (not i18n); no error/empty/loading-state vitest. (score display ✅ E6.) Batch later.
...
- **D-RAWSEARCH-E5B-LOW** (E5B /review-impl LOW-3/4/5) — (3) `_apply_rerank` silently drops candidates the rerank service omits from its response (we don't send `top_n`, so a conformant service returns all; an *empty* response is safely caught→degrade, a *partial* one isn't); (4) lexical hits reranked on a 160-rune snippet vs semantic full chunk (doc-length asymmetry; live scores fine); (5) no metrics on `/internal/rerank` (embed has them). Accept; revisit if a non-conformant rerank service or observability need surfaces.
- **D-RAWSEARCH-RERANK-LATENCY** (E5B) — warm rerank latency not benchmarked under load; service reported `vram_mb=8` (likely CPU). Confirm GPU vs CPU + measure warm per-call before relying on rerank in a latency-sensitive search box; consider `keep_warm` / pre-warm.
- **D-RAWSEARCH-E6-LOW** (E6 /review-impl LOW-3/4) — (3) no vitest for the api.ts hybrid→lexical fallback forwarding `granularity`; (4) on rerank-*degraded* hybrid the relevance bar shows raw cosine (compressed/uncalibrated, harmless). Accept; add the fallback test when batching FE tests.
- **D-RAWSEARCH-EVAL-CI** (P3-EVAL OUT) — wire the eval to a threshold gate + persist runs to `project_embedding_benchmark_runs`; add a 2nd-book golden + per-band metric breakdown (E5 COSMETIC) for higher-confidence numbers.
- **D-RAWSEARCH-E5-LOW** (E5 /review-impl LOW-3/5) — `relevance` "exact" uses Go `indexRunesFold` while the SQL ranks by Postgres `ILIKE` (agree on CJK; could drift on locale-cased Latin); chapter SQL computes `similarity()` twice (perf only). Accept; revisit if Latin corpora or perf surface it.
...
**NEXT options:** (1) **M6** human-fix flywheel (correction → glossary confirm → targeted re-translate + propagate — the last designed milestone) · (2) **D-TRANSL-EXTRASYSTEM-BUDGET** · (3) **push** (21 commits, needs explicit approval). All 5 original goals delivered; M0–M5 done. M6 is the remaining designed milestone.
...
- **D-TRANSL-M4D** (PO-deferred 2026-06-07) — timeline→memo (`memory_timeline` is MCP/JWT-only — needs a new internal-HTTP endpoint or an MCP client) + 2-pass cold-start mode. Revisit when richer cross-chapter context is wanted; M4a–c don't need it.
