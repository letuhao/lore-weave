# Session Handoff — `feat/auto-draft-factory-gaps` · HEAD `45fffdf5` · 2026-06-15
...
>
> **✅ T2 (PERSISTED BLOCK-RANGE SEGMENTS) COMPLETE — M1+M2+M3 all shipped (2026-06-15).** `D-TRANSL-PANEL-T2-SEGMENTS` done end-to-end (M1 data → M2.1/M2.2 status+dirty-only re-translate → M3 coverage+per-segment glossary staleness+matrix drill-down). The whole translation-panel arc (T1 per-block correction + AC4 banner + T2 segments) is shipped. Remaining deferred live-smokes: `D-TRANSL-T1-PATCH-LIVE-SMOKE`, `D-TRANSL-T2M1-LIVE-SMOKE`, `D-TRANSL-T2M1-BACKFILL-RUN`, `D-TRANSL-T2M2-LIVE-SMOKE`, `D-TRANSL-T2M3-LIVE-SMOKE`. **⚠️ Working-tree has uncommitted RAID `composition/`+`world/` FE changes (not mine — appeared mid-session); commits stage explicitly so they're excluded.**
...
**▶▶ TOP NEXT — UNIFIED JOB CONTROL PLANE (epic, DESIGN approved 2026-06-15).** User pain: long-running background jobs (e.g. a multi-hour glossary/knowledge extraction) with no GUI to see/cancel/pause/resume, and jobs scattered across services (5+ hand-rolled terminal-event consumers). **Spec:** [`docs/specs/2026-06-15-unified-job-control-plane.md`](../specs/2026-06-15-unified-job-control-plane.md). **PO decisions:** (1) aggregation = a **central `jobs-service` projection** (jobs emit lifecycle events → unified projection table → list/search/detail + SSE + control routing), NOT BFF fan-out; (2) build **the shared consumer SDK FIRST**. **Phases:** P1 `sdks/python/loreweave_jobs/` (canonical JobRecord contract + `BaseTerminalConsumer` deduping the 5 copies + `emit_job_event`) + migrate the 5 consumers · P2 `jobs-service` projection + API/SSE · P3 per-service cancel/pause/resume control endpoints (composition/translation/video-gen/lore-enrichment) + ownership backfill + control routing · P4 unified Jobs GUI (generalize `CampaignMonitor`/`MonitorControls`). Survey done: ownership ~80% already present; campaign + knowledge-extraction + llm_jobs already have control; campaign monitor FE is the reusable GUI base. **Immediate relief available NOW:** a stuck knowledge extraction can be cancelled via `POST /v1/knowledge/projects/{id}/extraction/cancel` (or campaign `/cancel`).
