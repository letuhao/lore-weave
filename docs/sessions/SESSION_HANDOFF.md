...
**▶ NEXT — W6b-2: capture-forward true old→new diff (cross-service XL, in progress).** Red/green text diff per staleness row. **Key constraint (W6b CLARIFY finding):** NO before-text is stored today (`source_ref` = ids; `build_inputs`/`source_usage.source_version` = hashes; entities/chapters keep only the current value + a change event). So a true diff is **capture-forward only** — snapshot the source text used at generation time, diff vs the current source; **existing articles can never get one**. PO chose to build it anyway. Likely shape: capture source text at M5 writeback (new storage — e.g. a `source_text` column on `wiki_article_source_usage` or a snapshot table) → a glossary diff endpoint {before: snapshot, after: current source fetched per `source_type`} → FE red/green via the W1 `wikiDiff` lib. Scope CLARIFY at the start (which source_types; storage shape; degrade kg/recipe which have no text before). **W1–W5 + W6a + W6b-1 shipped.** Other moves: a **PR** for `wiki/phase2-change-control` → main, or the **platform system-config UI** epic once its auth-conflict clears.
