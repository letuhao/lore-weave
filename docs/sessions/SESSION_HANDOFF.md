...
- **▶ NEXT: H5 remaining rows + gesture live-confirm.** (1) **Human/manual confirm** Row-1's drag end-to-end (open `plan-hub`, expand two arcs, drag a chapter across lanes, watch the DB `structure_node_id` flip + the card re-place). (2) **Rows 2-4:** arc band reorder/nest (`/arcs/{id}/move`), chapter/scene reorder-within + scene-reparent (`/outline/nodes/{id}/reorder`, OCC via `If-Match` → 412 reload+toast). (3) Optimistic re-place (avoid the brief refetch snap) + undo via `_meta.undo_hint`. The seed helper needs a real **Work** row (project_id→book_id) so the drawer's `getNode` resolves (currently the drawer errors gracefully on a Work-less seed). H7 view-modes deferred (P-10). Remaining i18n: only `en` for `plan-hub` + `planNav`; run `scripts/i18n_translate.py`.
