...
- **NEXT = build M3** (chat disconnect-cancel: the streaming path allocates no job-row, so explicit cancel-by-id needs a streaming-path job-row + a `jobCancels` registry entry + chat `request.is_disconnected()`/`GeneratorExit` → `cancel_job`; NB the slot ALREADY frees on disconnect via `r.Context().Done()`).
