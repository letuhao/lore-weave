...
1. **D-WIKI-M8-FEWSHOT** — inject gold AI→human pairs as few-shot exemplars into wiki generation (`prompt.py` `exemplars` param + a gold-pair fetcher from glossary `wiki_revisions` by `article_id` + thread through `generate.py`/orchestrator), gated `WIKI_FEWSHOT_ENABLED` (default OFF). Cross-service (knowledge ← glossary).
