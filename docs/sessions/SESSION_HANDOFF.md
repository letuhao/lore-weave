...
> - **`D-GLOSSARY-SORT-BE`** — extend `sort` whitelist (name/kind/status/counts/created/alive) + counts-sortability spike. Edits glossary-service `entity_handler.go` (RAID-coordinated).
> - **`D-GLOSSARY-RAW-SEARCH-BE`** — `search_mode=raw` lexical leg (ILIKE-exact + trigram) + `match` snippet/highlights + pg_trgm GIN on `entity_attribute_values.original_value` + `attribute_translations.value`. RAID-coordinated.
