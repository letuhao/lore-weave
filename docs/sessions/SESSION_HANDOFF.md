...
**State: M0 + M1(a–d) + M2 + M3 + M4(a–d) + config-plumbing + M5(a–d) + M6(a + b-1 + b-2) + M7(a + b + c-1 + c-2 + c-3 + d-1) DONE** (M7 feedback→learning: M7a–c✓ M7d-1✓ · M7d-2/d-3 left — the LLM-judge wiring) (**M1d + M4d-1 + M4d-2(a/b/c) + glossary-versioning(VG-1/2/3) + M6b-1/b-2 targeted-propagate DONE 2026-06-07/08**). **🏁 Translation V3 plan-of-record COMPLETE — all milestones M0–M6 shipped incl. the full human-fix flywheel (M6a confirm → M6b-1 targeted per-language stale → M6b-2 surface + user-triggered re-translate); M4d-2 full 2-pass cold-start + the glossary entity-versioning epic landed.**
...
**NEXT options:** (1) **M7d-2** — learning: `online_translation_judge.py` (`run_translation_judge` via the SDK `judge_translation_fidelity` + persist → `quality_scores(translation_judge_fidelity, source=auto, panel_safe=false)` with `origin_event_id='judge:<outbox_id>'` to avoid colliding with M7a's row) + config (`online_translation_judge_enabled` off + model/user) + a hook in `handle_translation_quality` (judge when enabled ∧ the event carries source+translated) + score_config seed · (2) **M7d-3** — translation worker `_emit_translation_quality` carries truncated source+translated text when a feed flag is on (off default) · (3) live-smokes on a rebuilt stack · (4) PR `feat/translation-pipeline-v3` → `main`. **PO: do NOT merge to main until translation is production-ready** — the M7 feedback track is part of that gate. **All 3 feedback channels' human + auto-log signals now collect** (M7a-log, M7b-accept, M7c-edit-gold + reachable FE, M7c-3-name-confirm); only M7d (online judge) + live-smokes + PR remain.
...
- **D-TRANSL-M6B-LIVE-SMOKE** (M6b-1) → **mostly CLEARED 2026-06-08 (live-smoke, rebuilt stack).** A 4-chapter-translation fixture (vi/en × used-E1/used-E2/no-index) + a real glossary translation PATCH proved the consumer's row-selection SQL live: **precise** (used-E1 vi → flagged), **per-language** (used-E1 *en* → NOT flagged), **precision** (used-E2 vi → NOT flagged), **legacy fallback** (no-index vi → flagged). The M6b-2 **coverage** endpoint surfaced `is_glossary_stale` correctly incl. the `latest`-version fallback branch (`/review-impl`'s mock-only concern — now live). **Residual (narrowed):** the **worker writing `chapter_translation_glossary_usage` rows during a real translation** is the only unproven link — it's a best-effort additive INSERT, unit-tested ×3, and its *output* (usage rows) is now proven consumed correctly live. Needs an LLM translation run (LM Studio Qwen3-35B is configured) to fully close; lower-risk.
...
- **D-TRANSL-EXTRASYSTEM-BUDGET** (M4c review-impl, MED → perf/correctness) — the V3 `extra_system` (romanization + knowledge brief + prev-memo block) is appended to the system prompt but NOT counted in `compute_input_budget`; with all three active on a small context window the prompt can overflow → truncation. Thread the `extra_system` token estimate into the batcher budget. Compounds M1c+M4b+M4c.
...
- **D-TRANSL-M7C-LIVE-SMOKE** (M7c-1) — chain `POST .../versions/edit` → human version + `translation.corrected` outbox → relay → `loreweave:events:translation` → learning `handle_translation_corrected` → `corrections` row (raw before/after). Same stack rebuild as M7a/b. Verify a real human edit lands a `corrections` row with the raw LLM/human bodies. Also pins the INSERT (`authored_by='human'`, parent link, `version_num=MAX+1`) which is mock-only in unit tests.
- **D-TRANSL-M7C3-LIVE-SMOKE** (M7c-3) — chain: a user confirm (`PATCH/POST .../translations` `confidence=verified`) → `glossary.name_confirmed` outbox → relay → `loreweave:events:glossary` → learning `handle_name_confirmed` → `quality_scores(glossary_name_confirmed, source=human)`. Same glossary→relay→stream the M6b smoke already proved live; emit-gating + handler unit-covered. Fold into the next stack rebuild.
...
- **D-TRANSL-M7B-LIVE-SMOKE** (M7b) — chain `set_active_version` → `translation.reviewed` outbox → relay → `loreweave:events:translation` → learning `handle_translation_reviewed` → `quality_scores(translation_human_accept, source=human)`. Same stream/relay as M7a (folds into the same stack rebuild); emit/handler unit-covered. Verify a real set-active (esp. `acknowledge_issues=true`) lands a `source=human` row.
- **D-TRANSL-M7A-LIVE-SMOKE** (M7a) — the chain (worker emit `translation.quality` → relay → `loreweave:events:translation` → learning `handle_translation_quality` → `quality_scores(target_kind=translation, source=auto)`) is NOT live-verified. Structurally identical to the Q3 chat-feedback chain (already live-E2E'd); emit/handler/score_config unit-covered both sides incl. the `/100` normalise. Needs a **learning-service rebuild** + either a real V3 translation or a `translation.quality` outbox injection. Do alongside the next stack rebuild.
- **D-TRANSL-M4D1-LIVE-SMOKE** (M4d-1) — still deferred (the other smokes cleared without needing it). The full chain (translation worker → `POST /internal/knowledge/timeline` → `knowledge_projects` resolve → `list_events_filtered` → memo in the Translator prompt) needs an **LLM translation run** + **seeded knowledge timeline events** + worker-log inspection (the memo is injected into `extra_system`, not a DB row). knowledge-service is now **FRESH** on the running stack and the endpoint probes present; endpoint + window math + client + wiring are unit-covered and the query layer is verbatim-reused from prod-proven `handlers.py`. Lowest-priority of the four; smoke alongside the M6b worker-usage run when doing a full LLM-translation pass.
