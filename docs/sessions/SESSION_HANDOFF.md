...
> - **Deferred:** `D-WORLD-PROJECT-BACKLINK` (knowledge-project detail → its book + world backlink; needs the project's book's `world_id`) · `D-WORLD-TIMELINE-ROLLUP` (`/worlds/{id}/timeline` union, OQ1).
