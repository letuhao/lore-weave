...
### §8.14 CM5 design — provenance (minimal) (CLARIFY-locked 2026-06-04)
**Scope:** §0 P4 / §2 CM5 — extraction accepts a `provenance` hint; `pass2_writer` stamps it on nodes. Vocab `{human_authored, ai_assisted, enrichment}` **aligned with enrichment H0, NO enrichment edits**. **No scoring change** (Cycle-0 tags only). Knowledge-internal. Default v2.2. **PO: ACCUMULATE** — a node's provenance is a deduped **array** (`provenances`), each merge contributing its origin (mirrors the existing `source_types` array pattern in the same Cypher).

- **API hint stays SINGULAR per call; the node property is a SET.** `PersistPass2Request.provenance: Literal['human_authored','ai_assisted','enrichment'] = 'human_authored'` (closed-set, 422 on invalid). The node accumulates across calls.
- **`merge_entity` / `merge_entity_at_id` (entities.py) · `merge_event` (events.py) · `merge_fact` (facts.py):** add `provenance: str = 'human_authored'` param. Cypher — exactly the `source_types` shape: `ON CREATE SET x.provenances = [$provenance]`; `ON MATCH SET x.provenances = CASE WHEN $provenance IN x.provenances THEN x.provenances ELSE x.provenances + $provenance END`. (New node property `provenances`; no read/scoring change — V1 composition-critic reads it.)
- **`resolve_or_merge_entity` (entity_resolver.py):** thread `provenance` → both `merge_entity` + `merge_entity_at_id`. Autocreate entities (pass2_writer:429) inherit the call's provenance (real entities from the same source text).
- **`write_pass2_extraction` (pass2_writer.py):** add `provenance: str = 'human_authored'` → pass to the entity (288,429) / event (534) / fact (596) merges. Relations (create_relation, 482) excluded — edges, V0 (plan = facts/entities). The 3 orchestrator call-sites + worker inherit the default (chapters = author-written).
- **`/persist-pass2` route (internal_extraction.py:485):** pass `provenance=body.provenance`.

**Verify (§2 CM5):** unit — `ai_assisted`-hinted merge → `provenances` contains `ai_assisted`; default → `['human_authored']`; ON MATCH dedupe-append (human then ai → `['human_authored','ai_assisted']`; same twice → one); `/persist-pass2` invalid provenance → 422; **enrichment path unchanged** (enrichment writes via `glossary_sync`, NOT pass2 → structurally untouched; assert no glossary_sync edit + a pass2-default-stays-human_authored regression-lock). Neo4j array persistence deferred to integration / D-CANON-CYCLE0-LIVE-SMOKE.

**Self-review:** mirrors the proven `source_types` accumulate pattern (low risk) ✅; singular-hint/array-property split is clean ✅; default keeps every existing caller at `human_authored` (no behavior change) ✅; closed Literal prevents junk values ✅; lore-enrichment untouched (different write path) ✅.

**/review-impl (POST-REVIEW deep pass) — 0 HIGH/MED, 1 LOW folded + 2 documented:** verified the load-bearing risk — `Entity`/`Event`/`Fact` are plain `BaseModel` (no `extra='forbid'`) so `_node_to_*`'s `model_validate(all_props)` **drops** `provenances` on read → no 500, no response/etag drift (a `forbid` config would have 500'd every read post-CM5, invisible to fakes that omit the property). Folded LOW#1: **converter-tolerance regression-locks** (`_node_to_entity/event/fact` fed a node WITH `provenances` → validates clean, property dropped) — the only test that would catch a future `extra` flip. Documented: LOW#2 retract-to-zero-then-recreate loses accumulated provenances (narrow; V0 accept); LOW#3 `merge_entity_at_id` redirect threading + internal-caller provenance is unvalidated string (API boundary validates; internal uses default; accept). Also accepted (self-review): anchor-hit entities skip the stamp (no-round-trip optimization). 6 CM5 tests pass.

### §8.13 CM-FE design — Publish affordance (CLARIFY-locked 2026-06-04)
**Scope:** §8.4 — a Publish/Unpublish control on the chapter editor so normal editors canonize chapters (canon=published now needs it). **FE-only** (gateway `pathFilter '/v1/books'` proxies CM1's `/publish`+`/unpublish` transparently; `getChapter` already returns `editorial_status`+`published_revision_id` per CM1 server.go:1188). Default v2.2. PO: **badge + Publish/Re-publish, no divergence detection** · **confirm dialog on unpublish**.

**`features/books/api.ts`:** `Chapter` += `editorial_status?: 'draft'|'published'` + `published_revision_id?: string|null`; `publishChapter(token,bookId,chapterId,expectedDraftVersion?)` → POST `…/publish` body `{expected_draft_version}`; `unpublishChapter(token,bookId,chapterId)` → POST `…/unpublish`.

**`features/books/hooks/usePublishChapter.ts`** (controller): `{token,bookId,chapterId,draftVersion,editorialStatus,onChanged}` → owns `busy` + `confirmOpen`; `publish()` calls api (passes `draftVersion` as `expected_draft_version`; **409 `CHAPTER_DRAFT_CONFLICT` → toast "draft changed, reload"**); `requestUnpublish()` opens confirm; `confirmUnpublish()` calls api. Success → toast + `onChanged()` (refetch). i18n via `useTranslation('editor')`. No status state (page owns it).

**`features/books/components/PublishControl.tsx`** (view, <100 lines): badge (Draft/Published) + action button (`Publish` when draft / `Re-publish` when published) + Unpublish button (published only) → shared `ConfirmDialog` (variant=destructive, explains KG+passage retract). **`dirty` prop:** publish DISABLED when the editor has unsaved changes (hint "save before publishing") — publish snapshots the SERVER draft, so local-unsaved must be saved first; this also makes `onChanged` refetch safe (no unsaved body to clobber).

**`ChapterEditorPage` wiring:** capture `editorial_status` into state in `load()` (already fetches `getChapter` :143); render `<PublishControl …>` in the toolbar by Save (:486); `onChanged` = a light `refreshEditorialStatus` (getChapter → setEditorialStatus only, NOT body/title — avoids clobbering the editor). Pass `dirty={isDirty}`, `draftVersion={version}`.

**i18n:** `editor.json` ×4 (en/ja/vi/zh-TW) — `publish.{publish,republish,unpublish,draft_badge,published_badge,confirm_title,confirm_body,published_toast,unpublished_toast,conflict_toast,save_first}`.

**Tests:** `usePublishChapter` (publish passes expected_draft_version; 409 → conflict toast no onChanged; unpublish via confirm calls api + onChanged) + `PublishControl` (badge reflects status; button label draft vs published; publish disabled when dirty; unpublish opens confirm — query `document` for the portal'd dialog). Local i18n mock encoding opts (toast-content lessons).

**Self-review:** FE-only confirmed (gateway transparent + getChapter exposes fields) ✅; MVC (hook=logic, component=view, no API in component) ✅; dirty-gate prevents publishing stale server-draft ✅; refetch-meta avoids editor clobber ✅; destructive confirm on the canon-retract ✅; **VERIFY wants a browser smoke (Playwright, test account) OR explicit defer — FE behavior (toast, confirm, badge) is the bug surface units miss.**

**/review-impl (POST-REVIEW deep pass) — 0 HIGH/MED, 3 LOW folded:** verified the load-bearing boundary — `apiJson` populates `.status`+`.code` (api.ts:55-57) so the 409 conflict path is real (not dead code) and the `|| status===409` fallback is drift-robust; `save()→load()→setVersion` confirmed so publish sends fresh draft_version. Folded: (1) **render null when `editorialStatus===undefined`** — fixes the pre-load "Draft/Publish" flicker AND degrades correctly against a pre-CM1 book-service (no publish affordance the BE can't honor); (2) test for bare `status:409`-no-code conflict; (3) **i18n regression-lock** asserting every `publish.*` key exists in `en/editor.json` (the `t:k=>k` mock can't catch a missing key). 9 component/hook tests pass.

### §8.12 CM4 design — dual-order population + backfills (CLARIFY-locked 2026-06-04)
**Scope:** populate `event_order` (reading) + `chronological_order` (in-story) + passage `chapter_index` — the axes the timeline ALREADY filters on but B1 left perpetually NULL (`before_chronological` was a no-op). **Knowledge-internal** (the chapter ordinal is already threaded via `HierarchyPaths.chapter_index` = book-service `sort_order`, `hierarchy_writer.py:53`; read-path filters wired at `events.py:657-672`). Default v2.2 (NOT `/amaw`). PO: **event_order anchors to EARLIEST mention (min)** · **backfill = per-project internal endpoint**.

**write-path — `pass2_writer.write_pass2_extraction` (events.py merge_event):**
- Event loop → `enumerate(event_list)`; when `hierarchy_paths is not None`: `event_order = hierarchy_paths.chapter_index * 1_000_000 + i` (chapter granularity dominant, intra-chapter extraction-order tiebreak). Legacy/chat (no hierarchy) → `event_order=None` → timeline null-sinks via `coalesce(event_order, 2147483647)`. Pass `event_order` to `merge_event`.
- **`merge_event` ON MATCH (events.py:187) — change coalesce→MIN-keep:** `e.event_order = CASE WHEN $event_order IS NULL THEN e.event_order WHEN e.event_order IS NULL THEN $event_order WHEN $event_order < e.event_order THEN $event_order ELSE e.event_order END`. **⚠️ BUILD correction:** `event_id` is keyed on `chapter_id` (events.py:64-89) → the same title in two chapters is TWO distinct nodes, so the PO's "cross-chapter re-mention" scenario does NOT collapse to one node. This ON MATCH only fires on **re-extraction of the same chapter** (re-publish; or an event surviving retract-then-write via cross-source evidence). Min-keep is still the right call: a **monotone-earliest invariant** — re-merge never pushes an event LATER in reading order, so an event already inside a `before_chapter` cutoff stays inside (idempotent under re-extraction). The PO premise was off (verify-prescriptions-against-code), but the chosen semantic stands for the actual scenario. `chronological_order` ON MATCH stays coalesce — overwritten wholesale by the rerank.

**`chronological_order` — new `events.py` repo `rerank_chronological_order(session, *, user_id, project_id)`:**
- Sequential date-rank (row-number over `(event_date_iso ASC, id ASC)`) for events with `event_date_iso IS NOT NULL`; events with NULL date → `chronological_order = NULL` (reading-order fallback, MED-1 CJK). Stable tiebreak by `id` (MED-3). Two passes: (a) NULL-out chrono for undated events; (b) UNWIND `range(0,N-1)` over the dated, ORDER BY date,id → `SET chronological_order = i+1`. (Sequential, not dense-rank: strictly monotonic → exact for the `< before_chronological` strict filter; per-project event counts are small.) **Debounced trigger:** `write_pass2_extraction` calls it after the event loop ONLY when ≥1 event with a non-null `event_date_iso` was written this chapter (`dated_written > 0`), in the same session/Tx. O(project events) per dated-chapter persist — acceptable at book scale (§5).

**passage `chapter_index` — `_ingest_published_passages` (handlers.py):**
- Fetch `sort_order` via `book_client.get_chapter_sort_orders([chapter_uuid])` (the method KEPT in CM3c — finds new use), pass `chapter_index=sort_order` into `ingest_chapter_passages` (CM3c left `None`). Metadata SET only, NO re-embed (the upsert path already carries `chapter_index`).

**backfill — `app/db/migrations/backfill_orders.py` `run_orders_backfill(session, book_client, *, user_id, project_id, book_id)` + per-project internal endpoint (mirrors `backfill_event_date.py` + CLI shim):**
- List project events `(id, chapter_id)` where `chapter_id IS NOT NULL`; distinct chapter_ids → `book_client.get_chapter_sort_orders`; group by chapter, sort by `id`, `event_order = sort_order*1e6 + idx` → SET per event (Python loop + per-row SET, mirrors C18). Min-keep N/A (one merged node per event, one chapter_id).
- List project passages `(source_id)` `source_type='chapter'` → `SET p.chapter_index = sortmap[source_id]` (metadata, no re-embed).
- Call `rerank_chronological_order(session, user_id, project_id)` at the end.
- **Internal route** `POST /internal/projects/{project_id}/backfill-orders` (`requireInternalToken` pattern) → resolves user_id+book_id from `knowledge_projects`, runs `run_orders_backfill`, returns stats. Idempotent (SET is overwrite; re-run stamps the same values). CLI shim for operator bulk-run (cross-project loop), mirroring `backfill_event_date.py:_cli_main`.

**/review-impl (POST-REVIEW deep pass) — 3 MED folded, 1 LOW documented:**
- **MED#1 (folded):** `event_order = chapter_index × 1e6` breached the timeline ORDER BY's INT32 null-sentinel (`2147483647`) for books >2147 chapters → null-order display corruption (filtering stayed correct). Bumped the 3 ORDER BY sentinels to INT64_MAX (`9223372036854775807`); web-novels run many thousands of chapters.
- **MED#2 (folded):** the Stage-2 "non-destructive gap-fill" backfill (`SET … WHERE event_order IS NULL`, idx over all events) could assign a NULL event the same `base+idx` an already-stamped sibling holds on a re-run-after-growth → duplicate order. Reverted to an **unconditional one-time SET** (collision-free, deterministic; mirrors C18 run-once semantics).
- **MED#3 (folded):** `_CHAPTER_ORDER_STRIDE` was duplicated in pass2_writer + backfill (drift → mismatched scales). Hoisted to one shared `EVENT_ORDER_CHAPTER_STRIDE` in events.py, imported by both.
- **LOW#4 (documented):** the stride is the chapter→order contract a future composition spoiler-cutoff (OI-1) needs (`before_order = N × EVENT_ORDER_CHAPTER_STRIDE`) — documented on the shared constant.

**Self-review (REVIEW-design, default v2.2):**
- Read-path already filters event_order/chronological_order (no read change) ✅; B1 closes once written.
- Min-keep event_order = spoiler-correct for cross-chapter re-mentions ✅; legacy/chat null-sinks ✅.
- Sequential chrono-rank monotonic → exact for strict range filter ✅; undated→NULL fallback ✅; stable id tiebreak ✅.
- Debounce: rerank only when a dated event changed → avoids O(N) on every chat/undated persist ✅.
- passage chapter_index reuses the CM3c-kept client method; metadata-only (vectors untouched) ✅.
- backfill idempotent (SET overwrite); per-project (book_id scoped); reuses C18 module+CLI pattern ✅.
- **Watch (BUILD):** the event loop index `i` must be the position in the FILTERED-written events (skip `name_clean.strip()` empties consistently) so event_order is gap-tolerant but monotonic — gaps are fine (strict filter), just keep it non-decreasing. Tests: event_order = chapter_index*1e6+i; min-keep on cross-chapter re-merge; rerank dated→ranked / undated→NULL / stable tie; debounce (no dated event → no rerank); backfill stamps events+passages+chrono idempotently.
