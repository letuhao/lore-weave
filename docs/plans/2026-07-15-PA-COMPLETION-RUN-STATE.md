...
| **C8** FE surfaces | SD-C8 | ✅ | (a) **ReflectionCard** — renders the latest reflection draft (from listDiaryEntries, journal_kind='reflection'), MOUNTED in AssistantHomeStrip; dismiss chain FE→BFF→chat live-smoked. NEW BFF `POST /v1/assistant/reflection-dismiss` (owner from JWT sub, never client). (b) **CoachingScorecard** — dimensions + a **quarantine badge**; `scorecardTrend.ts` EXCLUDES quarantine from any trend (SD-7). (c) **capture strip** — VERIFIED: CaptureRail reads `captureOn=consentEnabled` (persisted consent). **Tests:** FE 16 (scorecard quarantine/trend-exclusion, reflection render/dismiss/fail) + BFF 42 (incl. 3 reflection-dismiss: owner-from-JWT-not-client, 401, 400). **Live-smoke (PASTED):** login→JWT → FE→gateway→chat reflection-dismiss → landed owner-scoped in reflection_dismissals. **Cold review:** SD-7 trend-exclusion + tenancy/auth VERIFIED CORRECT (no drift); MED-3 (BFF identity test) FIXED; LOW-4/5/6 (dismiss error state, refresh-after, rejected test) FIXED. Two follow-on FEEDS tracked: D-COACHING-SCORECARD-MOUNT, D-REFLECTION-PATTERNS-FEED. |
...
- **C8 near-miss (fixed/tracked):** the two HIGH-risk axes — SD-7 (a quarantine score must never be
  trended) and the dismiss route's tenancy (owner from the JWT, never the client body) — were BOTH
  correct + well-tested (cold review confirmed, incl. a new BFF test proving a smuggled owner_user_id is
  ignored in favour of the JWT sub). The real gaps were reachability + tracking HONESTY: the scorecard
  component and the per-pattern dismiss are built + tested but can't execute in prod (no mount / no
  patterns feed), and the follow-on IDs lived only in code comments. Both now have tracked debt rows.
  The dismiss error-handling (a failed dismiss kept the pattern hidden) was hardened + tested.
...
- **D-COACHING-SCORECARD-MOUNT** (C8 cold review, MED-1) — `CoachingScorecard.tsx` (+ the SD-7 trend
  exclusion) is built + tested but NOT mounted anywhere, and there's no scorecard-read route. It needs a
  `getScorecard` gateway/api read (the evaluate output is a 'scorecard' ChatOutput) + a mount site (the
  interview-practice result view). Deliberately deferred until the coaching surface exists; the SD-7
  quarantine-badge/trend-exclusion logic is the shipped, tested part. Gate #2.
- **D-REFLECTION-PATTERNS-FEED** (C8 cold review, MED-2) — the ReflectionCard renders the reflection
  DRAFT (feedable now) but `useReflection` passes `patterns=[]` always, so the per-pattern dismiss buttons
  never render live (the dismiss CHAIN — button→api→BFF→chat tombstone — is built + fully tested + live-
  smoked). The backend must expose the reflection's structured patterns (detector_code, summary,
  pattern_key), filtered by the user's reflection_dismissals tombstones, so the card can render dismissable
  chips. Needs a worker-ai/book-service change to persist/expose the patterns alongside the draft. Gate #2.
