...
- **W6b-2 (cross-service XL) · capture-forward red/green diff. Sliced 2a/2b.** Capture-forward only (no before-text exists for old rows). PO: all source types + `source_text` column.
  ...
  - **W6b-2b (diff endpoint + FE) · XL · ⏳ NEXT:** before=stored `source_text`, after=knowledge re-gathers current context (one hop, all types) → glossary `staleness/{id}/diff` → FE "View diff" red/green via the W1 `wikiDiff` lib.
