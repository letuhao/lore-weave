...
- **W6b-2 (cross-service XL) · ⏳ IN PROGRESS (PO chose true snapshots):** red/green diff. **Capture-forward only** — `source_usage.source_version` is a hash (not text), entities/chapters keep no text history → existing articles can't get a diff. Capture source text at M5 writeback (new storage) → glossary diff endpoint {before: snapshot, after: current source per `source_type`} → FE `wikiDiff`. Own CLARIFY (source_types; storage shape; degrade kg/recipe).
