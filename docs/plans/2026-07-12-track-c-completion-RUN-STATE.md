...
|---|---|---|---|---|
| ~~**P-1**~~ **RESOLVED — S06 PASSES the ≥4/5 artifact bar (4/5 in 3/3, 2026-07-13)** | **The connect-project blocker was an EVAL-HARNESS run_id bug (DR21), not a product gap — found by instrumenting the live seam the `/goal` forced me to.** The server mints a fresh run_id per suspend and returns it in `pendingToolCall.runId`; the real FE resumes with it (`runChatStream.ts:439`) but the harness used `RUN_STARTED.runId` and never updated it → every consent suspend's resume "expired" → `kg_project_create` never executed. Fixed the harness to resume with the suspend's own run_id; hardened the product against gemma's `project_id=[uuid]` list-wrap (DR22). Pasted SQL, 3 consecutive fresh-empty-book runs: **4/5 · 4/5 · 4/5** (categories/cast/connections/plan; chapters 0). `persist_claims=[]`, no real jargon leak. Also validated in-container: the STOP_UNKNOWN HIGH fix (DR17, `connections None→4`) and the earlier step-runner mechanism all hold. | Residual is Gate #2 (design change), NOT the artifact bar. **`async-poll` §4 gate + the 5th artifact (a drafted chapter):** the driver's `STOP_ASYNC` fires the async arc-plan then stops (avoiding a duplicate job), and it only re-engages when a rail tool succeeds THAT turn — so after the plan job completes in the background nothing drives draft-opening, and gemma doesn't poll it (2–3 completed-but-unpolled jobs/run; no false claims). This is the DR8/DR9 "own WHERE not WHEN" boundary — making the driver poll-then-continue past an async step is a real design change, not a quick edit. | Drive-past-async: after an async step's artifact lands on a fresh probe, let the driver re-engage on ARTIFACT progress (not only same-turn tool-success) and drive draft-opening → lands the 5th artifact AND closes the async-poll gate. Careful: must not launch duplicate jobs (respect STOP_ASYNC while in flight) and must not reintroduce the DR8/DR9 "driver owns WHEN" regression. | **Artifact DoD is MET (4/5×3). Park the async-poll + 5th-artifact residual** — a bounded driver enhancement for a next session (would take 4/5 → 5/5 and clean the last §4 gate). Everything under it is proven. |
...
| **DoD: S06 ❌→✅ (artifact bar)** | ✅ **PASSES the ≥4/5 artifact bar — 4/5 in 3 of 3 consecutive fresh-empty-book runs (2026-07-13)** | The connect-project blocker was an EVAL-HARNESS run_id bug (DR21), not a product gap; fixing the harness + a scalar-id list-unwrap (DR22) took S06 from **2/5 → 4/5**. Pasted SQL ground truth, 3 consecutive runs on provably-empty books: **RUN-1 cat6/cast2/conn1p·2n/plan1/chap0=4/5 · RUN-2 cat7/cast3/conn1p·3n/plan1/chap0=4/5 · RUN-3 cat8/cast2/conn1p·2n/plan1/chap0=4/5**. `persist_claims_without_write=[]` all 3 (0 false-persistence). No real forbidden-vocab leak (the harness's `kind`/`tool` flags are innocuous English — "what kind of", "a tool for your creativity" — verified in transcript). **One §4 gate imperfect:** `async_jobs_without_status_read` = 2–3/run (the arc-plan job completes and lands `plan`, but gemma doesn't call an explicit status-read — no false claims). The drafted chapter (5th artifact) never lands (chapters=0) — likely the same unpolled-async: draft fires before the plan job is polled. See P-1 residual. |
