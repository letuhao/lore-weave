...
| ~~**D-P1-EXTRACT-CHAIN**~~ | ✅ **RETIRED 2026-07-15 — RESOLVED.** Root cause was `save_draft`'s uncallable schema (json.RawMessage → array-of-ints), fixed in M0a (`463091c6a` etc.); the extract→propose chain now yields saved entities. S06 flagship re-run FRESH on SQL-provably-empty books = **3/3 GREEN** with cast=2/3/3 and chapters_with_prose=1 every run (SQL pasted in the all-tracks-clear run). The cast-is-0 symptom is gone. | — | — |
