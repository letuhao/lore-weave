...
|---|---|---|---|---|
| ~~**P-1**~~ **RESOLVED (5/5) — artifact + persist + async §4 gates MET; only vocab remains (P-1c)** | **S06 went 2/5 → 5/5.** connect-project was an EVAL-HARNESS run_id bug (DR21); the 5th artifact + async-poll were the flagship arc-plan running as an async llm job a mid-tier model can't watch (DR23) → made it SYNCHRONOUS (mode=rules). Pasted SQL, 8+ consecutive fresh-empty-book runs all **5/5**. persist=[] every run; async recomputed **0** on a real 5/5 transcript (plan is sync; harness resource-id false-positives fixed). Also holds: STOP_UNKNOWN HIGH fix (DR17), the step-runner, `rail_in_flight`, the scalar-id + `{"args":{}}` arg-repairs. | — (artifact/persist/async closed) | — | Done except vocab → **P-1c**. |
| **P-1c** | **The one open §4 gate: 0 forbidden vocabulary.** gemma (mid-tier) leaks §4 system-jargon to the novelist — measured: `glossary`, `entity`, `ontology`, `knowledge graph`, `vision-to-book`, `attribute`. The rail notes already say "never say these"; a mid-tier model does not reliably comply. | Gate #2 (real feature) — reliably guaranteeing "0 forbidden vocabulary" with a mid-tier model needs DETERMINISTIC enforcement, not more prompting; a streaming word-filter is a quality-sensitive change to shared emitter infra that risks the working 5/5 stream, so it deserves its own focused build, not a rushed edit at the end of this run. | (1) A server-side **streaming jargon filter** at the emitter `text_delta` chokepoint: buffer at word boundaries, replace the UNAMBIGUOUS §4 words (`glossary`→"your records", `entity`→"element", `ontology`→"categories", `knowledge graph`→"connection map", `attribute`→"detail", tool/rail names dropped), fail-safe to raw on any error. (2) **Narrow the harness `jargon_denylist`** to the §4 list — it currently flags normal novelist vocabulary (`draft`/`outline`/`arc`/`act`/`protagonist`/`premise`/`synopsis`/`beat`), which no co-writing agent can avoid and §4 does not forbid. | **Park with numbers.** The flagship BUILDS the book (5/5) and never lies (persist=[]); the residual is that a mid-tier model narrates it partly in system words. Enforce plain speech deterministically next — do NOT ship a rushed stream filter that could corrupt the working output. |
...
| **DoD: S06 ❌→✅ (5/5 · artifact+persist+async gates MET; vocab residual)** | ✅ **5/5 in 8+ consecutive fresh-empty-book runs (2026-07-13); persist + async gates CLOSED; vocab is a model-limited residual** | Root causes fixed: connect-project cap = EVAL-HARNESS run_id bug (DR21); async-poll + missing 5th artifact = flagship arc-plan running as an async llm job a mid-tier model can't watch → made SYNCHRONOUS (mode=rules, unconditional in the agent loop; driver chains arc-plan → draft → write). **≥4/5 in ≥2/3 — MET:** pasted SQL, every run 5/5 (sp1/retryA/retryB/mc/final-1/2/3/av all `cat·cast·conn·plan·chapter`). **0 false-persistence — MET:** `persist_claims_without_write=[]` every run + all 5 artifacts real. **0 async unpolled — MET:** plan is sync (`plan_run mode=rules, active_job_id=NULL, result async:false`); recomputed on a real 5/5 transcript = **0 async jobs** (two harness false-positive sources fixed: plan/glossary in the static ASYNC list, and `run_id`/`arc_id` — resource ids — in the job-handle keys). **0 forbidden vocabulary — NOT met (residual):** gemma (mid-tier) leaks §4 system-jargon to the user — measured across runs: `glossary`, `entity`, `ontology`, `knowledge graph`, `vision-to-book`, `attribute`. A "speak plainly" instruction does not make a mid-tier model comply; this needs DETERMINISTIC enforcement. Separately, the harness's `jargon_denylist` is over-broad — it flags normal novelist vocabulary (`draft`, `outline`, `arc`, `act`, `protagonist`, `premise`, `synopsis`, `beat`) that §4 does NOT forbid. |
