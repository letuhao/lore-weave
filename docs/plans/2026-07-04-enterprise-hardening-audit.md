...
> **▶ NEXT (this track's handoff — SESSION_HANDOFF's NEXT block is owned by a concurrent studio/KG track, so this doc is the enterprise-hardening system-of-record).** **P0 fully clear** (8/8). **P1 fully clear.** **P2 spec** authored + adversarially verified: [`docs/specs/2026-07-04-enterprise-p2-structural.md`](../specs/2026-07-04-enterprise-p2-structural.md). **P2·B1 (KEK sha256-derivation) ✅ SHIPPED** — clears `D-REVIEW-AESKEY-DERIVE`. **P2·A2a (Python shared logging) ✅ SHIPPED** (`687fd4949`, `aba652a86`, `e7dbef061`, `178858d51`, `7500db04e`): `loreweave_obs.setup_logging` + dual-emit `otel_trace_id` (closes the Python half of A1); 3 copied `logging_config.py` retired; all runtime `basicConfig` migrated + lint-enforced. **P2·A1 (Go correlation-id) — substrate SHIPPED fleet-wide** (`e7043b7d2` SDK + `723984958` book-service reference sweep + `61cad8e9d` **fleet adoption ×10**): every tracing-wired Go service now installs `observability.SetupLogging` (arms `otel_trace_id` injection on `slog.*Context`); `go build` green ×10. The fleet **call-site sweep** is a per-service refactor (not mechanical — most bare-slog sites are startup/background/async; real ones need ctx threaded through helpers) → deferred `D-A1-CALLSITE-SWEEP` (sweep-on-touch; full value at cycle-37 Tempo). Also `D-A1-TIER2-NOTRACE` (4 no-tracing workers) + `D-A1-TIER3-STRUCTLOG` (ops-bots). **review-impl command hardened** (`1d43ff950`): mandatory Standards gate (index→ENFORCED/LOCKED→SoT-mirror→enforcement-exists) so a fresh/sub-agent author's missing-standard drift gets caught. **P2·B2 ✅ SHIPPED** (`2932fe2e9` retention sweeper · `30c500b5f` embed cost · `848b43e0c` route-parity+D-S4C): closes `D-REVIEW-EMBED-AUDIT-COST` + `D-S4C-STREAMING-REALCOST` (a real ledger bug — `/record` was dropping streaming's authoritative cost → flat rollup); parity contract test proven to red on the drift. New tracked: `D-B2-RETENTION-LIVE-SMOKE`, `D-B2-PARITY-LIVE-SMOKE`, `D-B2-RERANK-WEBSEARCH-PRICING`. **P2·C 4 of 6 SHIPPED** (`042786197` dedup — closes an at-least-once duplicate-row bug · `afedf0668` NoopNotifier warn-on-silent-drop · `e759aa6e3` body PII redaction · `35ea27165` per-user opt-out prefs). **Remaining 2 slices:** `D-C-PRODUCER-OUTBOX` (gate #2 structural cross-service) + `D-C-FE-I18N` (a FE *consolidation* — the main case already localizes via metadata; migrate onto the canonical `message_key` columns). Live-smoke: `D-C-DEDUP-LIVE-SMOKE` (covers dedup + opt-out gate runtime). **P2·D buildable core SHIPPED** — `contracts/slo/latency.yaml` (platform HTTP p95 SoT) + `scripts/slo-latency-lint.py` (blocking in lint-foundation p1-lints; proven red on 6 bad-row classes); registered in the standards index. Perf-nightly p95 assertion is the infra-gated tail `D-D-PERF-NIGHTLY` (no perf harness in-repo yet). **NEXT P2 items (recommended order):** F (tenant-boundary audit, first-access-per-session) → E (salience↔learning, decision-first); C's 2 slices (`D-C-PRODUCER-OUTBOX`, `D-C-FE-I18N`) + `D-D-PERF-NIGHTLY` when convenient. **Open forks:** A2b (Go `contracts/logging` adopt-vs-retire — CLARIFY needed; A1's review flagged the lint header still names `contracts/logging` as the Go idiom while the fleet adopted slog+SetupLogging — reconcile here); chat-service partial-tier logging adoption (tail).
...
|---|---|---|---|
| ~~`D-REVIEW-EMBED-AUDIT-COST`~~ ✅ **RESOLVED 2026-07-04** (P2·B2c, `30c500b5f`) | `internalEmbed` now SELECTs `um.pricing` + computes input-only cost via `billing.PriceEmbedding`, threaded through `recordSyncUsage`. (Landing it at the writer needed B2a below — the /record path was dropping the cost.) | — | resolved |
| ~~`D-S4C-STREAMING-REALCOST`~~ ✅ **RESOLVED 2026-07-04** (P2·B2a, `848b43e0c`) | `/record` (Route A) was dropping the caller's authoritative `total_cost_usd` (streaming's tallied cost + embed's cost) → flat fallback recorded, mis-counting the committed-spend rollup. `recordUsageRequest` now carries `total_cost_usd`; `recordUsageParams` honors it as the `recordCostUSD` override, matching the stream consumer. Route-parity contract test locks it (proven to red on the drift). | — | resolved |
| `D-B2-RETENTION-LIVE-SMOKE` | The retention sweeper's runtime row-selection (terminal-past-`expires_at` deleted, running/pending/fresh kept) is only statically asserted via the pgxmock SQL-regex; not proven against live PG. | #4 — needs an isolated/ephemeral PG (NOT the shared dev DB, which holds real `llm_jobs`) | an isolated-PG sweeper smoke, or the first ops run in staging |
| `D-B2-PARITY-LIVE-SMOKE` | The cost-parity fix is proven at the two pure builders + statically wire-aligned (`client.go` `total_cost_usd` ↔ struct tag), but not end-to-end (real streaming/embed → `usage_logs.total_cost_usd` = real cost) through the live stack. | #4 — needs the full provider-registry+usage-billing stack + a model (lm_studio) | a cross-service billing smoke on a stack-up |
| `D-B2-RERANK-WEBSEARCH-PRICING` | rerank + web_search record `nil` cost (flat fallback) — they carry 0/0 tokens, so a per-token cost is meaningless; they need a per-call/per-request pricing dimension before a real cost can land. | #2 — a new non-token pricing dimension (schema + resolution), consciously flat-rate + documented until then | when per-call/per-request pricing is added |
...
| `D-A1-CALLSITE-SWEEP` | Fleet Go call-site sweep (`slog.Xxx` → `slog.XxxContext(ctx,…)`) so request-path logs carry `otel_trace_id`. Substrate is armed fleet-wide (`61cad8e9d`) but the 10 services still log ctx-less. Calibration finding: it's a **per-service refactor, not a mechanical pass** — most `internal/` bare-slog sites are startup/background/async-goroutine (no span or a cancelled ctx; a blind convert is a bug), and real request-path sites often need ctx threaded through internal helpers (e.g. `catalog.fetchBookLanguages(bookID)` takes no ctx). book-service is the worked reference. | #2 structural (bounded-but-judgment-heavy refactor ×10 services) + value-throttled: the clickable join is Tempo-gated regardless | sweep-on-touch (convert a service's live-span sites when its request path is edited); or a dedicated fan-out; full value at cycle-37 Tempo |
| `D-A1-TIER2-NOTRACE` | 4 Go workers with a JSON logger but **no** `InitTracer` (publisher/retention-worker/breach-notifier/meta-outbox-relay) don't yet install `SetupLogging` — adopting now pulls the otel dep-graph for a permanently-dormant injector (no spans ever). | #2 — adopt with tracing, not standalone | when tracing (`InitTracer`) is wired into those workers |
| `D-A1-TIER3-STRUCTLOG` | Tier-3 Go services (admin-cli, ops-bots: alert-recorder/canary-controller/incident-bot/etc.) have no structured JSON logger at all — bringing them onto `SetupLogging` is Logging-Standard adoption (text→JSON), a superset of A1's trace-id goal (and JSON-to-stdout is wrong for the human CLI). | #1 out of A1 scope (Logging-Standard adoption, not correlation-id) | a Logging-Standard fleet-adoption pass |
