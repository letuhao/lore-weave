...
authorable-surface-gate: OK — 8 patch type(s) reachable from `RulesetPatch`, 72 authored key(s),
  ...
  their section advertises; 20 engine default(s) match the artifact
...
**WIRED, and stated here because a gate nobody runs is `contracts/.spectral.yaml`** — the exact
thing §1.3 gives as the reason `A2` had to exist. It runs from `.githooks/pre-commit`, deliberately
**without** `--staged`: the contract and the patch types live in different crates, so a change to
either side alone breaks the pair and a staged-only run would miss half the cases. `gate-wiring-gate`
is what confirms it rather than my saying so. (Added by audit — see `ASD-8`.)
...
| `ASD-8` | **The board never said the gate is WIRED**, which is the single fact that makes it enforcement rather than a script. `.githooks/pre-commit` runs it (unstaged, because the contract and the types live in different crates and a change to either breaks the pair), and `gate-wiring-gate` confirms it — *"101 gate(s) discovered, all wired or exempted"*. Found by audit. A gate nobody runs is `contracts/.spectral.yaml`, which is the very thing §1.3 of this board cites as the reason `A2` had to exist at all. |
| `ASD-9` | **A citation in §0 pointed at a drift row that did not exist.** This file's hazard list credits `WSD-8` for *"edit nothing while a sweep runs"* — a lesson I had genuinely learned, from an event I had genuinely caused twice, and never written down. So the reference resolved to nothing for two commits. `design-lint` refuses phantom "(registered …)" claims in the design corpus and **nothing checks drift ids anywhere**; the audit found it by diffing defined against referenced. `WSD-8` now exists and says what happened. |
