...
| **DR-6** | **I nearly reported the round as closer to sealing than it is, from memory.** Auditing the open rows on 2026-08-06 I reasoned: *"`F-8` was the stated reason not to seal `CMD-1`..`CMD-6`; §10.4b fixed `F-8`; therefore sealing is nearer than §10.5 says."* Every step is true and **the conclusion is backwards** — `R4` had attacked the replacement test (`R4-4`, `R4-5`, `O-CI-22`) and the licence to seal is *further* away, not nearer. Caught only because I grepped `V1\|V2\|V3\|bite-test` before writing it down instead of after. **The shape: a blocker was not removed, it MOVED, and a register that records blockers by their original grounds cannot show that.** Same session, same defect class as the round's own `R4-7` — and the third time in three days that reasoning over a register beat reading the file. |
| **DR-7** | **A falsified sentence survived its own deletion.** `DR-5` records deleting *"the cultivation genre… will fit by construction"* from dataflow §8. It was still sitting in this file's §8 checklist **four days later**, unqualified. Deleting a claim from the document that argued it, while a copy stays in the document that schedules it, is not deletion — and this repo's rule against commenting-out exists precisely because a surviving copy is indistinguishable from a live one. |
...
### 4e. Proposed after the review rounds — **DRAFT, awaiting PO. NOT sealed.**

> This section exists because `CMD-10` was first written into §4d, whose column header is
> **Decision**. A draft filed among sealed rows is read as sealed by the next person to open the
> file — `DR-7`'s failure mode with the sign flipped: there a deleted claim survived, here an
> undecided one would have been promoted. **The register's headers are load-bearing.**

| # | Proposal |
|---|---|
| **`CMD-11`** | **The offer REGISTRY — `offers_for(subject, tick)`.** Folded in from the quarantined 2026-08-05 round (its `§1`/`§3`), which is the prior-art pass this round's own `PO-5` asked for. **08-02 already has `from: Offered \| Any` on `RoleSpec`** — but as a *validation flag at pipeline stage 2*, and it never says **what produces an offer**. That is the gap this fills, and the red team made it larger rather than smaller: `THR-A4` does not guarantee the engine offered the target, because `candidates` is a **wire field the producer supplies** and `admit_signed` holds no island handle, so it cannot re-derive. **`THR-A4` therefore constrains the model relative to its driver, not the driver relative to the engine** — which makes a real registry necessary, not decorative. Where it runs is forced rather than chosen: offers need **live island state**, `game-server` holds no authority (`CWC-A1`, *the room is a lens*) and `world-service` holds only derived reads ⇒ **commit-service**, with delivery kept separate from computation |
| **`CMD-12`** | **`offer_id` is a keyed MAC over `(ruleset_digest, reality, tick, subject, provider, command, param-domains)`; entitlement RECOMPUTES and compares.** Folded in from 08-05. The point is that an offer must be **bound**, not merely unguessable — an id that is only hard to guess is a bearer token, and if it leaks it *is* authority. Recomputing buys three things: **stateless** (no live-offer table to leak, grow or evict wrongly), **expiry for free** (`tick` is an input, so a leaked id is worthless next tick with no cleanup job), and **tampering named** (changing subject, provider or a parameter domain changes the MAC input, so a mismatch says *which* field moved). **Stated limit, not glossed:** a leak *within the same tick* is still authority for that tick, and shortening a tick is not a security control. The fix when it matters is binding the session into the MAC input. ⚠ **The security lens found the load-bearing weakness is elsewhere:** `actorForUser` returns `LW_CHANNEL_DEFAULT_ACTOR ?? '1'` for any authenticated user absent from the env map, so **two users are legitimately bound to one subject** — a MAC over a subject the caller can already be is a lock on the wrong door. 08-02 §5 stage 0's *"the confused-deputy guard is already correct"* is **downgraded** |
| **`CMD-13`** | **Horizontal composition — a mechanism, not a policy choice.** Folded in from the 08-05 conflict-resolution proposal, which was deliberately left unnumbered until it reached this round. **The gap is exact:** S1's layered fold (`engine_default 0 → preset 10 → book 20 → reality 30 → forge_override 40`) resolves **vertical** conflict and is sealed; it says *"folded by merge strategy"* and **never defines what a merge strategy is**, while there are **N bundles per layer**. Four parts, each taken from a system that ships it: **①** a **match key** (Kubernetes `patchMergeKey`, *"matched irrespective of their array positions"*; Android's `android:name`) — a row with no key cannot merge, only append. **②** the merge strategy declared **in the SCHEMA, not by authors** (a Kubernetes struct tag on the API type; *a list with no strategy is replaced entirely*), so neither bundle author negotiates and neither needs to know the other exists: `requires[]`/`costs[]` **append**, `effects[]` **append in DECLARED ORDER**, `roles[]` merge by ordinal, and `cue`/`submitter_class`/resolution binding are **exclusive** — which is the only place conflict can arise at all. **③** an explicit `conflict_resolutions` row **naming both sides**, so it is checkable (do both bundles still exist? do both still set the field?) and a resolution that **outlives its conflict is a finding** — the same shrink-from-both-ends rule `orphan-model-gate` uses. **④** unresolved ⇒ **the build FAILS with a diagnostic naming both sides and recommending the marker** (Android's *"with a recommendation for how to resolve"*), which **we can afford only because we pre-compose**: a build has a place to fail, a launch does not. **This corrects `CMD-D2` twice:** *"amendments may never remove"* was over-correction — silent removal is the danger, not removal, and forbidding it pushes authors into declaring a near-duplicate verb, which is worse; and *"lists add"* stopped a step short, because addition commutes and **effect application does not** (A writes `heat += 1`, B's precondition reads `ValueAtLeast(heat, 1)`) |
| **`CMD-10`** | **The classification test gains a FOURTH question: *operand, not predicate*.** Answers `O-CI-22`; full argument in [dataflow §9.4](../specs/2026-08-02-command-interaction-dataflow.md). `R4-16` showed V1–V3 measure extension *mechanics* only, so giving any closed set a flag set scores it ✅✅✅ — including `CommandKind`, the thing this round exists to demolish. **V4: does every authored field enter the engine as an OPERAND — a value the engine's own fixed rule then judges — or can a field BE the judgement?** This is **`D-29` generalised** (*"a condition is a declared threshold, never a predicate grammar"*) from the condition layer to every authored field. `R4-16`'s own phrasing — *"can a member violate an engine invariant"* — was **refused as the question**: it needs an enumeration of invariants to be answerable at all, and an unanswerable fourth question would be `F-8` a **third** time, in the section written to fix it the second. V4 also supplies the boundary `R4` found missing in V3, and kills the `may_submit_engine_verbs` construction. **Two repairs travel with it:** V1 is asked of the **design under judgement**, never of the shipped code (dissolves `R4-4`); V2 is about an existing member's **score**, not its **rank** (dissolves `R4-5` — while both defects §9.2 caught in its own author stay caught, which is the test that the repair did not simply stop measuring). **Owes a bite**: author a member that sets an authority-bearing field, assert the build refuses it. Deferred to BUILD by `8b`, **recorded as owed, not done** |
...
- [x] **Red team** against dataflow §2–§4 — ran **four** times (`R1` determinism · `R2` extensibility · `R3` implementability · `R4`), **17 FATAL**, §9.5–§9.7. The nomination *"attack `O-CI-4` first"* was correct: it is the row that got decided
- [x] **Author agents, run BLIND** — four genres, §9.4. ⚠️ **The rider on this line was FALSIFIED and is deleted, not annotated** (`DR-5`): it read *"do not start with the cultivation genre; it is the home genre and will fit by construction."* Run last, blind and unprimed, cultivation scored **5.8 % ✅ — the lowest of the four.** The same sentence was deleted from dataflow §8 on 2026-08-02 and **survived here for four days**, which is how a falsified claim keeps being true somewhere
- [ ] **`O-CI-22` — the classification test's fourth question. This now BLOCKS sealing, and it is not the blocker §10.5 recorded.** §10.5 #1 said *"do not seal, `F-8` invalidates the test"*; `F-8` was then fixed by `V1`/`V2`/`V3` (§10.4b) — and `R4` attacked the replacement: `R4-4` (V1 returns opposite verdicts on one concern) · `R4-5` (`verb_declarations` fails V2) · `O-CI-22` (`A-4`'s flag-set argument **dissolves every closed set, including `CommandKind`**). **`F-8` could not fail; the replacement passes anything. Same defect, opposite sign.** Until the test discriminates, sealing `CMD-1`..`CMD-6` has no licence. → **PROPOSAL `CMD-10` (§4e) answers all three attacks**; it is DRAFT and the PO seals, not the author. Its own bite is **owed, not done**, so a seal taken on it today would rest on an argument rather than a mechanism — which is the exact trade `F-8` lost twice
- [ ] Re-present `CMD-1`..`CMD-9` for sealing — **unblocked by `O-CI-22`, not by the red team** (that condition has fired)
...
1. **Do NOT seal `CMD-1`..`CMD-6`.** `F-8` invalidates the test that would justify sealing them.
   ...
   > ⚠️ **STILL CORRECT 2026-08-06, but for a DIFFERENT reason — do not read this row as
   > dischargeable.** `F-8` *was* fixed (§10.4b: `V1`/`V2`/`V3`), so the stated ground is gone. The
   > conclusion survives because `R4` then attacked the replacement — `O-CI-22`. **A recommendation
   > whose premise has been discharged and whose conclusion has not is the most dangerous row in a
   > register:** it reads as stale, invites a quiet tick, and the blocker underneath it is bigger
