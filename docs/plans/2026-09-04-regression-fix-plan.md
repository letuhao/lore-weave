...
- [x] **D2 — DECIDED and BUILT** (owner, 2026-09-04: *frontend chrome only*), `b22858aca`. An empty assistant message now carries a badge — the same affordance interrupted/errored turns already use. **Chrome, not message text: the owner's declined generic failure line stands untouched**, nothing enters `content`, the stored row is unchanged. Excludes streaming, tool-calling, reasoning and interrupted/error turns (without the tool-call exclusion it fires on the 92 measured card-suspends). `message.empty_turn` registered in all 18 locales as real translations — AUDIT.md counts ECHOED as its own defect. Three bites, tsc clean, 6,433 FE tests pass. ⚠️ The 5 reds in `DefaultModelsCard.test.tsx` are PRE-EXISTING — verified by stashing and re-running.
  *Superseded row text:* was V3. ⚠️ **The owner's chosen route (fix upstream at V5) is REFUTED by the measurement they asked for** — see V5. D2 is back to a live choice, now with the number that was missing: a true blank bubble (no content, no tools, no card) runs at **~0.11% of assistant turns** — 7 in 6,640 in August, 1 in 625 in September. The outcome is already recorded correctly (`failed`); only the author's view is missing, and the frontend has no renderer for `is_error`/`error_detail` either. A failed turn renders as a blank bubble, and the fix runs straight into a **sealed owner decision**: `_last_tool_error_for_author`'s docstring records that *"the owner declined a generic failure line… a blank turn cannot be told apart from a crash, a refusal or a slow turn."* DQ-T33 surfaces the last TOOL error instead — and the measured turn called **zero tools**, so there was nothing to borrow. **New evidence the owner did not have:** this sub-population IS distinguishable — `input_tokens=0`, `output_tokens=0`, 0 tool calls, and provider-registry reporting `usage:false`. Does that change the answer? ⚠️ The frontend also has **no renderer** for `is_error`/`error_detail` on an assistant message, so this is two-sided work either way.
...
- [x] **V5 — MEASURED AND REFUTED, closed without the change** (owner chose "measure the store first", and the store said don't). The discriminator was wrong twice: `output_chars:0 AND usage:false` matches 14,823 rows of which **14,347 are `embed`**; both-tokens-zero would **not have matched the very instance I built it from** (`input_tokens=18221`, `output_tokens=0`). The surviving form — chat + `output_tokens=0` — selects **177** `usage_logs` rows against **1** `chat_messages` row in the same window, because `usage_logs` counts LLM **calls** (every pass, plus subagents/enrichment using `purpose='chat'`) and `chat_messages` counts **turns**. It would have flipped 177 rows to catch 1.
  *Superseded row text:* provider-registry logs `status:"success"` for a stream with `output_chars:0` **and** `usage:false` — the empty turn. That is where the blank bubble begins. ⚠️ `output_chars:0` ALONE is not the signal: legitimate tool-only turns show it with `usage:true`. Needs a measurement across the store before a status re-classification, since every metric over `status` moves with it.
...
- [x] **D1a — the follow-on, BUILT** `6ed5ac11a`. Two skill fixes: the create line said *"empty is fine"* — the surface recommending the shape that stranded four chapters, and louder than the tool schema I fixed in `0f56d7abb`; and the skill now states the ~1,200-1,500 word ceiling up front, points at the composition/scene path as the designed route for a long chapter, and **forbids the multi-part announcement** (Part Two arrived **0 of 4**). Cross-references composition, never re-teaches it (spec §8b.5) — a test holds that. ⚠️ These assert the SKILL TEXT, not obedience; the post-fix run is the behavioural evidence. Two bites, 3,938 tests pass.
- [x] **D1 — DECIDED by the owner, 2026-09-04: not a bug and no chat-side change.** *"This is model limit, we already have plan force with multiple scenes to fix it."* The ~1500-word single-call ceiling is the model's; the platform's answer to a long chapter is the composition path (PlanForge → multiple scenes), not the co-writer improvising a split. ⚠️ **Consequence worth noting, not built:** the chat assistant currently invents a two-part split when asked for 1800-2500 words, and Part Two arrived **0 times in 4 attempts** across two runs. Pointing that request at the scene path instead is a separate piece of work.
  *Superseded row text:* F4 is a product decision, not a bug: a single draft call holds
