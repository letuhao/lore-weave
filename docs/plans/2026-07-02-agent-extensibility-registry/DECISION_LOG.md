...
- **Deferred to P3 (by design):** redirect-based SSRF (the overlay's fetch/MCP-client follows redirects — a user server could 302 to a metadata endpoint) + full egress allowlist are the P3 security layer; the P2 guard is scoping only.
