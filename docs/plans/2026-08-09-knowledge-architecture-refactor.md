...
**RESUME: `T37b` — the planforge caller (append the role at plan time, and CLOSE roles a plan revision no longer implies) + the studio's backend declare endpoint. SPEC §4.2b, decided by the PO 2026-08-14: roles get TWO producers, plan-time and explicit author action; compose-time was rejected because it cannot guard the drafting chapter's own first draft. The studio UI rides with T51 (§6.5). Acceptance is one number — T36 measured `relation 0`; T37 is done when a real run makes it non-zero and the canon check reads it.** ✅ `A8` · ✅ `T24b` · ✅ `T25 ②` · ✅ `A9` · ✅ `T35a/b/c` · ✅ `T36` · ✅ `T37a` · ✅ `T38` · ✅ `T42a`. Gates: port-adoption **64 → 57**, derived-entity-id **5 → 4**, conformance **40 → 82**.
