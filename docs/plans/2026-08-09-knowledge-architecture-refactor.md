...
**RESUME: `T35` — decide whether it CLOSES. Minting half closed (T35a/b/c); **T35d closed the event half** — an Event's id derives from its TITLE, so Kuzu and the fake forked the event on every re-extraction after an author rename; both fixed, `_EXPECTED_DIVERGENCES` empty again. Its own test passes 9/9, `derived-entity-id-gate` PASS at 4, §4.1 says nothing further is owed on entities. ⏸ It carries a POST-REVIEW checkpoint. Verify by RUNNING, then tick it or write what it owes. Then `T32` → `T33` ⛔.**
