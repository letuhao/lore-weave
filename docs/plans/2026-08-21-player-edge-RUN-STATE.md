...
| ~~`EO-1`~~ | **CLEARED 2026-08-21 — and the deferral reason was FALSE.** I wrote that it needed "the migrate-existing-realities path", which `services/migration-orchestrator/cmd/migrate` already provides (`cmdApply`, over `contracts/migrations/per_reality`). I deferred against infrastructure I never looked for — the lazy tell CLAUDE.md names, and a defer row built on a premise this repo disproves in one `ls`. `0023_actors_entity_id_nonneg` + manifest row. **Bitten on a real table**: before, the column accepted `-1`; applying it over that row was REFUSED (*"violated by some row"* — which is why `NOT VALID` was deliberately not used); with the row gone it applies, `-1` is refused and `0` and `99` still insert. Original text: `actors.entity_id` has **no `CHECK (entity_id >= 0)`**. `checked_island_id` closes it at both code edges, but the column stays permissive, so a future writer that skips the helper is unguarded. Not fixed here because `0022` is applied **per reality at provision time**, so a new migration only reaches worlds provisioned after it — this needs the migrate-existing-realities path, not a new file. | `checked_island_id`'s test is the code-side guard (bitten). The column-side row is declared here rather than left as a comment. |
