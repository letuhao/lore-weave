...
### DEBT — dispositioned 2026-07-17 (each S6-owned item resolved to a decision, not left vague)
- **focusRuleId edit deep-link → ✅ CLEARED** (`5b466a94d`): quality-canon "Edit rule" opens quality-canon-rules focused (edit mode + scroll). Bar #6 canon pair loop closed.
- **kind field (F-Q10) → ⛔ WON'T-FIX** (gate #5): VERIFIED `canon_rule.kind` is WRITE-ONLY — no engine/critic/packer reads it (`str` default "generic", no enum, no behavior). Exposing it in the FE would ship a write-only control (the exact stored-but-unread anti-pattern CLAUDE.md forbids). Neither agent nor human setting it does anything; the "inverse gap" is a non-issue. Do not build.
- **412 conflict rich-diff-UI → ⏸ DEFER** (gate #2 structural): bar #4 is MET (patch `onError→toast.error` surfaces the conflict; OCC `If-Match` guarantees no silent overwrite). The draft's "show `current`, keep your draft, re-apply onto v8" diff-UI needs api-layer 412-body plumbing (`apiJson` must surface the `current` payload) + a conflict panel — a real enhancement beyond the bar, not a quick edit.
- **reverse badge (canon-rules → quality-canon "N broken") → ⏸ DEFER** (gate #2): needs per-rule violation counts inside canon-rules (which only has the rules, not the violations) — a cross-data fetch. The forward hop (canon → canon-rules) ships; this reverse hop is additive.
- **flywheelEffects Lane-B + F2 auto-open → ⏸ DEFER** (gate #4 cross-service signal): VERIFIED the flywheel delta is produced by an ASYNC knowledge-service extraction job, NOT an agent-tool effect the Lane-B reconciler observes (E2). Refreshing on it needs a notification (notification-service "extraction complete") or a post-publish poll — a genuine cross-service signal design, tracked as `flywheelEffects: s6-m5` in the coverage ledger. F2 auto-open keys on the same signal.
- **conformanceEffects (S4) · BE-9a authoring_run_units.job_id correction-capture (S1) → 🚫 CROSS-CHARTER** — not S6's to build (S4 owns the conformance panel's Lane-B; S1 owns the authoring-run producer seam, E6). Filed for the owning session.
