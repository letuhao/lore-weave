...
- **conformanceEffects → ✅ DONE by S4** (its owner; registered in the barrel, `studioConformanceEffects`). **BE-9a authoring_run_units.job_id correction-capture → 🚫 S1's producer seam** (§5 charter boundary — building it under S6 would cross-edit S1's authoring-run engine, E6). Both genuinely not S6-buildable; filed for the owner.
