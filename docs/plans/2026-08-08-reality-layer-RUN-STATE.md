...
| meta database | **exists**, **29 tables, 41 migrations** (was 28/35 on 2026-08-08 — `036`/`037` ownership, `038` orphan_scan_finding, `039` session_registry, `040` tier_policy, `041` the actor_control_binding live-unique repair) | `psql -d loreweave_meta -tAc "SELECT count(*) FROM pg_tables WHERE schemaname='public'"`; `ls migrations/meta/*.up.sql \| wc -l` |
