...
| **`CLAIM-ROT`** | **THE MISSING PIECE, opened 2026-08-11 by the audit in `BDR-91`. A document asserting that something is unbuilt, blocked, absent or `none` is a CLAIM about the world, and nothing compares it to the world.** Eight instances measured in this run alone, every one optimistic-for-work (claiming blocked what was built), every one a `SELECT` or an `ls` away: the measured-state table's `world-service` row and its reality count · the pipeline index's `G-S7a`/`G-S7b`/`G-S8a`/`G-S8b` · `world-service/README.md` (*"Cycle 0 scaffold — no behavior"* for a crate with nine binaries) · the pgvector Dockerfile comment, **stale from the hour it shipped**. The run that found four of them created the other four. **Measured Phase 0: 197 documents make a claim of this shape, and prose-matching them is NOT the mechanism** — the corpus is dominated by hypotheticals, past states and quoted error text (*"SQL `column does not exist`"*), so a grep-and-verify gate would be false positives with a real one buried in them. **The shape that works is already in the tree and is per-island:** `actor-hub-figures-gate` (figures, keyed to measurement commands, five recurrences behind it), `design-lint`'s count-drift + phantom-registration arms, `gate-wiring-gate`'s `KNOWN_RED` rows that **fail when the gate turns green**, `dp-oracle-coverage-gate`'s `NO_PRODUCER` shrink arms, `deferral-gate`'s `PROSE_ONLY`. Five mechanisms, five islands, and a run-state's measured-state table is on none of them | **spec first — `docs/specs/2026-08-15-claim-rot.md`, then CLARIFY with the PO.** This is a repo-wide check with a real false-positive budget and a registration cost per claim; it is exactly the *"large / structural, needs a serious plan"* gate-2 case, and building it straight from a run-state row would repeat the mistake it exists to catch |
