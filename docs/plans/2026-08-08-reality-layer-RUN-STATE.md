...
| a reality's database | `lw_reality_cd0747d24b94`, **19 tables** — 13 until 2026-08-22, when `A2` of the world-in-a-running-reality board brought all ten realities to the manifest head and six space tables landed (`map_layout`, `entity_binding`, `place`, `portal`, `layer_registry`, `encounter`). 12 until 2026-08-20, when `0022_actors` was applied to the exemplar so the actor-control live smoke had a registry to mint into. (The 12 itself corrected a ~~13~~ miscount in the first draft, settled 2026-08-08 by diffing the second reality's schema against it.) | `SELECT count(*) FROM pg_tables WHERE schemaname='public'` |
| its migration ledger | **26 applied** — 15 until 2026-08-22. This exemplar was in the `0019_channels` cohort, so `A2` applied eleven: `0020`–`0030`. **The ledger moving at all is itself new**: until that run `migrate` wrote only the META ledger, so a reality's own `schema_migrations` stood still while its schema changed underneath it. | `SELECT count(*) FROM schema_migrations` |
...
| meta database | **exists**, **32 tables, 41 migrations** (was 29/41 until 2026-08-20: the migration FILES were all committed, but `039` session_registry, `040` tier_policy and `041` had never been APPLIED to the dev stack — the actor-control live smoke failed on `relation "session_registry" does not exist` and that is how anyone found out. Before 2026-08-08 it was 28/35 — `036`/`037` ownership, `038` orphan_scan_finding) | `psql -d loreweave_meta -tAc "SELECT count(*) FROM pg_tables WHERE schemaname='public'"`; `ls migrations/meta/*.up.sql \| wc -l` |
...
| game-tier services in compose | `game-server` and `world-service`; `commit-service` still absent. `WS-COMPOSE` is CLOSED — the gap this row named was that world-service had a server binary with no compose entry and no general `Dockerfile`, and `F2` (2026-08-21, demo-path board) wrote both, because a control-plane route nothing in the stack can call is reachable only by whoever remembers the incantation | `grep -cE '^  (world-service|commit-service|game-server):' infra/docker-compose.yml` → **2** of 3 |
...
| `W8` | capacity: make the real path read `shard_utilization` (the drill fakes its snapshot) | ✅ **CLOSED — verified 2026-08-22.** *"Subsumed by `W3`"* was true and left as `⬜`. `capacity_glue::live_snapshot` reads `SELECT DISTINCT ON (shard_host) … FROM shard_utilization` (`capacity_glue.rs:103`), and the REAL path calls it at both entry points — `provision_flow.rs:177` (placement, under the advisory lock) and `:224` (resume). Its own comment states the property: *"a fabricated snapshot here would reintroduce the drill's defect"*. |
...
| `3E` | **adoption** — the bare `reality_id` sites | ✅ **CLOSED 2026-08-22 — SUPERSEDED BY A GATE, which is a stronger mechanism than this row's number ever was.** `scripts/reality-id-adoption-gate.py` reports **0 ADOPTABLE** for both game-layer services, and its scope is not a list but a DERIVATION from the locked `01_scope_and_boundary.md` §4 (`DPA-SCOPE` names exactly `world-service` and `commit-service`), anchored so a reword of §4 reds the gate. The residue is structurally out of scope: of 195 bare `reality_id: Uuid` declarations repo-wide, **71 are in `dp-kernel`, which CANNOT hold a `RealityId`** — it carries `dp-crate = true`, it IS the data plane, and `new_verified` is `pub(crate)` to `dp`. The rest are `meta-rs`/`rebuilder`, not game-layer. **This row's headline figure was corrected five times (457 → 884 → 178 → 84 → 76 → a three-way split) and each correction narrowed it** — a count that keeps being wrong is a proxy, and the gate replaced it with the property. |
