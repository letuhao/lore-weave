...
| Tool | Tier | Notes |
|---|---|---|
| `jobs_list`, `jobs_summary`, `job_get` | R | the read-SSOT for all job state; via S-KIT-PY (jobs-service is Python). Moved here from S-TRANSL (SF-1: jobs-service was unowned). |
...
- **S-JOBS.** `/mcp` server via S-KIT-PY exposing `jobs_list/jobs_summary/job_get` (reads) over jobs-service's existing store. DoD: kit identity+scope tests; tier metadata present.
