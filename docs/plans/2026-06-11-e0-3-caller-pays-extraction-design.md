...
>
> **▶ SCOPE DISCOVERY (2026-06-11 build session) — embedding surface is 3 sites, not 1.** §4 framed passage-embedding caller-pays as a single synchronous `persist-pass2` change. Tracing the code, extraction-triggered embeddings fan out across **three** provider-call sites, two on async RabbitMQ pipelines: `passage_ingester.embed` (sync, in persist path), `summary_processor.embed` (async — billing identity must ride the `summary.*` message), `entity_embedder.embed` (async). The **BYOK invariant is absolute**: the route cannot move OWNER→EDIT until *every* one of these (plus the LLM) is caller-attributed — a single owner-billed summary embedding on a collaborator's job is still a breach. So **route-reopen is coupled to full embedder coverage.** Phase 2 therefore lands in sub-slices (PO-approved 2026-06-11):
> - **2a — billing plumbing (additive, NULL=legacy, route STAYS owner-only, ZERO behavior change).** Lands in 2 commits: **(2a-1)** migration (3 cols) + `extraction_jobs` repo (model/SELECT/INSERT) + worker-ai `JobRow`/SELECT + `eff_*` resolution helpers + billing contextvar + LLM (`extract_pass2`) caller-attribution + fail-safe deny. **(2a-2)** thread billing identity through the 3 embedder sites (`persist-pass2` params → `passage_ingester`; billing fields on `summary.*` message → `summary_processor`; `entity_embedder`). Storage tag stays the project's canonical `embedding_model` at every site.
> - **2b — route reopen.** `require_project_principals` → set `billing_*` on collaborator jobs; need OWNER→EDIT; dimension-guard 409; benchmark stays owner/project-scoped. Needs the ≥3-service live-smoke. Safe because 2a made every provider call caller-attributable.
