...
| `TL-STRING-PRODUCER` | **a string literal counts as a producer.** `dp-oracle-coverage-gate`’s `NO_PRODUCER` shrink arm strips comments but not STRINGS, so the marker `TurnBoundary` inside a test’s assert message — `"the TurnBoundary event is tagged with the new turn"` — was reported as the producer that retired the row. The verdict was right (the doc does now have one) and the evidence was wrong, which is the failure mode that looks most like success. It is the same fixture-not-producer trap the actor-hub incident turned on: *every `pc.created` in the tree was a fixture* | the next `NO_PRODUCER` row retiring. Check WHICH file the arm names before believing it. The fix is to strip string literals as well as comments in `producers_for`, which needs a per-language tokenizer and is why it is a row and not an edit |
...
| ~~`TL-PGVECTOR`~~ **✅ DISCHARGED 2026-08-11** by the world-service server track, and the fix was not code. `infra/docker-compose.yml` has declared `image: loreweave/postgres-pgvector:18` since 2026-08-08 and **the image was already built** — the running container was still `postgres:18-alpine`. `docker compose up -d postgres` recreated it: `vector 0.8.1` available, **161 databases intact**, and a reality provisioned end to end through all 15 migrations. The row's own prescription (*"install pgvector in the image, or drop the stale `pg_extension` row from `template1`"*) was already half-done and nobody had run the other half. Kept below because the diagnosis is what made it findable in one look. See `WSD-3` in [the world-service board](2026-08-13-world-service-server-RUN-STATE.md) — **the general shape is that a compose file and a running container disagreed for three days and nothing said so.** ⟨original⟩ **`template1` on the dev cluster records `vector 0.8.1` as INSTALLED while the cluster has no pgvector files** (PG 18.1 / Alpine), so every database `CREATE DATABASE` makes from it inherits a `pg_extension` row pointing at a missing shared library. `0006_projections` dies on `could not access file "vector"`. This is `W7-TEMPLATE1`'s hazard arriving by its **inverse**: that row warns the template silently *carries* pgvector into every new DB; the live failure is that it silently *claims* to, after the image stopped providing it — so provisioning yields databases that look correct and break on the first vector DDL. Not this track's subject and not fixed here | anyone provisioning a reality that needs embeddings on this cluster, or CI adopting this image. The fix is infra: install pgvector in the image, or drop the stale `pg_extension` row from `template1`. **Do not paper over it in a migration** |
