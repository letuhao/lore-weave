...
| DQ-T36 | The world/map family is UUID-only and the model will not walk the id chain. What is the fix? | **OPEN — owner call.** Measured batch 29, 100 live runs over 4 arms. Every tool takes a UUID, none accepts a name, and the only suppliers are `world_list` → `world_id` and `world_map_list` → `map_id`. The model never chained once. Declaring the supplier on every id argument was SHIPPED and MEASURED: it changed the model's words ("I'll list your worlds now to find it") and not its behaviour — it states the plan and ends the turn. Candidates: (a) accept a NAME on these tools and resolve server-side; (b) a retrieval layer that can rank on a proper noun (the scorer is token-overlap, and `world_get` ties at 0.250 with every tool containing the word "world"); (c) make a second tool call after a refusal a turn-level behaviour. Three tools stay UNCONCLUDED because their replies are a FALSE ABSENCE — they tell the author a world they own does not exist, and one offers to rebuild it. |
