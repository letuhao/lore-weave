...
## Demo milestone
After **C14** the P1 path is end-to-end: ingest Fengshen source → gap-detect → template + cultural-retrieval enrichment → schema-governed proposals (enriched, quarantined) → review → **author promote** → write-back to glossary (+ K14 auto-sync to KG, D4-03 wiki body). That is the demo-able vertical slice. C15+ adds the quality gate and the higher-cost techniques (c/d).
...
| **C6** | Gap MODEL spec (M1a) | Define the canonical **dimension set per entity-kind**, partial-coverage detection rule, gap **ranking**. The product crux — spec + fixtures before engine. | review: dimension model + ranking on a Fengshen fixture | C1, C2 |
...
| **C10** | Strategy (b) retrieval | P1 external cultural retrieval: `source_corpus` ingest + embed via provider-adapter + RAG; `cultural_grounding_ref` populated. | live smoke: embed + retrieve over a seeded corpus chunk | C8, C9 |
...
| **C15** | Eval harness + gate | Cultural-fidelity harness (schema/canon/anachronism/provenance scoring) + promotion quality gate. | eval scorecard; gate blocks below threshold | C14 |
