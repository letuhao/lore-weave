...
| **C15** | Eval (EXTEND framework) + gate | **Extend the existing eval framework additively** — `scripts/enrichment_eval.py` + `eval/enrichment-eval-suite.toml` (weighted sub-scores: schema/canon/anachronism/provenance/usefulness + regression thresholds + baseline-diff) + versioned `eval/baselines/enrichment-vX.json` + in-service persist `enrichment_eval_runs` (mirror knowledge-service benchmark). Reuse LM-Studio quality-gate pattern. NEVER edit climate/geo eval files. | eval scorecard vs baseline; gate blocks below threshold; run persisted | C14 |
