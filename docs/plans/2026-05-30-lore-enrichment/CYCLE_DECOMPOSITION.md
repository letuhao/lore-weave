...
| **C6** | Gap MODEL spec (M1a) | Define dimension set per entity-kind — **demo anchors on entity-kind = LOCATION**: dimensions 历史/地理/文化/features/inhabitants; gap = canon-mentioned place missing these; + ranking. Spec + fixtures (3–5 target places: 玉虛宮/碧遊宮/蓬萊山/城池·关隘) before engine. | review: location dimension model + ranking on Fengshen fixtures | C1, C2 |
