...
- **Cost posture = conservative / autonomous.** RAID runs with **low DPS (2–3)** + **pause-on-quota** (Max subscription). Per the RAID spec the Coordinator runs **C0→C18 in one invocation**, halting only on **escalation / quota / cost-exceeded / secret-leak** — there is **NO human review between cycles or batches** (that is AMAW/v2.2, not RAID; RAID Phase 9 POST-REVIEW is the AUTO Scope Guard). **Human review points = pre-flight sign-off (before the run) + Coordinator final report (after the run).** Demo = milestone at **C14**; the **C15 eval gate auto-blocks** the higher-cost techniques (C16 fabrication / C17 re-cook) unless quality clears threshold, and **escalates to the human on failure** — this is the agent-run cost-control checkpoint, not a manual gate.
- **Application LLM = Qwen 3.6 via LM Studio** (strong Classical-Chinese / 文言文 — addresses the CHisAgent Chinese-reasoning gap). **Resolved via provider-registry, NOT hardcoded** — registry entry points to the LM Studio OpenAI-compatible endpoint (e.g. `http://host.docker.internal:1234/v1`), must be reachable from the service container. **Embedding model** for retrieval (C10) still to confirm in LM Studio (e.g. bge-m3 / nomic-embed). Both used by reference; no model name in code. **VERIFIED LIVE 2026-05-30:** `qwen/qwen3.6-35b-a3b` + `text-embedding-bge-m3` (+ judge `qwen/qwen3-30b-a3b`, gemma) registered & active in provider-registry; LM Studio reachable from container (200). **JIT auto-load:** models load on demand (enrich ≠ judge can't co-reside) — call by id, tolerate first-call load latency; this is the practical throughput limit (not API cost, since local). Matches other branches + eval framework.
...
- **Eval = EXTEND the existing framework, additively.** The platform eval is a script-based pattern: `scripts/climate_eval.py` + `eval/*-suite.toml` (weights + regression thresholds + baseline-diff) + versioned `eval/baselines/*.json`, plus knowledge-service `app/benchmark/` (load→run→**persist** to a runs table) with multilingual LM-Studio quality gates. Enrichment **mirrors the methodology in SEPARATE files** — `scripts/enrichment_eval.py`, `eval/enrichment-eval-suite.toml`, `eval/baselines/enrichment-vX.json`, + an in-service benchmark module persisting `enrichment_eval_runs` — and **does NOT modify the climate/geo eval files** (isolation: those are the geo pipeline's). Records results per version → longitudinal improvement space. **Adopt the judge-ENSEMBLE methodology now on main** (`tests/quality/`: multi-judge majority vote — gemma + qwen-30b + claude-4.7-opus — + Fleiss' κ agreement + partial-credit; `compute_ensemble_macros.py`, `llm_judge.py`) — ideal for SUBJECTIVE cultural-fidelity scoring; reuse it for enrichment quality judging (Qwen already a judge). Chinese fixtures (西游记) already present.
