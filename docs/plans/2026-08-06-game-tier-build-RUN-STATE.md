...
missed defect. It returned **BLOCK**, and it was right on every point. It also **verified the
`1b.13` mutation claim independently, with its own harness rather than mine, and got 7/7 red** —
which is the part that makes the rest credible.

| id | what |
|---|---|
| `1b14-01` | 🔴 **`1b12-05`'s fix introduced a retry regression, and I had already written down why it was wrong.** `apply_migrations` looped the manifest UNCONDITIONALLY — a whole-history replay — and carried the comment *"re-running the whole chain on an existing reality is a no-op rather than an error."* `scripts/dp-migration-chain-smoke.py`'s own docstring, written by me two commits earlier, separates RETRY-SAFETY from WHOLE-HISTORY REPLAY by name and says a versioned runner never performs the second. **This performed the second.** Measured: the second pass dies at `0001_initial`. Worse than a failed retry — `provisioner.rs` documents steps 4-8 as re-drivable, so a reality that crashed mid-provision could NEVER be completed, and the orphan scanner would collect it. **The unit test covering re-entry stayed green because `FakeEffects::apply_migrations` is a `HashSet::insert`: the mock had the property and the live code did not.** Fixed with a `schema_migrations` ledger in the reality DB, and covered by a live test that drives `apply_pending` ITSELF — a second re-implementation inside a test would have repeated the exact mistake. |
| `1b14-02` | 🔴 **`UPDATE channels SET reality_id = …` put a live child under a dissolved parent.** `parent` alone does not identify the parent row — `(reality_id, parent)` does — and the guard's firing condition read `NEW.parent IS DISTINCT FROM OLD.parent`. So moving the ROW re-pointed it at a different channel with the guard seeing no change: one ordinary statement, no deferral, no DDL, no superuser, no concurrency. **A guard whose firing condition does not cover the whole key it guards is wrong regardless of who can reach it today** — and this is the state `1b7gap-H3` and `1b12-01` were both opened for, through a third door. |
| `1b14-03` | 🟠 **The `COMMENT ON TABLE` asserted BOTH claims the slice retracted, 200+ lines below their own retractions in the same file** — and it is **the only artifact of slice 1b that is written into a real per-reality database.** `dp-channels-schema-gate` does not read `COMMENT ON`: default-uncovered, `NV-3`. |
| `1b14-04` | 🟠 The same two retracted claims survived verbatim at three sites in the LOCKED spec. **`1b5-H1`'s "the fix reached one site of N", third occurrence.** |
| `1b14-05` | 🟠 **`0008_pgvector_setup` was registered, so the provisioner applied it unconditionally — and `postgres:18-alpine` does not provide pgvector.** Provisioning died at migration 8 of 15 on every new reality. A second regression from `1b12-05`. It now joins `0009`–`0012` as unregistered with a stated reason and a trigger; the manifest's *"Verified: the full chain applies"* was false and is corrected — the chain smoke SKIPS `0008` and says so, which is precisely the fact the claim needed and did not carry. |
| `1b14-06` | 🟡 `provisioner_live.rs:12` still read *"apply_migrations → run `0001_initial.up.sql`"* — **the precise false sentence `1b12-05` blocked on, left in the header of the file whose body was rewritten to fix it.** |
| `1b14-07` | 🟡 Recorded, not fixed: `metadata JSONB` accepts scalars/arrays/`null`; `display_name` may be blank where `level_name` may not; `dissolved_at` may precede `created_at`. No LOCKED doc requires otherwise. |

#### What the refuter confirmed TRUE, which is why the BLOCK is worth acting on

It re-ran the load-bearing claims instead of reading them: `provisioner_live.rs` really does apply
the whole chain; the live smoke really reports 59/59 with `roots per reality = [1,1]`; the chain
smoke really applies **and retries** 18 and reverses to an empty schema; both gates' self-tests hold.
It also found `1b12-01`'s fix **generalises further than its own test** — a route nobody logged
(`DELETE` the parent and re-`INSERT` the same id already dissolved, inside the deferral hatch) is
refused by `channels_dissolve_order_trg`. And **DELETE is covered by arithmetic rather than by a
rule, and that turns out to be enough**: the tree cannot be orphaned by any deletion order.

#### The one it named against itself

> **The weakest point in my own BLOCK:** `1b14-02` is not reachable in the production per-reality
> deployment shape, and a reader who decides `reality_id` is redundant could downgrade it to LOW.

Recorded because it is the right way to hold a finding. It was fixed anyway, for the reason it gave:
the slice treated the identical scenario (`1b12-04`) as a defect worth changing the migration for.

### `1b.12` — **a third refuter verified the `1b.10` fixes. Three of six did not hold.**

Briefed to attack the FIXES rather than re-audit the schema, and to assume they were wrong until
they proved otherwise. **`1b.10`'s claim — *"every HIGH and MEDIUM discharged, re-verified live:
54/54"* — was false**, and the 54/54 was the problem rather than the evidence: it was green on all
three surviving defects.

| id | what the fix missed |
|---|---|
| `1b12-01` | 🔴 **Both guards only inspected rows that ALREADY EXIST**, so REVERSING THE INSERT ORDER inside the migration's own sanctioned `SET CONSTRAINTS ... DEFERRED` hatch walked around them: insert the child (its parent does not exist yet, so the lookup finds nothing), then insert the parent already `dissolved` (`NEW.parent IS NULL`, so that guard skips; the dissolve-order trigger was `AFTER **UPDATE**`, so it never fired). The FK is satisfied at COMMIT. **Then, because `DP-Ch33` checks children rather than descendants ON THE STRENGTH OF THAT RULE, every ancestor dissolves legally** — leaving a fully dissolved tree with a live leaf. Fixed by firing on INSERT. *The hatch is not the defect; a rule that only inspects the past is.* |
| `1b12-02` | 🔴 **Plain READ COMMITTED write-skew — no deferral, no DDL, no superuser.** Both guards were unlocked predicate reads: T1 inserts a child under an active parent (reads `active`, passes, uncommitted) while T2 dissolves that parent (`EXISTS` cannot see T1's row, passes). Both commit. **Two predicate reads that each pass are not the same as the conjunction holding.** Fixed with `FOR UPDATE` on the parent row, so both paths contend on one row in either order. |
| `1b12-03` | 🔴 **`channels_parent_depth_derived` closed only the probe I imagined.** `parent_depth` is NULLABLE: after `DROP EXPRESSION` you do not FORGE a value, you **OMIT** the column. The CHECK evaluates to NULL — not FALSE, so it passes — and the MATCH SIMPLE foreign key skips a key with a NULL member. A self-parent and a full 2-cycle both INSERT, under a constraint reporting `convalidated = t`. Fixed with `IS NOT NULL`. **Three-valued logic is where a CHECK stops being a check.** |
| `1b12-04` | 🟠 **The `M21` tenancy defect, RECREATED on the trigger added to fix `H3`.** Deleting the `reality_id` filter from `channels_no_child_of_dissolved` is a genuine cross-tenant read and the harness stayed **54/54**. Third occurrence of one defect in one slice. |
| `1b12-05` | 🟠 **Registration is not application, and my commit message said otherwise.** `provisioner_live.rs` applied `0001_initial.up.sql` and nothing else; `migrate` takes ONE id and has no apply-pending path. So a freshly provisioned reality got the cycle-5 skeleton and none of the eighteen migrations after it. **`1b7gap-H1` was half-fixed and claimed whole.** |
| `1b12-06` | 🟠 `0017`/`0018` drop **ten tables** between them and were registered `breaking: false`, so they fan out destructively with no canary — against the manifest's own stated criterion, which `0002`/`0003`/`0004` obey. Nothing checked `breaking` at all. |

#### The pattern, and it is the one this slice keeps producing

**Every fix was correct and none of them was protected.** Five mutations of the shipped migration
were applied one at a time to a pristine file with the whole harness re-run, and **four stayed
green** — including three that reverted the fixes made hours earlier. A fix without a leg is a fix
the next edit silently removes, and *"I verified it live"* is not the same as *"the suite would
notice."* Four legs added: the reverse-insert attack, the omitted-column probe, a real two-session
concurrency leg (the only one in the suite that needs two overlapping sessions, and the only shape
that could ever have caught `1b12-02`), and the tenancy collision.

`1b12-04` needed a change to the MIGRATION to become testable at all: `SELECT ... INTO` takes an
arbitrary row when the tenancy filter is deleted, so the defect was **nondeterministic and a leg
could not catch it reliably**. `INTO STRICT` states what the primary key already guarantees — exactly
one row matches — and turns the deleted filter into a hard `TOO_MANY_ROWS` error. *Sometimes the
thing that makes a defect detectable is a change to the subject, not to the test.*

#### And one in the gate I had just written

`migration-manifest-gate`'s `R6` shipped **vacuous**. The `DROP TABLE` detector's regex contained a
literal **backspace byte** where `\b` should be — a heredoc ate the escape — so `destructive` was
always empty and R6 could never fire on the real tree. Its self-test passed, because that path
passes `destructive` explicitly. **The rule was tested; the wiring was not.** Found by biting the
real tree rather than the fixture, and `cat -A` is what showed it.
...
| **3** | `RealityId` + `SessionContext` | ✅ **BUILT — via the reality-layer board, not one written here. Verified 2026-08-22.** `crates/dp/src/ids.rs` carries all four newtypes; `session.rs` carries `CapabilityToken`, `trait ControlPlane` and `SessionContext`. Checked by reality-layer `3C`'s OWN criterion — `cargo clippy -p dp --all-targets -- -D warnings` **exits 0**, i.e. `new_verified` is no longer dead code — and by its own bite: field → `pub` reds `tests/ui/forged_reality_id.rs`. **The ADOPTION half is reality-layer `3E` and is still open**; this row's *"re-priced by the 457 bare `reality_id` sites"* is that half, and `3E` re-measured it. *No game-tier board was ever written for this slice, and none is needed — the work landed elsewhere and the register was not told.* |
...
| `1b.12` | 🔴 **BLOCK, and the claim in `1b.10` was FALSE.** Three of six fixes did not hold — the reverse-insert order walked around both guards, plain READ COMMITTED write-skew defeated them concurrently, and `channels_parent_depth_derived` closed only the probe its author imagined (`parent_depth` is nullable: OMIT the column and NULL is not FALSE). Plus: registration is not application (`provisioner_live.rs` applied only `0001_initial`), ten `DROP TABLE`s registered non-breaking, and the `M21` tenancy defect recreated on the trigger added to fix `H3`. |
| `1b.14` | 🔴 **BLOCK from a fourth refuter, and TWO findings were regressions from `1b.12`'s own fixes** — the provisioner's manifest loop was an un-retryable whole-history replay (a distinction I had written down myself two commits earlier), and it applied `0008_pgvector_setup`, which `postgres:18-alpine` cannot provide, so provisioning died at migration 8 of 15. Plus a third door to a live-child-under-dissolved-parent, and the retracted claims surviving in the `COMMENT ON` that lands in every real database. |
| `1b.15` | ✅ **all seven discharged and bitten.** A `schema_migrations` ledger with a live test driving the real function (the unit test was green against a `HashSet` mock); the guard's firing condition widened to the whole key; `0008` unregistered with a trigger; the retracted claims deleted from five sites. Live smoke **60/60**. |
| `1b.13` | ✅ **all six fixed AND BITTEN.** Five mutations that previously stayed green now turn the harness red, including a two-session write-skew leg that needed a HANDSHAKE rather than a sleep (docker+psql startup exceeded it, so the "concurrent" test ran serially) and a `lock_timeout` (with the lock working, T2 blocking is the correct outcome, and waiting forever was a deadlock built into the test). `1b12-04` needed `INTO STRICT` in the MIGRATION before any leg could catch it deterministically. |
| `1b.10` | ⚠️ **claimed "every HIGH and MEDIUM discharged, re-verified live: 54/54" — and `1b.12` disproved it.** `0019` was applied by NOTHING (the manifest ended at `0013`, six migrations unregistered, past a row that tracked four); the cycle guarantee lived in DDL rather than data; three semantic mutations left both gates green; a one-statement subtree dissolve was impossible in every row order; and `orphan-model-gate` was cited twice for something it cannot see. |
...
| `1b14-07` | 🟡 **Three column domains nothing constrains, and no LOCKED doc requires otherwise.** `metadata JSONB` accepts a scalar, an array or `null` where every reader will assume an object (`DP-Ch2` calls it *"feature-level bag; DP does not interpret"*, which is a reason to leave it open and not a reason to accept `null`); `display_name` may be blank while `level_name` may not (`channels_level_name_nonempty` exists and has no sibling); `dissolved_at` may precede `created_at`. **Deliberately not fixed**: `1b5-M4` added the `id` and `level_name` domains because `REC-103` had carried a WIDTH across without its DOMAIN — a specific, cited defect. These three have no such warrant, and adding constraints because they are cheap is how a schema acquires rules nobody can justify later. | the first writer of `channels` — the same trigger as `FLOW-19`. A real producer settles what `metadata` may be, and then the constraint has a reason rather than a preference | prose |
...
| `BDR-43` | 🔴 **A gate's self-test can pass while its WIRING is dead, and the shape is worth naming.** `migration-manifest-gate`'s `R6` shipped vacuous: a heredoc turned `\b` into a literal **backspace byte** inside the `DROP TABLE` regex, so the `destructive` set was always empty and R6 could never fire on the real tree. The self-test passed because it hands `destructive` in explicitly — **the RULE was tested and the FEED was not.** ⇒ Bite the gate against the REAL TREE, not only against its fixture; a self-test exercises `check()`, and `main()` is a second, unexercised program. And the recurring mechanical cause is now four-for-four in this run: **`\b`, `\n`, `\u{74}` and `\\n` have all been eaten by a heredoc.** Use the Edit tool or a scratchpad file for anything containing a backslash; `cat -A` is what finally showed it. |
