...
- **D-A4-LIVE-SMOKE** — the full-stack interview live-smoke (a real `faang_swe` session wrapping at 5 / at
  budget + a quarantined `/evaluate` scorecard). **Gate:** a rebuild+redeploy of chat-service + roleplay-service
  with the A4 code (the running images are stale) on a clean checkout (not mid-concurrent-session). All A4
  logic is unit+DB proven; this is the deployed E2E.
