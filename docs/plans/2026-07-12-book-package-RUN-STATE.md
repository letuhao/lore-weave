...
|---|---|---|---|
| P-01 | **27 V2-C2 — the `plan_pass` WORKER op** (dispatch + finalize hook + `sync_from_job`). | C1's registry/fingerprint/derivation are built + tested, but nothing yet ENQUEUES a pass as a `generation_job`. A real slice (worker op + job_consumer dispatch + finalize hook), not a line. | Build C2; then C4 (pass-4 hoist) + C5 (artifact-I/O adapters) make passes 1–7 runnable. |
| P-02 | **27 V2-D (B3)** — `propose_seed` quarantine · checkpoint extension · `roster_bindings` write. | Nothing to checkpoint until a pass can RUN (P-01). | After P-01. |
| P-03 | **27 V2-E3** — bootstrap [A] stamps `outline_node.chapter_id`. | The linker deliberately writes NULL ("planned, not yet written"); stamping is bootstrap's job, and bootstrap runs after the manuscript chapters exist. | After P-01/P-02. |
| P-04 | **27 V2-F/G (B5)** — 3 MCP tools + HTTP mirrors ∥ fixture severing. | The tools would expose a runner that cannot run (P-01). **V2-G is independently REAL:** the rules parser is hard-bound to the POC fixture — numbered `# N.` sections, literal `## Arc 1`/`## Arc 2`, `### Event N`, and HARDCODED Vietnamese themes/summaries for every arc. **A generic book cannot be planned in rules mode at all.** That is a correctness bug, not tech debt. | V2-G can and should be done independently of P-01. |
| P-05 | **27 V2-H (B6)** — S06 replay gate + the pillar-27 DoD. | It is the ship signal for a compiler whose passes do not run yet. | After P-01→P-04. |
