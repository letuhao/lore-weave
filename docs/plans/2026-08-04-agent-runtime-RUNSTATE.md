...
| **CP-5 · the tool contract** | β | 📋 **SPEC 2026-08-09, NOTHING BUILT — and it BLOCKS tool v2 by PO directive.** **17 contract members: 5 exist, 12 are missing.** Measured over 4,175 failed `tool_calls`: repeat semantics **48.8%** · typed inputs **16.8%** · preconditions **10.0%** · argument supplier **9.6%** · partial outcome 2.1% · output contract 1.0% · consent 0.9% · closed vocabularies 0.6% · concurrency 0.4% · empty-change 0.4% · conditional params (tail) — **90.6% directly attributable**, plus 4.7% to *the registry must be the only source of callable names* (`glossary_propose_entity_edit`: **101 calls, 0% success, not in the catalogue**). 🔴 **Member 17, result completeness, scores 0% BY CONSTRUCTION** — `book_list` returns 20 of 197 with `is_complete: false` and the model is never told, so it cannot fail loudly. That is the quiet-failure class V-METRIC exists to detect, sitting inside a tool result, and a contract that fixes only the loud ones converts nothing. **Enforcement ladder is 6 rungs and rung 6 needs no other team**: admission refuses to promote an incomplete contract, so an unmigrated tool stays `draft` and never serves. See `docs/specs/2026-08-09-v2-tool-contract/CP-5.md` |
