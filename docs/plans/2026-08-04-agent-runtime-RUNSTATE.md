...
| 🔴 **`resolve_arguments` has ZERO production callers** | **CP-3's CENTRAL MECHANISM IS NOT WIRED**, found 2026-08-09 by asking what the V-METRIC probe had actually measured. Brick 4 claims *the executor supplies the identifier the model already saw*; today the plan reaches the model as a system message and the model **retypes** the id from it. The dispatcher never receives a bound argument. **This is the run's headline open item**, and it is the same shape as `sweep_expired_runs` — a mechanism whose tests all pass and whose callers do not exist | **CP-3** |
| **the per-declaration metric has n=0 CALLS on the new arm** | The frozen baseline measures over **declaration calls** (7,718 raw / 6,498 organic; carry-forward 38.6% organic, 56.3% raw). The V-METRIC probe asked text questions and produced **0 tool calls**, so it measured a prompt-level proxy, not the baseline's unit. ✅ **The machinery is complete and proven** — the new arm dispatches (`book_list`, `ok`, 79 ms) and each call record carries `declaration` · `runtime_variant` · `source` · `latency_ms` · `ok` · `error`, which is every join key the baseline needs. What is missing is **traffic through it**, and the traffic is only worth generating once the row above is wired | **CP-3** |
