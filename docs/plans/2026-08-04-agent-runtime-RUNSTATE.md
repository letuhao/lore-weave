...
**FINAL STATE — 12 of 13 CLOSED, 1 withdrawn, 1 on a computed `CANNOT DETERMINE`.**
...
| **5.10** | 🟡 **CODE + DATA; LIVE `CANNOT DETERMINE`, computed.** Genuine inventions are **2 of 363 sessions (0.55%)** once the misread 101 are removed. 45 driven sessions expect **0.25** occurrences (P(zero)=0.78); **543** are needed for a 95% chance of one. Not drivable — the mechanism is deployed and awaits a real occurrence |

🔴 **The one open leg is a MEASUREMENT limit, not unbuilt work**, and it is published with its `n` and its bound rather than relabelled — the same discipline that withheld §6.3's rate figure.
