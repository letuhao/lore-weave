...
| **5.8** | 🟡 **HALF BUILT — pre-dispatch done; the ADVERTISEMENT gate deliberately not attempted** | 🔴 **MEASURED: 414 calls / 82 sessions — the largest remaining population by the honest denominator** — and **every failing tool ALREADY DECLARES what it needs.** `_meta.scope` is set across the whole catalogue (**194 `book` · 65 `user` · 33 `project` · 23 `none`**) and nothing consulted it, so the model learned the requirement from a round trip and a backend error like `no project in scope`. A project-scoped tool with no project in scope is now refused **before the wire**, typed `refused` (5.7), naming `kg_project_list` / `kg_project_create` as the way out. ⭐ **GATED ON `project` ONLY, AND THAT IS THE LOAD-BEARING DECISION, VERIFIED BEFORE BUILDING:** `scope: book` is a **scope KEY, not a precondition** — `book_list` is itself `scope: book` and is how a model FINDS a book, so gating it would make books unreachable; and `kg_project_create` / `kg_project_list` are `scope: user`, so the path to create a project stays open under this gate. Both facts are guarded. ✖ **The §4.3 advertisement half is NOT attempted and that is a judgement, not an oversight:** withholding tools from the model is the highest-blast-radius change in this checkpoint — a wrong gate silently starves the surface — and it deserves its own verification pass rather than the tail of a long session |
...
from OBSERVED traffic once the mechanism is deployed.** All three mechanisms are deployed; the
evidence is now a matter of waiting for the next occurrence, and the rows stay open until then
rather than being called closed on two legs.
