...
| 🔴 **the discriminating V-METRIC needs DISTANCE** | The probe put `book_list` and `book_read` adjacent, so the conversation was not lossy and the control retyped correctly 15/15. The baseline's 61.8% is eviction across turns — *"tool results beyond the newest 3"*. **Design for the next run:** the bound value must be one the control cannot RE-DERIVE (re-listing reproduces it, so `book_list` cannot be the source); use a value that appeared once — e.g. a throwaway book created in turn 1 — then ≥3 further tool-bearing turns to evict it, then ask for the read. Both arms identical; grade the DISPATCHED args. **Until that runs, no reduction figure is published** | **CP-3** |
