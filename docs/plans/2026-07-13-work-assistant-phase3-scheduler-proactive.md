...
|---|---|---|---|---|
| **WS-3.1** | Scheduler substrate | `scheduled_agent_runs` table + the tick-driver loop (claim/lease/re-arm/breaker) + a per-user `enabled` setting. Enqueues onto existing consumers. | — | The load-bearing new infra. Copy the authoring-run driver; `FOR UPDATE SKIP LOCKED`. |
| **WS-3.2** | Auto end-of-day distill | The tick enqueues `eod_distill` at the user's local EOD → the EXISTING `assistant.distill` consumer. Draft-into-inbox inherited. | WS-3.1, distiller (built) | Low — pure wiring onto a proven seam. |
| **WS-3.3** | Catch-up sweep (P-10(d)) | A returning user's missed days journaled, **bounded** + **spend-capped** (period-digest for >5-day gaps, T20). | WS-3.2, spend cap (WS-2.8, built) | Q4/T20 — MUST be cost-capped or a multi-day sweep blows the budget. The cap now exists, so this is unblocked. |
| **WS-3.4** | Away marker | `assistant_away_periods`; detectors + nudges exclude away days. | — | Small; unblocks safe nudges. |
| **WS-3.5** | Proactive-turn seam | §4 mini-spec: `chat_messages.initiated_by`, the headless `stream_response` path, `proactive_enabled` (default false), spend-metered, away-aware. | WS-3.1, WS-3.4, chat stream (built) | 🔴 The net-new architecture. Build behind `proactive_enabled=false`; live-smoke before enabling. |
| **WS-3.6** | Content-free nudges | The `nudge` job → a **content-free** notification (unfinished entry, etc.). Reuses the locked content-free path; the distiller failed-error suppression constraint (below) lands here. | WS-3.1, notification (content-free locked) | Low — the invariant is already tested. |
| **WS-3.7** | Costed weekly rollup | A scheduled weekly summary DRAFT (not auto-canon), spend-capped; the md5 cache never hits at book level so it costs real tokens. | WS-3.1, WS-3.3 | Cost — schedule + cap; a draft the user reviews. |
| **WS-3.8** | Weekly reflection (spec 08 §weekly only) | The reflection as a **proactive turn** (WS-3.5) or a draft. NOT the coaching scorer (that is P5). | WS-3.5 | Scope-guard: reflection ≠ coaching. No rubric/score here. |
