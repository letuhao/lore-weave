...
- Pure helpers `parseRequestSteps` + `buildStepOutcomes` in `scope-filter.ts`; for a batch (`Array.isArray(body)`) annotate the response **per item** with an additive `_meta.step_outcome`, single requests unchanged (backward-compat). Build the outcome **as the edge gates** (source of truth), refine `relayed`→`failed` by matching the upstream entry's id. Batch-level rate-limit/upstream-fail stays a single error (no per-step). Smallest slice.
  - **PO DECISION (2026-06-28, at POST-REVIEW):** keep the **bare JSON-RPC array** shape (H-M transport-transparency) and attach each step's verdict to that item's `_meta.step_outcome` — NOT a `{step_outcomes, result:[…]}` wrapper (a wrapper would deviate from a bare-array batch response; a strict client would have to unwrap). `annotateBatchStepOutcomes` mutates each item in place, merging into any existing `_meta`.
