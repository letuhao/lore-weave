...
| # | Slice | Status | Evidence |
|---|---|---|---|
| B1 | AN-C2 discovery scent (`stream_service.py`) — unblocks P-08→S06→DoD of 23/27/28 | [x] | **UN-PARKED (other session done, PO confirmed).** `_ORIENTATION_SCENT` module constant appended to `book_context_note`, naming the 3 orientation reads + the F7 verify-nudge; `test_orientation_scent.py` 3 pass. The false C2/C3 [x] (DR-A) is now REAL. Unblocks S06. |
