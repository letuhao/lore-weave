...
| **WS-3.3** catch-up sweep (spend-capped) | ✅ **DONE (ee3dee3c2)** — distill trigger `catchup_days` (bounded to 14, reuses the whole distill path, idempotent kept-day 409-skip) + scheduler daily eod_distill carries `catchup_days:3`. Chat catchup test green (100→15 bounded). |
