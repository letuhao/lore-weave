...
| **WS-4.1** shared-inner-generator refactor · **WS-4.2a** LLM-token billing · **WS-4.2b** STT/TTS usage plumbing (+ unbuilt `lane` col) · **WS-4.3** retention→per-user (≤48h) · **WS-4.4** audio joins D-R27 erase · **WS-4.5** affordance gate (live bug) | ⬜ | **WS-4.1 investigation DONE (resume-ready):** voice (`voice_stream_service.voice_stream_response`) calls `stream_service._stream_via_gateway` (RAW stream, ~line 407), NOT `_stream_with_tools` — so a voice turn (a) can't call tools, (b) **never fires `maybe_capture_canon`** (the WS-4.1 capture gap; capture lives in `_emit_chat_turn` post-turn ~line 5299), (c) hardcodes `input_tokens=0,output_tokens=0` for billing (`voice_stream_service.py:544-545`) + `finish-message` usage `0/0` (`:529`) — the WS-4.2a gap. `_stream_with_tools` is the ~440-line generator @ `stream_service.py:1214`; `_emit_chat_turn` @ `:4378`. The content-dict seam yields `{"content","reasoning_content",...}`; voice's `content = chunk_data["content"]` (`:416`) KeyErrors on tool_call/suspend/agent_surface chunks — must handle. **WS-4.1 is 🔴 High + touches the MOST load-bearing path (main chat streaming); acceptance live-smoke = "voice turn calls a tool + fires capture + bills real tokens" ⇒ needs LM Studio = DOWN.** lane column UNBUILT; audio sweeper EXISTS; voice NOT gated for assistant sessions today (WS-4.5 live bug). |
