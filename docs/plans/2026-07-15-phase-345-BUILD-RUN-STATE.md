...
| **WS-4.1-tools** (voice → `_stream_with_tools` for mid-turn tool calls) · **WS-4.2b** STT/TTS usage plumbing (+ unbuilt `lane` col) | ⬜ **LARGE/STRUCTURAL remainder (gate #2)** | Both NOT acceptance-critical (the P4 live-smoke passed w/o them). **WS-4.1-tools** = route voice's LLM stream through the ~60-param `_stream_with_tools` (@ stream_service.py:1214) handling tool_call/suspend/agent_surface chunks (buffer only content for TTS); large refactor of the most load-bearing path. **WS-4.2b** = cross-service (provider-registry STT/TTS adapters EMIT usage) + **loreweave_llm SDK** change (`stream_tts` discards UsageEvent @ SDK client.py) + **UNBUILT spend `lane` ×3 tables (T-8)** — needs its own plan. **FE gate (WS-4.5) STAYS until WS-4.2b lands** (plan: "until 4.1/4.2 land"; STT/TTS metering is the last billing gap — $0-capable locally but un-metered). |
