...
| **WS-4.1** shared-inner-generator refactor · **WS-4.2b** STT/TTS usage plumbing (+ unbuilt `lane` col) | ⬜ **LARGE/STRUCTURAL remainder (gate #2)** | **P4 milestone review DONE (cold-start adversarial on WS-4.0/4.2a/4.3/4.4/4.5): CLEAN** — 1 LOW (WS-4.3 session door doesn't validate `audio_retention_hours`, but retention is account-scoped by design + no writer sends it there — accept+documented); standards gate PASS (Settings-Boundary SET-6 + per-user-not-env, tenancy owner-scoped, no direct-SDK/model-literal). 223 affected tests green. **WS-4.1** = 🔴 High refactor of the MOST load-bearing path (`_stream_with_tools` @ stream_service.py:1214 / `_emit_chat_turn` @ :4378 / capture @ :5299; voice seam @ voice_stream_service.py:407/416); acceptance live-smoke "voice turn calls a tool + fires capture" ⇒ **needs LM Studio = DOWN (gate #4)**. **WS-4.2b** = Med-High cross-service (provider-registry STT/TTS adapters EMIT usage) + **loreweave_llm SDK** change (stream_tts discards UsageEvent @ SDK client.py:326) + the **UNBUILT spend `lane` ×3 tables (T-8)** — needs its own plan (gate #2). Both genuinely large/structural, NOT fix-now. |
