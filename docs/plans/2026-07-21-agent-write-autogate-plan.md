...
**M0c prerequisite to verify FIRST (chat-service suspend):** confirm `book_update_meta` returning a `{confirm_token,...}` card actually SUSPENDS the run / surfaces the card the same way delete/publish do. It's Tier-W and mints (no write), so the confirm-token path should fire — but verify in `stream_service.py` that a Tier-W tool RESULT carrying `confirm_token` is surfaced to the FE as a proposal (grep the confirm_token handling around the tool-exec envelope). If book_update_meta's card isn't surfaced, that's the real M0c blocker, not the FE render.
