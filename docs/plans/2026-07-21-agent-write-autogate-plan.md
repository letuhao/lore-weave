...
- 2026-07-21 (M0d) — **NEW FINDING, breaker gap:** the reasoning-loop breaker's steer forces `reasoning_effort="none"` on the retry, but **lm_studio/Gemma IGNORES it** and keeps emitting reasoning — so the steered retry re-loops instead of acting. The cap still prevents an infinite hang, but the steer doesn't *resolve* the loop on local reasoning models. Follow-up: strengthen the steer (lower the cap to 1, and/or de-advertise the specific tool the model is thrashing between, and/or a harder "call the tool NOW, no analysis" system message), and don't rely on reasoning-off alone. Filed for a dedicated fix.
