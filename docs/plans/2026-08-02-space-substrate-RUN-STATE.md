...
**RESUME: `SDF-R3` is APPLIED and built — a seeded world is now traversable. Next: the `ChannelTier` sweep at 73 code sites / 22 files, which needed `SPG-R14`'s mapping and now has it, and which is a SEMANTIC remap (five rungs → three kinds plus recursion), not a rename. Then `SDF-R5`·`R7`·`R8`·`R9` and the inherited `SPG-R5`·`R11`·`R12`·`R13`, `WSA-R19..R24`. `SDF-Q15` unblocks once a space view exists. NOT queued: `SDF-Q12`, blocked on a `PROG_001` parameter this tier may not invent. Also owed, recorded not assumed: `VALIDATE CONSTRAINT` on `0027` once the 313 measured orphan rows are reconciled.**
