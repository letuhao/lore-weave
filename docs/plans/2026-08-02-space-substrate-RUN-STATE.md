...
**RESUME: `SDF-R1`·`R2`·`R3`·`R7` applied; `SPG-A3`, `SPG-R13`, `SPG-R14` done; `ChannelTier` gone from code. Next: `SDF-R8`/`R9` against doc 37 (snapshot-compaction, and per-table scope) — both are doc amendments, so pair each with something bitten. `SDF-R5` is BLOCKED: layers bind to `MapKind` and the layer registry `T4` is unbuilt. Then inherited `SPG-R5`·`R11`·`R12`, `WSA-R19..R24`. Owed and recorded: `VALIDATE CONSTRAINT` on `0027` once the 313 measured orphans are reconciled; the `tier` IDENTIFIER rename, behind `SPG-F4`'s vocabulary split. NOT queued: `SDF-Q12`, blocked on a `PROG_001` parameter this tier may not invent.**
