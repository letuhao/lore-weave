...
**RESUME: `SPG-R11` and `SPG-R12` are VERIFIED — the first narrowed, the second confirmed with a second finding. Next, in order: (1) `SPG-R12`'s second finding is now the sharpest open defect in the corpus — `TVL_004`'s `progress_fraction: f32` is a replay divergence, and fixing it is a row in the TRAVEL tier with `SDF-R2`'s precedent to follow. (2) `SPG-R9` is still `verify` and its target `TMP_001` was touched this session. (3) `SPG-R8` is still `verify`. (4) `SPG-R5` half (1), `SDF-R4`, `SDF-R6` — each needs a `_boundaries` claim or another tier. (5) `SDF-R5` needs the layer registry `T4`. Owed with triggers: `VALIDATE CONSTRAINT` on `0027` · the `tier` identifier rename · two gates refused on measurements, each with the shape its replacement needs.**
