...
**RESUME: the space substrate is BUILT and the design register is closed to what this tier can decide alone. Shipped this run: four migrations (`0024` kind · `0025` siting · `0026` place · `0028` portal) + `0027` discharging `FLOW-19`, the `world_seed` bootstrap, `SPG-A3`'s matrix and `SDF-A19`'s scale bound in code, a parity gate, and six of nine `SDF-R*` applied. **An actor stands in a seeded place, verified end to end.** Every remaining row needs one of three things this tier cannot supply alone: a `_boundaries` LOCK CLAIM (`SPG-R5` — `cell_scene_layout` is a registered aggregate), ANOTHER TIER'S decision (`SDF-R4` combat · `SDF-R6` `CSC_001` · `SPG-R11`/`R12` · `WSA-R19..R24` · `SDF-Q12` needs a `PROG_001` distribution), or ROUND-3 INFRASTRUCTURE (`SDF-R5` needs the layer registry `T4` — the whole layer model). Owed with named triggers: `VALIDATE CONSTRAINT` on `0027` after the 313 measured orphans are reconciled · the `tier` identifier rename behind `SPG-F4` · a reality_id scope gate needing live-schema introspection.**
