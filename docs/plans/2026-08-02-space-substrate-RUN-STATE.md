...
**RESUME: every `SPG-R*` row is now APPLIED, VERIFIED, or RETIRED — none is left unexamined. The sharpest open defect in the corpus is `SPG-R12`'s SECOND finding: `TVL_004`'s `progress_fraction: f32` is a float in a replayed simulation, which `WDS-A7` ruled out and `SDF-R2` removed from `Transform` today. It is a TRAVEL-tier row with `SDF-R2`'s precedent to follow. After that: `SPG-R5` half (1) and `SDF-R4`/`R6` need a `_boundaries` claim or another tier; `SDF-R5` needs the layer registry `T4`; `WSA-R19..R24` are doc 32's. Owed with triggers: `VALIDATE CONSTRAINT` on `0027` · the `tier` identifier rename · two gates refused on measurements, each with the shape its replacement needs.**
