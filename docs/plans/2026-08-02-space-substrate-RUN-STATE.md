...
**RESUME: SIX of nine `SDF-R*` are applied. What is left is genuinely BLOCKED or belongs to another tier, and each says which: `SDF-R5` needs the layer registry `T4`, which is the whole layer model and round-3 scope · `SDF-R4` is `COMB_*`'s Encounter closure · `SDF-R6` is `CSC_001`'s 16×16 default · inherited `SPG-R5`·`R11`·`R12` and `WSA-R19..R24` are other features' docs. Owed and recorded: `VALIDATE CONSTRAINT` on `0027` after the 313 measured orphans are reconciled · the `tier` IDENTIFIER rename, behind `SPG-F4`'s vocabulary split · a reality_id scope gate, which needs live-schema introspection because migration text gives a wrong answer. NOT queued: `SDF-Q12`.**
