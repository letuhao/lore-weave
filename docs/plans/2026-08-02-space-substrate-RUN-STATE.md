...
**RESUME: 2-5 is DONE — an actor now stands in a seeded place, end to end. Next is the queue behind it: `SDF-R3` (PortalSet), which BLOCKS the `connections` column deliberately left out of both `0024` and `0026` and is the last thing between a seeded world and a traversable one. Then the `ChannelTier` sweep at 73 code sites / 22 files, which needed `SPG-R14`'s mapping and now has it. Then `SDF-R5`·`R7`·`R8`·`R9` and the inherited `SPG-R5`·`R11`·`R12`·`R13`, `WSA-R19..R24`. `SDF-Q15` unblocks once a space view exists; `SDF-Q12` is not queued — it is blocked on a `PROG_001` parameter this tier may not invent.**
