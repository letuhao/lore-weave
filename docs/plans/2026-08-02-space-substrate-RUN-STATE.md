...
**RESUME: `SPG-R13` is resolved and its presets renamed. Next: the `ChannelTier` WIRE enum — 29 Rust sites in `tilemap-service`, ~26 TS sites in `frontend-game` + `poc`, and ONE string in `tests/golden/tilemap_baseline.json`. It is `serde(rename_all = "snake_case")`, so Rust, TypeScript and the golden must move in ONE commit or the wire breaks; the mapping is `SPG-R14`'s (Continent/Country/District → `Region`, Town → `Locale`, Cell → `Domain`) and nothing branches on the value except `poc/tilemap_world_view/src/llm/validator.ts:379`. Then `SDF-R5`·`R7`·`R8`·`R9`, inherited `SPG-R5`·`R11`·`R12`, `WSA-R19..R24`. Owed: `VALIDATE CONSTRAINT` on `0027` once the 313 measured orphans are reconciled. NOT queued: `SDF-Q12`.**
