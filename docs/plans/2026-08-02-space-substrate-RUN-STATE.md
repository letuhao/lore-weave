...
| **2-5** | The production birth path | **STARTED — first table landed 2026-08-22** | **`0024_map_layout`**: `MapKind` was designed 2026-07-29 and until now appeared in **zero `.rs` files and zero tables** — `channels` shipped the tree, nothing shipped what a node IS. One row per channel: `kind` (closed CHECK, the retired ladder's rungs now **unwritable**) + parent-relative `0..1000` position. **No `materialization` column** — `SDF-R1` applied first, deliberately, because a column is cheap to add and expensive to remove and this one was already MEASURED wrong. **Verified against real Postgres 18**: applied, re-applied (idempotent), all six constraints bitten (FK, unknown kind, reserved `vessel`, out-of-frame position, cross-reality, CASCADE), down applied, down re-applied, up round-tripped, scratch DB dropped. **Still to come:** siting (`EF_001` `entity_binding`, `SDF-A34`) · `place` rows (`PF_001`) · a bootstrap that writes them |
