...
**RESUME: the space substrate's amendment work is COMPLETE. All nine `SDF-R*` applied · every `SPG-R*` applied/verified/retired/decided · `WSA-R19..R24` measured, with `WSA-R23` verified and its mechanism corrected · `T3`/`T4`/`T9` built · `SPG-F4`'s `tier` collision closed in code. **TWO items remain and BOTH are decided rather than open:** `SDF-Q12` cannot be closed by this tier at all — it needs a `PROG_001` realm distribution, and inventing one from the map tier's chair is the encroachment this whole round refused (`SDF-F8`); `VALIDATE CONSTRAINT` on `0027` is an OPERATIONAL step, decided so because a migration that fails on 313 measured historical orphans would block provisioning for every reality that ever acquired a lease. Two gates were refused on their measurements, each recorded with the shape its replacement needs.**
