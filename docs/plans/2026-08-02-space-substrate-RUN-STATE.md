...
| **2-5** | The production birth path | **all 3 tables landed 2026-08-22; the BOOTSTRAP remains** | **`0024_map_layout`** a node gets a KIND (no `materialization` column — `SDF-R1` landed first) · **`0025_entity_binding`** WHERE THINGS ARE, implementing `EF_001` §3.1 with a **sum-type CHECK** and **no fine position** (`SDF-A34`), FK **RESTRICT** so `R-52`'s *evacuate, never delete* is the database's refusal · **`0026_place`** SEMANTIC IDENTITY, implementing `PF_001` §3.1, with the 1:1 *a place only on a `Domain`* rule made **structural** by `0019`'s own generated-column-into-composite-FK device (and carrying `0019`'s later honesty up front: `DROP EXPRESSION` means this is stronger than a check, not unrepresentable). **All three verified on throwaway databases** — applied, re-applied, **13 + 10 bites each proven to REJECT**, down, down-again, up round-trip, dropped, zero scratch DBs left. **Left: the `PF_001` §5 five-step bootstrap whose step 5 is the spawn** |
...
**RESUME: 2-5 — all three tables are in (`0024` kind · `0025` siting · `0026` semantic identity). What remains is the BIRTH PATH: `PF_001` §5 numbered bootstrap at `RealityManifest` ingestion — (1) DP creates the channel hierarchy, (2) place rows from `places: Vec<PlaceDecl>`, (3) a REFUSING validator `place.missing_decl`, (4) fixture seeds, (5) **NPC + PC canonical seeds place actors at nodes whose place rows are now valid — step 5 IS THE SPAWN.** Nothing in production creates a channel today; every `INSERT INTO channels` in the repo is in a test. Verify against a throwaway database the way `0024`–`0026` were: apply, re-apply, bite every refusal, down, down-again, round-trip, drop.**
