...
**RESUME: every `SPG-R*` row is APPLIED, VERIFIED or RETIRED, and the sharpest defect they surfaced — a float in the replay path — is fixed at its ROOT in `TVL_001`, not just where the row pointed. What remains needs something this tier cannot supply alone, and each says which: `SPG-R5` half (1) needs a `_boundaries` claim (`cell_scene_layout` and `csc.*` are registered) · `SDF-R4` is `COMB_*`'s Encounter closure · `SDF-R6` is `CSC_001`'s 16×16 default · `SDF-R5` needs the layer registry `T4`, which is the whole layer model · `WSA-R19..R24` are doc 32's · `SDF-Q12` needs a `PROG_001` distribution. Owed with named triggers: `VALIDATE CONSTRAINT` on `0027` after the 313 measured orphans · the `tier` identifier rename behind `SPG-F4` · two gates refused on measurements, each recorded with the shape its replacement needs.**
