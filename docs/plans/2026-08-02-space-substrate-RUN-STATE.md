...
**RESUME: SEVEN of nine `SDF-R*` applied and every `SPG-R*` row applied, verified, retired or decided. TWO rows remain and neither is unfinished work this tier is avoiding — each is work this tier MAY NOT DO: `SDF-R4` is `COMB_*`'s Encounter closure, and designing another feature's combat from the map tier's chair is the encroachment the actor round exists to stop; `SDF-R5` needs the layer registry `T4`, which is the entire layer model and round-3 scope. Inherited `WSA-R19..R24` belong to doc 32. Owed with named triggers: `VALIDATE CONSTRAINT` on `0027` after the 313 measured orphans · the `tier` identifier rename behind `SPG-F4`'s vocabulary split · two gates refused on measurements, each recorded with the shape its replacement needs.**
