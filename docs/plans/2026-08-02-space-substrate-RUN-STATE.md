...
**RESUME: ALL NINE `SDF-R*` are APPLIED, every `SPG-R*` row is applied/verified/retired/decided, and `T3`/`T4`/`T9` — the three tables doc 41 called absent or wrong-shaped — are BUILT. What is left is not amendment work: the `tier` IDENTIFIER rename behind `SPG-F4`'s vocabulary split (the word also means treasure tier in the same files) · `VALIDATE CONSTRAINT` on `0027`, an OPERATIONAL step rather than a migration because a migration that fails on historical data blocks provisioning · `WSA-R19..R24`, doc 32's own inherited set · `SDF-Q12`, needing a `PROG_001` distribution. Two gates were refused on their measurements, each recorded with the shape its replacement needs.**
