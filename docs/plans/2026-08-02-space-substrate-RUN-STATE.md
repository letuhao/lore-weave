...
**RESUME: `SPG-A3` is enforced and `ChannelTier` is gone from code. Next: `SDF-R7` — give `NodeDecl` a `WorldScale` and bound `Domain → World` to `Pocket`, which is where the matrix implementation already says it will hook and which `SDF-F7` prices at up to 500× the authored world. Then `SDF-R5` (blocked: needs the layer registry `T4`, unbuilt) · `SDF-R8`/`R9` against doc 37 · inherited `SPG-R5`·`R11`·`R12`, `WSA-R19..R24`. Owed and recorded: `VALIDATE CONSTRAINT` on `0027` once the 313 measured orphans are reconciled; the `tier` IDENTIFIER rename, blocked behind `SPG-F4`'s vocabulary split. NOT queued: `SDF-Q12`.**
