...
**RESUME: `ChannelTier` is gone from code — the retired ladder now survives only where docs cite its retirement. Next: `SDF-R5` (layers bind to `MapKind`) · `SDF-R7` (the scale matrix, which `SDF-F7` prices at up to 500× the authored world) · `SDF-R8`/`R9` (both against doc 37) · inherited `SPG-R5`·`R11`·`R12` and `WSA-R19..R24`. Owed and recorded: `VALIDATE CONSTRAINT` on `0027` once the 313 measured orphans are reconciled; the `tier` IDENTIFIER rename, blocked behind `SPG-F4`'s vocabulary split because the word also means treasure tier. NOT queued: `SDF-Q12`, blocked on a `PROG_001` parameter this tier may not invent.**
