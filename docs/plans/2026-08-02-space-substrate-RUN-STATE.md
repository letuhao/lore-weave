...
| **NEW** | **A "half-applied annotation" gate was prototyped and REFUSED on its measurement.** Twice this session a summary moved and its declaration did not (`PF_001`'s header, `CSC_001` §4.2), so a check for *"the doc says `was X` while `X` still appears live in a fence"* looked obvious. Prototyped across 386 docs: **47 candidates, mostly false** — `was` matches ordinary prose (*"the register's highest id was `REC-98`"*), and substring matching catches `tier` inside `tier_metadata`. **Same failure mode as check E's first draft**, and shipping it would have put a 47-finding baseline in front of a real one | a token-level matcher scoped to `pub NAME: TYPE` declarations, which is a parser rather than a regex |
| **NEW** | **A "every per-reality table carries `reality_id`" gate CANNOT be written from migration text, and that was measured rather than assumed.** `WDS-A10` makes the scope rule explicit, so mechanising it looked obvious. Scanning `CREATE TABLE` across `contracts/migrations/per_reality/` reports **13 of 30 tables missing the column** — and the number is WRONG: it reads migration HISTORY, and several of those tables were later dropped (`0017`, `0018`) or gained the column in a later migration. The final schema is the RESULT of 30 migrations including drops and alters, so a correct check must introspect a LIVE database rather than parse text. **A gate built on the text scan would have shipped a 13-finding baseline that is mostly false** | a live-schema introspection harness, which `provisioner_reentry_live` already has the shape of |
| **NEW → PROVEN 2026-08-22** | **`VALIDATE CONSTRAINT` on `0027` — the discharge PROCEDURE is now run, not described.** The migration left it owed with a trigger, and **a deferral whose discharge procedure has never been executed is a promise, not a plan.** Three things nobody had checked: does `VALIDATE` actually FAIL while an orphan is present, or does `NOT VALID` make it permanently unvalidatable · does reconciliation make it succeed · is it STILL A RATCHET afterwards. `writer_state_validate_live.rs` answers all three against a real database, and **the first is the bite** — a validation that could not fail would certify a database that still holds the defect. Mutation-verified: a reconciliation that deletes nothing reds with *"reconciliation removed 0 rows, wanted 1"* | **the operational run against the 313 measured orphans — now a procedure with evidence rather than a sentence** |
...
| 13 | **A mutation script died on escaping and I nearly read the resulting green as a bite.** The patch's Python literal ended in a backslash, so it raised `SyntaxError`, wrote nothing, and the `cargo test` in the same command printed `ok` — for the **unmutated** test. **A bite that did not apply looks exactly like a bite that found nothing.** Caught by checking the script's own output rather than the test's; re-applied through a file with a backslash-free anchor, and it reds. Fourth escaping failure this session, and the first that could have produced a FALSE CLAIM OF EVIDENCE rather than a broken artifact. |
| 12 | **My own verification grep could not fail.** Checking migrations for errors with `grep -i "^ERROR"` **never matches**, because `psql -f file` prefixes every diagnostic with `psql:/tmp/x.sql:NN:` — while `psql -c` does not, which is why the pattern looked fine everywhere else. So `0027` reported *"no ERROR = applied"* against a database where `channel_writer_state` **did not exist**. Caught by the next assertion disagreeing, not by the check. **NV-1 in my own harness, in the middle of a run whose rule 2 is that a check which cannot fail is not a check.** |
| 11 | **A live test reported `ok` in 0.00 s and that was the SKIP path** — `LOREWEAVE_TEST_PG_ADMIN_URL` was unset, so the gated body never ran. A skipped test reporting success is the vacuous-check shape arriving through a gate deliberately built to be skippable. Caught by reading the DURATION rather than the verdict; with a real DSN it takes 0.6–0.8 s. |
...
| **2-4** | ~~The map hub contract~~ **the RECONCILIATION** | **DONE 2026-08-22** | **`SPG-R14` written and APPLIED.** Doc 36 had thirteen amendment rows correcting nine feature docs and **missed `PF_001`** — the CANDIDATE-LOCK doc that owns the `place.*` namespace and is where an actor comes into existence. **The mapping is `SPG-A3`'s own consequence, not a new decision: five rungs become THREE KINDS PLUS RECURSION** (`Continent`/`Country`/`District` → nesting `Region`, `Town` → `Locale`, `Cell` → `Domain`), because `MapKind::Region` is declared recursive and `Country` was never geography. **A place is 1:1 with a `Domain`.** `PlaceId(ChannelId)` needed **no** change — `PF_001` reached `SDF-A31` from the other side in April. Reject-rule ids deliberately **not** renamed: an id is a contract, the words inside it are not. **Definitional sites swept, illustrative ids explicitly NOT** — stated in the amendment note so a half-application cannot be read as a full one (REC-97) |
| **2-5** | The production birth path | **DONE 2026-08-22 — a world is seeded and an actor stands in it** | Three tables (**`0024`** kind · **`0025`** siting · **`0026`** semantic identity) plus **`world-service/src/world_seed.rs`**, the world-structure step of bootstrap. Measured first: `reality_seeder` already runs in the `seeding` stage and seeds **canon**, not space — it creates no channel, and every `INSERT INTO channels` in the repo was in a test. **One documented deviation from `PF_001` §5**: validate FIRST, so a refusal writes nothing, because a half-written bootstrap manufactures the state `orphan_scan` exists to collect. **Bitten twice, not asserted once** — disarming the Domain-must-have-a-place rule reds exactly one unit test; making it write before validating reds the live test with *"a REJECTED seed wrote 1 row(s) into channels"*. Both restored byte-exact. **9 unit + 1 live green, 168/168 lib, zero stray databases** |
| **2-7** | `SDF-R3` — the connective relation | **DONE 2026-08-22** | **`SPG-A18`** written into doc 36 §3.2 **and `0028_portal` built**, because a prose-only cycle is not a cycle. `parent` is containment and cannot express a door. **ONE ROW, TWO ENDS** + a unique index over `LEAST/GREATEST` makes `R-59`'s one-sided door link **unrepresentable**; ends are `a`/`b` because `from`/`to` invites the directed reading that caused it. **10 bites, each proven to REJECT** — including #8, a DIFFERENT pair on the same node, which proves the uniqueness is not over-constrained. This unblocks the `connections` column both `0024` and `0026` deliberately left out |
| **2-8** | `SPG-R13` — the tilemap presets, and the `ChannelTier` sweep MEASURED | **`SPG-R13` DONE; the wire enum SPLIT OUT with a reason** | **Rule 3 shrank the row, then a bite grew it back.** Measured before touching code: **nothing branches on the tier** (11 constructions in `src/`, one branch in the whole repo, in `poc/`), `grid_size` is **already a field**, and the `256/192/128/64` constants say *"author-configurable"* in their own source with two consumers. So the collapse costs **naming, not semantics**: they were never a per-kind map but a **ZOOM LADDER**, and `SPG-A3` removed ladders — under a matrix, depth is not a kind, so a per-kind default cannot reproduce per-rung sizes and should stop pretending to. Renamed to `ZOOM_256/192/128/64`. **THEN THE BITE FOUND THE REAL DEFECT:** mutating `ZOOM_64` to 32² left **all 521 tests passing** — the tier NAME had been carrying the tie to `TMP_001` §2 by implication, and renaming to a size role dissolved it. A spec-conformance test now pins the values and the strict descent; the same mutant reds instantly. **554 passing.** ⚠ **The `ChannelTier` WIRE enum is NOT swept here** — it is `serde(rename_all)`, so moving it changes the wire for `frontend-game` **and** a golden fixture in one step. That is its own row, not a leftover |
| **2-9** | The `ChannelTier` WIRE enum — Rust + TypeScript + golden, one commit | **DONE 2026-08-22** | `ChannelTier` → `MapKind` across **16 Rust files**, 8 TypeScript files and the golden. **Three defects the remap surfaced, none of which a rename would have found:** (1) `generates_tilemap()` was `!matches!(self, Cell)` — a NEGATION correct only because the set had five members, which at seven silently claims `Universe`/`Passage`/`Arena` bear tilemaps; rewritten as a positive list. (2) A backward-compat test reds — a payload with `"tier": "country"` no longer deserialised. **`SDF-A12` on the wire: retire a decoder, never delete it.** Deserialise-only `#[serde(alias)]` per absorbed rung; deleting them reds that test, bitten. (3) The mechanical replace produced a UI dropdown with **three identical `region` entries**, because the ladder's three middle rungs are one recursive kind — rewritten by hand, and the grid defaults stopped being keyed by kind. **554 Rust + 214 TS passing, `tsc` rc=0.** ⚠ The IDENTIFIER `tier` stays: the same word already means **treasure tier** in the same files, which is `SPG-F4` live, and renaming it blind would corrupt treasure code |
| **2-10** | `SPG-A3`'s containment matrix — IMPLEMENTED | **DONE 2026-08-22** | **The central axiom of doc 36 was enforced NOWHERE.** Grepped: `containment_violation` appeared in exactly two places in the repository, both of them `world_seed` naming it for its STRUCTURAL half (cycles, roots); **`allowed(parent, child)` was implemented nowhere.** The matrix was sealed on 2026-07-29, cited by fifteen amendment rows, and never once consulted. Now a `const fn` transcribing doc 36 §3.1 **as a row per parent kind in the doc's own order**, so a reviewer can diff the two side by side rather than audit 49 hand-typed booleans. Tests pin the two edges that prove it is a RELATION and not a ladder — `Universe → Universe` and `Domain → World` — plus `Arena` as a leaf against all seven kinds. **Bitten:** widening `Locale` to accept `Region` reds two tests; restored byte-identical. **172 lib + 1 live passing.** ⚠ `SDF-R7`'s SCALE half is deliberately unchecked and says so at the variant: bounding `Domain → World` to `Pocket` needs a scale on `NodeDecl`, which is its own row |
| **2-11** | `SDF-R7` — the SCALE bound, and a mirror with a mechanism | **DONE 2026-08-22** | `SDF-A19` bounds `Domain → World` to `Pocket` rather than forbidding it — a quota is arbitrary, fails at runtime instead of authoring time, and tells an author nothing. `NodeDecl` gains a `scale`; the edge is refused at **Megaplanet** and at **None**, because *silence is not consent* — an undeclared scale inherits a generator default that is not `Pocket`. And the bound applies **only to that edge**: a `Gigaplanet` under a `Universe` passes, since checking every `World` would be the quota the axiom refuses. **`WorldScale` is MIRRORED, not imported** — `world-gen` pulls `image`, `clap` and a triangulator, and a provisioning service importing a renderer for one enum is the worse trade. **The mirror gets a MECHANISM:** `scripts/world-scale-parity-gate.py`, set equality in BOTH directions (a one-way check misses the likelier drift, where the source grows and the mirror does not), with an empty parse and a renamed enum both treated as **findings** rather than passes. **--selftest: 5 cases, each proven to fire.** 176 lib + 1 live passing |
| **2-12** | `SDF-R8` + `SDF-R9` — the two doc-37 amendments | **DONE 2026-08-22** | **`WDS-A9`** snapshot-compaction, whose absence was found by **grep** (`compact`/`truncate`/`fold-baseline` = zero hits the day after doc 37 shipped) and whose finding is that **refusing alone is worse than `R-61`'s NMS behaviour** — NMS degrades, a bare refusal permanently locks an owner out of their own world. **`WDS-A10`** scope-follows-derivation, carried in with the MEASURED numbers rather than the claim (251 B/node, 92 % saved at N=100) and the note that it holds only because `SDF-A31` keeps generated cells out of the row space. **Six of nine `SDF-R*` now applied** |
| **2-13** | The remaining amendment rows — MEASURED, so the next session does not re-measure | **each row now carries WHY, not just THAT** | `SPG-R5` (`CSC_001`) — I suspected the header was stale, because §71/§80 read like applied forms. **Reading the target corrected me:** `[[char; 16]; 16]` still appears **twice**, `0..15` once, `cell` **102×** against `Domain` **10×**, and the aggregate is still `cell_scene_layout`. The header is HONEST — those lines are annotations saying *"was X, it becomes Y"*, not the change. **Applying it needs a `_boundaries` lock claim**, because `cell_scene_layout` is a REGISTERED aggregate name and `csc.*` a registered namespace. `SPG-R11` (`03_multiverse` + `GEO_001`) and `SPG-R12` (`TVL_*`) are other features' docs. `SDF-R4` is `COMB_*`'s, `SDF-R6` is `CSC_001`'s, `SDF-R5` is blocked on the unbuilt layer registry `T4`. **Every remaining row needs a boundary claim, another tier's decision, or round-3 infrastructure** — which is a different kind of work from the twelve rows above it |
| **2-14** | `SPG-R5` half (2) applied · `SPG-R11` and `SPG-R12` VERIFIED | **DONE 2026-08-22** | **`SPG-R5`** — the §2 summary rows were rewritten to `Grid<char>` on 2026-07-30 and **§4.2's actual struct still declared `[[char; 16]; 16]`**, so the doc described a default in one place and enforced an invariant in another for three weeks. Declaration fixed, `Grid<T>` defined rather than assumed, status corrected. The `cell → Domain` VOCABULARY half stays PROPOSED **on purpose**: `cell_scene_layout` and `csc.*` are registered contracts, and an id is a contract while the words inside it are not — the same trade `SPG-R14` took. **`SPG-R11` — the count was WRONG.** There are **two** strength orders, not three: `GeographyDelta` is *"Replay = base + deltas in order"*, a divergence LOG whose order is TIME, and `WDS-A9` folds it into a new baseline — meaningless for a strength order. Folding it in would have asked *"can a stronger source override a later delta?"*, which has no answer. **Struck from the row.** **`SPG-R12` — the gap is real and brought a SECOND finding:** `Passage` appears in neither travel doc, so a journey is a weighted edge and a timer — `SPG-A13`'s *"an ambusher has nowhere to stand"* in a struct — **and `TVL_004`'s `progress_fraction: f32` is a float in a replayed simulation**, which `WDS-A7` ruled out and `SDF-R2` removed from `Transform` the same day |
| **2-15** | `SPG-R8` APPLIED · `SPG-R9` VERIFIED | **DONE 2026-08-22** | **`SPG-R8` — the rot was live.** `08_realtime_movement_authority.md` still carried `RTM-Q4` as **"✅ Instanced — dedicated encounter scene"**, a RESOLVED decision, three weeks after `SPG-D1` reversed it. **The reversal had been recorded in the doc that MADE it and never at the doc it OVERRULED** — a reader of doc 08 alone would have built instanced combat. Annotated in place with `SPG-A16`'s reason and an explicit note that §5.4's checkpoint/restore survives. **`SPG-R9` — verified, and its SCOPE was ambiguous.** `TMP_001` already carries the restatement and calls it *"sharper, not different"*; but the row says `Locale` **singular**, while a faithful remap of *"every non-cell tier"* yields `Region` **and** `Locale`. The two readings differ in whether a `Region` renders at all. `tilemap-service` had to pick one today and picked `Region \| Locale`, because the target's own words are *"nothing in the pipeline changes"*. **Recorded rather than silently chosen** |
| **2-16** | `SPG-R12`'s second finding — the float in the replay path | **DONE 2026-08-22, and the ROOT was one doc upstream of the row** | The row pointed at `TVL_004`, whose encounter schedule carries a fraction. Following it upstream: **`TVL_001.actor_travel_state.progress_fraction: f32` is the root**, and `TVL_004`'s points are COMPARED AGAINST it — so converting `TVL_004` alone would have left the crossing test on a float and **fixed nothing while reading like a whole fix**. Both converted to `progress_permille: u16`, `MAP_001`'s existing `0..1000` convention. **The float was doing all three prohibited things:** advanced per tick, compared for ordering, and — via `TVL_004` §2 — **blake3-hashed into `EncounterId`**, so one bit of cross-platform difference yields a DIFFERENT ENCOUNTER IDENTITY, which is divergence rather than drift. `WDS-A7` had ruled f32 unproven cross-platform; `SDF-R2` removed the same type from `Transform` the same day |
| **2-17** | `SDF-R6` APPLIED · `SPG-R5` half (1) DECIDED, not deferred | **DONE 2026-08-22** | **`SDF-R6` was half-done and the missing half was the important one.** `R-62`'s justification (Edgar: *"fewer than 30 rooms; not suitable for large levels"*, so recursion is mandatory) landed with `SPG-R5`; **`R-48`'s COST did not** — over-fragmenting makes a continuous field numerically unstable, *"large spikes of water throwing the crew about"*, and a palace as a Domain-of-Domains with an atmosphere layer IS that graph. Both now stated together, **because a justification without its cost reads as "recursion is free"**. `SDF-A27` pays it at no new authoring surface. **`SPG-R5` half (1) is DECIDED AGAINST, not blocked:** `cell_scene_layout` and `csc.*` are registered contracts, and an id is a contract while the words inside it are not — the third time this session that trade was taken (`place.invalid_place_type_for_channel_tier`, `place.missing_decl`, now these). Renaming them for prose would cost a migration and a `_boundaries` claim for zero semantic gain |
| **2-18** | `T4` BUILT · `SDF-R5` APPLIED | **DONE 2026-08-22** | I had called this **blocked on unbuilt infrastructure**, which is exactly the *"blocked meaning I would have to build it"* the goal refuses. **`0029_layer_registry` is that infrastructure.** `home_kinds` `NOT NULL` with `cardinality > 0` — an empty set means *every node*, `R-29`'s 300 000-row outcome arriving as an OMISSION — and every member a real `MapKind`. **Every policy column `NOT NULL` with NO `DEFAULT`**, because `R-30` prices a wrong default at ~2000× over budget before any work, and a column with a `DEFAULT` is that mistake with a database behind it. `layer_id` is blake3-128 of the name — derived, never allocated, never reused (`R-19`) — and `retired_at` exists because `SDF-A12` retires rather than deletes. **10 bites, each proven to REJECT**, including #9: the same layer name under a DIFFERENT ruleset digest is ALLOWED, which proves the per-ruleset scope is not over-constrained |
| **2-19** | `SDF-R4` — the last of the nine | **DONE 2026-08-22** | **`SPG-D1` as written was NOT SATISFIABLE**, and `R-7` is why: XCOM 2 replays because a tactical mission is a CLOSED world, while an in-place fight in a shared `Locale` is an OPEN one. The reframing rescues the PO's decision rather than reversing it — *in place means no separate SPACE, not no ISOLATION BOUNDARY*. **`SPG-A19` + `0030_encounter`**: `carved_arena` NULL/not-null is the entire branch, `opening_frame` is the closure, and **multiple encounters at one node are ALLOWED** (`R-6`, Foundry) — a unique index would have broken that and passed every other bite. **10 bites, each proven to REJECT.** ⚠ **Damage, initiative, turn order and participants are deliberately absent** — `COMB_*`'s, and a column here would decide them by accident |
| **2-20** | The `tier` IDENTIFIER — `SPG-F4`'s collision closed in code | **DONE 2026-08-22** | I had called this blocked because *"the word also means treasure tier in the same files"*. **Measured instead of assumed: it does not.** 7 `tier: MapKind` bindings against 119 treasure uses **confined to two files**, and `engine/mod.rs` already distinguishes them by name (`treasure_tiers`). So the split was by TYPE and it was clean. **5 internal bindings renamed to `kind`; the 2 WIRE fields kept** — `RenderRequest` deserializes and `TilemapView` serializes into the golden, and a serialized field name is a contract in the same way a reject id is (the fourth time that trade was taken today). **The rename also exposed two stale doc comments** it would otherwise have left: `channel.rs` still said *"mirrors MAP_001 `MapKind` (5 V1 closed enum)"* for a **seven**-variant set, and *"Cell tier is excluded"*. **Bitten:** hard-coding the emitted kind reds `golden_baseline_byte_identical`; restored byte-identical. 554 passing |
| **2-21** | `WSA-R19..R24` — the inherited set, MEASURED | **DONE 2026-08-22** | Measured rather than recalled: **five of six were already `verified`**; only `WSA-R23` was outstanding. Verified it by opening `AIT_001`, and **the gap is real while the row's MECHANISM is wrong**. `AIT_001` is entirely NPC-shaped — `NpcTrackingTier { Major \| Minor }`, caps ≤20/≤100 — so loci are genuinely absent. But *extending* it as written would put a per-row TIER ENUM on every locus, which is precisely what `SDF-R1` measured at **92.4× worse**. **And `AIT_001` already uses the right mechanism for its own lowest tier:** *"Untracked = **absence** of `actor_progression` aggregate"* — **absence IS index membership**. So a locus's tier is membership in the live set (`T2`), never a column. **The row survives; the word "extend" was doing the damage** |
| **2-22** | `FLOW-19`'s owed half — PROVEN | **DONE 2026-08-22** | I had closed this **by decision** ("operational, not a migration"), which is true and was not enough: **nobody had ever run the discharge.** Three unknowns, all now answered against a real database — `VALIDATE` **does** fail with an orphan present (so `NOT VALID` does not make it permanently unvalidatable), reconciliation makes it succeed, and it is **still a ratchet** afterwards: a new orphan is refused and a legitimate lease is not. **The first assertion is the bite** — a validation that could not fail would certify a database still holding the defect. **Mutation-verified**, and the first mutation attempt SILENTLY FAILED to apply (drift 13), so the green that followed was the unmutated test |
| **2-23** | `SDF-Q12` CLOSED — `SDF-F8` was a UNIT ERROR | **DONE 2026-08-22** | I had said twice that this *"cannot be closed by this tier"*. **Re-reading §11.6 after `SDF-A31` landed shows the premise is gone.** §11.6 wrote *"an inner world bounded to `Pocket` is 1 024 CELLS — DOUBLE the per-player allowance"*, and the allowance is **500 NODES**. `SDF-A31`, written six sections later in the same document, settled that a generated cell is **never a row**. So 500 holders cost **500 node rows, not 512 000** — 126 KB, **0.05 %** of the envelope rather than 51 %. The binding constraint is the **baseline**, and `SDF-A30` shares it by digest: 500 holders on one template seed cost **~0.94 MB total**. **`SDF-F8` was not wrong about arithmetic — it was wrong about UNITS, and the axiom that exposed it was written by the same doc six sections later.** The `PROG_001` dependency survives but shrinks to *"how many DISTINCT seeds"* and can no longer break the design |
| **2-24** | `SDF-Q15` MEASURED — the last open row | **DONE 2026-08-22** | *"Needs a measured cost that does not exist"* is a blocker only until the thing is built. Built `space_view` (`SDF-A23`'s three producers behind `SDF-A26`'s budget), then measured it against real Postgres, best-of-7, on the market square §8.1 named as its own falsifier — 20 doors, 200 occupants. **12.5 B per included node, 14.10 ms per assembly.** §8.1's stated falsifier did NOT fire (occupancy 64 B vs ancestors 272 B) — **but the lean was still wrong. "Ancestors are free" is false: BOUNDED IS NOT FREE.** Four ancestors are 53 % of the payload; at `DP-Ch1`'s full depth the section alone is ~1 KB, four times everything else. **And the real constraint is neither cap** — 14.10 ms is 14 % of a tick, and the shape is an N+1. Tokens deliberately NOT measured: no tokenizer here, and CJK bytes-per-token would make conversion invented precision. **Bitten:** silencing the truncation flag reds with *"a reader could not tell a crowded room from an empty one"* |
...
**RESUME: THE REGISTER IS EMPTY. All eighteen `SDF-Q*` resolved · all nine `SDF-R*` applied · every `SPG-R*` and `WSA-R*` applied/verified/retired/decided · `T3`/`T4`/`T9` built · `FLOW-19`'s discharge PROVEN · `SPG-F4`'s `tier` collision closed in code. Seven migrations (`0024`–`0030`), three world-service modules, one new gate, and an actor standing in a seeded place — all bitten. **What the last two closures have in common is the lesson of the run:** `Q12` was a UNIT error its own document exposed six sections later, and `Q15` was blocked only on an artefact nobody had built. **The next work is not on this board** — §19 names two findings the measurement produced (the ancestor payload, and an N+1 in the view), and neither is a row here yet.**
