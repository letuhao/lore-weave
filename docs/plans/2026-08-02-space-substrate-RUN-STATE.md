...
**RESUME: EIGHT of nine `SDF-R*` applied, and `T4` is built. ONE row remains: `SDF-R4` — in-place combat needs an Encounter closure, and `Arena` and `Encounter` are different things. Its target is `SPG-D1`, which is **doc 36's own decision**, so the STRUCTURAL half is this tier's to state (the closure is the correctness boundary and exists in BOTH branches, per `R-7`); the MECHANIC half — what a fight does — is `COMB_*`'s and must not be written from this chair. Then: the `tier` identifier rename behind `SPG-F4` · `VALIDATE CONSTRAINT` on `0027` · `WSA-R19..R24`, doc 32's.**
