...
| **2-5** | The production birth path | **2 of 3 tables landed 2026-08-22** | **`0024_map_layout`** — a node gets a KIND (closed CHECK; the retired ladder's rungs now **unwritable**) + parent-relative `0..1000` position, and **no `materialization` column** because `SDF-R1` landed first. **`0025_entity_binding`** — WHERE THINGS ARE, implementing `EF_001` §3.1 rather than inventing `T7`: `EntityLocation`'s four variants as a discriminator + per-variant columns with a **sum-type CHECK**, and **no fine position** (`SDF-A34`; the enum's own `InCell` carries `cell_id` and nothing else, so it had already refused what `T7` proposed). FK is **RESTRICT, not CASCADE** — `R-52`'s *evacuate, never delete* is now the database's refusal. Both verified on throwaway databases: applied, re-applied, **13 bites each proven to REJECT**, down, down-again, up round-trip, dropped. **Left: `PF_001`'s `place`, then the §5 bootstrap** |
...
**RESUME: 2-5 continues — siting DONE, next is `PF_001`'s `place` row, then the bootstrap that writes all three. `0024_map_layout` gives a node a KIND and `0025_entity_binding` says where things ARE; nothing yet gives a node its SEMANTIC identity, which is what an LLM needs to narrate a scene. Build `place` per `PF_001` §3.1 — 1:1 with a `Domain` (`SPG-R14`), closed `PlaceType`, structural state — then the §5 five-step bootstrap whose step 5 is the spawn. Verify each against a throwaway database the way `0024` and `0025` were: apply, re-apply, bite every constraint, down, down-again, round-trip, drop.**
