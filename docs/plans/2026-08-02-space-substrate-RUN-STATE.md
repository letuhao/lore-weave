...
**RESUME: the board is finished. The three items the original goal named — `entity_binding`, `place`, the §5 bootstrap — landed as `0025` (`f24f3b12e`), `0026` (`b917da700`) and `world_seed` (`9fcd6835e`), each bitten against a throwaway database. Since then: all nine `SDF-R*` applied · every `SPG-R*` and `WSA-R*` applied/verified/retired/decided · `T3`/`T4`/`T9` built · `SPG-F4`'s `tier` collision closed in code · `FLOW-19`'s owed discharge PROVEN rather than promised · `SDF-Q12` CLOSED, because `SDF-F8` turned out to be a unit error. **ONE row remains in the whole register: `SDF-Q15`**, the space-view fan-out and occupant caps — and it is not blocked by a decision but by an ARTEFACT: there is no space view to measure a prompt-assembly cost against, and building one is round 3. Everything else is closed with evidence.**
