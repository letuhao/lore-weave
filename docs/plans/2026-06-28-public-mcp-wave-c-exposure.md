...
- **Slice 4 — per-provider live-smoke campaign** (`D-PMCP-PRICED-EXPOSURE-FLIP`). **KEY FINDING: there is no code "advertise flip".** Exposure is purely scope-driven (`isToolAllowed` = tier scope + domain scopes); write/priced tools are ALREADY reachable by any key holding those scopes, gated only by the `PUBLIC_MCP_ENABLED` kill-switch. So Slice 4 = proving each provider live with a properly-scoped real key (+ attribution for priced), not flipping a bit. **Results:**
  ...
  - ✅ **translation (write_confirm, priced):** default key → `translation_start_job` → **diverted to approval** (write_confirm divert holds for translation too); self-confirm key → `translation_start_job` → **real cost ESTIMATE + confirm_token** returned — the priced propose works end-to-end for a public key. **Self-confirm EXECUTION now PROVEN — see Slice 5.**
  - **Net:** all 5 providers + all 4 tiers proven at the EDGE (scope-gate + relay + divert + cross-tenant + paid_read + cost-estimate), AND self-confirm real-LLM execution proven (Slice 5).
  - **Deploy note:** the dev Docker build serves stale images (build cache + `docker cp` mtime quirk); the working deploy path this session was `docker cp` the changed files → clear `__pycache__` → `docker restart` → restart `ai-gateway` to re-federate. (Slice 5's env change was runtime-only → a plain `compose up -d` recreate sufficed, no rebuild.)

- **Slice 5 — self-confirm real-LLM execution** (`D-PMCP-SELFCONFIRM-ROUTES`). The last unproven leg of Slice 4. **Root cause:** auth-service's `DomainConfirmServiceURLs` (the replay target map) is loaded by `config.go:loadDomainConfirmURLs` from `*_SERVICE_URL` env vars — **none were set on the dev auth-service**, so every self-confirm `confirm_action`→domain replay returned `AUTH_CONFIRM_DOMAIN_UNROUTABLE`. **Fix (deploy-config only, no code):** added the 6 `*_SERVICE_URL` vars to the auth-service env in [docker-compose.yml](../../infra/docker-compose.yml) (`BOOK_=8082`, `COMPOSITION_=8093`, `TRANSLATION_=8087`, `GLOSSARY_=8088`, `KNOWLEDGE_=8092`, `PROVIDER_REGISTRY_=8085`) + recreated auth-service (env-only). **Live-proven (test account, local lm_studio gemma model = $0, real inference):**
  - Minted a self-confirm key (`[write_confirm, domain:translation]`, `allow_self_confirm=true`, cap $2) → `translation_start_job` propose through the edge → **token KEPT** (self-confirm key, not diverted) with a real estimate (local `user_model` 019ebb72, target `vi`, `$0.00117`).
  - `confirm_action(confirm_token, domain=translation)` through the edge → edge→auth `/internal/mcp-keys/confirm`→auth replays→translation `/v1/translation/actions/confirm` = **`status: executed`** (previously `AUTH_CONFIRM_DOMAIN_UNROUTABLE`) → job `019f0f22…` created with `mcp_key_id` + `spend_cap_usd=2` on the row.
  - Job ran to **`completed`** on the local gemma model → **3 `usage_logs` rows tagged `mcp_key_id=019f0f21…`, 9079 tokens total** — real LLM spend, attributed to the agent's key through the self-confirm execution path. Smoke key revoked.
  - **The full public-MCP write/priced trust boundary is now end-to-end proven:** propose (divert OR self-confirm) → confirm → domain execute → key-attributed spend + per-key cap.
