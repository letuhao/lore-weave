...
| S2-B2d · D-ARC-TEMPLATE-BOOK-TIER (34a: schema + tenancy) | DOING | Design 34a (MIRRORS proven motif.book_shared — no novel tenancy). **Schema DONE + verified**: `book_id`+`book_shared` cols, `arc_template_book_shared_shape` CHECK (verified rejects book_shared+NULL-book live), per-user dedup rescoped to `book_id IS NULL` + `uq_arc_template_book_shared` per-book, old `uq_arc_template_user` dropped-after-create (migrate.py + applied to dev DB). **Left: repo (create/adopt target=book_shared + list branch), REST+MCP EDIT-gate, FE tier, MANDATORY tenancy tests (non-grantee blocked).** |
