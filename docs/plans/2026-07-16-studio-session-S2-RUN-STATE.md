...
| S2-B1f · registration (catalog/enum/contract/i18n/guide) + cross-service live-browser smoke | DOING | catalog row + import (S2 block) ✓; `panel_id` enum + tool description (frontend_tools.py) ✓; contract regen (WRITE_FRONTEND_CONTRACT=1 pytest 20 pass) ✓; i18n en + 17 locales (i18n_translate.py) ✓; guideBody ✓. Drift-locks 42/42 (panelCatalog + frontendToolContract + useStudioCommands + UserGuide). arcEffects already live (B1d sweep). **Left: /review-impl QC + live-browser smoke.** |
