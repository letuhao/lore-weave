...
- **SHARED-FILE COLLISION (B1d) — same-tree hazard, handled.** `agent/handlers/index.ts` + `agent/__tests__/effectCoverage.contract.test.ts` were being edited CONCURRENTLY by S6 (wave-1 `compositionEffects`, split into flywheel/conformance) in the SAME working tree. My removal of S6's import to isolate my hunk momentarily broke THEIR tree — restored immediately. Resolution: B1d commits arcEffects.ts DORMANT (mine, no collision) + excludes the 2 colliding shared files; arcEffects registration + coverage un-pend move to B1f (with catalog/enum), staged only when they can be isolated from S6's uncommitted lines. HEAD stays self-consistent (arc pending + unregistered). This is the residual same-folder risk §8 named; worktrees would have avoided it.
