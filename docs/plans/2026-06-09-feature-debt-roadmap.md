...
> 🔴 **EVAL GATE RESULT (LOOM-67, 2026-06-10):** the FD-8 dropped-promise-rate eval is **NULL** — OFF 0.454 vs ON 0.429 (Δ=+0.025, within noise; ON wins 1/3 books; n=3 powered, disjoint judge). The spine (FD-6/7/8) is **built + live-proven functional**, but the §8 measured-lift gate is **NOT met**. Therefore **FD-9, FD-10, FD-12, FD-13 stay DEFERRED/speculative** — do not build them on this eval. To revisit: a discriminating eval that beats the confounds (longer arcs to escape saturation; shared-plan-per-arm rigor; control the re-injection introduced-inflation). FD-14 (cheap template seed) remains a free standalone anytime.
...
| **FD-27** | **Zero-event / reasoning-model extraction guard** (filed from FD-4/067). A reasoning model (qwen3.x) silently returns empty extraction output (burns the token budget on `<think>` before any JSON) → 0 events → 0 entities/status, with no signal. Add a warning/metric when an extraction job yields 0 events on non-empty input, and/or a capability check that flags a reasoning model configured for extraction (recommend `reasoning_effort=none` or a non-reasoning model). Prevents a silent "extraction did nothing" class (the original cycle-8 symptom). | worker-ai runner + knowledge extraction metrics | S-M |
