...
| **FD-5** | **full context mode may be unbuilt** — `full.py` references a `builder.py` path that still `raise NotImplementedError`. | knowledge `app/context/modes/full.py:23` | XS-S (verify first) | NO | **CLARIFY = verify reachability first.** If full-mode is reachable in prod → fix; if dead/unreachable → remove or guard. May be a non-issue. |
...
| **FD-22** | **Worker-notification "deferred to K16.6"** — extraction worker notify path marked deferred; agent suspects **stale** (K16.6 may have landed). | knowledge `app/routers/public/extraction.py:366` | XS (verify) → S (fix if real) | NO | **CLARIFY = verify whether K16.6 landed.** If landed → delete the stale marker; if not → the worker isn't being notified (polling fallback?) — real gap, fix. |
