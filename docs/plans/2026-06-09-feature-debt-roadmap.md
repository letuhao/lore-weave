...
| **FD-8** 🟢 S4a DONE | **S4 — arc-end DEBT check + eval** (§7). ✅ **S4a (debt-check BE, LOOM-66):** `GET …/narrative-threads` (`open`=the unpaid debt | `all`=full ledger; `open_count`= a true COUNT, not capped by the list LIMIT) + an advisory `open_promise_count` on the chapter-generate result; read-only, advisory per D4. 7 unit tests; /review-impl folded (MED capped-count → `count_open`). **Remaining S4b:** the eval arm (dropped-promise-rate) + the bundled full-loop live-smoke (needs the stack-up). | S4a DONE; S4b NOT-BUILT | M | **3** | the gate for FD-6/7 |
