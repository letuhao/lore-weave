...
if ! command -v gitleaks >/dev/null 2>&1; then
  ...
  echo "{\"ts\":\"$NOW\",\"event\":\"secret_scan_skipped\",\"cycle\":$CYCLE,\"scope\":\"cycle\",\"reason\":\"gitleaks_not_installed\"}" >> "$AUDIT_LOG"
  exit 0
fi
...
if [ "$LEAKS" -eq 0 ]; then
  ...
  echo "{\"ts\":\"$NOW\",\"event\":\"secret_scan_clean\",\"cycle\":$CYCLE,\"scope\":\"cycle\"}" >> "$AUDIT_LOG"
  exit 0
fi
echo "[secret-scan-cycle] $LEAKS leak(s) detected — halt rebase" >&2
echo "{\"ts\":\"$NOW\",\"event\":\"secret_leak\",\"cycle\":$CYCLE,\"scope\":\"cycle\",\"leak_count\":$LEAKS}" >> "$AUDIT_LOG"
exit 1
