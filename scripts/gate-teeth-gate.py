...
# 0 -> 4 at the feat/game-logic merge, and the direction is DOWN.
#
# feat/game-logic drove this to ZERO by proving every gate on its own tree (46
# gates, 457 mutations). refactor/entity-lifecycle was carrying 55. The merge is
# not a regression in their work: their zero described a tree that did not contain
# these four, and each is a knowledge-refactor gate that never had a proof on
# EITHER branch:
#
#   alive-column-deprecation-gate.py   pins the deprecated `alive` column's
#                                      shrink-only reader set
#   derived-entity-id-gate.py          pins the derived entity_canonical_id
#                                      call sites
#   entity-lifecycle-outbox-gate.py    every lifecycle mutation emits
#   gateway-domain-logic-gate.py       no domain logic in the gateway
#
# All four PASS today and all four are unproven, which is the exact distinction
...
def verify_proofs(invoked: dict) -> int:
    ...
    * Exit 0 means the self-test RAN and passed. It does not mean the arms
    ...
    print(f"gate-teeth-gate --verify-proofs: {len(targets)} advertised self-test(s) (floor {VERIFY_PROOF_FLOOR}) RAN and "
          f"passed.")
...
def main() -> int:
    ...
    if rc == 0:
        ...
        # The floor is PRINTED even when it equals the count. It used to reach the output
        # only because `len(invoked)` happened to BE the floor, so the moment the census
        # grew by one the ratchet became invisible and `gate-number-visibility-gate` caught
        # it (L2). feat/game-logic renamed the constant CI_SCOPE_FLOOR -> CI_GATE_FLOOR;
        # this print survived the merge naming the old one, which is a NameError on the
        # PASS path — the branch nothing exercises until the day it passes.
        print(f"  {proven} carry a red-ability proof; {len(unproven)} held at baseline; "
              f"census floor CI_GATE_FLOOR = {CI_GATE_FLOOR}.")
    return rc
