...
#
# 86 -> 90 AT THE MERGE OF refactor/entity-lifecycle (2026-08-31). The four are the
# SAME class this row already describes, arriving from the other side of the merge:
#   services/composition-service/scripts/eval_cast_prompt.py       (operator eval)
#   services/composition-service/scripts/live_smoke_t37_revision.py (live smoke)
#   services/knowledge-service/app/benchmark/vector_backend_bench.py (benchmark)
#   services/knowledge-service/app/db/migrations/neo4j_to_age.py    (migration CLI)
# Each is run BY A HUMAN and its stdout IS its output -- a benchmark that logs its
# table through a structured logger has no table, and the migration prints the
# per-graph counts an operator reads to decide whether to continue. Measured, not
# assumed: feat/game-logic scores exactly 86 (its ratchet) and the merged tree 90,
# and the diff of the two offender lists is these four files and nothing else.
