...
DONE_RE = re.compile(
    r"\*\*DONE\.?\*\*|✅ \*\*|CLOSED \d{4}-\d{2}-\d{2}|\d+ passed|RETRACTED|DISCHARGED",
    re.I,
)
...
OWED_RE = re.compile(
    r"⬜|still owed|owes|NOT built|not started|⛔|remains|Unfinished, not undecided|OWED",
    re.I,
)
...
def selftest() -> int:
    ...
    print("[plan-row-honesty-gate] SELFTEST "
          + ("PASS — flags a finished-looking open row, stays quiet on one that names owed "
             "work and on an already-ticked row (non-vacuous both ways)" if ok else "FAIL"))
...
def main() -> int:
    ...
    for name, done, owed in suspects:
        print(f"    {name:<8} completion-markers={done:<3} owed-markers={owed}")
