...
# 27 known duplications from the enterprise-hardening audit (Area 8): JWT
# verifier x8 (+ alg-pin x8), logging_config trio x3, TerminalEvent dup x2.
...
BASELINE = {
    ...
    'jwt-alg-pin|services/book-service/internal/api/server.go|if t.Method != jwt.SigningMethodHS256 {',
    'jwt-alg-pin|services/glossary-service/internal/api/server.go|if t.Method != jwt.SigningMethodHS256 {',
    'jwt-alg-pin|services/notification-service/internal/api/server.go|if t.Method != jwt.SigningMethodHS256 {',
    'jwt-alg-pin|services/provider-registry-service/internal/api/server.go|if t.Method != jwt.SigningMethodHS256 {',
    'jwt-alg-pin|services/sharing-service/internal/api/server.go|if t.Method != jwt.SigningMethodHS256 {',
    ...
    'jwt-verifier|services/book-service/internal/api/server.go|tok, err := jwt.ParseWithClaims(tokenStr, &accessClaims{}, func(t *jwt.Token) (any, error) {',
    'jwt-verifier|services/glossary-service/internal/api/server.go|tok, err := jwt.ParseWithClaims(tokenStr, &accessClaims{}, func(t *jwt.Token) (any, error) {',
    'jwt-verifier|services/notification-service/internal/api/server.go|tok, err := jwt.ParseWithClaims(strings.TrimPrefix(auth, "Bearer "), &accessClaims{}, func(t *jwt.Token) (any, error) {',
    ...
    'jwt-verifier|services/sharing-service/internal/api/server.go|tok, err := jwt.ParseWithClaims(tokenStr, &accessClaims{}, func(t *jwt.Token) (any, error) {',
