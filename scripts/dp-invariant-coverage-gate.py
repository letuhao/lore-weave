...
BASELINE: dict[str, tuple[int, int]] = {
    ...
    "DP-A": (0, 5),   # 6 -> 5 at the feat/game-logic merge: the gate measured 5 unproven
                      # on the merged tree and asked to be lowered. A ratchet that is not
                      # tightened when it can be stops being one.
    "DP-R": (0, 0),
    "DP-T": (0, 0),
    "DP-Ch": (0, 11),
}
...
def run() -> int:
    ...
    print(f"[dp-coverage] PASS — {len(decl)} declared invariant(s) across {len(FAMILIES)} "
          ...
          f"(reach floors >= {MIN_DECLARED} declared, >= {MIN_FILES} file(s)).")
