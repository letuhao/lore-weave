...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.write:
        ...
        UNPROVEN.write_text(
            ...
            "#\n"
            "# THE FIVE ROWS FROM tests/test_cp0_merge_db.py ARE A THIRD THING, AND THEY ARE HERE\n"
            ...
            "#\n"
            "# THIS SAID 'THE 13 ROWS' AND THE COUNT WAS A COINCIDENCE THAT COST A FINDING.\n"
            "# CI (no Postgres) reports 13 NOT FALSIFIABLE, and this note said 13 rows come from\n"
            "# that file - so the whole set read as one environment artefact. Measured: the file\n"
            "# defines 20 tests and exactly FIVE carried falsifier rows. A matching count is not a\n"
            "# matching population, and reading it as one closes real findings. The five are now\n"
            "# here; what the other eight are is a separate question, answered by running this in\n"
            "# an environment with no Postgres rather than by arithmetic.\n"
            "#\n"
