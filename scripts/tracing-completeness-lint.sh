...
# 49 -> 52 AT THE MERGE OF refactor/entity-lifecycle (2026-08-31), and the split is
# measured rather than assumed. Running this scan on both trees:
#   feat/game-logic  = 51 against its own ratchet of 49  (it shipped 2 over)
#   merged           = 52
# so ONE of the three is ours, and the diff of the two file lists names it:
#   services/glossary-service/internal/api/command_transport.go
# That file has no handler in it. It matches because GO_HANDLER_RE contains
# http.Handle, which is a prefix of the http.Handler and http.HandlerFunc TYPES --
# the file is transport middleware whose whole job is to record which transport
# dispatched a command. Adding the tracing import to silence a prefix match would
# be writing code for the regex rather than for the request path, so the count
# moves and the file stays as it is.
# The other two are on the far side of the merge and predate it; this commit does
# not adopt them as correct, it stops a red that was already red from hiding the
# next NEW one, which is the only thing this ratchet is for (see the note above).
