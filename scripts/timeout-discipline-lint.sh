...
# 3 -> 4 at the feat/game-logic merge. The ratchet is THEIRS (this branch had no
# Python leg), so the merge subjected refactor/entity-lifecycle's code to it for the
# first time and found one real site: age_bootstrap.py:149, an asyncpg.create_pool
# with no timeout. A second apparent offender at :122 was a COMMENT -- the leg now
# strips those, so this is 4 rather than 5.
PY_NEW_BASELINE=4
...
# A TRAILING CR IS PART OF THE RECORD, AND IT MADE THIS LEG REPORT ITS INTERPRETER.
# The tree is a Windows checkout: the .py files scanned here are CRLF
# (services/translation-service/app/database.py, measured: 23 CRLF, 0 bare LF).
# msys awk opens files in text mode and drops the CR, so snippet came out clean and
# the fingerprint rel<TAB>snippet matched its baseline row. The awk reached through
# the bash that Python spawns (cwd=REPO, stdin=DEVNULL -- how gate-wiring-gate
# invokes every gate) does NOT drop it: every snippet ended in CR, so every
# fingerprint missed its baseline row and every hit looked new. It prints as a
# SYMMETRIC failure -- 8 baseline row(s) matching no current hit AND 8 offender(s)
# outside the baseline -- which reads like the tree moved under the gate, and is why
# this was first mis-diagnosed as a lost tab separator. Same commit, same tree, PASS
# from one shell and FAIL from the other.
#
# NO APOSTROPHES BELOW THIS LINE: the awk program is a single-quoted shell string,
# so one apostrophe in a comment ends it and the rest becomes shell. That happened
# while writing this very comment.
{ sub(/$/, "") }
{
  if (!capturing) {
    # A COMMENT IS NOT A CALL SITE. `age_bootstrap.py:122` is prose ABOUT
    # `asyncpg.create_pool(server_settings=...)` and this leg counted it as an
    # offender. That is the mentions-vs-uses false positive the TS leg further down
    # already guards against -- its note records reporting 8 instead of 3 for
    # exactly this. Found when the merge first ran this leg over
    # refactor/entity-lifecycle code: 5 offenders, one of which was a sentence.
    stripped_py = $0; sub(/^[ \t]+/, "", stripped_py)
    if (substr(stripped_py, 1, 1) == "#") next
...
run_lint() {
  ...
  local n_go n_rs n_py n_ts
  n_go=$(count_ext go "$services" "$contracts")
  n_rs=$(count_ext rs "$services" "$crates")
  n_py=$(count_ext py "$services")
  # The TS leg (refactor/entity-lifecycle, T48aw) counts only `services/*/src/**`, not
  # every .ts in the tree — node_modules dwarfs the source and would make the floor
  # meaningless.
  n_ts=$(find "$services" -path "*/src/*" -name "*.ts" -not -path "*/node_modules/*" \
  ...
  # The TS leg's floor WARNS instead of returning 2, and the asymmetry is the point: that
  # leg is ADVISORY, so its silence never claims compliance in the first place. Making it
  # hard broke all 16 of this gate's own selftest cases at once — their synthetic trees
  # carry .go/.rs/.py and no .ts, and a fixture is not a moved repo.
  if [[ "$n_ts" -eq 0 ]]; then
    echo "[timeout-discipline] note — TS leg scanned 0 file(s) under services/*/src;" >&2
    echo "  advisory, so this is a note, but a zero on the REAL tree means a moved path." >&2
  fi
  ...
  # ── TypeScript leg (T48aw, from refactor/entity-lifecycle) ─────────────────
  #
  # The discipline scanned .go, .rs and .py and NEVER .ts — so it skipped the two services
  # whose whole job is outbound calls: the KAL and the BFF. Measured 2026-08-30 across
  # services/*/src/**/*.ts: 18 real `fetch(` sites, 15 carrying a signal or timeout, 3
  # carrying neither (all in mcp-public-gateway).
  #
  # Comments are STRIPPED before matching. The first version counted `// … over fetch` in a
  # module doc as a call site and reported 8 offenders instead of 3 — the mentions-vs-uses
  # false positive T48aj found in a route census, made again.
  #
  # ADVISORY like the Python leg: the findings are pre-existing and live in another track's
  # service, so this warns rather than failing a merge that did not cause them.
  local ts_files ts_new
  ts_files=$(find "$services" -path "*/src/*" -name "*.ts" -not -path "*/node_modules/*" \
               2>/dev/null || true)
  if [[ -n "$ts_files" ]]; then
