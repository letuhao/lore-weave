...
- **`glossary-service`** = authored SSOT for lore AND hosts the **wiki** (`wiki_*` tables — wiki is NOT a separate service). **`knowledge-service`** = the extracted, story-positioned graph (Postgres + Neo4j), joined to glossary by the `glossary_entity_id` anchor. ⚠️ **NOT a derived layer** — this line said *"derived fuzzy/semantic layer"* until 2026-08-14 (T44/T47), and measured live the KG holds truth glossary **cannot represent**: 1184 `:Event` (glossary has no events table), 35 `:EntityStatus` of which 0 are anchored, 567 unanchored `:Entity`. `entity_facts.entity_id` is an FK to `glossary_entities`, so those rows are not merely absent from the "SSOT" — they are unwritable there. Two stores with **disjoint truth joined by an anchor**; the anchor is a join key, not a derivation. One store is the destination (O1; T46 merges them). See [scope-separation SCOPE-3](docs/standards/scope-separation.md).
