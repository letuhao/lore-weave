...
def test_python_can_unwrap_a_dek_that_GO_wrapped():
    ...
    GO_WRAPPED = "1O8sFld1A+G3vyjsWv3jODDj1V98ptRYftU2ptFOg+9B17IXFVx4IrU8B2JIpS9Rcezhl+LOJ1QHT7z+"
    GO_KEY_REF = "a89a34849ab8ed7f"

    kek = _coerce_key("golden-test-kek")
    ring = Keyring(active=kek, active_ref=key_ref(kek))

    # 1. Python's key_ref must equal Go's, byte for byte — otherwise the operator's
    #    "which users are still on the retired KEK?" rotation checklist lies.
    assert key_ref(kek) == GO_KEY_REF, (
        "Python and Go disagree about key_ref. The rotation checklist would be wrong."
    )

    # 2. Python must recover EXACTLY the DEK Go wrapped.
    dek = unwrap_dek(ring, GO_WRAPPED)
    ...
    # 3. And that DEK really works end-to-end.
