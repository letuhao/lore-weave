...
- Every LLM call goes through the injected `JudgeLLMClient` (see
  `_client.py`) → `loreweave_llm` SDK → provider-registry gateway. NO
  direct provider SDK calls (gateway invariant). The caller supplies the
  concrete client (knowledge-service `LLMClient`, learning-service's, or a
  test double).
...
from ._client import JudgeLLMClient
...
async def run_dump_judge(
    ...
    *,
    ...
    # Q0 lift: judge_ensemble is now a sibling module in this package.
