...
func TestRunUnknownProfile(t *testing.T) {
	if code := run(io.Discard, 1, "nope", false, false, false, false, false, false, false, "", 0, 0); code != 2 {
		t.Errorf("unknown profile must exit 2, got %d", code)
	}
}

func TestRunEmitRequiresDSN(t *testing.T) {
	if code := run(io.Discard, 1, "micro", true, false, false, false, false, false, false, "", 0, 0); code != 2 {
		t.Errorf("-emit without -dsn must exit 2, got %d", code)
	}
}

func TestRunVerifyRequiresDSN(t *testing.T) {
	if code := run(io.Discard, 1, "micro", false, true, false, false, false, false, false, "", 0, 0); code != 2 {
		t.Errorf("-verify without -dsn must exit 2, got %d", code)
	}
}

func TestRunCheckProjectionsRequiresDSN(t *testing.T) {
	if code := run(io.Discard, 1, "micro", false, false, true, false, false, false, false, "", 0, 0); code != 2 {
		t.Errorf("-check-projections without -dsn must exit 2, got %d", code)
	}
}

func TestRunCheckChecksumRequiresDSN(t *testing.T) {
	if code := run(io.Discard, 1, "micro", false, false, false, false, true, false, false, "", 0, 0); code != 2 {
		t.Errorf("-check-checksum without -dsn must exit 2, got %d", code)
	}
}

func TestRunMicroBenchRequiresDSN(t *testing.T) {
	if code := run(io.Discard, 1, "micro", false, false, false, false, false, true, false, "", 0, 0); code != 2 {
		t.Errorf("-micro-bench without -dsn must exit 2, got %d", code)
	}
}

func TestRunDryRunEmitsValidJSONL(t *testing.T) {
	var buf bytes.Buffer
	if code := run(&buf, 1, "micro", false, false, false, false, false, false, false, "", 0, 0); code != 0 {
